...
module TestResult : sig
  ...
  (** Aggregate statistics for a test run.
      ...
      The ['stset] parameter is the type of sets of final states,
      as returned by [states]. *)
  ...
  type ('es, 'exec, 'stats) t =
    { event_structures : 'es list;
      (** Event structures generated for the test. *)

      exec_iter : ('exec -> unit) -> 'stats
      (** [exec_iter f] iterates over a sequence of executions, applying [f] to
          each. It then returns aggregate statistics for the test run. *)
    }

  module Make (S : SemExtra.S) : sig
    (** This module provides specialized versions of {!TestResul} types for a
        concrete semantics module [S], as well as helper functions on
        {!execution} and {!stat} instances that only make sense on specialized
        versions on them, that therefore cannot live in the generic
        {!TestResult} module. *)
    ...
    (** [count_prop ~byte test stats] counts the final states in [stats] that
        satisfy [test]'s condition. *)
  end
end
...
module Make (O: Config)(M:XXXMem.S) : sig
  ...
  (** [run test] executes [test] according to the configured model and returns
      the possibly updated test together with its results.

      The sequence of executions exposed by the returned result's [exec_iter]
      iterator depends on the configuration. In particular, it may be filtered
      and capped by parameters such as [O.show] and [O.nshow]. *)
end
