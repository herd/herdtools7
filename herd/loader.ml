...
struct
  ...
  let rec load_code addr mem rets link_num = function
    | [] -> mem,[],rets,link_num
    | ins::code ->
      load_ins addr mem rets link_num code ins

  and load_ins addr mem rets link_num code = fun x ->
    match x with
    | A.Nop ->
      load_code addr mem rets link_num code
    | A.Instruction ins ->
      if A.is_link ins then
        let new_mem,start,new_rets,new_num = load_code (addr+4) mem rets (link_num+1) code in
        ...
        let newer_mem =
            ...
            Label.Map.add lbl start new_mem in
        let newer_rets = IntMap.add addr lbl new_rets in
        newer_mem,(addr,ins)::start,newer_rets,new_num
      else
        let mem,start,new_rets,new_num = load_code (addr+4) mem rets link_num code in
        mem,(addr,ins)::start,new_rets,new_num
    | A.Label (lbl,ins) ->
      let mem,start,new_rets,new_num = load_ins addr mem rets link_num code ins in
      if Label.Map.mem lbl mem then
        Warn.user_error
          "Label %s occurs more that once" lbl ;
      Label.Map.add lbl start mem,start,new_rets,new_num
    | A.Symbolic _
  ...
  let load prog =
    ...
    | ((proc,_),code)::prog ->
      ...
      let fin_mem,start,fin_rets,fin_num = load_code addr mem rets new_num code in
