 ...
 *)
  ...
  val calculate_rf_with_cnstrnts :
      S.test -> Label.Set.t -> S.event_structure -> S.M.VC.cnstrnts ->
        (S.concrete -> S.M.VC.cnstrnt option -> S.M.VC.arch_solver_state -> 'a -> 'a ) -> (* kont *)
          'a -> 'a
  ...
  val solve_mem :
      S.test ->S.E.event_structure -> S.read_from S.RFMap.t -> S.M.VC.cnstrnt list ->
        (S.E.event_structure ->
          S.read_from S.RFMap.t -> S.M.VC.cnstrnt list -> S.M.VC.arch_solver_state -> 'a -> 'a) ->
            'a -> 'a
  ...
  val when_unsolved :
      S.test -> S.E.event_structure -> S.read_from S.RFMap.t -> S.M.VC.cnstrnt list ->
        S.M.VC.arch_solver_state -> 'a -> 'a

  val compute_final_state :
    S.test -> S.read_from S.RFMap.t -> S.E.EventSet.t -> S.A.state * S.A.FaultSet.t

  val check_filter : S.M.VC.arch_solver_state -> S.test -> S.A.state * S.A.FaultSet.t -> bool
 ...
 *   First, all "atomic" effects are grouped first by thread
 * and then by location. Those "atomic" effects are
 * then partitioned according to their generating instruction,
 * AMO's (atomic proper) or load-reserve and store-conditional
 * (exclusives below). Atomic proper effect are further partitioned
 * into explicit effects (atms below, generated by LDADD etc. instructions)
 * and non-explicit effects (nexps below, hardware PTE updates).
 *   Then, the atomic pairs are computed by OCaml code that
 * follows the Cat defitions of stdlib.cat. However, the
 * OCaml code takes advantage of the fact that effects are grouped
 * by thread and location.
 *)

    let make_atomic_load_store _test es =

      let atms,excls,nexps = U.collect_atomics_and_exclusives es in

      (*
       * Compute `[NExp & PTE & R]; iico_data & loc; [NExp & PTE & W])`,
       * by simply restricting iico_data domain and range to
       * implicit reads and writes.
       * This assumes that all non-explicit atomic access modify operations
       * are PTE updates.
       *)

      let make_atomic_pairs_nexps evts =
       ...
       * Notice that is assumed that an instruction execution generates:
       *   + one explicit atomic effect per location exactly, or
       *   + one explict atomic read and one explicit
       *     atomic write per location.
       * Other situations are flagged by a fatal error.
       *)

      and make_atomic_pairs_exps atms =
        List.fold_left
          (fun k (_,atms) ->
            U.LocEnv.fold
              (fun _ evts k ->
                IntMap.fold
                  (fun _ es k ->
                    match es with
                    | [_] -> k (* failing CAS or RMW effect *)
                    | [e1; e2;] ->
                       let er,ew =
                         if E.is_load e1 && E.is_store e2 then e1,e2
                         else if E.is_store e1 && E.is_load e2 then e2,e1
                         else assert false in
                       (er,ew)::k
                    | []|(_::_::_::_) -> assert false)
                  (U.group_by_po evts) k)
              atms k)
        [] atms |> E.EventRel.of_list

      (* Computes `[R & EX]; po \ (po; [M & EX]; po); [W & EX]`,
         using OCaml code. *)
      and make_atomic_pairs_exclusives excls =
        List.fold_left
          (fun k (_,evts) ->
            let po = (* Reconstruct po by iterating on all unordered pairs. *)
              Misc.doubleton_fold
                ...
                [] evts |> E.EventRel.of_list in
            (* Notice: po is from exclusive to exclusive *)
            let open E.EventRel in
            diff
              (restrict_domains E.is_load E.is_store po)
              (sequence po po) (* local  po is already from EX to EX *)
           ::k)
        [] excls |> E.EventRel.unions in

      let amo =
        E.EventRel.union
          (make_atomic_pairs_nexps nexps)
          (make_atomic_pairs_exps atms)
      and lxsx = make_atomic_pairs_exclusives excls in
      E.EventRel.union amo lxsx
