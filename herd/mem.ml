    ...
    let fetch_addr check_back seen proc_jmp addr_jmp lbl addr =
      ...
      let env0 = get_all_mem_locs test in
        ...
        let pc_value_of_target = function
          | CodeEnd proc ->
              A.V.nameToV (Printf.sprintf "END_%s" (Proc.pp proc))
          | FaultHandlerEnd proc ->
              A.V.nameToV (Printf.sprintf "FH_END_%s" (Proc.pp proc))
          | Addr (proc, addr) ->
            begin
              match Label.norm (labels_of_instr addr) with
              | Some lbl -> addr2v proc (Label.pp lbl)
              | None -> V.intToV addr
            end
          | Value v -> v
        in
...
(* Call instruction semantics proper *)
      ...
      let rec add_next_instr re_exec fetch_proc proc env seen addr inst nexts =
        wrap re_exec fetch_proc proc inst addr env sem_instr >>> fun branch ->
          ...
          and szo =
            ...
            | St -> None
            | Ld sz -> Some sz in
          let add_pc branch pc_reg =
            match branch with
            | B.Next bds ->
                let target =
                  match nexts, re_exec with
                  | (addr_next, _) :: _, _ ->
                      Addr (proc, addr_next)
                  | [], true ->
                      FaultHandlerEnd proc
            ...
            | B.Jump (tgt, bds) ->
              let target =
                ...
                | B.Lbl lbl -> Value (addr2v proc (Label.pp lbl)) in
              let pc_val = pc_value_of_target target in
                set_pc proc pc_val;
            ...
            | B.IndirectJump (v, lbls, bds) ->
              let target =
                ...
                | Some (proc_lbl, lbl) ->
                    (try Value (addr2v proc_lbl (Label.pp lbl)) with _ ->
                       (try Addr (proc_lbl, Label.Map.find lbl prog)
                        with Not_found -> Value v))
                | None -> Value v
              in
              let pc_val = pc_value_of_target target in
                set_pc proc pc_val;
      ...
      | [] ->
          ...
          | Some _ ->
              let target =
                if re_exec then FaultHandlerEnd fetch_proc
                else CodeEnd fetch_proc in
              let pc_val = pc_value_of_target target in
