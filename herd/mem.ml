 ...
 *)
    ...
    let make_atomic_load_store _test es =
      ...
      and make_atomic_pairs_amos es k =
        let rs,ws = List.partition E.is_load es in
        let ws = StoreSet.of_list ws in
        List.fold_left
          (fun k er ->
      ...
      and make_atomic_pairs_exclusives es k =
        let rs,ws = List.partition E.is_load es in
        ...
        List.map
          (fun (_,m) ->
           U.LocEnv.fold
             (fun _loc exps k ->
                let amos,excl =
                  List.partition E.is_amo exps in
                make_atomic_pairs_exclusives excl
                  (make_atomic_pairs_amos amos k))
             m E.EventRel.empty)
