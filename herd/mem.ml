        ...
        A.state_fold
          (fun loc v locs ->
            let locs =
              ...
              | A.Location_global _
                -> loc::locs
              | A.Location_reg (_, r) when A.is_sysreg r -> loc::locs
      ...
      let locs_final =
        A.LocSet.filter
          (function
           ...
           | A.Location_reg (_, r) when A.is_sysreg r -> true
              ...
              (fun locs (_,ins) ->
                A.fold_addrs
                  (fun x ->
                    ...
                    | A.Location_reg (_, r) when A.is_sysreg r -> A.LocSet.add loc
                    | _ -> fun locs -> locs)
                  locs ins)
              locs code)
          locs
...
let match_reg_events es =
  let loc_loads = U.collect_non_sysreg_loads es
  and loc_stores = U.collect_non_sysreg_stores es in
    ...
    let solve_mem_non_mixed test es rfm cns kont res =
      ...
      if self then
        ...
        let kont es rfm cns res =
          ...
          let is_load e = E.is_mem_load e || E.is_sysreg_load_any e in
          let is_store e = E.is_mem_store e || E.is_sysreg_store_any e in
...
(* Store to final state comes last *)
    ...
    let all_finals_non_mixed test es =
      let loc_stores = U.remove_spec_from_map es (U.collect_mem_and_sysreg_stores es) in
      let loc_stores =
