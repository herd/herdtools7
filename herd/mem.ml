 ...
 * AMO's (atomic proper below) or load-reserve and store-conditional
 * (exclusives below).
 *   For exclusives, to each read we associate the
 ...
 *  For atomics proper, we proceed as above to associate an atomic  write
 * to each atomic load, we then check that the read and its associated write
 * are generated by the same instruction.
 ...
 *)
    ...
    let make_atomic_load_store _test es =
      ...
      let module EvtSet = EvtSetByPo(struct let es = es end) in
       ...
       * by matching each explicit atomic read with its (generalised)
       * po-successor explicit atomic write to the same address.
       * It is a fatal error for this write not to be emited
       * by the same instruction instance as the read.
       *)
        ...
        List.fold_left
          (fun k (_,atms) ->
            U.LocEnv.fold
              (fun _ evts k ->
                let evts =
                  (* Get rid of RMW effects that are both read and write *)
                  List.filter (fun e -> E.is_load e <> E.is_store e) evts in
                let rs,ws = List.partition E.is_load evts in
                let ws = EvtSet.of_list ws in
                List.fold_left
                  (fun k er ->
