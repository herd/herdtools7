...
module Make (C : Config) = struct
  ...
  module Mixed (SZ : ByteSize.S) : sig
    ...
    let p1r name arg ~returns ?(se = false) ?parameters f =
      let f ii_env _ = function
        | [ v ] -> return [ f ii_env v ]
    ...
    let p2r name arg1 arg2 ~returns ?(se = false) ?parameters f =
      let f ii_env _ = function
    ...
    let p1a1r name param1 arg1 ?(se = false) ~returns  f =
      ...
      in
      build_primitive
        ?returns:(Some returns) ~args:[ arg1; ] ~parameters:[param1] ~se
        name f

    let p1a2 name param1 arg1 arg2 ?(se = false) f =
      let f ii_env params args =
        match params,args with
        | [v1],[v2; v3; ] -> f ii_env v1 v2 v3
        | _ -> Warn.fatal "Arity error for function %s." name
      in
      build_primitive ~args:[ arg1; arg2; ] ~parameters:[param1;] ~se name f

    let p1a2r name param1 arg1 arg2 ~returns  ?(se = false) f =
      let f ii_env params args =
        match params,args with
        | [ v1; ], [ v2; v3; ] -> return [ f ii_env v1 v2 v3; ]
        | _ -> Warn.fatal "Arity error for function %s." name
      in
      build_primitive ?returns:(Some returns) ~se
        ~args:[ arg1; arg2; ] ~parameters:[param1;]
        name f

    let p1a3 name param1 arg1 arg2 arg3 ?(se = false) f =
      let f ii_env params args =
        match params,args with
        | [v1],[v2; v3; v4; ] -> f ii_env v1 v2 v3 v4
        | _ -> Warn.fatal "Arity error for function %s." name
      in
      build_primitive ~args:[ arg1; arg2; arg3; ] ~parameters:[param1;] ~se name f
    ...
    let extra_funcs =
      ...
      [
        ...
        p1a2 "write_memory" ~se
          ("size", None)
          ("addr", bv_64) ("data", bv_var "size")
          write_memory;
        p1a3 "write_memory_gen" ~se
          ("size", None)
          ("addr", bv_64)
