...
(*************)
    ...
    type event_structure = {
      ...
      intra_causality_order : EventRel.t ; (* Just order *)
        control : EventRel.t ;
        addr_ports : EventSet.t ; success_ports : EventSet.t ;
    ...
    let map_event_structure f es =
      ...
      { procs = es.procs ;
        ...
        addr_ports = map_set es.addr_ports ;
    ...
    let empty =
      { procs = [] ; events = EventSet.empty ;
        ...
        addr_ports = EventSet.empty ; success_ports = EventSet.empty ;
    ...
    let is_empty_event_structure es =
      ...
      EventSet.is_empty es.addr_ports &&
...
(*******************************)
    ...
    let inst_code_comp (*poi*) es1 es2 =
      { procs = [] ;
        ...
        addr_ports = EventSet.union es1.addr_ports es2.addr_ports;
    ...
    let inst_code_comp_spec es1 es2 es3 =
      ...
      | [lo;ro;] ->
          ...
          { procs = [] ;
            ...
            addr_ports =
            EventSet.union3 es1.addr_ports es2.addr_ports es3.addr_ports;
            success_ports =
            EventSet.union3
...
(* Standard union of two structures, specific fields to be completed *)
    let union es1 es2 =
      { procs = [];
        ...
        addr_ports =
          EventSet.union es1.addr_ports es2.addr_ports ;
        success_ports =
    ...
    let exch_comp rs1 rs2 ws1 ws2 =
      { procs = [] ;
       ...
       intra_causality_order =
        ...
        addr_ports =
        EventSet.union4 rs1.addr_ports rs2.addr_ports ws1.addr_ports ws2.addr_ports;
        success_ports =
...
(* Notice
 ...
 (* Notice similarity with data composition.  *)
        ...
        EventSet.union5
          rloc.addr_ports
          rmem.addr_ports rreg.addr_ports wmem.addr_ports wreg.addr_ports;
        success_ports =
        EventSet.union5
          rloc.success_ports
...
(* disjointness is awful *)
    ...
    let linux_exch re rloc rmem wmem =
      ...
      { procs = [];
        ...
        addr_ports =
        EventSet.union4
          re.addr_ports rloc.addr_ports rmem.addr_ports wmem.addr_ports;
        success_ports =
        EventSet.union4
...
(* Success *)
    let linux_cmpexch_ok rloc rold rnew rmem wmem =
      ...
      { procs = [];
        ...
        addr_ports=
        EventSet.union5 rloc.addr_ports rold.addr_ports rnew.addr_ports
          rmem.addr_ports wmem.addr_ports;
        input = None; data_input = None;
        output=Some (get_output rmem); ctrl_output = None ;
        success_ports=
   ...
   new iico_control edge. *)
    ...
    let linux_cmpexch_no rloc rold rmem =
      ...
      { procs = [];
        ...
        addr_ports=
        EventSet.union3 rloc.addr_ports rold.addr_ports rmem.addr_ports;
        success_ports=
          EventSet.union3
...
(**************)
    ...
    let linux_add_unless_ok loc a u rmem wmem retbool =
      ...
      { procs = [];
        ...
        addr_ports =
        EventSet.union5
          loc.addr_ports a.addr_ports u.addr_ports
          rmem.addr_ports wmem.addr_ports;
        success_ports =
        EventSet.union5
    ...
    let linux_add_unless_no loc u rmem retbool =
      ...
      { procs = [];
        ...
        addr_ports =
        EventSet.union3 loc.addr_ports u.addr_ports rmem.addr_ports;
        success_ports =
...
(* RISCV Store conditional *)
    ...
    let riscv_sc success resa data addr wres wresult wmem =
      ...
      { procs = [];
        ...
        addr_ports =
        EventSet.union
          (EventSet.union3 resa.addr_ports data.addr_ports addr.addr_ports)
          (EventSet.union3 wres.addr_ports wresult.addr_ports wmem.addr_ports);
        success_ports =
        EventSet.union
...
(* AArch64 CAS, failure *)
    let aarch64_cas_no is_phy add_ctrl rn rs wrs rm br =
      ...
      { procs = [] ;
        ...
        addr_ports =
        EventSet.union5
          rn.addr_ports rs.addr_ports
          wrs.addr_ports rm.addr_ports br.addr_ports;
        success_ports =
        EventSet.union5
...
(* AArch64 CAS, success *)
    let aarch64_cas_ok is_phy prov_data rn rs rt wrs rm wm br =
      ...
      { procs = [] ;
        ...
        addr_ports =
        (EventSet.unions
           [rn.addr_ports; rs.addr_ports; rt.addr_ports;
            wrs.addr_ports; rm.addr_ports; br.addr_ports; wm.addr_ports]);
        success_ports =
        (EventSet.unions
...
(* Temporary morello variation of CAS *)
    let aarch64_cas_ok_morello rn rt rm wm =
      ...
      { procs = [] ;
        ...
        addr_ports =
        (EventSet.union4
           rn.addr_ports rt.addr_ports rm.addr_ports wm.addr_ports);
        success_ports =
        (EventSet.union4
...
(* Dijointness not checked..., useless *)
    let stu rD rEA wEA wM =
      ...
      { procs = [] ;
        ...
        addr_ports =
        EventSet.union4
          rD.addr_ports rEA.addr_ports wEA.addr_ports wM.addr_ports ;
        success_ports =
        EventSet.union4
