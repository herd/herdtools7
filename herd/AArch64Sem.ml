...
(* Basic read, from register *)
      ...
      | _ ->
          M.read_loc is_addr (mk_read quad Annot.N aexp) (A.Location_reg (ii.A.proc,r)) ii

      let read_reg_morello is_addr r ii =
      ...
      let read_reg_neon_elem r idx ii = match r with
      | AArch64Base.Vreg (_,(_,esize)) ->
          read_reg_neon r ii >>= fun cur_val ->
          neon_getlane cur_val idx esize
      | _ -> assert false

      let read_reg_sz sz is_addr r ii = match sz with
      | MachSize.S128 -> read_reg_morello is_addr r ii
      | MachSize.Quad when not morello || not is_addr -> read_reg is_addr r ii
      | MachSize.Quad|MachSize.Word|MachSize.Short|MachSize.Byte ->
          read_reg is_addr r ii >>= uxt_op sz

      let read_reg_ord sz = read_reg_sz sz false
      let read_reg_addr = read_reg_sz quad true
      let read_reg_addr_sz sz = read_reg_sz sz true
      let read_reg_nzcv = read_reg_ord quad AArch64.NZCV
...
(* Complete effective adress computation *)
      ...
      let post_kr rA addr kr ii =
        ...
        | RV(v,rO) -> read_reg_ord (tr_variant v) rO ii in
        get_k >>= fun k ->
...
(* Ordinary loads *)
                  ...
                  (fun ac a _ ii ->
                    (add_if post k a_virt >>=
                      ...
                      let (>>|) =
                        ...
                        | _ -> (>>|) in
                      ((read_reg_ord sz rs1 ii >>> fun v ->
                        do_write_mem sz an aexp ac a v ii) >>|
                         (add_size a sz >>= fun a ->
                          read_reg_ord sz rs2 ii >>> fun v ->
                          do_write_mem sz an aexp ac a v ii)))
                  sz Annot.N
                  ma (M.unitT V.zero)
                  ii >>=
      ...
      let stlr sz rs rd ii =
        do_str rd (do_write_mem sz Annot.L aexp) sz Annot.L
          (read_reg_addr rd ii) (read_reg_ord sz rs ii) ii
...
(* AMO instructions *)
      ...
      let swp sz rmw r1 r2 r3 ii =
        lift_memop r3 Dir.W true (* swp is a write for the purpose of DB *)
          ...
          (to_perms "rw" sz)
          (read_reg_addr r3 ii)
          (read_reg_ord sz r1 ii)
          (rmw_to_read rmw)
          ii

      let cas sz rmw rs rt rn ii =
        let an = rmw_to_read rmw in
        let read_rs = read_reg_ord sz rs ii
        ...
        M.altT (
          ...
          lift_memop rn Dir.W true memtag mop_success (to_perms "rw" sz)
            (read_reg_addr rn ii) (read_reg_ord sz rt ii) an ii
        )( (* CAS fails *)
          M.altT (
      ...
      let casp sz rmw rs1 rs2 rt1 rt2 rn ii =
        let an = rmw_to_read rmw in
        let (>>>) = M.data_input_next in
        let read_rs = read_reg_ord sz rs1 ii
          >>| read_reg_ord sz rs2 ii
        ...
        let mop_success ac ma _ =
          ...
          let read_rt = read_reg_ord sz rt1 ii
                >>| read_reg_ord sz rt2 ii
        ...
        M.altT (
          ...
          lift_memop rn Dir.W true memtag mop_success
            ...
            (read_reg_ord sz rt1 ii >>> fun _ -> read_reg_ord sz rt2 ii)
            an ii
        )( (* CASP fails *)
          M.altT (
      ...
      let cas_morello sz rmw rs rt rn ii =
        ...
        lift_morello
          (fun ac ma mv ->
            ...
            let mrs = read_reg_ord sz rs ii in
            let mrt = mv in
      ...
      let ldop op sz rmw rs rt rn ii =
        ...
        lift_memop rn Dir.W true memtag
          ...
          (to_perms "rw" sz)
          (read_reg_addr rn ii)
          (read_reg_ord sz rs ii >>= tr_input)
          an ii
      ...
      let do_simd_str an sz ma rd ii =
        ma >>|
        read_reg_neon rd ii >>= fun (addr,v) ->
      ...
      let addv var r1 r2 ii =
        ...
        let rec reduce n op =
          match n with
          | 0 -> op >>| read_reg_neon_elem r2 0 ii >>= sum_elems
          | _ ->
             reduce (n-1)
               (op >>| read_reg_neon_elem r2 n ii >>= sum_elems)
        in
      ...
      let uaddv var r1 p src ii =
        ...
        read_reg_predicate p ii >>= fun pred ->
          get_predicate_any pred psize nelem >>= fun any ->
            M.choiceT
            any
            (read_reg_scalable src ii >>= fun src ->
      ...
      let add_sv r1 r2 r3 ii =
        ...
        read_reg_scalable r3 ii >>|
        read_reg_scalable r2 ii >>= fun (v1,v2) ->
          let add cur_val idx =
      ...
      let while_op compare unsigned p var r1 r2 ii =
        ...
        read_reg_ord sz r1 ii >>|
        read_reg_ord sz r2 ii >>= fun (v1,v2) ->
          ...
          ( let last idx = get_predicate_last new_val psize idx in
            ...
            let flags = n >>| z >>= fun (v1,v2) ->
              M.op Op.Or v1 v2 >>| c >>= fun (v1,v2) ->
                M.op Op.Or v1 v2 in
            flags >>= fun flags -> write_reg AArch64.NZCV flags ii
          ) >>! new_val
        ...
        read_reg_ord quad ri ii >>= fun index ->
          M.add index (V.intToV k) >>= fun slice ->
            M.op Op.Rem slice (V.intToV dim) >>= fun slice ->
              read_reg_za src ii >>|
                read_reg_predicate pg ii >>|
                  read_reg_scalable dst ii >>= fun ((src,pred),orig) ->
                    let mova cur_val idx =
                      get_predicate_last pred psize idx >>= fun last ->
                        M.choiceT
                        last
        ...
        read_reg_za acc ii >>|
          read_reg_predicate pslice ii >>|
            read_reg_predicate pelem ii >>|
              read_reg_scalable src ii >>= fun (((acc,p1),p2),src) ->
...
(******************************)
      ...
      let movk var rd k os ii =
        ...
        | V32, S_NOEXT | V64, S_NOEXT ->
            read_reg_ord sz rd ii >>= m_movk msk (V.intToV k)
        | V32, S_LSL(0|16 as s)
        | V64, S_LSL((0|16|32|48 as s)) ->
            let msk = V.op1 (Op.LeftShift s) msk in
            let v1 = V.op1 (Op.LeftShift s) (V.intToV k) in
            read_reg_ord sz rd ii >>= m_movk msk v1
        | _, S_LSL(n) ->
          Warn.fatal
 ...
 *)
      ...
      let do_store_elem an i r addr ii =
        let access_size = AArch64.simd_mem_access_size [r] in
        read_reg_neon_elem r i ii >>= demote >>= fun v ->
     ...
     (* Single structure memory access *)
      ...
      let stg d rt rn k ii =
        ...
        and mv = read_reg_ord quad rt ii >>= tag_extract in
        let do_stg ac ma mv =
          let __do_stg a v =
...
(********************)
      ...
      let do_build_semantics test inst ii =
        ...
        | I_BR r as i ->
            read_reg_ord quad r ii >>= do_indirect_jump test [] i ii

        | I_BLR r as i ->
           let v_ret = get_link_addr test ii in
           let read_rn = read_reg_ord quad r ii in
        ...
        | I_ERET ->
           ...
           commit_eret ii >>=
             fun () -> read_reg_ord quad AArch64.elr_el1 ii >>=
             eret_to_addr
        ...
        | I_MOV_TG(_,r1,r2,i) ->
            check_neon inst;
            !(read_reg_neon_elem r2 i ii >>= demote >>=
        ...
        | I_LD1SP (var,rs,p,rA,MemExt.ZReg (rM,sext,s)) ->
          ...
          !(let sz = tr_simd_variant var in
            let ma = read_reg_addr rA ii in
            let mo = read_reg_scalable rM ii in
        ...
        | I_SMSTART (None) ->
           ...
           read_reg_ord quad AArch64.SM ii >>|
           read_reg_ord quad AArch64.ZA ii >>= fun (sm,za) ->
            M.op Op.Ne sm V.one >>|
        ...
        | I_SMSTART (Some(SM)) ->
           ...
           read_reg_ord quad AArch64.SM ii >>= fun sm ->
        ...
        | I_SMSTART (Some(ZA)) ->
           ...
           read_reg_ord quad AArch64.ZA ii >>= fun sm ->
            M.op Op.Ne sm V.one >>= fun diff ->
              M.choiceT
              diff
              (List.fold_right (>>::) ops (M.unitT [()]) >>= M.ignore >>= fun () -> M.unitT (B.Next vals))
        ...
        | I_ALIGND(rd,rn,k) ->
            check_morello inst ;
            !((read_reg_ord MachSize.S128 rn ii >>=
        ...
        | I_BUILD(rd,rn,rm) ->
            ...
            !(begin
              read_reg_ord MachSize.S128 rn ii >>|
              read_reg_ord MachSize.S128 rm ii
            end >>= fun (a,b) ->
        ...
        | I_CHKSLD(rn) ->
            check_morello inst ;
            !(read_reg_ord MachSize.S128 rn ii >>= fun v ->
        ...
        | I_GC(op,rd,rn) ->
            check_morello inst ;
            !(read_reg_ord MachSize.S128 rn ii >>= begin fun c -> match op with
        ...
        | I_SC(op,rd,rn,rm) ->
            ...
            !(begin
              read_reg_ord MachSize.S128 rn ii >>|
              read_reg_ord MachSize.Quad rm ii
            end >>=
        ...
        | I_STCT(rt,rn) ->
            ...
            lift_morello
              ...
              (read_reg_addr rn ii)
              (read_reg_ord MachSize.Quad rt ii)
              Dir.W Annot.N ii
        | I_LDCT(rt,rn) ->
            check_morello inst ;
            (* NB: only 1 access implemented out of the 4 *)
        ...
        | I_RBIT (v,rd,rn) ->
            ...
            >>= nextSet rd
        | I_SXTW(rd,rs) ->
            read_reg_ord MachSize.Word rs ii
            >>=  sxtw_op
            >>= fun v -> write_reg_dest rd v ii
            >>= nextSet rd
        | I_ABS (v,rd,rs) ->
        ...
        | I_REV (rv,rd,rs) ->
           ...
           let margs =
             let sz = tr_variant v in
             let mn = read_reg_ord sz rn ii in
             begin
               ...
               | Reg (r,s) ->
                  mn >>|  (read_reg_ord sz r ii >>= opext_shift sz s)
             end in
        ...
        | I_ADDSUBEXT (v,op,r1,r2,(v3,r3),(e,ko)) ->
           let op =
             ...
             | Ext.SUBS -> SUBS in
           let sz = tr_variant v in
           let m2 = read_reg_ord sz r2 ii in
           let m3 =
             read_reg_ord (tr_variant v3) r3 ii
             >>= ext_sext e ko in
        ...
        | I_MOPL ((s,op),rd,rn,rm,ra) ->
           ...
           begin
             (read_reg_ord MachSize.Word rn ii >>= ext)
             >>| (read_reg_ord MachSize.Word rm ii >>= ext)
             >>| read_reg_ord MachSize.Quad ra ii
           end >>= fun ((vn,vm),va) ->
           M.op Op.Mul vn vm
           >>= M.op op va
           >>= fun v -> write_reg_dest rd v ii
          ...
          begin
            ...
            | _ -> begin
              ...
              | true, Some off ->
                ...
                ldr sz xt rs e ii
              | _, _ ->
                read_reg_ord sz (SysReg sreg) ii
                >>= fun v -> write_reg_dest xt v ii
                >>= nextSet xt
            end
          end
