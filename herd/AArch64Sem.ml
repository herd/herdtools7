...
(* Basic write, to register  *)
      ...
      let write_nzcv v ii =
        let write_one_bit i reg =
          M.op1 (Op.ReadBit i) v >>= fun b ->
          write_reg (AArch64.PState reg) b ii
        in
            write_one_bit 3 AArch64.PSTATE.N
        ||| write_one_bit 2 AArch64.PSTATE.Z
        ||| write_one_bit 1 AArch64.PSTATE.C
        ||| write_one_bit 0 AArch64.PSTATE.V

      let write_nzcv_tuple (n, z, c, v) ii =
             ...
             match op_set_flags op v with
              ...
              | Some get_flags ->
                 let compute_and_write_flags res v1 v2 =
                   ...
                   write_nzcv_tuple flags ii >>= fun li ->
                   M.unitT (Some li)
                 in
                 fun (v1, v2 as p) ->
          ...
          ( let last idx = get_predicate_last new_val psize idx in
            ...
            let flags = n >>| z >>= fun (v1,v2) ->
              M.op Op.Or v1 v2 >>| c >>= fun (v1,v2) ->
                M.op Op.Or v1 v2 in
            flags >>= fun flags -> write_nzcv flags ii
          ) >>! new_val
