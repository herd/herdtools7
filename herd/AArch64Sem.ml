...
(* Pointer Anthentication Code `FEAT_Pauth2` *)
        | I_PAC (key, rd, rn) ->
            if pac then begin
              ...
              (* Here the variant `pac` represent the activation of the key, and
                `pac* Xd,Xn` is implemented by `mov Xd,Xd` if `SCTLR_EL1.En* = 0`*)
        ...
        | I_AUT (key, rd, rn) ->
            if pac then begin
              ...
              (* Here the variant `pac` represent the activation of the key, and
                `aut* Xd,Xn` is implemented by `mov Xd,Xd` if `SCTLR_EL1.En* = 0`*)
              read_reg_ord rd ii >>= fun v ->
              write_reg_dest rd v ii >>= fun v ->
              B.nextSetT rd v
