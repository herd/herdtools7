      ...
      let lift_kvm dir updatedb mop ma an ii mphy instr_at =
        let mfault ma a ft =
          ...
          if not instr_at then
            ...
            let parel1_v = V.Val (Constant.AddrReg (V.Cst.AddrReg.default 1)) in
            (* address translation faults are asynchronously reported in PAR_EL1 *)
            let set_par_el1 = write_reg AArch64Base.par_el1 parel1_v ii in
                let ma = ma >>= (fun a -> (set_par_el1) >>! a) in ma >>! B.Next []
        in
        let maccess a ma =
 ...
 *)
      ...
      let do_at op rd ii =
        ...
        lift_memop rd dir false memtag
          (fun _ ma _ -> ma >>= (fun a -> write_reg_dest sreg a ii))
      ...
      let cas sz rmw rs rt rn ii =
        ...
        M.altT (
          ...
          lift_memop rn Dir.W true memtag mop_success (to_perms "rw" sz)
            (read_reg_ord rn ii) (read_reg_data sz rt ii) an ii false
        )( (* CAS fails *)
          M.altT (
        ...
        if AArch64Base.DC.sw op then
          M.mk_singleton_es (Act.CMO (AArch64.CMO.DC op, None)) ii >>= B.next1T
            ...
            lift_memop rd dir false false
              ...
              (read_reg_ord rd ii) mzero Annot.N ii false
          end
