...
(***********************)
      ...
      and lsr_op k = do_shift (Op.LogicalRightShift k) k
      and asr_op k = do_shift (Op.ArithRightShift k) k

      let ror_op sz k v =
        let n = MachSize.nbits sz in
        let m = k mod n in
        begin lsr_op m v >>| lsl_op (n-m) v end
...
(* Complete effective adress computation *)
      ...
      let get_ea_reg rs _v ri sext s ii =
        let open AArch64Base.MemExt in
        (read_reg_ord rs ii >>| read_reg_ord ri  ii)
        >>= fun (v1,v2) -> memext_sext sext s v2
        >>= fun v2 -> M.add v1 v2
