 ...
 *)
     ...
     (* Single structure memory access *)
      ...
      let fault_if_streaming action ii =
        let streaming =
          let open FaultType.AArch64 in
          SME Streaming in
        let sm = if sme then read_reg_ord AArch64.(PState PSTATE.SM) ii else mzero in
        sm >>= fun sm ->
          M.choiceT sm (mk_sme_fault streaming ii) action

      let fault_if_not_streaming action ii =
        let not_streaming =
          let open FaultType.AArch64 in
          SME NotStreaming in
        let sm = if sme then read_reg_ord AArch64.(PState PSTATE.SM) ii else mzero in
        sm >>= fun sm ->
          M.choiceT sm action (mk_sme_fault not_streaming ii)

      let fault_if_inactive_za action ii=
        let open FaultType.AArch64 in
        let inactive_za =
          let open FaultType.AArch64 in
          SME InactiveZA in
        let za = if sme then read_reg_ord AArch64.(PState PSTATE.ZA) ii else mzero in
        za >>= fun za ->
          M.choiceT za action (mk_sme_fault inactive_za ii)
