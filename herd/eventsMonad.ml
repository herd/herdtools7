...
(* For combining conditions and branches of an if, as above + instruction dependencies *)
    ...
    let add_addr_ports st = { st with E.addr_ports = st.E.events; }
    let add_success_ports st = { st with E.success_ports = st.E.events; }

    let as_addr_port : 'a t -> 'a t =
      let f (a, cs, es) = (a, cs, add_addr_ports es) in
      fun m -> map_elt f m

    let do_make_one_event_structure_addr is_addr =
      if is_addr then
        fun a iiid -> do_make_one_event_structure a iiid -- add_addr_ports
      else
    ...
    let do_read_loc is_addr mk_action loc iiid = fun eiid ->
      ...
      let v = match iiid,loc with
          ...
          | _ -> V.fresh_var () in
      let m =
        do_make_one_event_structure_addr is_addr (mk_action loc v) iiid ++
        make_one_monad v [] in
      m eiid
...
(* Build mixed size event structure *)
      ...
      let read_mixed is_addr sz mk_act a ii =
        fun eiid ->
          ...
          let st =
            if is_addr then { st with E.addr_ports = es } else st in
