...
(* For combining conditions and branches of an if, as above + instruction dependencies *)
    ...
    let read_loc is_data mk_action loc ii = fun eiid ->
      ...
      let m =
        make_one_event_structure_data is_data (mk_action loc v) ii ++
        make_one_monad v [] in
      m eiid
...
(* Build mixed size event structure *)
        ...
        fun eiid ->
          ...
          let eiid,es =
            fold_pp
              (fun es (ea,v) ->
                make_one_event
                  (mk_act SZ.byte (A.Location_global ea) v) ii --
                add_evt es)
              E.EventSet.empty eavs
            eiid in
          let eiid,e_full =
            make_one_event (mk_act sz (A.Location_global a) v) ii eiid in
      ...
      let write_mixed sz mk_act a v ii =
        fun eiid ->
          ...
          let eiid,e_full =
            make_one_event
              (mk_act sz (A.Location_global a) v) ii eiid in
          let st = make_mixed es e_full in
