...
(* For combining conditions and branches of an if, as above + instruction dependencies *)
    ...
    let set_port port es =
      E.EventSet.fold
        (fun e k -> E.EventMap.add e port k) es E.EventMap.empty
    ...
    let as_port : Port.t -> 'a t -> 'a t =
      function
      | Port.No -> Fun.id
      | port ->
          let f (a, cs, es) = (a, cs, add_ports port es) in
          fun m -> map_elt f m

    let as_addr_port : 'a t -> 'a t = fun m -> as_port Port.Addr m

    let do_make_one_event_structure_ports port a iid =
      do_make_one_event_structure a iid -- add_ports port
    ...
    let do_read_loc port mk_action loc iiid = fun eiid ->
      ...
      let v = match iiid,loc with
          ...
          | _ -> V.fresh_var () in
      let m =
        do_make_one_event_structure_ports port (mk_action loc v) iiid ++
        make_one_monad v [] in
      m eiid

    let read_loc port mk_action loc ii =
      do_read_loc port mk_action loc (E.IdSome ii)
...
(* Build mixed size event structure *)
      ...
      let read_mixed port sz mk_act a ii =
        fun eiid ->
...
(* Notice that size does not depend upon offset.
   ...
   share the same size *)
    ...
    let assume_collision v1 v2 =
      make_one_monad ()
      [VC.Predicate (A.pred_assume_collision v1 v2)] E.empty_event_structure

    let assume_no_collision v1 v2 =
      make_one_monad ()
      [VC.Predicate (A.pred_assume_no_collision v1 v2)] E.empty_event_structure
