...
and type arch_pred = A.arch_pred
and type state = A.state =
  struct
    ...
    let pp_cnstrnt cnstr =  match cnstr  with
      | Predicate pred -> A.pp_predicate pred
      | Assign (v,rval) ->
    ...
    let fold_predicate_vars f pred acc =
      A.fold_predicate_vars f pred acc
...
(* Collect all variables in partition *)
    ...
    let fold_vars_eq f e t =
      ...
      | Predicate pred -> A.fold_predicate_vars (fold_var f) pred t
      | Failed _ | Warn _ -> t
...
(* Simplify equations *)
    ...
    let subst_cn m cn =
      ...
      | Predicate pred ->
        let pred = A.map_predicate (subst_atom m) pred in
        Some (Predicate pred)
...
(* Phase 3, substitution *)
    ...
    let simplify_vars_in_cnstrnt soln cn =
      ...
      | Assign (v,rval) ->
          ...
          Assign (v,rval)
      | Predicate pred ->
          Predicate (A.map_predicate (simplify_vars_in_atom soln) pred)
      | Failed _ | Warn _ -> cn
...
(*********************************)
    ...
    *)
    ...
    let solve_one c sol eqs =
      ...
      | Predicate pred ->
        let pred = A.map_predicate (simplify_vars_in_atom sol) pred in
