...
module type S = sig
  ...
  type answer = (solution * cnstrnts * arch_solver_state) answer_gen

  val pp_answer : answer -> string

  val fold_predicate_vars : (atom -> 'a -> 'a) -> arch_pred -> 'a -> 'a
...
and type arch_solver_state = A.CS.arch_solver_state
...
and type state = A.state =
  struct
    ...
    type arch_pred = A.CS.arch_pred
    type arch_solver_state = A.CS.arch_solver_state
    ...
    let pp_cnstrnt cnstr =  match cnstr  with
      | Predicate pred -> A.CS.pp_predicate pred
      | Assign (v,rval) ->
    ...
    let fold_predicate_vars f pred acc =
      A.CS.fold_predicate_vars f pred acc
    ...
    let solve_predicates cns =
      let rec do_rec solver = function
        | Predicate pred::cns -> begin
          match A.CS.add_predicate pred solver with
...
(* Collect all variables in partition *)
    ...
    let fold_vars_eq f e t =
      ...
      | Predicate pred -> A.CS.fold_predicate_vars (fold_var f) pred t
      | Failed _ | Warn _ -> t
...
(* Simplify equations *)
    ...
    let assign_vals v w =
      match (v, w) with
      | (V.Val _ as val_), (V.Var _ as var_) -> Assign (var_, Atom val_)
      | _ -> Assign (v, Atom w)

    let assign_expr v e =
      match (v, e) with
      | (V.Val _ as val_), Atom (V.Var _ as var_) -> Assign (var_, Atom val_)
      | _ -> Assign (v, e)

    let subst_cn m cn =
      match cn with
      | Assign (v, Atom w) ->
          let v = subst_atom m v and w = subst_atom m w in
          if V.compare v w = 0 then None else Some (assign_vals v w)
      | Assign (v, e) ->
          let v = subst_atom m v and e = subst_expr m e in
          Some (assign_expr v e)
      | Predicate pred ->
        let pred = A.CS.map_predicate (subst_atom m) pred in
        Some (Predicate pred)
...
(* Phase 3, substitution *)
    ...
    let simplify_vars_in_cnstrnt soln cn =
      ...
      | Assign (v,rval) ->
          ...
          Assign (v,rval)
      | Predicate pred ->
          Predicate (A.CS.map_predicate (simplify_vars_in_atom soln) pred)
      | Failed _ | Warn _ -> cn
...
(* Phase 2, "solving": just collect equations S := cst / cst := S *)
    ...
    (** [add_sol x cst (sol, csn)] updates [sol] to map to [cst], raising [Contradiction] in case of conflicting solutions, and adding a predicate to [csn] if needed. *)
    let add_sol =
      let exception FoundPredicate of arch_pred in
      let unify_solution cst cst' =
        if V.Cst.eq cst cst' then Some cst' else
          match A.CS.eq_satisfiable cst cst' with
          | Some pred -> raise (FoundPredicate pred)
      ...
      fun x cst (sol, csn) ->
        try
          let sol =
            V.Solution.update x
              (function None -> Some cst | Some cst' -> unify_solution cst cst')
              sol
          in
          (sol, csn)
        with FoundPredicate pred -> (sol, Predicate pred :: csn)
...
(*********************************)
    ...
    *)
    ...
    let solve_one c sol eqs =
      ...
      | Predicate pred ->
        let pred = A.CS.map_predicate (simplify_vars_in_atom sol) pred in
        sol, Predicate pred :: eqs
      | Assign (v0,e) ->
         begin
           let () =
             if debug > 1 then
    ...
    let topo_step =
      ...
      let solve_step sol scc =
        List.fold_right (fun c (sol,scc) -> solve_one c sol scc)
          scc (sol, [])
      in
      (* Iterate solve, until fixpoint *)
      let rec solve_rec sol0 scc0 acc =
        let sol, scc = solve_step sol0 scc0 in
        (* same_eqs scc0 scc implies sol == sol0, but the physical test is
           faster, so we use it as first approximation. *)
        if sol == sol0 && same_eqs scc0 scc
        then (sol, scc @~ acc)
        else solve_rec sol scc acc
      in
      (* General function, we distinguish the simple case where the scc is a singleton. *)
      fun cs (sol, eqs) ->
        match cs with
        | [] -> assert false
        | [c] -> solve_one c sol eqs
        | scc -> solve_rec sol scc eqs
    ...
    let solve_topo_step cs =
      ...
      if debug > 0 then begin
        ...
        *)
        ...
        debug_topo stderr cs r
      end ;
      EqRel.scc_kont topo_step (V.Solution.empty,[]) cs r
