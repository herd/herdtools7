...
module Make (TopConf : AArch64Sig.Config) (V : Value.AArch64ASL) :
  ...
  module Mixed (SZ : ByteSize.S) : sig
    ...
    let decode_inst ii =
      ...
      | I_LDXP (v,t,rt,rt2,rn) ->
          let acqrel, fname =
            match t with
            | XP -> false, "LDXP_LP32_ldstexclp.opn"
            | AXP -> true, "LDXAP_LP32_ldstexclp.opn"
          in
          let acqrel = acqrel && rt != ZR && rt2 != ZR in
          Some
            ( "ldst/ldstexclp/" ^ fname,
              stmt [
                  "t" ^= reg rt;
                  "t2" ^= reg rt2;
                  "n" ^= reg rn;
                  "elsize" ^= variant v;
                  "datasize" ^= liti (2 * variant_raw v);
                  "acqrel" ^= litb acqrel;
                  "tagchecked" ^= litb (rn <> SP);
                  "rt_unknown" ^= litb false;
              ])
