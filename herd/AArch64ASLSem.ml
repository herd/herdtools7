  ...
  module Mixed (SZ : ByteSize.S) : sig
    ...
    module Translator : sig
      ...
      let tr_execution ii (conc, cs, set_pp, vbpp) =
        ...
        let is_bcc =
          let iico_ctrl =
            match List.assoc_opt  aarch64_iico_ctrl vbpp with
            | Some rel -> rel
            | None -> ASLE.EventRel.empty in
          let bcc =
            ASLE.EventRel.fold
              (fun (e1,e2) k ->
                if
                  ASLE.is_commit e1
                  && ASLE.is_reg_store_any e2
                  &&
                    begin
                      match ASLE.location_reg_of e2 with
                      | Some (ASLBase.ArchReg AArch64Base.PC)
                        -> true
                      | _ -> false
                    end
                then ASLE.EventSet.add e1 k
                else k)
              iico_ctrl ASLE.EventSet.empty in
