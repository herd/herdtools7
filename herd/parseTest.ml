...
module Top (TopConf : sig
  ...
  (* This preserves the old behaviour where graph data was collected when
     herd output was directed to stdout or an output directory. The flag is
     exposed here only because [Top] no longer depends on the concrete output
     configuration.

     Filippo: I think this is an awkward optimization knob for a library-facing
     API. Its performance benefit should be established, and if it is not
     useful in practice, this flag should be removed. *)
end) = struct
      ...
      let () =
        ...
        if pauth1 && pauth2 then begin
          let msg =
            ...
            "\"pauth1\" represents \"FEAT_PAuth\" without " ^
            "\"FEAT_PAuth2\", while \"pauth2\" represents both " ^
