  ...
  Make
	...
	module Mixed(SZ : ByteSize.S) = struct
      ...
      let read_loc is_addr mo =
        M.read_loc is_addr (fun loc v -> Act.Access (Dir.R, loc, v, mo, nat_sz))

      let read_reg is_addr r ii =
        read_loc is_addr no_mo (A.Location_reg (ii.A.proc,r)) ii

      let read_mem is_addr mo a =
        read_loc is_addr mo (A.Location_global a)
      ...
      let rec build_semantics_expr is_addr e ii : V.v M.t =
        match e with
        | JavaBase.LoadReg reg -> read_reg is_addr reg ii

        | JavaBase.LoadMem (vh, am) -> (read_reg true vh ii) >>=
                              (fun l -> read_mem is_addr am l ii)
        ...
        | JavaBase.CAS (vh, (read_am, write_am), expect, dest) ->
              (read_reg true vh ii) >>= fun loc_vh ->
              (build_semantics_expr false expect ii) >>= fun v_expect ->
              (read_mem false read_am loc_vh ii) >>*= fun v_vh ->
              M.altT
                ((M.neqT v_vh v_expect) >>! v_vh)
                ((build_semantics_expr false dest ii) >>= fun v_dest ->
                    (M.mk_singleton_es
      ...
      let rec build_semantics test ii : (A.program_order_index * B.t) M.t =
        let ii =
            ...
            | JavaBase.StoreReg (reg, exp) -> (
                        (build_semantics_expr false exp ii) >>=
