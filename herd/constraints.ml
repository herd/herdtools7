...
module type S = sig
  ...
  type final_state = A.rstate * A.FaultSet.t * A.CS.arch_solver_state
...
(* pretty_print *)
        ...
        module Mixed (SZ : ByteSize.S) = struct
          ...
          type 'a msolver = A.CS.arch_solver_state -> ('a * A.CS.arch_solver_state) list
          let (let*) x f = fun st ->
            Misc.concat_map (fun (a,s) -> f a s) (x st)
          let pure : 'a -> 'a msolver = fun x st -> [x,st]
          let contradiction : 'a msolver = fun _ -> []
          let test_cond c = if c then pure () else contradiction
          let alt xs = fun st -> Misc.concat_map (fun x -> x st) xs
          let rec seq : unit msolver list -> unit msolver = function
            | x :: xs ->
                let* _ = x in
                seq xs
            | [] -> pure ()

          let add_equality x y : unit msolver = fun st ->
            match x, y with
            | V.Val c1, V.Val c2 -> begin
              match A.CS.eq_satisfiable c1 c2 with
              | Some pred -> begin
                match A.CS.add_predicate pred st with
          ...
          let add_inequality x y : unit msolver = fun st ->
            match x, y with
            | V.Val c1, V.Val c2 -> begin
              match A.CS.eq_satisfiable c1 c2 with
              | Some pred -> begin
                match A.CS.add_predicate (A.CS.enforce_no_collision pred) st with
                | None -> contradiction st
                | Some st -> pure () st
              end
              | None ->
          ...
          let normalize_fatom : A.fatom -> A.fatom msolver = fun fatom st ->
            [A.map_fatom (V.map_const (fun cst -> A.CS.normalize cst st)) fatom,st]

          let normalize_flts : A.FaultSet.t -> A.FaultSet.t msolver = fun flts st ->
            let flts =
              A.FaultSet.map (A.map_fault (V.map_const (fun cst ->
                A.CS.normalize cst st))) flts
            in [flts,st]
          ...
          let do_check_prop solver look_type look_val flts =
            let rec do_rec sign p : unit msolver = match p with
              ...
              | Or ps ->
                  ...
                  else seq (List.map (do_rec sign) ps)
              | And ps ->
                  if sign
                  then seq (List.map (do_rec sign) ps)
