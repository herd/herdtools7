...
Option `-restrict observed` restricts executions to only those 'observed' by the post-condition,
and suppresses output on 0 executions.

  $ herd7 -set-libdir ../libdir MP-true.litmus -restrict none | sed '/^Time/d'
  Test MP-true Required
  States 1
  ...
  Observation MP-true Always 4 0
  Hash=91e4b747d0f94b9b380b24f9db7c989f
  
  $ herd7 -set-libdir ../libdir MP-true.litmus -restrict observed

Option `-restrict none` prints stdout stats when there are no executions.

  $ herd7 -set-libdir ../libdir Loop-zero.litmus -restrict none -candidates true | sed '/^Time/d'
  Warning: File "Loop-zero.litmus": unrolling limit exceeded at L0, legal outcomes may be missing.
  Test Loop-zero Required
  States 0
  Loop Ok
  Witnesses
  Positive: 0 Negative: 0
  Condition forall (true)
  Observation Loop-zero Never 0 0
  Candidates Loop-zero 0
  Hash=89723a07ff97592f57f8dbfa0871cfdd
  

Option `-restrict nonambiguous` suppresses stdout if multiple executions result in the same final state.

  $ herd7 -set-libdir ../libdir MP+CAS-rfi-ctrl+acq.litmus -outcomereads false -restrict none | sed '/^Time/d'
  ...
  $ herd7 -set-libdir ../libdir MP+CAS-rfi-ctrl+acq.litmus -outcomereads false -restrict nonambiguous

Option `-restrict condone` suppresses stdout if the number of positive executions matches the number of final states satisfying the test condition.

  $ herd7 -set-libdir ../libdir MP+CAS-rfi-ctrl+acq.litmus -outcomereads false -restrict condone

Option `-restrict` has no impact on DOT graph printing.

  $ herd7 -set-libdir ../libdir MP+CAS-rfi-ctrl+acq.litmus -outcomereads false -restrict condone -show prop -o - | grep -E 'digraph'
  digraph G {
  digraph G {
