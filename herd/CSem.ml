...
(****************************)
    ...
    module Mixed(SZ : ByteSize.S) = struct
      ...
      let read_loc is_addr mo =
        M.read_loc is_addr (fun loc v -> Act.Access (Dir.R, loc, v, mo, false, nat_sz))

      let  read_exchange is_addr vstored mo a ii =
        M.read_loc is_addr
      ...
      let xchg is_addr rloc re a ii =
        ...
        and ar =
          (if lkmmv1 then
            ...
            MOorAN.AN a) in
        let rmem = fun loc -> read_mem_atomic is_addr ar loc ii
      ...
      let cxchg is_addr rloc re mo v_loc ii =
        let m = match mo with
        | MemOrder.SC
        | MemOrder.Rlx -> (mo, mo)
        ...
        | None -> fun loc -> read_mem_atomic is_addr (MOorAN.MO (fst m)) loc ii
        | Some x -> fun loc -> read_mem_atomic_known is_addr (MOorAN.MO (fst m)) loc x ii
      ...
      let rec build_semantics_expr is_addr e ii : V.v M.t = match e with
      | C.Const v -> M.unitT (V.maybevToV v)
      | C.LoadReg r -> read_reg is_addr r ii
      | C.LoadMem(loc,mo) ->
          let open MemOrderOrAnnot in
          (match mo with
          | AN [] | MO _ -> build_semantics_expr is_addr loc ii
          | AN (_::_) ->  begin match loc with
            | C.LoadMem (loc,AN []) ->
                build_semantics_expr is_addr loc ii
            | _ ->
...
(* Cannot do this with a macro, by lack of sequencing expression operator *)
                read_mem is_addr an_once l ii >>*=
                fun v -> mk_rb_dep ii >>! v
            | _ ->
                read_mem is_addr mo l ii
            end
      ...
      | C.Op(op,e1,e2) ->
          (build_semantics_expr is_addr e1 ii >>|
          build_semantics_expr is_addr e2 ii) >>= fun (v1,v2) ->
            M.op op v1 v2

      | C.Exchange(l,e,(MOorAN.AN a)) ->
          let re = build_semantics_expr false e ii
          and rloc =  build_semantics_expr true l ii in
          xchg is_addr rloc re a ii
      ...
      | C.Exchange(l,e,MOorAN.MO mo) ->
          if Conf.variant Variant.NoRMW then
            let re = build_semantics_expr false e ii
            and rloc = build_semantics_expr true l ii in
            cxchg is_addr rloc re mo None ii
          else
            (build_semantics_expr false e ii >>|
            build_semantics_expr true l ii)
              >>= (fun (v,l) ->
                read_exchange is_addr v mo l ii)

      | C.CmpExchange (eloc,eold,enew,a) ->
          let mloc =  build_semantics_expr true eloc ii
          and mold =  build_semantics_expr false eold ii in
          let add_mb r =
            (if lkmmv1 then
              (match a with
              | ["mb"] ->
          ...
          M.altT
            (let r =
              let mnew = build_semantics_expr false enew ii
              and rmem vloc =
                read_mem_atomic false arok vloc ii
              and wmem vloc w =
      ...
      | C.ECas(obj,exp,des,success,failure,strong) ->
          ...
          build_semantics_expr true exp ii >>= fun loc_exp ->
            (* Obtain location of object *)
            build_semantics_expr true obj ii >>= fun loc_obj ->
              (* Non-atomically read the value at "expected" location *)
              read_mem false no_mo loc_exp ii >>*= fun v_exp ->
                (* Non-deterministic choice *)
                M.altT
                  (read_mem false (mo_as_anmo failure) loc_obj ii >>*= fun v_obj ->
                  ...
                  (build_semantics_expr false des ii >>= fun v_des ->
                    if Conf.variant Variant.NoRMW then
                      let re = build_semantics_expr false des ii
                      and rloc = build_semantics_expr true obj ii in
                      cxchg is_addr rloc re success (Some v_exp) ii >>!
                      V.one
                    else
      ...
      | C.AtomicAddUnless (eloc,ea,eu,retbool,a) ->
          (* read arguments *)
          let mloc = build_semantics_expr true eloc ii
          and mu =  build_semantics_expr false eu ii
          and mrmem loc =
            read_mem_atomic false
              (if lkmmv1 then an_once else (MOorAN.AN a)) loc ii in
          M.altT
            (let r =
              M.linux_add_unless_ok mloc (build_semantics_expr false ea ii)
                mu mrmem
                (fun loc v ->
                   write_mem_atomic
      ...
      | C.ExpSRCU(eloc,a) ->
          ...
          | _ -> None in
          build_semantics_expr true eloc ii >>=
          fun (vloc) ->
      ...
      and build_atomic_op ret a_read a_write eloc op e ii =
        ...
        (build_semantics_expr true eloc ii >>=
         fun loc ->
           (read_mem_atomic false (MOorAN.AN a_read) loc ii >>| M.unitT loc))
      ...
      let rec build_semantics test ii : (A.program_order_index * B.t) M.t =
        ...
        | C.StoreMem(loc,e,mo) ->
            (begin
              let open MemOrderOrAnnot in
              match mo with
              | AN [] | MO _ ->  build_semantics_expr true loc ii
              | AN (_::_) -> match loc with
                | C.LoadMem (loc,AN []) -> build_semantics_expr true loc ii
                | _ ->
