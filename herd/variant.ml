...
let pp = function
  ...
  | ETS -> "Enable Enhanced Translation Synchronization (deprecated)"
  | ETS2 -> "Enable FEAT_ETS2"
  | ETS3 -> "Enable FEAT_ETS3"
  | ExS -> "Enable Enhanced Exception Synchronization (FEAT_ExS; AArch64 only)"
  ...
  | ConstPacField -> "When using pointer authentication, enable FEAT_CONSTPACFIELD (AArch64 only)"
