...
(* Scalable vector Extension *)
    ...
    | Preg (_,s) -> "^" ^ io ^ string_of_int (i+offset) ^
       (try Misc.lowercase (List.assoc s sve_arrange_specifier) with Not_found -> assert false)
    | PMreg (_,s) -> "^" ^ io ^ string_of_int (i+offset) ^
       (try Misc.lowercase (List.assoc s sve_pred_modifier) with Not_found -> assert false)
    | _ -> assert false
