        ...
        let module DC =
          CompCond.Make(O)
            (struct
              ...
              | Constant.Symbolic (Constant.Virtual {Constant.name=s; Constant.tag=Some(t); _}) ->
      ...
      let init_mem_loc indent clean env test a =
        ...
        let pp_const v =
          ...
          | Symbolic (Virtual {name=s; tag=Some t; offset=0; _}) ->
            sprintf "tagged((%s)_vars->%s,%d)" (CType.dump at) s (Misc.int_of_tag t)
          | Label (p,lbl) ->
          ...
          A.RLocSet.iter
            (fun rloc ->
               if U.is_rloc_label rloc env then
                 ...
                 O.fii "%s = tagged(idx_addr((intmax_t *)%s,_vars),tag_of(%s));"
