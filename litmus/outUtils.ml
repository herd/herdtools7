...
let fmt_lbl_instr (p,lbl) = sprintf "instr_P%i_%s" p lbl
let fmt_lbl_instr_offset (p,lbl) = sprintf "off_instr_P%i_%s" p lbl
...
module Make(O:Config)(V:Constant.S) = struct
  ...
  | Symbolic (Virtual {name=Symbol.Data a;tag=None;cap=0L;offset=0;}) -> dump_addr a
