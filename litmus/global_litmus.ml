...
let tr_symbol =
  ...
  function
    | C.Virtual {C.name=s; C.tag=None; C.cap=0L; C.offset=0; _} -> Addr s
    | C.Virtual {C.name=s; C.tag=Some(t); C.cap=0L; C.offset=0; _} -> AddrT (s,Misc.int_of_tag t)
...
let get_base_symbol =
 let open Constant in
  function
    ...
    | Virtual {name=s; tag=Some t; cap=0L; _ } -> AddrT (s,Misc.int_of_tag t)
    | Physical (s,_) -> Phy s
    | System (PTE,s) -> Pte s
