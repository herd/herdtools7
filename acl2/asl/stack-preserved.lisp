...
(defret <fn>-stack-preserved
  (equal (global-env->stack_size (env->global new-env))
         (global-env->stack_size (env->global env)))
...
(defret <fn>-stack-preserved
  (and (implies (env_result-case res :lk_local)
                (equal (global-env->stack_size (env->global (lk_local->val res)))
                       (global-env->stack_size (env->global env))))
       (implies (env_result-case res :lk_global)
                (equal (global-env->stack_size (env->global (lk_global->val res)))
                       (global-env->stack_size (env->global env)))))
  :hints(("Goal" :in-theory (enable <fn>)))
  :fn env-assign)
...
;; (defret <fn>-preserves-static-env
;;   (equal (global-env->stack_size (env->global new-env))
;;          (global-env->stack_size call-env))
;;   :hints(("Goal" :in-theory (enable <fn>)))
;;   :fn env-pop-stack)

(local (include-book "centaur/vl/util/default-hints" :dir :system))

(local (in-theory (enable env-pop-stack env-push-stack)))

(progn 
  (defconst *stack-preserved-form*
    '(std::defret-mutual-generate <fn>-stack-preserved
       :return-concls (((expr_eval_result-p res)
                        (implies (eval_result-case res :ev_normal)
                                 (equal (global-env->stack_size
                                         (env->global
                        ...
                        (implies (eval_result-case res :ev_normal)
                                 (b* ((res (ev_normal->res res)))
                                   (and (implies (control_flow_state-case res :returning)
                                                 (equal (global-env->stack_size (returning->env res))
                                                        (global-env->stack_size (env->global env))))
                                        (implies (control_flow_state-case res :continuing)
                                                 (equal (global-env->stack_size (env->global (continuing->env res)))
                                                        (global-env->stack_size (env->global env))))))))
