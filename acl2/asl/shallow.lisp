...
(defines expr-vars
    ...
    (b* ((x (expr->desc x)))
      (expr_desc-case x
        ...
        :e_pattern (union (expr-vars x.expr)
                          (pattern_matcher-vars x.pattern))
        :otherwise nil)))
  ...
  (define pattern_matcher-vars ((x pattern_matcher-p))
    :measure (pattern_matcher-count x)
    :returns (vars (and (identifierlist-p vars)
                        (setp vars)))
    (patternlist-vars (pattern_matcher->patterns x)))
