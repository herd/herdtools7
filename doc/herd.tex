...
a possible computation of coherence orders\footnote{The actual computation of coherence orders slightly differs. Interested readers can look at the
...
Executing the model means allowing or forbidding that candidate
execution.
...
their name is always uppercase and derives from the mnemonic of
...
the second event is in a branch controlled by the value loaded by the
...
\subsubsection*{\aname{operator}{Operator} expressions}
...
In practice, the list of instructions defined by the included model file
are executed.
...
identify last writes in coherence orders, and pass the model a
...
It defines a few convenient relations that are thus
...
second event additionally contains
a \texttt{isync} fence preceding that instruction.
...
{\tt gv} viewer.
\item[{\tt -dumpes <bool>}]
Dump generated event structures and exit. Default is \opt{false}.
...
jumps $n$ times. This is a workaround for one of \herd{} main limitations:
...
false and the unskipped checks yield true. This option comes in handy
when one wants to observe the executions that fail one (or several) checks
...
tests that perform accesses of different sizes, as well as indexed accesses.
...
has no effect on colors given explicitly with the
...
When the size of nodes is not fixed (\emph{i.e.} \opt{-fixedsize false} and default), \herd{} computes the width of nodes by counting characters in node
...
However, some edges can be controlled only with options (or configuration
...
This setting applies at the very last moment and thus cancels any
...
Notice that the argument is an arbitrary string, so as to allow DOT general
...
\item[{\tt -oknames <name,...,name>}] Run \herd{} only on tests whose names are
given as a comma-separated list.
\item[{\tt -excl <file>}] Do \emph{not} run \herd{} on tests whose names are
listed in \texttt{<file>}.
\item[{\tt -nonames <name,...,name>}] Do \emph{not} run \herd{} on tests whose names are given as a comma-separated list.
...
it results in adding the directory name of the
configuration file to the search path.
...
the final test condition, while \ahrefloc{opt:doshow}{\opt{-doshow prop}}
