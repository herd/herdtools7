...
\item[{\tt -oknames <name,...,name>}] Run \litmus{} only on tests whose
names are given as a comma-separated list.
\item[{\tt -excl <file>}] Do \emph{not} run \litmus{} on tests whose names are
listed in \texttt{<file>}.
\item[{\tt -nonames <name,...,name>}] Do \emph{not} run \litmus{} on tests whose names are given as a comma-separated list.
...
\begin{description}
\item[{\tt -names <file>}] Run \klitmus{} only on tests whose names are
listed in \texttt{<file>}.
\item[{\tt -oknames <name,...,name>}] Run \klitmus{} only on tests whose
names are given as a comma-separated list.
\item[{\tt -excl <file>}] Do \emph{not} run \klitmus{} on tests whose names are
listed in \texttt{<file>}.
\item[{\tt -nonames <name,...,name>}] Do \emph{not} run \klitmus{} on tests whose names are given as a comma-separated list.
\item[{\tt -rename <file>}] Change test names.
...
\begin{verbatim}
% litmus7 -mach kvm-aarch64 -o M.tar MP-TTD+DMB.ST+DMB.LD.litmus  MP-TTD+DMB.ST+DSB-ISB.litmus
% scp M.tar MACH:
\end{verbatim}
...
Histogram (4 states)
1717461:>1:X7=0; ~fault(P1:L0,x,MMU:Translation);
127644:>1:X7=1; ~fault(P1:L0,x,MMU:Translation);
167   *>1:X7=1; fault(P1:L0,x,D-MMU:Translation);
154728:>1:X7=0; fault(P1:L0,x,D-MMU:Translation);
Ok

Witnesses
Positive: 167, Negative: 1999833
Condition exists (1:X7=1 /\ fault(P1:L0,x,MMU:Translation)) is validated
Hash=e6d0396aa32c40b307db5f6921404c07
EL0=P1
Observation MP-TTD+DMB.ST+DMB.LD Sometimes 167 1999833
Topology 167   :> part=0 [[0],[1]]
Faults MP-TTD+DMB.ST+DMB.LD 154895 P1:154895
Time MP-TTD+DMB.ST+DMB.LD 2.67
...
Test MP-TTD+DMB.ST+DSB-ISB Allowed
Histogram (3 states)
1816612:>1:X7=0; ~fault(P1:L0,x,MMU:Translation);
132549:>1:X7=1; ~fault(P1:L0,x,MMU:Translation);
50839 :>1:X7=0; fault(P1:L0,x,D-MMU:Translation);
No
...
Faults MP-TTD+DMB.ST+DSB-ISB 50839 P1:50839
Time MP-TTD+DMB.ST+DSB-ISB 2.67
...
\end{verbatim}
...
Synonymous: \texttt{precise}, \texttt{sync} and~\texttt{synchronous}.
...
table entry.
Synonymous: \texttt{imprecise}, \texttt{async}  and~\texttt{asynchronous}.
