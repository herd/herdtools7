...
the \texttt{-a}, \texttt{-s} and \texttt{-a} setting, to which we add
the \texttt{-n <num>} option (number of instances).
...
Observe that non-implemented options are
silently ignored.
...
instruction \texttt{LDR X1,[X2]}, where the register \texttt{X1} holds
the valid descriptor and the register \texttt{X2} points to the page table
entry of~\texttt{x}. The two tests differ by their synchronisation
...
Compilation to qemu binary file arguments implies many settings. For
user convenience, we provide litmus configuration
