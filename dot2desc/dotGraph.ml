...
let tr_stmt acc stmt param_map =
  ...
  | ParsedStmt.Node n ->
    ...
    if is_init_event n then
      ...
      let require_address_reg f name =
        let func loc reg is_explicit = match reg with
      ...
      let tag_write = require_address_reg DescDict.tag_read "tag write" in
      let pte_read = require_address_reg DescDict.tag_read "PTE read" in
      let pte_write = require_address_reg DescDict.tag_read "PTE write" in
      let pa_read = require_address_reg DescDict.tag_read "PA read" in
      let pa_write = require_address_reg DescDict.tag_read "PA write" in
