...
module Make(C:Builder.S)
    ...
    =
  struct
    let debug fmt = O.Debug.debug Debug_gen.Top fmt
...
(* Prefix *)
    ...
    let extract_po rs =
      ...
      | m ->
          fun _ _ _ ->
            Warn.fatal "Function po_safe called in mode %s\n" (pp_check m)
...
(* Is xs a prefix of s@p ? *)
    ...
    (* TODO: explain the parameter here.
       Parameter `f` is a function check the result? *)
