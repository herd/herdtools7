      ...
      | _ ->
          ...
          | Code _ ->
              ...
              | Some r -> (* fetch! *)
                  ...
                  m,F.add_final_v p r (IntSet.singleton (C.PteVal.value_to_int (U.fetch_val n)))
                    fenv
              end),
          st
      end
...
let max_set = IntSet.max_elt
  ...
  let check_writes env_wide atoms =
    ...
    let rec check_rec p i =
      ...
      | (x,vs)::xvs ->
         let vs = List.map ( List.map ( fun (v, vset) -> (Array.map C.PteVal.value_to_int v, vset) ) ) vs in
  ...
  let do_observe_local_simd st p i code f x bank nxt =
    ...
    let f =
      List.fold_right2
        (fun r v -> F.add_final_loc p r (v |> List.map C.PteVal.value_to_int |> Code.add_vector O.hexa))
        rs vs f in
    i,code@c,f,st
  ...
  let add_co_local_check avoid_ptes lsts ns st p i code f =
    ...
    if U.check_here lst then
      ...
      | Data x,(Ord|Pair|Instr) -> (* TODO check for -obs local mode and pairs *)
         ...
         | _ ->
            let v = C.PteVal.value_to_int nxt.C.v
            and prev_v = C.PteVal.value_to_int lst.C.evt.C.v in
            let all_lst =
      ...
      | Data x,Tag ->
          let v = C.PteVal.value_to_int lst.C.next.C.evt.C.v in
      ...
      | Data x,(VecReg _)->
         ...
         begin match bank with
         | Ord|Pair ->
            let v = C.PteVal.value_to_int nxt.C.v in
    ...
    if O.verbose > 1 then
      Printf.eprintf "Last_Ptes: %s\n"
        (String.concat ","
           (List.map
              (fun (loc,v) ->
                Printf.sprintf "%s->%s" loc (C.PteVal.pp_pte v)) last_ptes)) ;
