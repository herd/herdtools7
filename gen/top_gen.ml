            ...
            if O.variant Variant_gen.NoFault then
              ...
              let pos_flts,neg_flts,_ = List.fold_left
                (fun (pos_flts,neg_flts,previous_multi_intr_rmw) n ->
                  ...
                  | Some label_list,Data x,Ord ->
                    let proc = n.C.evt.C.proc in
                    let flts = List.map (fun (lbl, _) ->
                        ((proc, (Label.Set.singleton lbl)), Some (A.Loc x), None, None)) label_list in
                    (* Assume the `label_list` has at least one element,
                       also all the elements have the same `do_fault` *)
                    let (_,do_fault) = List.hd label_list in
                    (* zip the `to_add` to the begining of `flts_list`,
                      e.g. `to_add` = [a1, a2] and `flts_list` = [[f1], [f2],...]
                      the result will be [[a1,a2,f1], [f2],...] *)
                    let add_to_first_element to_add flts_list = match flts_list with
                      | [] -> assert false
                    ...
                    let new_pos_flts,new_neg_flts =
                      ...
                      | false,false -> pos_flts,[flts] @ neg_flts
                      | false,true -> pos_flts,add_to_first_element flts neg_flts
                      | true,false -> [flts] @ pos_flts,neg_flts
                      | true,true -> add_to_first_element flts pos_flts,neg_flts in
