...
(********)
      ...
      let rec dump_exp e =
        ...
        | AtomicExch (MemOrder.SC,loc,v) ->
            sprintf "atomic_exchange(%s,%s)" (dump_exp loc) (C.PteVal.pp_v v)
        | AtomicExch (mo,loc,v) ->
            sprintf "atomic_exchange_explicit(%s,%s,%s)"
              (dump_exp loc) (C.PteVal.pp_v v) (dump_mem_order mo)
        | AtomicFetchOp (MemOrder.SC,loc,v) ->
            sprintf "atomic_fetch_add(%s,%s)" (dump_exp loc) (C.PteVal.pp_v v)
        ...
        | Const v -> sprintf "%s" (C.PteVal.pp_v v)
      ...
      let rec dump_exp e =
       ...
       | AtomicFetchOp (mo,loc,v) ->
        ...
        | AssertVal (AtomicLoad _|Load _ as e,v) ->
            sprintf "%s.readsvalue(%s)" (dump_exp e) (C.PteVal.pp_v v)
        | AssertVal _ ->
 ...
 *)
      ...
      let test_of_cycle name ?com ?(info=[]) ?(check=(fun _ -> true)) ?scope ?(init=[])
          es c =
        ignore (scope) ;
        assert (init=[]) ;
