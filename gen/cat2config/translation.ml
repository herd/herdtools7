...
let apply_prim_set : partial_effect -> prim_set -> partial_effect option =
  ...
  fun eff x ->
    ...
    | Prim "A" -> build_atom_eff eff (make OrdinaryAccess OrderAcquire)
    | Prim "Q" -> build_atom_eff eff (make OrdinaryAccess OrderAcquirePc)
    | Prim "L" -> build_atom_eff eff (make OrdinaryAccess OrderRelease)
    | _ -> None
