...
module CoSt = struct
  ...
  let add_physical_address st physical_address init_value n =
    let co_cell =
      Array.make (Array.length (get_cell st)) (Value.from_int init_value) in
...
(* For each contiguous elements to location `x` in the input,
...
let check_cycle c =
  ...
  (* group all the RMW events by locations *)
  |> by_loc
  (* check if there exists invalid RMW events for a location *)
  |> List.exists (fun e -> not @@ E.is_valid_rmw @@ List.flatten @@ snd e)
  |> ( fun exist_invalid_rmw -> if exist_invalid_rmw then
    Warn.fatal "There are two `Amo.St*` of the same type on the same location. A cycle will not form due to commutativity." )
...
(* Set the values of write events *)
  ...
  let do_set_write_val st nss env initial_value =
 ...
 *)
    ...
    ) (* END of the function applying to `fold_left` *) (env, initial_value, st) nss in
    env
      ...
      List.fold_left
        (fun init_env ns ->
          ...
          | n::_ ->
              ...
              do_set_write_val init_st ns init_env init_val )
        [] nss
...
(* find changing location *)
  ...
  (* Helper function that extracts all write nodes
     satisfying `filter_predicate`. *)
  ...
  let coherence_by_memory_tag nodes =
    let to_tagloc = function
      | Data s -> Data (Misc.add_atag s)
      | Code s -> Code (Misc.add_atag s) in
    let get_tag_locs n = (to_tagloc n.evt.loc,n) in
    List.map get_tag_locs nodes |> by_loc

  let coherence_by_physical_address nodes =
    List.map (fun node -> (node.evt.pa,node)) nodes |> by_loc

  (* Extract ordinary writes that affect the coherence check, then group
     them by physical address. *)
  let coherence n =
    let start_node,changes_location =
      match find_change n,O.same_loc with
      | Some start_node, _ -> start_node,true
      | None, true -> n,false
      | None, false -> Warn.fatal "Unique location" in
    let ordinary_writes = get_ord_writes start_node in
    let write_set =
      let ord_ws_by_pa = coherence_by_physical_address ordinary_writes in
       ...
       * label accordingly. *)
      (* A single-location cycle already has one coherence sequence. Do not
         duplicate it as tag coherence, which would unnecessarily expose the
         internal tag location in the generated postcondition. *)
      let tag_ws = if do_memtag && changes_location then
        coherence_by_memory_tag ordinary_writes else [] in
  ...
  let last_ptes n =
    ...
    | Some n ->
        ...
        let r = List.map (fun n -> (n.evt.loc,n)) ws |> by_loc in
        List.fold_right
