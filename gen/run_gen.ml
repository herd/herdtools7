...
module Make (O:Config) (C:ArchRun.S) :
    sig
      val run : C.C.event list list -> C.A.location C.C.EventMap.t
        -> (C.A.location * C.C.PteVal.v) list list

      val dump_cond :  (C.A.location * C.C.PteVal.v) list list -> string
    end
    =
  struct
    module A = C.A
...
(* Communication edges *)
    ...
    module StateSet =
      MySet.Make
        (struct
          type t = C.PteVal.v State.t
          let compare = State.compare C.PteVal.value_compare
        end)
...
(* Dump condition *)
    ...
    module OV = struct
      type t = C.PteVal.v
      let compare = C.PteVal.value_compare
    end
    ...
    and do_dump = function
      ...
      | Or fs -> do_dumps " \\/ " fs
      | And fs -> do_dumps " /\\ " fs
      | Atom (loc,v) -> sprintf "%s=%s" (A.pp_location loc) (C.PteVal.pp_v v)
