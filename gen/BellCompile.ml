...
(*jade: l'idee c'est de tout faire par les labelled fences en LISA*)
    ...
    | Some W,Data loc ->
        ...
        let cs2 =  [calc_zero r2 r1;Instruction (addk r2 r2 (PteVal.value_to_int e.v));] in
    ...
    let emit_access_ctrl st p init e r1 v1 =
      if Cfg.realdep then
        ...
        let c =
          [Instruction (movne rd r1 (PteVal.value_to_int v1)) ;
...
(* Check load *)
    ...
    let do_check_load p st r e =
      let ok,st = A.ok_reg st in
      (fun k ->
        Instruction (movne tempo1 r (PteVal.value_to_int e.v))::
