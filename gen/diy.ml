...
let () =
  Util.parse_cmdline
    ~usage_suffix:Config.diy7_parser_syntax_doc
    (Config.diy_spec ())
    get_arg;
  begin
  ...
  try
    ...
    | [lhs;rhs] ->
        ...
        |> List.iter ( fun (l, r, result) ->
            ...
            (Alt.PredicateRelax.pp_list [l]) (Alt.PredicateRelax.pp_list [r])
    ...
    | _ -> (* The common path to generate tests *)
      if !Config.unfold_only then
        printf "***relax***\n%s\n***safe***\n%s\n***reject***\n%s\n"
          (Alt.PredicateRelax.pp_list relax)
          (Alt.PredicateRelax.pp_list safe)
          (Alt.PredicateRelax.pp_list reject)
      else
