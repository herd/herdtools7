...
(*********)
    ...
    module LDNW = struct
      ...
      let emit_load_reg n st init rA idx =
        let start,st = ZR,st in
        let finish,st = next_reg st in
        let pred,st = next_preg st in
        let acc,st = next_vreg st in
        let whilelt = [I_WHILELT (pred,V32,start,finish)] in
        let setup = [do_movi V32 finish (SIMD.nelements n)]@whilelt in
        ...
        rX,init,lift_code (setup@load@reduce@fmov),st
...
(**********)
    ...
    module STNW = struct
      let emit_store_reg n st init rA v idx =
        ...
        let setup = [do_movi V32 finish (SIMD.nelements n)]@mov_sv@whilelt in
      ...
      let emit_store_dep n vdep st init rA v =
        ...
        let pred,st = next_preg st in
        let whilelt = [I_WHILELT (pred,V32,start,finish)] in
        let setup = dup_sv@mov_sv@add_sv@[do_movi V32 finish (SIMD.nelements n)]@whilelt in
        let store = [stnsv n (r::rs) pred rA idx] in
        init,lift_code (setup@store),st
    end
