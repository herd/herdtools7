...
(**********)
    ...
    | None,init,st ->
	let rA,st = next_reg st in
	rA,init,[Instruction (_mov rA v)],st
    | Some rA,init,st ->
	rA,init,[],st
...
(***************************)
    ...
    let emit_sta rw st p init loc v =
      let rA,init,st = next_init st p init loc in
      let rW,init,csi,st = emit_mov st p init v in
...
(**********)
    ...
    let emit_exch st p init er ew =
      ...
      let rR,st = next_reg st in
      let rW,init,csi,st = emit_mov st p init ew.v in
...
(* Dependencies *)
    ...
    let emit_access_dep_addr st p init e rd =
	  ...
	  | W,Some (Mixed (sz,o)) ->
		...
		STORE
	      ...
	      let rA,init,cs_mov,st = emit_mov_sz sz st p init e.v in
    ...
    let emit_access_dep_data st p init e  r1 =
	  ...
	  | Some (Mixed (sz,o)) ->
	      let rA,init,csA,st = emit_mov_sz sz st p init e.v in
	      let r2,st = next_reg st in
	      let cs2 =
