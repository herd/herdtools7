...
(*************)
    ...
    let emit_exch st p init er ew =
      ...
      let rW,init,csi,st = U.emit_mov st p init (PteVal.to_int ew.v) in
      let rR,st = next_reg st in
      let cs,st = emit_pair p st rR rW rA in
    ...
    let emit_exch_dep_addr st p init er ew rd =
      ...
      let init,csw,st = emit_one_strex_reg st p init tempo1 (PteVal.to_int ew.v) in
      r,init,c@csr@csw,st
      ...
      | Some W ->
          ...
          let cs2 =
            [Instruction (calc0 r2 r1) ;
             Instruction (I_ADD (DontSetFlags,r2,r2,(PteVal.to_int e.v))) ; ] in
...
(* Fences *)
    ...
    let do_check_load p st r e =
      let ok,st = A.ok_reg st in
      (fun k ->
        Instruction (I_CMPI (r, (PteVal.to_int e.v)))::
