...
(* Signatures as strings, for the sake of compacity *)
  ...
  let sig_of_dir = function
    | Dir W -> 'W'
    | Dir R -> 'R'
    | Irr -> '*'

  let sig_of_ie = function
    | Int -> 'i'
    | Ext -> 'e'

  let sig_of_sd = function
    | Same -> 's'
    | Diff -> 'd'
  ...
  let sig_of_tedge buff e =
    ...
    | Fr ie ->
        Buffer.add_char buff '2' ;
        Buffer.add_char buff (sig_of_ie ie)
    | Ws ie ->
        Buffer.add_char buff '3' ;
        Buffer.add_char buff (sig_of_ie ie)
    | Po (sd,e1,e2) ->
        Buffer.add_char buff '4' ;
    ...
    | Hat ->
        Buffer.add_char buff '7'
    | RfStar ie ->
        Buffer.add_char buff '9' ;
        Buffer.add_char buff (sig_of_ie ie)
    | Rmw ->
...
(* Check edge signatures to be pairwise distinct *)          
  let () =
    ...
    let all =
      List.sort
        (fun (_,s1) (_,s2) -> String.compare s1 s2)
        all in
    let rec uniq = function
      | []|[_] -> ()
      | (e1,s1)::((e2,s2)::_ as rem) ->
          if s1=s2 then
            Warn.fatal
