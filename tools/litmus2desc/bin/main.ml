...
let () =
    ...
    [
      ...
      ( "--describe-dep-path",
        ...
        "Include intermediate instructions when describing dependencies. \
         Default = disabled." );
      ( "--print-litmus",
        Arg.Unit (fun () -> print_litmus := true),
        "Include contents of input litmus test file in the tool's output. \
         Default = disabled." );
    ]
  in
  let process_arg arg =
  ...
  | [] ->
      ...
      exit 1
  | _ ->
      if !print_litmus then begin
        let contents = String.trim contents in
        if !latex then
          Printf.printf "\\begin{verbatim}\n%s\n\\end{verbatim}\n" contents
        else print_endline contents;
        print_endline ""
      end;
      print_endline (format_descriptions ~latex_compat:!latex descriptions)
