...
module Instruction = struct
  ...
  let opcode_name ins =
    let str = A.show_instruction ins in
...
struct
  ...
  let describe_mem_event (ev : regular_mem_event) (ins : Instruction.t) =
    ...
    in
    let ins_full = Instruction.render ins in
    let value_descr =
  ...
  let describe_dep (dep : Component.dependency) : string =
    ...
    let describe_path_ev ev =
      ...
      let ins_full = Instruction.render ev.ins in
        ...
        let loc_descr, instr_descr =
          if should_dump_instr then
            let instr_descr =
              ...
              Format.sprintf " generated by %s" (Instruction.render ins)
            in
...
let describe_execution ~describe_regs ~num_of_execs (test : LitmusTest.test)
    (src_exec : Execution.t) : string list =
...
let explain_test_path ~(libdir : string option) ~describe_regs file_path :
    string =
  let test = A.parse_from_file file_path in
  let executions = LitmusTest.run_herd ~libdir file_path in
  let executions =
  ...
  let exec_sections =
    ...
    |> List.mapi (fun ix expl ->
        if multiple_executions then
          Format.sprintf "=== %s execution ===@.@.%s@."
            (Util.verbalize_index ix) expl
        else expl)
    |> String.concat "\n"
  in
