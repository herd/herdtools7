...
(* Recursive module! Notice however that M.equal_node is physical equality *)
    module rec M : Hashcons.S with type key = t_node = Hashcons.Make
        (struct
          type t = t_node
          ...
          | Cons (x1,r1),Cons (x2,r2) ->
              I.equal_node x1 x2 && M.equal_node r1 r2
          let hash = function
            | Nil -> 0
            | Cons (x,r) ->
