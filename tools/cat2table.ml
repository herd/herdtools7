...
struct
  ...
  *)
    ...
    | Var (_, id) as e ->
      if StringMap.mem id crt_nums then
        ...
        if is_rec && id = def_id then
          ...
          else make_var (to_alias id 1)
        else
          make_var (to_alias id 0)
  ...
  let execute fname e1 e2 start_rel =
    let valid_primitives = StringSet.of_list ["M"; "PTE"; "PTEV"; "T"; "Instr";
      "R"; "W"; "IW"; "Exp"; "NExp"; "A"; "L"; "Q"; "B"; "BCC"; "EXC-RET";
      "EXC-ENTRY"; "TagCheck"; "FAULT"; "F"; "Rreg"; "Wreg"; "ADDR"; "DATA";
      "NoRet"; "AF"; "DB"; "AccessFlag"; "Permission"; "Translation"; "MMU";
      "SVC"; "UndefinedInstruction"; "TLBI"; "TLBIIS"; "TLBInXS"; "DMB.ISH";
      "DMB.ISHLD"; "DMB.ISHST"; "DSB.ISH"; "DSB.ISHLD"; "DSB.ISHST"; "DMB.SY";
      "DMB.ST"; "DMB.LD"; "DSB.SY"; "DSB.ST"; "DSB.LD"; "DMB.OSH"; "DSB.OSH";
      "DMB.OSHLD"; "DSB.OSHLD"; "DMB.OSHST"; "DSB.OSHST"; "ISB"; "DC.CVAU";
      "IC.IALLUIS"; "IC.IALLU"; "IC.IVAU"] in
