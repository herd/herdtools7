...
struct
  ...
  let rec fold_left_short f stop acc = function
    ...
    | x :: xs ->
        let acc' = f acc x in
        if stop acc' then acc' else fold_left_short f stop acc' xs
  ...
  let is_universe = function Konst (_, Universe _) -> true | _ -> false

  (* A character that we know can't be part of a variable name *)
  let sep = '$'
  let to_alias s n = s ^ String.make 1 sep ^ string_of_int n
  ...
  let pp_expr =
    ...
    and do_pp_expr paran = function
      ...
      | Try (_, e1, e2) ->
          Printf.sprintf "try %s with %s" (do_pp_expr false e1)
            (do_pp_expr false e2)
      | If (_, _, e1, e2) ->
          Printf.sprintf "if _some_cond then %s else %s" (do_pp_expr false e1)
            (do_pp_expr false e2)
      | e -> pp_expr_with_loc e
    in
    do_pp_expr false
  ...
  let compare_expr e1 e2 =
    let is_commutative = function Union | Inter | Add -> true | _ -> false in

    let do_compare_pairs (a1, b1) (a2, b2) cmp1 cmp2 =
      let c = cmp1 a1 a2 in
      if c != 0 then c else cmp2 b1 b2
    in

    let rec do_compare_variant_cond vc1 vc2 =
      match (vc1, vc2) with
      ...
      | OpAnd (vc11, vc12), OpAnd (vc21, vc22) ->
          do_compare_pairs (vc11, vc12) (vc21, vc22) do_compare_variant_cond
            do_compare_variant_cond
    ...
    let rec do_compare_expr e1 e2 =
      ...
      | If (_, c1, e11, e12), If (_, c2, e21, e22) ->
          do_compare_pairs
            (c1, (e11, e12))
            (c2, (e21, e22))
            do_compare_cond do_compare_expr_pairs
      | _, _ ->
    ...
    and do_compare_cond c1 c2 =
      match (c1, c2) with
      | Eq (e11, e12), Eq (e21, e22) ->
          do_compare_expr_pairs (e11, e12) (e21, e22)
      | Subset (e11, e12), Subset (e21, e22) ->
          do_compare_expr_pairs (e11, e12) (e21, e22)
      | In (e11, e12), In (e21, e22) ->
          do_compare_expr_pairs (e11, e12) (e21, e22)
    ...
    let rec sort_subexprs = function
      ...
      | Op (loc, op, es) ->
          ...
          let es =
            if is_commutative op then List.sort do_compare_expr es else es
          in
          Op (loc, op, es)
      | App (loc, e1, e2) -> App (loc, sort_subexprs e1, sort_subexprs e2)
    ...
    and sort_cond = function
      ...
      | VariantCond vc -> VariantCond (sort_variant_cond vc)
    in
  ...
  module MakeGraph (VSet : MySet.S) (VMap : MyMap.S with type key = VSet.elt) =
  struct
    module VSet = VSet
    module VMap = VMap
    ...
    let neighbours (v : vertex) (g : t) : vertex list =
      match VMap.find_opt v g with Some l -> l | None -> []
    ...
    let dfs_fold ~(per_vertex : vertex -> int -> 'acc -> 'acc)
        ~(per_neighbour : vertex -> vertex -> 'acc -> 'acc) (g : t)
        (start : vertex) (seen : VSet.t) (acc0 : 'acc) : VSet.t * 'acc =
    ...
    let dfs_fold_multiple_starts ~(per_vertex : vertex -> int -> 'acc -> 'acc)
        ~(per_neighbour : vertex -> vertex -> 'acc -> 'acc) (g : t)
        (acc0 : 'acc) : 'acc =
      let _, res =
        VMap.fold
  ...
  *)
  let get_axioms ast =
    ...
    let add_to_map key value els =
      ...
      StringMap.add key (value :: existing_els) els
    in

    let invalid_assert_err e name =
      Warn.fatal
        "Assertion %s inside axioms file %s contains invalid expression: %s\n"
        (pp_opt_string name) O.axioms_file (pp_expr e)
    in

    List.fold_left
      (fun (supersets, intersects, rel_effects) ins ->
        match ins with
        | Test
            ( ( _,
                _,
                Yes TestEmpty,
                Op (_, Diff, [ subs_expr; Var (_, super) ]),
                name ),
              Assert ) ->
            (* subs is either a primitive or a disjunction of primitives *)
            let supersets =
              match subs_expr with
              ...
              | Op (_, Union, subs) ->
                  List.fold_left
                    ...
                    supersets subs
              | e -> invalid_assert_err e name
            in
            (supersets, intersects, rel_effects)
        | Test
            ( (_, _, Yes TestEmpty, Op (_, Diff, [ Var (_, rel); e ]), name),
              Assert ) ->
            let rel_effects =
              match e with
              | Op (_, Cartesian, [ e1; e2 ]) ->
                  add_to_map rel (e1, e2) rel_effects
              | Op (_, Union, es) ->
                  List.fold_left
                    (fun rel_effects e ->
                      match e with
            ...
            (supersets, intersects, rel_effects)
        | Test
            ( (_, _, No TestEmpty, (Op (_, Inter, [ e1; e2 ]) as e), name),
              Flagged ) ->
            let intersects =
              match (e1, e2) with
              | Var (_, id1), Var (_, id2) ->
                  VarGraph.add_edge id1 id2 intersects
              | _, _ ->
                  Warn.fatal
                    "Flag %s inside axioms file %s contains invalid expression \
                     %s\n"
                    (pp_opt_string name) O.axioms_file (pp_expr e)
            in
            (supersets, intersects, rel_effects)
      ...
      (fun id _ intersects ->
        ...
        StringSet.fold
          (fun id2 intersects ->
            ...
            let id2_supersets =
              get_children id2 supersets |> StringSet.add id2
            in
            let id1_diff_supersets =
              StringSet.diff id_supersets id2_supersets
            in
            let id2_diff_supersets =
              StringSet.diff id2_supersets id_supersets
            in

            let update_intersects id1_diff_supersets id2_diff_supersets
                intersects =
              StringSet.fold
                (fun e intersects ->
                  let new_intersects =
                    get_children e intersects
                    |> StringSet.union id1_diff_supersets
                  in
                  StringMap.add e new_intersects intersects)
                id2_diff_supersets intersects
            in
  ...
  let rec is_subset_of defs e1 e2 =
    match (e1, e2) with
    ...
    | e1, App (_, f, e2) ->
        ...
        is_subset_of defs e1 e2
    | Op1 (_, Comp, e1), Op1 (_, Comp, e2) -> is_subset_of defs e2 e1
    | Op1 (_, Comp, e1), e2 -> is_universe (unite defs e1 e2)
    | e1, Op1 (_, Comp, e2) -> is_empty (intersect defs e1 e2)
    | e1, Op (_, Inter, es) -> List.for_all (fun e -> is_subset_of defs e1 e) es
    | Op (_, Union, es), e2 -> List.for_all (fun e -> is_subset_of defs e e2) es
    | e, Op (_, Union, es) ->
    ...
    | e, Op (_, Diff, e1 :: es) ->
        is_subset_of defs e e1
        && List.for_all (fun el -> is_empty (intersect defs e el)) es
    | Op (_, Diff, e1 :: _), e ->
    ...
    | _ ->
        Printf.eprintf "is_subset_of not supppoted for %s and %s\n" (pp_expr e1)
          (pp_expr e2);
        false
  ...
  and intersect defs e1 e2 =
    let recover_ids (id1, e1) (id2, e2) = function
      ...
      | e ->
          ...
          else Op (TxtLoc.none, Inter, [ res1; res2 ])
    in

    let is_optimised_inter e1 e2 inter =
      let unoptimised =
        ASTUtils.flatten (Op (TxtLoc.none, Inter, [ e1; e2 ]))
      in
      not (expr_equal inter unoptimised)
    in
      ...
      is good enough. Need to make sure that A and B are irreducible. *)
        let rec traverse_inter els e = function
          ...
          | x :: xs ->
              ...
              else traverse_inter els inter xs
        in
        ...
        let rec traverse_union acc = function
          ...
          | x :: xs ->
              if is_subset_of defs e x then [ e ]
                ...
                else traverse_union (inter :: acc) xs
        in
  ...
  and unite defs e1 e2 =
    ...
    | Op (_, Union, es1), Op (_, Union, es2) ->
        Op (TxtLoc.none, Union, es1 @ es2)
    | Op (_, Union, es), e | e, Op (_, Union, es) ->
        Op (TxtLoc.none, Union, e :: es)
  ...
  and diff defs e1 e2 =
    ...
    | Op (_, Union, es), e ->
        let es = List.filter (fun el -> not (is_subset_of defs el e)) es in
        let union = List.fold_left (unite defs) empty_effect es in
        if List.exists (fun el -> not (is_empty (intersect defs el e))) es then
          Op (TxtLoc.none, Diff, [ union; e ])
        else union
  ...
  and apply_func defs f e =
    let f =
      ...
      | Var (_, id) -> from_alias id
      | e ->
          Warn.fatal "%s is applied as a function but is not a var" (pp_expr e)
    in
    let f =
      match f with
      ...
      | "at-least-one-writable" -> (
          fun _ expr ->
            ...
            | e ->
                Warn.fatal
                  "at-least-one-writable called on invalid expression %s"
                  (pp_expr e))
      | "oa-changes" -> (
          fun _ expr ->
            match expr with
            | Op (_, Tuple, [ e; _ ]) -> e
            | e ->
                Warn.fatal "oa-changes called on invalid expression %s"
                  (pp_expr e))
      | _ -> Warn.fatal "Function %s not supported" f
    in
    f defs e
  ...
  and tr_effect defs = function
    ...
    | ( Tag (_, _)
      ...
      | ExplicitSet (_, _)
      | Match (_, _, _, _)
      | MatchSet (_, _, _, _) ) as e ->
        Warn.fatal "%s is not an effect" (pp_expr e)
    | e -> e

  and domain looked_up_ids defs expr =
    match expr with
    | Var (_, id) ->
        if StringMap.mem id defs then
          if StringSet.mem id looked_up_ids then empty_effect
            ...
            domain
              (StringSet.add id looked_up_ids)
              defs (StringMap.find id defs)
        else
          ...
          if id = "same-instance" then universe_effect
            ...
            | Some pairs ->
                List.fold_left
                  (fun acc (eff, _) -> unite defs acc eff)
                  empty_effect pairs
            | None ->
                Printf.eprintf
                  "Could not find relation %s in map of primitives\n" id;
                empty_effect
          end
    ...
    | Op (_, (Seq | Diff), rels) ->
        let first_non_opt =
          List.find (function Op1 (_, Opt, _) -> false | _ -> true) rels
        in
  ...
  and range looked_up_ids defs expr =
    match expr with
    | Var (_, id) ->
        if StringMap.mem id defs then
          if StringSet.mem id looked_up_ids then empty_effect
          ...
          if id = "same-instance" then universe_effect
            ...
            | Some pairs ->
                List.fold_left
                  (fun acc (_, eff) -> unite defs acc eff)
                  empty_effect pairs
            | None ->
                Printf.eprintf
                  "Could not find relation %s in map of primitives\n" id;
    ...
    | Op (_, (Seq | Diff), rels) ->
        let first_non_opt =
          List.find
            (function Op1 (_, Opt, _) -> false | _ -> true)
            (List.rev rels)
        in
  ...
  let tr_rel e1 e2 id defs =
    let any_match e2 =
      List.exists (fun e -> not (is_empty (intersect defs e e2)))
    in

    let add_to_sol id chain sols =
      let rel = make_var id in
      ExprGraph.add_edge rel chain sols
    in
    ...
    let rec do_tr_effect e1s = function
      ...
      | e ->
          List.filter
            (fun e -> not (is_empty e))
            (List.map (intersect defs e) e1s)
    in
    ...
    (* Traverses a cat clause as part of a relation. Takes as arguments:
        ...
        *)
    let rec do_tr_chain (looked_up_ids : StringSet.t) (ret_sol : bool)
        (sol : ExprGraph.t) (e1s : AST.exp list) (is_inv : bool) = function
      | Op (_, Seq, es) ->
          ...
          let e2s =
            fold_left_short
              (fun e1s e ->
                let _, e1s =
                  do_tr_chain looked_up_ids false ExprGraph.empty e1s is_inv e
                in
                e1s)
              (fun l -> l = [])
              e1s es
      ...
      | Op1 (_, (Star | Opt), e) ->
          ...
          (sol, e1s @ e2s)
      | Op (_, Cartesian, [ e1; e2 ]) ->
          if not (do_tr_effect e1s e1 = []) then (sol, [ e2 ]) else (sol, [])
      | Op (_, Cartesian, _) as e ->
          Warn.fatal
            "Cartesian product of more than 2 effects is not a relation: %s\n"
            (pp_expr e)
      | App (_, f, e) ->
          let e = apply_func defs f e in
          ...
          ( sol,
            Misc.fold_cross results
              (fun effs acc ->
                ...
                in
                match inter with Konst (_, Empty _) -> acc | _ -> inter :: acc)
              [] )
      | Op (_, Union, es) ->
      ...
      | e ->
          ...
          (sol, [])
    and do_tr_rel (looked_up_ids : StringSet.t) (ret_sol : bool)
        (sol : ExprGraph.t) (e1s : AST.exp list) (is_inv : bool) (id : string) =
      if StringMap.mem id defs then begin
        if StringSet.mem id looked_up_ids then (sol, [])
          ...
          if ret_sol then
            let chains =
              match chain with Op (_, Union, es) -> es | e -> [ e ]
            in
            List.fold_right
              (fun chain (sol, e2s_acc) ->
                let sol, e2s =
              ...
              chains (sol, [])
          else do_tr_chain looked_up_ids ret_sol sol e1s is_inv chain
      end
      else
        let id = from_alias id in
        if id = "same-instance" then (sol, e1s)
        else
          let default = [ (universe_effect, universe_effect) ] in
          let pairs = StringMap.safe_find default id rel_effects in
          let pairs =
            if is_inv then List.map (fun (a, b) -> (b, a)) pairs else pairs
          in
          let res =
            List.fold_left
              (fun res (e_left, e_right) ->
                if any_match e_left e1s then e_right :: res else res)
              [] pairs
          in
          (sol, res)
    in
    ...
    let sol, _ =
      do_tr_rel StringSet.empty true ExprGraph.empty [ e1 ] false id
    in
    sol
    ...
    let rec run = function
      ...
      | MatchSet (loc, e1, e2, cl) ->
          let cl =
            ...
            | PreEltPost (p1, p2, p3, e) -> PreEltPost (p1, p2, p3, run e)
          in
  ...
  let convert_defs num_defs crt_nums def_id is_rec expr =
    let get_def_count id num_defs =
      match StringMap.find_opt id num_defs with
      | Some n -> n
      | None -> Warn.fatal "Found definition for %s that was not counted" id
    in
    ...
    let rec run = function
      ...
      | Var (_, id) as e ->
          if StringMap.mem id crt_nums then
            let total_num = get_def_count id num_defs in
          ...
          else if is_rec && id = def_id then
            let total_num = get_def_count id num_defs in
            if total_num = 1 then e else make_var (to_alias id 1)
          else make_var (to_alias id 0)
  ...
  let tr_bds defs kont bds =
    List.fold_left
      (fun defs (_, p, expr) ->
        match p with Pvar (Some id) -> kont defs id expr | _ -> defs)
      defs bds
  ...
  let rec tr_ast defs kont parsed_files fname =
    ...
    List.fold_left
      (fun (defs, parsed_files) ins -> tr_ins defs kont parsed_files ins)
  ...
  let execute fname e1 e2 start_rel =
    ...
    let stdlib_defs, crt_nums =
      get_defs stdlib_fname ~existing_defs:StringMap.empty ~num_defs
        ~crt_nums:StringMap.empty
    in
    let defs, _ =
      get_defs fname ~existing_defs:stdlib_defs ~num_defs ~crt_nums
    in
    let defs =
      StringMap.map
        (fun def -> replace_ifs_and_tries (ASTUtils.flatten def))
        defs
    in
    match (e1, e2) with
    | Some e1, Some e2 ->
        ...
        if sol = ExprGraph.empty then Printf.printf "No solutions\n"
        else
          ignore
            (ExprGraph.dfs_fold
               ~per_vertex:(fun rel lvl _ ->
                 Printf.printf "%s%s\n" (String.make lvl '-') (pp_expr rel))
               ~per_neighbour:(fun _ _ _ -> ())
               sol (make_var start_rel) ExprGraph.VSet.empty ())
...
let axioms_file = ref (Filename.concat !libdir "aarch64axioms.cat")
let e1 = ref None
let e2 = ref None
...
let options =
  [
    ...
    ArgUtils.parse_string "-axioms-file" axioms_file "path to axioms file";
