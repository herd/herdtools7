...
struct
  ...
  let compare_expr e1 e2 =
    ...
    | Op (_, op1, es1), Op (_, op2, es2) ->
      do_compare_pairs (op1, es1) (op2, es2) compare do_compare_expr_list
    | Op _, _ -> -1
    | _, Op _ -> 1
    ...
    | If _, _ -> -1
    | _, If _ -> 1
    | _, _ -> 0 (* We don't care about the rest, since they are not effects *)
    ...
    and do_compare_variant_cond vc1 vc2 = match vc1, vc2 with
    ...
    and do_compare_expr_list es1 es2 = match es1, es2 with
    | [], [] -> 0
    | [], _ -> -1
    | _, [] -> 1
    | e1 :: e1s, e2 :: e2s ->
      do_compare_pairs (e1, e1s) (e2, e2s) do_compare_expr do_compare_expr_list in
    ...
    and sort_variant_cond = function
