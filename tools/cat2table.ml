  ...
  *)
  let get_assumptions ast =
    ...
    let invalid_assert_err e name =
      Warn.fatal
        "Assertion %s inside assumptions file %s contains invalid expression: %s\n"
        (pp_opt_string name) O.assumptions_file (pp_expr e)
    in
      ...
      (fun (supersets, intersects, rel_effects) ins ->
        ...
        | Test
            ...
            let intersects =
              ...
              | _, _ ->
                  Warn.fatal
                    "Flag %s inside assumptions file %s contains invalid \
                     expression %s\n"
                    (pp_opt_string name) O.assumptions_file (pp_expr e)
            in
            (supersets, intersects, rel_effects)
...
let options =
  [
    ...
    ( "-herd-libdir",
      Arg.Unit
        (fun () ->
          print_endline !herd_libdir;
          exit 0),
      " show herd installation directory and exit" );
    ( "-set-herd-libdir",
      Arg.String (fun s -> herd_libdir := s),
    ...
    ArgUtils.parse_string "-assumptions-file" assumptions_file
