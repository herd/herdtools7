...
(defun loop-local-vars-final-env (local-vars)
  (if (atom local-vars)
    ...
    (let* ((rest (loop-local-vars-final-env (cdr local-vars)))
      ...
      (case-match first
        ((& str final-val)
         `(val-imaplist-assign ,str ,final-val ,rest))
        (& rest)))))

;; (defun loop-local-vars-final-env (local-vars)
;;   `(cons ,(loop-local-vars-final-env-aux local-vars)
;;          (cdr env.local.storage)))
...
@({
 (defthm user-loopname-correct
   (b* (((env env))
        ...
        (i-look (val-imaplist-lookup \"loop_index_var\" env.local.storage))
        ((v_int i) (cdr i-look))
        (n-look (val-imaplistlookup \"asl_var_n\" env.local.storage))
        ((v_real n) (cdr n-look))
        (res-look (val-imaplist-lookup \"result\" env.local.storage))
...
(define subprogram-arg-hyp ((var symbolp)
  ...
  (b* ((env-term `(change-env
                   ...
                   :local (change-local-env
                           (empty-local-env)
                           :storage (list ,local-storage-term))))
