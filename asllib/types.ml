...
module Domain = struct
  ...
  module StaticApprox = struct
    ...
    (** Util to raise [CannotOverApproximate]. *)
    let assert_under approx =
    ...
    let make_interval approx z1 z2 =
      ...
      else assert_under approx

    let intset_map f s =
      IntSet.elements_individual s |> List.map f |> IntSet.of_list

    let intset_cross_map f s1 s2 =
      let l1 = IntSet.elements_individual s1
      and l2 = IntSet.elements_individual s2 in
      list_cross f l1 l2 |> IntSet.of_list
    ...
    let iterated op =
      ...
      and iter = function
        | [] -> raise (Invalid_argument "iterated")
        | [ x ] -> x
        | li -> pairwise [] li |> iter
      in
      iter
    ...
    let rec approx_expr approx env e =
      ...
      | E_Literal _ -> assert_under approx
      | E_Var x -> (
          match approx with
          | Over -> approx_type Over env (SEnv.type_of env x)
          | Under -> IntSet.empty)
      | E_Unop (op, e') ->
          intset_map (apply_unop e op) (approx_expr approx env e')
      | E_Binop (op, e1, e2) ->
          intset_cross_map (apply_binop e op)
            (approx_expr approx env e1)
            (approx_expr approx env e2)
      | E_Arbitrary t -> approx_type approx env t
      | E_Cond _ -> (* Not yet implemented *) assert_under approx
      | _ -> assert_under approx
    ...
    and approx_type approx env t =
      ...
      | T_Real | T_String | T_Int (UnConstrained | Parameterized _) ->
          assert_under approx (* We can't iterate on those. *)
      | T_Bool | T_Bits _ | T_Enum _ ->
          (* Not yet implemented *) assert_under approx
      | _ -> assert false

    and approx_constraints approx env cs =
      let op =
        match approx with Under -> IntSet.inter | Over -> IntSet.union
      in
      List.map (approx_constraint approx env) cs |> iterated op
    ...
    and approx_constraint approx env = function
      ...
      | Constraint_Range (e1, e2) -> (
          ...
          with Not_found | CannotOverApproximate -> assert_under approx)
