...
module type INTERVAL_SET = sig
  ...
  type interval
  (** An interval: a range (x, y) of set values where all the elements from x to
      y inclusive are in the set *)
  ...
  (** [fold_individual f t acc] folds [f] across all the individual elements of
      [t] *)
  ...
  val max_elt : t -> elt
  (** [max_elt t] returns the largest (in terms of the ordering) element in [t],
      or raises [Not_found] if the set is empty. *)

  val min_interval : t -> interval
  (** [min_interval t] returns the smallest (in terms of the ordering) interval
      in [t], or raises [Not_found] if the set is empty. *)

  val max_interval : t -> interval
  (** [max_interval t] returns the largest (in terms of the ordering) interval
      in [t], or raises [Not_found] if the set is empty. *)
  ...
  *)
  ...
  (** [take n] returns [Some a, b] where [cardinal a = n] and [diff t a = b] or
      [None] if [cardinal t < n] *)
  ...
  (** [find_next_gap from t] returns the next element that's absent in set [t]
