...
let list_take_until =
  let rec aux pred accu = function
    ...
    | x :: xs ->
        let accu' = x :: accu in
        if pred x then List.rev accu' else aux pred accu' xs
  in
...
let get_cycle =
  ...
  *)
  ...
  let rec dfs m path above e seen =
    if ISet.mem e above then
      let cycle =
        try list_take_until (String.equal e) path
      ...
      ISet.fold (dfs m path' above') succs seen'
  in
  fun m ->
    try
      let above0 = ISet.empty and seen0 = ISet.empty and path0 = [] in
      let _ = IMap.fold (fun x _ -> dfs m path0 above0 x) m seen0 in
      None
