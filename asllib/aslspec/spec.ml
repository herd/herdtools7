...
module Error = struct
  let undefined_reference id context =
    let msg = Format.asprintf "Undefined reference to '%s' in %s" id context in
    raise (SpecError msg)
...
let msg = Format.sprintf "Encountered undefined element: %s" id in
  ...
  let duplicate_definition id =
    let msg = Format.asprintf "Duplicate definition of '%s'" id in
    raise (SpecError msg)
  ...
  let invalid_application_of_constant_in_expr name expr =
    let msg =
      Format.asprintf "Invalid application of constant '%s' to expression %a"
        name PP.pp_expr expr
    in
    raise (SpecError msg)

  let invalid_application_of_type_in_expr name expr =
    let msg =
      Format.asprintf "Invalid application of type '%s' to expression %a" name
        PP.pp_expr expr
    in
    raise (SpecError msg)
  ...
  let multiple_output_judgments relation_name =
    let msg =
      Format.sprintf
  ...
  let missing_output_judgment relation_name =
    let msg =
      Format.sprintf "The rule for relation %s must end with an output judgment"
        relation_name
    in
    raise (SpecError msg)
...
let make_id_to_definition_node definition_nodes =
  List.fold_left
    (fun acc_map node ->
      ...
      if StringMap.mem name acc_map then Error.duplicate_definition name
...
(** A module for performing resolution transformations on rules to make them
    amenable for further checking and for rendering. *)
module ResolveRules = struct
  ...
  let rec resolve_expr id_to_defining_node expr =
    ...
    | Application { applicator; args } -> (
        ...
        | EmptyApplicator ->
            Application { applicator = EmptyApplicator; args = resolved_args }
        | Relation name ->
            Application { applicator = Relation name; args = resolved_args }
        | TupleLabel _ -> assert false
        (* Tuple labels appear only post resolution so not expected here. *)
        | ExprOperator op ->
            Application { applicator = ExprOperator op; args = resolved_args }
        | Fields path ->
        ...
        | Unresolved (Var id) -> (
            ...
            | Some (Node_Constant { Constant.name }) ->
                Error.invalid_application_of_constant_in_expr name expr
            | Some (Node_Type { Type.name }) ->
                Error.invalid_application_of_type_in_expr name expr
            | _ ->
  ...
  (** [resolve_judgment_form id_to_defining_node conclusion_lhs form] performs
      resolution transformations on [form] using [id_to_defining_node] to lookup
      type variants and relations and [conclusion_lhs] as the LHS of the
      conclusion when [form] is an output expression. *)
  let rec resolve_judgment_form id_to_defining_node conclusion_lhs form =
    match form with
    | Expr expr -> Expr (resolve_expr id_to_defining_node expr)
    | Output expr ->
        Transition
          {
            lhs = conclusion_lhs;
            rhs = resolve_expr id_to_defining_node expr;
            short_circuit = None;
  ...
  (** Transforms relations in three ways:
      {ul
       ...
       {- Transforms the conclusion judgment from an expression to a transition.
          ...
          identifiers to definition nodes, updated for the newly created
          relation nodes.
       }
      } *)
...
let from_ast ast =
  ...
  let ast, id_to_defining_node = ResolveRules.resolve ast id_to_defining_node in
