...
module ResolveRules = struct
  ...
  let rec resolve_application_expr id_to_defining_node expr =
    ...
    | Application ({ applicator; args } as app) -> (
        let resolved_args =
          List.map
            (fun e -> resolve_application_expr id_to_defining_node e)
            args
        in
    ...
    | Transition { lhs; rhs; short_circuit } ->
        ...
        let resolved_short_circuit =
          Option.map
            (List.map (fun e -> resolve_application_expr id_to_defining_node e))
            short_circuit
        in
        Transition
          {
                      ...
                      List.map
                        (fun rule_elem ->
                          resolve_rule_element id_to_defining_node
                            conclusion_lhs first_output_type rule_elem)
                        elements
                    in
