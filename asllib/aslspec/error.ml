...
(** [spec_error loc msg] raises a [SpecError] with a message [msg] and source
    location [loc]. *)
let spec_error loc msg = raise (SpecError { loc; msg })

(** [spec_error loc msg] raises a [SpecError] with a message [msg] and source
    location [loc]. *)
let spec_error loc msg = spec_error loc msg
...
let undefined_element loc id =
  spec_error loc @@ Format.asprintf "Encountered undefined element: %s" id

let duplicate_definition loc id =
  spec_error loc @@ Format.asprintf "Duplicate definition of %s" id

let unmatched_variables_in_template loc template unmatched_vars =
  spec_error loc
  @@ Format.asprintf
...
let empty_rule { loc; Relation.name } =
  spec_error loc @@ Format.asprintf "The rule for relation %s is empty." name
...
let missing_type_for_constant loc id =
  spec_error loc @@ Format.asprintf "Missing type for constant %s" id

let missing_case_in_rule loc case_name relation_name case_path =
  spec_error loc
  @@ Format.asprintf
