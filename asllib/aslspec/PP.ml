...
let pp_short_circuit fmt short_circuit =
...
let rec pp_judgment_form fmt =
  ...
  | Output e -> fprintf fmt "%a%a" pp_output_opt true pp_expr e
  | Transition { lhs; rhs; short_circuit; is_output } ->
      fprintf fmt "%a%a -> %a%a" pp_output_opt is_output pp_expr lhs pp_expr rhs
        pp_short_circuit short_circuit
