...
let pp_attribute fmt (key, value) = fprintf fmt {|%s = "%s"|} key value

let pp_attributes fmt attributes =
  if Utils.list_is_empty attributes then ()
  else fprintf fmt "<<@.%a@.>>" (pp_sep_list ~sep:",@," pp_attribute) attributes

let rec pp_type_term fmt = function
  | Label name -> pp_print_string fmt name
  | Powerset elt_term -> fprintf fmt "powerset(%a)" pp_type_term elt_term
  | Option elt_term -> fprintf fmt "option(%a)" pp_type_term elt_term
  ...
  | List { maybe_empty; member_type } ->
      let list_label = if maybe_empty then "list0" else "list1" in
      fprintf fmt "%s(%a)" list_label pp_type_term member_type
  | ConstantsSet constants ->
      fprintf fmt "set(%a)" (pp_sep_list ~sep:"," pp_print_string) constants
  | Function { from_type; to_type; total } ->
      let keyword = if total then "fun" else "partial" in
...
let pp_type_term_with_attributes fmt variant =
  ...
  fprintf fmt "%a@.%a" pp_type_term (TV.term variant) pp_attributes
...
let pp_relation_definition fmt relation =
  ...
  fprintf fmt "relation %s(%a) -> %a@.%a;" (Rel.name relation)
    (pp_sep_list ~sep:", " pp_named_type_term)
    (Rel.input relation) pp_variants (Rel.output relation) pp_attributes
    (Rel.attributes_to_list relation)

let type_kind_to_string = function
  | TypeKind_Generic -> "typedef"
  | TypeKind_AST -> "ast"
