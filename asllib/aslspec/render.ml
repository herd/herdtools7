  ...
  module RenderRule = struct
    ...
    let pp_field_path fmt path =
      ...
      let pp_dot_field fmt field_name =
        fprintf fmt ".%s" (field_name_to_latex field_name)
      in
    ...
    let rec pp_expr fmt (expr, layout) =
      match expr with
      | Var name ->
          if Spec.is_constant S.spec name then pp_id_as_macro fmt name
          else pp_var fmt name
      ...
      | Application { applicator = ExprOperator op; args }
        when is_prefix_operator op ->
          pp_one_arg_macro fmt
            (operator_to_macro_name op)
            (pp_sep_elements ~pp_sep:pp_comma ~alignment:"l" pp_expr layout)
            args
      | Application { applicator; args } ->
          ...
          fprintf fmt {|%a%a|} pp_lhs applicator
            ...
            args
      | Record { label; fields } ->
          fprintf fmt {|%a%a|} pp_id_as_macro label (pp_fields pp_expr)
            (fields, layout)
      | ListIndex { var; index } ->
    ...
    (** [pp_short_circuit ~is_premise relation_name fmt short_circuit] renders
        the optional short-circuit override expressions [short_circuit] for the
        relation [relation_name] with the formatter [fmt]. If no short-circuit
        expressions are provided, then they are generated from the definition of
        the relation. If [is_premise] is true, then the short-circuit
        expressions are rendered; otherwise, nothing is rendered, since
        conclusion judgments should not include short-circuit expressions. *)
    let pp_short_circuit ~is_premise relation_name fmt short_circuit =
      let short_circuit_separator_macro_name = "terminateas" in
      match (is_premise, short_circuit) with
      | true, None -> (
    ...
    (** [pp_judgment_form ~is_premise fmt (form, layout)] renders the judgment
        form [form] with the formatter [fmt] and laid out according to [layout].
        If [is_premise] is true, then any short-circuit expressions are
        rendered; otherwise this judgment is a conclusion, and they are omitted.
    *)
    let rec pp_judgment_form ~is_premise fmt (form, layout) =
      match form with
      | Expr expr -> pp_expr fmt (expr, layout)
      | Indexed { index; list; body } ->
          let list_of_indices_macro_name = "listrange" in
          fprintf fmt {|%a \in %a(%a) \;:\; %a|} pp_var index pp_macro
            list_of_indices_macro_name pp_var list
            (pp_judgment_form ~is_premise)
            (body, layout)
      | Output _ ->
      ...
      | Transition
          ...
          let pp_rhs_with_short_circuit fmt rhs_with_layout =
            fprintf fmt "%a%a" pp_expr rhs_with_layout
              (pp_short_circuit ~is_premise relation_name)
              short_circuit
          in
      ...
      | Transition _ ->
          let msg =
            Format.asprintf "Unexpected LHS in transition judgment: %a"
              PP.pp_judgment_form form
          in
          raise (SpecError msg)

    (** [pp_judgment ~is_premise fmt judgment] renders the judgment [judgment]
        with the formatter [fmt]. If [is_premise] is true, then any
        short-circuit expressions are rendered; otherwise this judgment is a
        conclusion, and they are omitted. *)
    let pp_judgment ~is_premise fmt ({ Rule.form } as judgment) =
      let layout = Rule.judgment_layout judgment in
      fprintf fmt "{%a}" (pp_judgment_form ~is_premise) (form, layout)
    ...
    let pp_case_name_opt fmt name_opt =
      match name_opt with
      | None -> ()
      | Some name -> fprintf fmt "[%s]" (Latex.escape_underscores name)

    let pp_math_expanded_rule fmt { ExpandRules.name_opt; judgments } =
      let premises, conclusion = Utils.split_last judgments in
      let pp_premise fmt premise = pp_judgment ~is_premise:true fmt premise in
      let pp_conclusion fmt conclusion =
        pp_judgment ~is_premise:false fmt conclusion
      in
