...
```
relation choose(flag: Bool, a: Num, b: Num) -> Num {} =
    case Left {
        flag = True;
        --
        a;
...
This produces rule and prose macros named `choose` for the relation `choose`.
...
```
render rule choose_left = choose(Left);
render rule choose_nested = choose(Outer.Inner);
render rule choose_all = choose(-);
```

The path in parentheses is a dot-separated prefix of the expanded case path.
For example, `choose(Outer)` selects rules under `Outer` and
`choose(Outer.Inner)` selects only rules under `Outer.Inner`. The special path
`-` is the empty path, which is a prefix of every expanded rule path, so
`choose(-)` selects all rules of `choose`.
...
shorthands. For list-shaped objects they use the default layout for every
element, regardless of list length. For example, `x { [_] }`, `r(a, b) { (_) }`,
and `make_set(a, b, c) { [_] }` are all valid.
