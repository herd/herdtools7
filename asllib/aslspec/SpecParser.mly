...
let relation_definition :=
    RELATION; name=IDENTIFIER; input=plist0(named_type_term); ARROW; output=type_variants;
...
let type_attributes ==
    LANGLE_LANGLE; pairs=tclist0(type_attribute); RANGLE_RANGLE; { pairs }

let type_attribute :=
    | PROSE_DESCRIPTION; EQ; template=STRING; { (Attributes.prose_description, template) }
    | MATH_MACRO; EQ; template=STRING; { (Attributes.math_macro, template) }

let relation_attributes ==
    LANGLE_LANGLE; pairs=tclist0(relation_attribute); RANGLE_RANGLE; { pairs }

let relation_attribute :=
    | PROSE_DESCRIPTION; EQ; template=STRING; { (Attributes.prose_description, template) }
    | PROSE_APPLICATION; EQ; template=STRING; { (Attributes.prose_application, template) }
    | MATH_MACRO; EQ; template=STRING; { (Attributes.math_macro, template) }
