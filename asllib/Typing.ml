...
module Annotate (C : ANNOTATE_CONFIG) = struct
  ...
  and annotate_constant_slices ~loc env slices =
    ...
    let+ () =
      ...
      Error.(fatal_from loc (UnpureExpression (E_Slice (var_ "slices", slices) |> add_dummy_pos)))
    in
    slices'
  ...
  and annotate_expr env (e : expr) : ty * expr * side_effects =
    ...
    (* Begin EConcatEmpty *)
    | E_Concat [] ->
        (T_Bits (expr_of_int 0, []) |> here, e, SE.pure)
    ...
    | E_GetFields (e_1, fields) -> (
        let t_e', e_2, _ = annotate_expr env e_1 in
  ...
  let rec annotate_lexpr env le t_e : lexpr * side_effects =
    ...
    | LE_SetFields (le', fields) ->
        ...
        let le', _ses = annotate_lexpr env le' t_le' in
  ...
  let rec annotate_stmt env s : stmt * StaticEnv.env * side_effects =
    ...
    | S_Assign (le, re, ver) ->
        (let () =
         ...
         match reduced with
         | Some (s, ses) -> (s, env, ses)
         | None ->
             let t_e, e1, ses1 = annotate_expr env re in
             let env1 =
               match ver with
               | V1 -> env
               | V0 -> (
             ...
             in
             let le1, ses2 = annotate_lexpr env1 le t_e in
             let ses = SE.union ses1 ses2 in
             (S_Assign (le1, e1, ver) |> here, env1, ses))
        |: TypingRule.SAssign
    (* End *)
    (* Begin SCall *)
  ...
  let declare_global_storage loc gsd env =
    ...
    let typed_initial_value =
      ...
      | Some e -> Some (annotate_sef_expr env e)
      | None -> None
    in
    let declared_t =
