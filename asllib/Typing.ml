...
let conflict ~loc expected provided =
  fatal_from ~loc
    (Error.ConflictingTypes (Error.UnexpectedType, expected, provided))

let type_satisfaction_conflict ~loc expected provided =
  fatal_from ~loc
    (Error.ConflictingTypes (Error.TypeSatisfaction, expected, provided))
...
module Annotate (C : ANNOTATE_CONFIG) : S = struct
  ...
  let check_type_satisfies ~loc env t1 t2 () =
    ...
    else type_satisfaction_conflict ~loc [ t2.desc ] t1
  ...
  and annotate_type ?(decl = false) ~(loc : 'a annotated) env ty : ty * SES.t =
    ...
    | T_Array (e_length, t_elem) ->
        let annot_t_elem, ses_t = annotate_type ~loc env t_elem
        and annot_length, ses_length =
          annotate_symbolic_constrained_integer ~loc env e_length
        in
        let ses = SES.union ses_t ses_length in
        (T_Array (annot_length, annot_t_elem) |> here, ses) |: TypingRule.TArray
        ...
        | _ -> (
            ...
            | T_Int _ | T_Bits _ ->
                ...
                *)
                ...
                |: TypingRule.ESlice
            (* End *)
            | T_Array (_, ty') when e'.version = V0 -> (
                match slices with
  ...
  let check_can_be_initialized_with ~loc env s t () =
    ...
    else type_satisfaction_conflict ~loc [ s.desc ] t
  (* End *)
