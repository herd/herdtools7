...
let reduce_constants env e =
  ...
  let eval_expr env e =
    ...
    with NotYetImplemented -> Error.(fatal_from e (UnsupportedExpr e))
  in
  try eval_expr env e
  with StaticEvaluationUnknown -> (
    ...
    with StaticEvaluationUnknown -> Error.(fatal_from e (UnsupportedExpr e)))
...
module Annotate (C : ANNOTATE_CONFIG) = struct
  ...
  let get_variable_enum' env e =
    match e.desc with
    | E_Var x -> (
        let t = T_Named x |> add_pos_from e in
        match (Types.make_anonymous env t).desc with
  ...
  let annotate_func_sig ~loc env (f : AST.func) : env * AST.func =
    ...
    let env2, declared_params =
      let with_def = get_undeclared_defining env1 f in
      let () =
        if false then
      ...
      let folder (env1', acc) (x, ty_opt) =
        ...
        let+ () =
          check_true (ISet.mem x with_def) @@ fun () ->
