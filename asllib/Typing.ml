...
module Annotate (C : ANNOTATE_CONFIG) : S = struct
          ...
          (let () =
           ...
           try
             ...
             | ty, LDK_Constant ->
                 ...
                 in
                 (ty, e, SES.empty)
             | ty, _ -> (ty, e, SES.read_local x)
           with Not_found -> (
             try
               match IMap.find x env.global.storage_types with
               | ty, GDK_Constant -> (
                   match IMap.find_opt x env.global.constant_values with
                   | Some v -> (ty, E_Literal v |> here, SES.empty)
                   | None -> (ty, e, SES.empty))
               | ty, _ -> (ty, e, SES.read_global x)
             with Not_found ->
               let () =
                 if false then
                   Format.eprintf "@[Cannot find %s in env@ %a.@]@." x pp_env
                     env
               in
  ...
  let rec annotate_lexpr env le t_e =
    ...
    | LE_Concat (les, _) ->
        ...
        let annotate_lebitslice (les, widths, debug_sum, sess) le =
          ...
          (le1 :: les, width :: widths, binop PLUS debug_sum width, ses :: sess)
  ...
  let rec annotate_stmt env s =
    ...
    | S_Decl (ldk, ldi, e_opt) -> (
        ...
        | LDK_Var, None ->
            ...
            (S_Decl (LDK_Var, ldi1, None) |> here, new_env, SES.empty)
    ...
    let env2 =
      ...
      | GDK_Constant, Some (_, e, ses_e) ->
          let+ () =
            ...
            fatal_from loc Error.(ImpureExpression (e, ses_e))
          in
