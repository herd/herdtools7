...
module Annotate (C : ANNOTATE_CONFIG) : S = struct
  ...
  let rec annotate_bitfield ~loc env width bitfield : bitfield =
    let check_static_slices ses =
      check_true (SES.is_empty ses) @@ fun () -> assert false
    in
    match bitfield with
    | BitField_Simple (name, slices) ->
        let slices1, ses = annotate_slices ~loc env slices in
        let+ () = check_static_slices ses in
  ...
  and annotate_slices env =
    ...
    *)
    ...
    in
    fun ~loc slices ->
      let slices, sess = List.map tr_one slices |> List.split in
      let ses =
  ...
  and annotate_call_arg_typed loc env name caller_args_typed call_type =
    ...
    *)
    ...
    (name1, args1, eqs3, ret_ty_opt, ses3) |: TypingRule.AnnotateCallArgTyped
  (* End *)
  ...
  and annotate_expr env (e : expr) : ty * expr * SES.t =
    ...
    (* Begin ELit *)
    | E_Literal v ->
        (annotate_literal v |> here, e, SES.empty) |: TypingRule.ELit
    (* End *)
    (* Begin ATC *)
    | E_ATC (e', ty) ->
        let t, e'', ses = annotate_expr env e' in
        ...
        (if Types.subtype_satisfies env t ty' then (ty', e'', ses)
        ...
        if should_reduce_to_call env x ST_EmptyGetter then
          ...
          try
            ...
            | ty, LDK_Constant ->
                ...
                in
                (ty, e, SES.empty)
            | ty, _ -> (ty, e, SES.read_local x) |: TypingRule.ELocalVar
            (* End *)
          with Not_found -> (
            try
              ...
              | ty, GDK_Constant -> (
                  ...
                  | Some v ->
                      (ty, E_Literal v |> here, SES.empty)
                      |: TypingRule.EGlobalVarConstantVal
                  (* End *)
                  (* Begin EGlobalVarConstantNoVal *)
                  | None ->
                      (ty, e, SES.empty) |: TypingRule.EGlobalVarConstantNoVal)
              (* End *)
              (* Begin EGlobalVar *)
              | ty, _ -> (ty, e, SES.read_global x) |: TypingRule.EGlobalVar
              (* End *)
              (* Begin EUndefIdent *)
            with Not_found ->
              let () =
    ...
    | E_Unknown ty ->
        ...
        (ty1, E_Unknown ty2 |> here, SES.non_deterministic)
        |: TypingRule.EUnknown
    (* End *)
    | E_Slice (e', slices) -> (
        ...
        | _ -> (
            ...
            | T_Array (size, ty') -> (
                ...
                | [ Slice_Single e_index ] ->
                    ...
                    in
                    let ses = SES.union ses1 ses_index in
    ...
    | E_GetField (e1, field_name) -> (
        ...
        | None -> (
            ...
            | T_Exception fields | T_Record fields -> (
                ...
                | Some t ->
                    (t, E_GetField (e2, field_name) |> here, ses)
  ...
  let rec annotate_lexpr env le t_e =
    ...
    | LE_Slice (le1, slices) -> (
        ...
        | T_Array (size, t) -> (
            ...
            | [ Slice_Single e_index ] ->
                ...
                in
                let ses = SES.union ses1 ses_index in
                (LE_SetArray (le2, e_index') |> here, ses)
    ...
    | LE_Concat (les, _) ->
        ...
        let t_e_eq, _e_eq, _ses_eq = annotate_expr env e_eq in
        let+ () = check_bits_equal_width le env t_e_eq t_e in
        let annotate_one (les, widths, sum, sess) le =
          let e = expr_of_lexpr le in
          let t_e1, _e, _ses = annotate_expr env e in
          let width = get_bitvector_width le env t_e1 in
          let t_e2 = T_Bits (width, []) |> add_pos_from le in
          let le1, ses = annotate_lexpr env le t_e2 in
          (le1 :: les, width :: widths, binop PLUS sum width, ses :: sess)
        in
        let rev_les, rev_widths, _real_width, sess =
        ...
        (LE_Concat (les1, Some widths) |> add_pos_from le, ses)
        |: TypingRule.LEConcat
  (* End *)
  ...
  let add_immutable_expressions ~loc env ldk typed_e_opt x =
    ...
    | (LDK_Constant | LDK_Let), Some (_, e, ses_e)
      when is_statically_evaluable ~loc env ses_e -> (
        ...
        | Some e' -> add_local_immutable_expr x e' env
        | None -> env)
    | _ -> env
  ...
  let rec annotate_stmt env s =
        ...
        (let () =
           if false then
             Format.eprintf "@[<3>Annotating assignment@ @[%a@]@]@." PP.pp_stmt
               s
         in
         let t_re, re1, ses_re = annotate_expr env re in
         let reduced =
           setter_should_reduce_to_call_s env le (t_re, re1, ses_re)
         in
         match reduced with
    ...
    | S_Return e_opt ->
        ...
        (* Begin SReturnNone *)
        | None, None ->
            (S_Return None |> here, env, SES.empty) |: TypingRule.SReturnNone
        (* End *)
        (* Begin SReturnSome *)
        | Some t, Some e ->
            let t_e', e', ses = annotate_expr env e in
    ...
    (* Begin SCase *)
    | S_Case (e, cases) ->
        let t_e, e1, ses_e = annotate_expr env e in
        let annotate_case case =
          let { pattern = p0; where = w0; stmt = s0 } = case.desc in
          let p1 = annotate_pattern e1 env t_e p0
          and s1, ses0 = try_annotate_block env s0
          and w1 =
            match w0 with
            | None -> None
            | Some e_w0 ->
                let twe, e_w1 = (annotate_static_expr env) e_w0 in
                let+ () = check_structure_boolean e_w0 env twe in
                Some e_w1
          in
          (add_pos_from_st case { pattern = p1; where = w1; stmt = s1 }, ses0)
        in
        let cases1, sess = List.map annotate_case cases |> List.split in
        let ses = SES.unions (ses_e :: sess) in
    ...
    | S_Repeat (s1, e1, limit1) ->
        let s2, ses1 = try_annotate_block env s1 in
        let limit2 = annotate_loop_limit ~loc env limit1 in
        let t, e2, ses2 = annotate_expr env e1 in
        let+ () = check_type_satisfies e2 env t boolean in
        let ses = SES.union ses1 ses2 in
    ...
    | S_Decl (ldk, ldi, e_opt) -> (
        ...
        (* Begin SDeclSome *)
        | _, Some e ->
            let ((t_e, e', ses) as typed_e) = annotate_expr env e in
            let env1, ldi1 =
            ...
            in
            (S_Decl (ldk, ldi1, Some e') |> here, env2, ses)
            |: TypingRule.SDeclSome
        (* End *)
        (* Begin SDeclNone *)
        | LDK_Var, None ->
            let env', ldi' = annotate_local_decl_item_uninit loc env ldi in
            (S_Decl (LDK_Var, ldi', None) |> here, env', SES.empty)
    ...
    | S_Throw (Some (e, _)) ->
        ...
        (S_Throw (Some (e', Some t_e)) |> here, env, ses2)
        |: TypingRule.SThrowSome
    (* End *)
    (* Begin SThrowNone *)
    | S_Throw None ->
        (* TODO: verify that this is allowed? *)
        (s, env, SES.throw_exception "TODO") |: TypingRule.SThrowNone
    (* End *)
    (* Begin STry *)
  ...
  and annotate_loop_limit ~loc env = function
    | None -> None
    | Some limit ->
        let t, limit' = annotate_static_expr env limit in
        let+ () = check_constrained_integer ~loc env t in
  ...
  and annotate_catcher loc env ses_in (name_opt, ty, stmt) =
    ...
    and ses_filtered =
      ...
      SES.filter_thrown_exceptions
        ...
        ses_in
    in
    (ses_filtered, ((name_opt, ty, new_stmt), ses_block))
  ...
  and set_fields_should_reduce_to_call ~loc env x fields (t_e, e, ses_e) =
    ...
    let name, args, eqs, ret_ty, ses_call =
      annotate_call_arg_typed loc env name typed_args ST_Setter
    in
    ...
    Some (S_Call (name, args, eqs) |> add_pos_from loc, ses)

  and setter_should_reduce_to_call_recurse ~loc env (t_e, e, ses_e) make_old_le
      sub_le =
    ...
    in
    let s1, ses_s1 =
      (S_Decl (LDK_Var, ldi_x, Some sub_re) |> here, ses_sub_re)
    in
    let s2, ses_s2 =
      let old_le = make_old_le (LE_Var x |> here) in
      let old_le', ses_old_le = annotate_lexpr env1 old_le t_e in
      (S_Assign (old_le', e, V1) |> here, SES.union ses_old_le ses_e)
    in
            ...
            if List.for_all Option.is_none subs then None
              ...
              let produce_one i sub_le t_sub_e_i = function
                | None ->
                    ...
                    in
                    (S_Assign (sub_le', sub_e i, V1) |> here, sub_le_ses)
                | Some (s, ses) -> (s, ses)
              in
              let stmts, sess =
  ...
  *)
  ...
  (* Begin Subprogram *)
  let annotate_subprogram (env : env) (f : AST.func) : AST.func * SES.t =
    let () =
      if false then
    ...
    let ses' = SES.remove_locals ses in
    let () =
      if false then
  ...
  let try_annotate_subprogram env f =
    best_effort (f, SES.empty) (fun _ -> annotate_subprogram env f)
  ...
  let declare_one_func loc (func_sig : func) env =
    ...
    let init_ses =
      ...
      | SB_ASL _ | SB_Primitive true -> SES.recursive_call name'
      | SB_Primitive false -> SES.empty
    in
  ...
  let declare_global_storage loc gsd genv =
    ...
    (* AnnotateTypeOpt) *)
    (* AnnotateExprOpt( *)
    and initial_value_type, initial_value', typed_initial_value =
      match initial_value with
      | Some e ->
          let t, e', ses_e = annotate_expr env e in
          (Some t, Some e', Some (t, e', ses_e))
      | None -> (None, None, None)
      (* AnnotateExprOpt) *)
    in
    let declared_t =
    ...
    let env2 =
      ...
      | GDK_Let, Some (_, e, ses_e) when is_statically_evaluable ~loc env1 ses_e
        -> (
          match StaticModel.normalize_opt env1 e with
    ...
    let env_and_fs =
      List.map
        (fun d ->
          ...
          | D_Func f ->
              let loc = to_pos d in
              let env', f = annotate_func_sig ~loc genv0 f in
              (env'.local, f, loc)
          | _ ->
              fatal_from d
      ...
      List.map
        (fun (lenv, f, loc) ->
          ...
          | SB_ASL _ ->
              ...
              in
              let new_f, ses_f = try_annotate_subprogram env' f in
              (D_Func new_f |> here, (new_f, ses_f))
          | SB_Primitive side_effecting ->
              let new_f = rename_primitive loc env' f in
              let ses =
                if side_effecting then SES.recursive_call new_f.name
                else SES.empty
              in
              (D_Func new_f |> here, (new_f, ses)))
        env_and_fs2
      |> List.split
    in
    let env2 =
      ...
      List.fold_left
        (fun env1' ((new_f : func), ses_f) ->
          StaticEnv.add_subprogram new_f.name new_f ses_f env1')
        (StaticEnv.with_empty_local genv1)
        sess
    in
    (List.rev_append ds acc, env2.global) |: TypingRule.TypeCheckMutuallyRec
  (* End *)
