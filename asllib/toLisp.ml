...
module Make (Conf : PrinterConf) = struct
  (* -------------------------------------------------------------------------
  ...
  let rec tree_of_list_aux x n =
    ...
    | _ ->
        let half = n / 2 in
        Cons
          (tree_of_list_aux x half, tree_of_list_aux (nth_tl x half) (n - half))
  ...
  let of_annotated of_a x =
    if Conf.compact then of_a x.desc
    else
      aslsym_alist
  ...
  let rec of_expr_desc x =
    tagged_list_of_list
      ...
      | E_Binop (op, x, y) ->
          [ key "E_BINOP"; of_binop op; of_expr x; of_expr y ]
      ...
      | E_Pattern (x, p) -> [ key "E_PATTERN"; of_expr x; of_pattern_matcher p ])
  ...
  and of_type_desc x =
    tagged_list_of_list
      ...
      | T_Array (i, t) -> [ key "T_ARRAY"; of_expr i; of_ty t ]
  ...
  let rec of_lexpr_desc x =
    tagged_list_of_list
      ...
      | LE_SetField (lx, i) ->
          [ key "LE_SETFIELD"; of_lexpr lx; of_identifier i ]
      ...
      | LE_Destructuring lx ->
          [ key "LE_DESTRUCTURING"; of_list_map of_lexpr lx ])
  ...
  let rec of_stmt_desc x =
    tagged_list_of_list
      ...
      | S_Cond (x, s1, s2) ->
          [ key "S_COND"; of_expr x; of_stmt s1; of_stmt s2 ]
  ...
  and of_case_alt_desc x =
    aslsym_alist
      [
        ("PATTERN", of_pattern_matcher x.pattern.desc);
  ...
  let of_imap of_a x =
    of_list_map
      (fun (k, v) -> Cons (String k, of_a v))
      (ASTUtils.IMap.bindings x)
