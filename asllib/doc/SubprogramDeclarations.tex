...
The function
\[
\extractparameters(\overname{\staticenvs}{\tenv}, \overname{\func}{\funcsig}) \aslto \overname{\identifier^*}{\allparameters}
  \cup \overname{\TTypeError}{\TypeErrorConfig}
\]
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \Proseeqdef{$\allparameters$}{the empty list}.
  \end{itemize}

  \item \AllApplyCase{non\_empty}
  \begin{itemize}
    \item applying $\funcsigtypes$ to $\funcsig$ yields the list of types $\vt_{1..k}$;
    \item applying $\paramsofty$ to $\tenv$ and $\vtypes[\vi]$ yields the list of parameter names $\vparams_\vi$, for every $\vi=1..k$;
    \item \Proseeqdef{$\allparameters$}{the concatenation of lists $\vparams_\vi$, for every $\vi=1..k$};
    \item checking that $\allparameters$ does not contain duplicate names yields $\True$\ProseOrTypeError.
  \end{itemize}
\end{itemize}
...
\inferrule[empty]{
  ...
  \extractparameters(\tenv, \funcsig) \typearrow \overname{\emptylist}{\allparameters}
}
\end{mathpar}
...
\inferrule[non\_empty]{
  ...
  \vi=1..k: \paramsofty(\tenv, \vtypes[\vi]) \typearrow \vparams_\vi\\
  \allparameters \eqdef \vparams_1 \concat \ldots \concat \vparams_k\\
  \checktrans{\uniquelist(\allparameters) = \allparameters}{\BadSubprogramDeclaration} \typearrow \True \OrTypeError
}{
  \extractparameters(\tenv, \funcsig) \typearrow \allparameters
}
\end{mathpar}
