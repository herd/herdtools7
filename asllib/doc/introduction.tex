...
all understand. It can intentionally under-specify behaviours in the architecture being described.
...
More specifically, a \specificationterm{} is a set of \globaldeclarationsterm{}
written in ASL code that describe an architecture.
...
\begin{itemize}
    ...
    \item A function \texttt{MyOR} demonstrating a simple bitwise OR function over two bitvectors.
    \item Initialisation of \texttt{R0} and \texttt{R1} bitvectors.
...
\item A procedure \texttt{ColdReset} to initialise the state of the system when power is applied and the system is reset.
...
\begin{itemize}
    \item A function \texttt{Dot8}, which operates on two bitvectors, one byte at a time.
...
\begin{description}
    ...
    \item[AST build rules] Rules for building an AST from parse trees.
        ...
        and are defined in terms of \inferencerules;
    \item[Typing rules] Rules expressing the type system.
        ...
        and a paragraph titled \emph{Formally}, which defines the rule in terms of \inferencerules.
        Typing rules are accompanied by examples;
    \item[Dynamic semantics rules] Rules expressing the dynamic semantics.
        ...
        which defines the rule in prose,
        and a paragraph titled \emph{Formally}, which defines the rule in terms of \inferencerules.
        Semantics rules are accompanied by examples.
\end{description}
...
\begin{itemize}
    ...
    \item \chapref{TypeChecking} and \chapref{Semantics} introduce basic
        definitions needed to formalise the type system and dynamic semantics;
    ...
    \item \chapref{TopLevel} is where all of the formalisms are used together to
        demonstrate how they can be utilised to form an interpreter for an ASL specification;
