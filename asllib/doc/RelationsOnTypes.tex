...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \begin{itemize}
        \item $\vt$ is a well-constrained integer type with constraints $\cst$ and precision $\vpone$;
        \item $\vs$ is a well-constrained integer type with constraints $\css$ and precision $\vpone$;
        \item applying $\precisionjoin$ on $\vpone$ and $\vptwo$ yields $\vp$;
        \item $\tty$ is the well-constrained integer type with constraints $\cst \concat \css$ and precision $\vp$.
      \end{itemize}
...
{
  \typeequal(\tenv, \vt, \vs) \typearrow \False\\
  \vt \eqname \TInt(\wellconstrained(\cst, \vpone))\\
  \vs \eqname \TInt(\wellconstrained(\css, \vptwo))\\
...
\begin{itemize}
  \item $\op$ is $\NEG$;
  \item obtaining the \wellconstrainedstructure\ of $\vt$ yields the well-constrained integer type with constraints $\vcs$ and precision $\vp$\ProseOrTypeError;
  \item negating the constraints in $\vcs$ (see $\negateconstraint$) yields $\vcsnew$;
  \item $\vs$ is the well-constrained integer type with constraints $\vcsnew$ and precision $\vp$, that is, \\
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item $\vtone$ is the well-constrained integer type with constraints $\csone$ and precision $\vpone$;
    \item $\vttwo$ is the well-constrained integer type with constraints $\cstwo$ and precision $\vptwo$;
    ...
    \item $\vt$ is the well-constrained integer type with constraints $\vc$ and precision $\vp$\
  \end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item \Proseeqdef{$(\csonearg, \cstwoarg, \vp)$}{$(\csonee, \cstwoe, \PrecisionFull)$ if \\
            $\vexpectedconstraintlength$ is
            less than $\maxconstraintsize$ and $(\csone, \cstwof, \PrecisionLost)$, otherwise};
    \end{itemize}
...
\inferrule[exploding]{
  ...
  {
  (\csonearg, \cstwoarg, \vp) \eqdef \begin{cases}
    (\csonee, \cstwoe, \PrecisionFull) & \text{if }\vexpectedconstraintlength < \maxconstraintsize\\
    (\csone, \cstwof, \PrecisionLost ) & \text{else}
  \end{cases}
  }\\\\
  \commonsuffixline\\\\
...
\]
returns the indicator denoting if either $\vpone$ or $\vptwo$ denote a precision loss.

\ProseParagraph
\OneApplies
\begin{itemize}
  \item $\vpone$ is $\PrecisionLost$ and $\vp$ is $\PrecisionLost$;
  \item $\vptwo$ is $\PrecisionLost$ and $\vp$ is $\PrecisionLost$;
  \item $\vpone$ and $\vptwo$ are $\PrecisionFull$ and $\vp$ is $\PrecisionFull$.
\end{itemize}
