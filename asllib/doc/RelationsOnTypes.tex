...
\begin{itemize}
  \item $\op$ is $\NEG$;
  \item obtaining the \wellconstrainedstructure\ of $\vt$ yields the well-constrained integer type with constraints $\vcs$ and \precisionlossindicatorterm{} $\vp$\ProseOrTypeError;
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item checking that $\vtone$ is a \singulartype{} yields $\True$\ProseOrTypeError;
    \item checking that $\vttwo$ is a \singulartype{} yields $\True$\ProseOrTypeError;
  ...
  \begin{itemize}
    ...
    where $\vlone$ and $\vltwo$ are the AST labels of any \singulartypes{}:
    \[
    \begin{array}{lll}
      \hline
