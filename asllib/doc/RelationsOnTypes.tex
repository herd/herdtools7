  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item \Proseeqdef{$(\csonearg, \cstwoarg)$}{$(\csonee, \cstwoe)$ if \\
            ...
            less than $2^{17}$ and $(\csone, \cstwof)$, otherwise};
    \end{itemize}
...
\inferrule[exploding]{
  ...
  (\csonearg, \cstwoarg) \eqdef \begin{cases}
    (\csonee, \cstwoe) & \text{if }\vexpectedconstraintlength < 2^{17}\\
...
\subsubsection{Prose}
The value $\vb$ is $\True$ if and only if $\vztwo-\vzone$ is greater than $2^{14}$.

\subsubsection{Formally}
\begin{mathpar}
\inferrule{}{
  \intervaltoolarge(\vzone, \vztwo) \typearrow \overname{\vztwo-\vzone > 2^{14}}{\vb}
}
\end{mathpar}
