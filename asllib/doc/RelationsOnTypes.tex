...
\begin{itemize}
  ...
  \item The Lowest Common Ancestor of two types (\secref{TypingRule.LowestCommonAncestor})
  \item Applying a unary operator to a type (\secref{TypingRule.ApplyUnopType})
  \item Applying a binary operator to a pair of types (\secref{TypingRule.ApplyBinopTypes})
\end{itemize}

\subsubsection{TypingRule.Subtype\label{sec:TypingRule.Subtype}}
...
\subsubsection{TypingRule.SubtypeSatisfaction\label{sec:TypingRule.SubtypeSatisfaction}}
...
\subsubsection{TypingRule.TypeSatisfaction \label{sec:TypingRule.TypeSatisfaction}}
...
\subsubsection{TypingRule.TypeClash\label{sec:TypingRule.TypeClash}}
...
\subsubsection{TypingRule.LowestCommonAncestor \label{sec:TypingRule.LowestCommonAncestor}}
...
\subsubsection{TypingRule.ApplyUnopType \label{sec:TypingRule.ApplyUnopType}}
...
\inferrule[neg\_error]{
  ...
  \applyunoptype(\tenv, \overname{\NEG}{\op}, \vt) \typearrow \TypeErrorVal{InappropriateTypeForNeg}
}
\end{mathpar}
...
\subsubsection{TypingRule.ApplyBinopTypes\label{sec:TypingRule.ApplyBinopTypes}}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item applying \normalize{} to $\vw$ in $\tenv$ yields $\wp$;
    \item $\vt$ is the bitvector type of width $\wp$ and empty list of bitfields, that is, \\ $\TBits(\vw, \emptylist)$.
  \end{itemize}
...
\subsubsection{TypingRule.FindNamedLCA \label{sec:TypingRule.FindNamedLCA}}
...
\subsubsection{TypingRule.AnnotateConstraintBinop\label{sec:TypingRule.AnnotateConstraintBinop}}
...
\subsubsection{TypingRule.BinopFilterRhs \label{sec:TypingRule.BinopFilterRhs}}
...
\subsubsection{TypingRule.RefineConstraintBySign \label{sec:TypingRule.RefineConstraintBySign}}
...
\subsubsection{TypingRule.ReduceToZOpt \label{sec:TypingRule.ReduceToZOpt}}
...
\subsubsection{TypingRule.RefineConstraints \label{sec:TypingRule.RefineConstraints}}
...
\subsubsection{TypingRule.RefineConstraintForDiv \label{sec:TypingRule.RefineConstraintForDiv}}
...
\inferrule[div]{
  ...
  \checktrans{\vres \neq \emptylist}{\BinaryOperationFailsAllConstraints} \checktransarrow \True \OrTypeError
}{
...
\subsubsection{TypingRule.FilterReduceConstraintDiv \label{sec:TypingRule.FilterReduceConstraintDiv}}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item $\langle\vc\rangle$, if $\vztwo$ is positive and $\vztwo$ divides $\vzone$;
      \item $\None$, otherwise.
    \end{itemize}
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item $\vc$ if if $\vzoneopt$ is $\None$ and $\vztwoopt$ is $\None$.
    \end{itemize}
  \end{itemize}
\end{itemize}
...
\inferrule[exact]{
  \getliteraldivopt(\ve) \typearrow \langle(\vzone, \vztwo)\rangle \terminateas \None\\\\
  {
    \vcopt \eqdef
    \begin{cases}
      \langle\vc\rangle & \text{if }\vztwo > 0 \land \frac{\vzone}{\vztwo} \in \Z\\
      \None & \text{else}
    \end{cases}
  }
}{
...
\subsubsection{TypingRule.GetLiteralDivOpt \label{sec:TypingRule.GetLiteralDivOpt}}
...
\subsubsection{TypingRule.ExplodeIntervals \label{sec:TypingRule.ExplodeIntervals}}
...
\subsubsection{TypingRule.ExplodeConstraint \label{sec:TypingRule.ExplodeConstraint}}
...
\subsubsection{TypingRule.IntervalTooLarge \label{sec:TypingRule.IntervalTooLarge}}
...
\subsubsection{TypingRule.BinopIsExploding\label{sec:TypingRule.BinopIsExploding}}
...
\subsubsection{TypingRule.GetBitvectorConstWidth}
