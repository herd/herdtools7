...
\begin{itemize}
  \item Same named types (\TypingRuleRef{SameNamedType})
...
\TypingRuleDef{SameNamedType}
\RenderRelation{same_named_type}

In \listingref{same_named_type}, \verb|time| and \verb|space| are both named types of the same underlying type \verb|integer|,
but they are not the same named type.
\ASLListing{Same-named type}{same_named_type}{\typingtests/TypingRule.SameNamedType.asl}

\RenderProseAndFormally{same_named_type}
\CodeSubsection{\SameNamedTypeBegin}{\SameNamedTypeEnd}{../types.ml}
...
\listingref{subtypesat3} shows more examples of legal and illegal assignments in the context
of subtype-satisfaction.
...
In \listingref{typing-typesat3}, checking whether \verb|(integer, T2)|
...
\listingref{example-lca-bad} shows an example of an erroneous conditional expressions
where no lowest common ancestor exists.
\ASLListing{Lowest common ancestor error}{example-lca-bad}{\typingtests/TypingRule.LowestCommonAncestor.bad.asl}
...
constraints.
For example, given a range constraint \verb|A..B| and an exact constraint \verb|2|,
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item define $\vf$ as the specialisation of $\refineconstraintbysign$ for the predicate
...
\inferrule[no\_filtering]{
  ...
  \binopfilterrhs(\vapprox, \tenv, \op, \cs) \typearrow \overname{\cs}{\newcs}
}
\end{mathpar}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item applying $\reducetozopt$ to $\ve$ in $\tenv$, in order to symbolically simplify $\ve$ to an integer,
          yields $\some{\vz}$;
    \item $\vcopt$ is $\some{\vc}$ if $\vp$ holds for $\vz$ and $\none$ otherwise.
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item if $\vp$ is $\False$ for both $\vzone$ and $\vztwo$, define $\vcopt$ as $\none$.
    \end{itemize}
  \end{itemize}
...
\inferrule[exact\_reduces\_to\_z]{
  ...
  \vcopt \eqdef \ifthenelseop[H]{\vp(\vz)}{\some{\vc}}{\none}
}{
...
\inferrule[only\_e1\_reduces\_to\_z]{
  ...
  \reducetozopt(\tenv, \veone) \typearrow \some{\vzone}\\
  \reducetozopt(\tenv, \vetwo) \typearrow \none\hva\\
...
\inferrule[only\_e2\_reduces\_to\_z]{
  \vc = \ConstraintRange(\veone, \vetwo)\\
  \reducetozopt(\tenv, \veone) \typearrow \none\\
  \reducetozopt(\tenv, \vetwo) \typearrow \some{\vztwo}\\
...
and one that cannot be reduced into an integer constant, which yields $\none$.
...
used by $\refineconstraintbysign$, the resulting $\none$ is also filtered out.
...
\begin{mathpar}
\inferrule[empty]{}{
  \refineconstraints(\vapprox, \vf, \overname{\emptylist}{\cs}) \typearrow \overname{\emptylist}{\newcs}
}
\end{mathpar}
...
\getliteraldivopt(\AbbrevEBinop{\DIV}{\AbbrevEVar{\vA}}{\ELInt{2}}) &\typearrow& \none\\
\end{array}
\]
...
\makeconstraintrangeopt(42, 0) &\typearrow& \none\\
\end{array}
\]
...
\ExampleDef{Converting Parameterised Integer Types to Well-constrained Integer Types}
...
\begin{mathpar}
\inferrule{}{
  \towellconstrained(\overname{\unconstrainedinteger}{\vt}) \typearrow \unconstrainedinteger
}
\end{mathpar}
