...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vtone$ is a named type with name $\idone$, that is $\TNamed(\idone)$;
    \item $\vttwo$ is a named type with name $\idtwo$, that is $\TNamed(\idtwo)$, such that $\idone$ is different from $\idtwo$;
    \item the \globalstaticenvironmentterm{} maintains that $\idone$ does not subtype any named type;
    \item $\vb$ is $\False$.
  \end{itemize}
...
\inferrule[transitive]{
  \idone \neq \idtwo\\
  \tenv.\staticenvsG.\subtypes(\idone) = \idthree\\
...
\inferrule[no\_supertype]{
  \idone \neq \idtwo\\
  \tenv.\staticenvsG.\subtypes(\idone) = \bot
}{
...
\item \AllApplyCase{t\_tuple}
  \begin{itemize}
  \item $\vs$ has the \underlyingtypeterm\ of a tuple with type list $\vlis$ ($\TTuple(\vlis)$);
  \item $\vt$ has the \underlyingtypeterm\ of a tuple with type list $\vlit$ ($\TTuple(\vlit)$);
...
\inferrule[t\_int]{
  \makeanonymous(\tenv, \vt) \typearrow \vttwo\\
  \makeanonymous(\tenv, \vs) \typearrow \vstwo\\
  \astlabel(\vttwo) = \astlabel(\vstwo) = \TInt\hva\\
...
\inferrule[t\_bits]{
  ...
  \bitfieldsincluded(\tenv, \bfss, \bfst) \typearrow \True \OrTypeError \hva\\
...
\inferrule[t\_array\_expr]{
  ...
  \typeequal(\tenv, \vtys, \vtyt) \typearrow \True \terminateas \False\hva\\
  \booltrans(\astlabel(\vlengths) = \astlabel(\vlengtht)) \booltransarrow \True \terminateas \False\\
  \vlengths \reverseeqdef \ArrayLengthExpr(\vlengthexprs)\hva\\
  \vlengtht \reverseeqdef \ArrayLengthExpr(\vlengthexprt)\\
...
\inferrule[t\_array\_enum]{
  \makeanonymous(\tenv, \vs) \typearrow \TArray(\vlengths,\vtys) \\
  \makeanonymous(\tenv, \vt) \typearrow \TArray(\vlengtht,\vtyt) \hva\\
  \typeequal(\tenv, \vtys, \vtyt) \typearrow \True\\
  \booltrans(\astlabel(\vlengths) = \astlabel(\vlengtht)) \typearrow \True \terminateas \False \hva\\
  \vlengths \reverseeqdef \ArrayLengthEnum(\vnames, \Ignore)\\
  \vlengtht \reverseeqdef \ArrayLengthEnum(\vnamet, \Ignore)\\
...
{ \makeanonymous(\tenv, \vs) \typearrow\TTuple(\vlis)\\
  ...
  \equallength(\vlis, \vlit) \typearrow\True \terminateas \False\hva\\
...
\inferrule[structured]{
  ...
  \makeanonymous(\tenv, \vt)\typearrow L(\vfieldst) \hva\\
  \vnamess \eqdef \fieldnames(\vfieldss)\\
  \vnamest \eqdef \fieldnames(\vfieldst)\\
  \booltrans(\vnamess \subseteq \vnamest) \booltransarrow \True \terminateas \False\hva\\
...
\inferrule[anonymous]{
  ...
  \isanonymous(\tenv, \vs) \typearrow \vbtwo\hva\\
  \vbone \lor \vbtwo\\
...
\inferrule[t\_bits]{
  ...
  \tstruct(\tenv, \vs) \typearrow \TBits(\widths, \Ignore) \OrTypeError\hva\\\\
...
\inferrule[otherwise1]{
  ...
  \isanonymous(\tenv, \vs) \typearrow \vbtwo\hva\\
  \subtypesatisfies(\tenv, \vt, \vs) \typearrow \vbthree\\
  \neg((\vbone \lor \vbtwo) \land \vbthree)\hva\\
...
\inferrule[otherwise2]{
  ...
  \subtypesatisfies(\tenv, \vt, \vs) \typearrow \vbthree\\
  \neg((\vbone \lor \vbtwo) \land \vbthree)\\
  \tstruct(\tenv, \vs) \typearrow \vsstruct\hva\\
...
\inferrule[okay]{
  ...
  \checktypesatisfies(\tenv, \vt, \vs) \typearrow \True
}
\end{mathpar}
...
\inferrule[error]{
  ...
  \checktypesatisfies(\tenv, \vt, \vs) \typearrow \TypeErrorVal{\TypeSatisfactionFailure}
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{check_type_satisfies}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \begin{itemize}
        \item neither $\vt$ nor $\vs$ is an unconstrained integer type;
        ...
        \item $\tty$ is the lowest common ancestor of $\vtone$ and $\vsone$ in $\tenv$\ProseOrTypeError.
      \end{itemize}
      ...
      \item \AllApplyCase{t\_bits}
      \begin{itemize}
        \item $\vt$ is a bitvector type with length expression $\vet$ ($\TBits(\vet, \Ignore)$);
        \item $\vs$ is a bitvector type with length expression $\ves$ ($\TBits(\ves, \Ignore)$);
...
\inferrule[named\_subtype1]{
  ...
  \namedlca(\tenv, \namesubs, \namesubt) \typearrow \None \OrTypeError\hva\\\\
  \makeanonymous(\tenv, \vs) \typearrow \vanons \OrTypeError\hva\\\\
  \makeanonymous(\tenv, \vt) \typearrow \vanont \OrTypeError\hva\\\\
...
\inferrule[t\_int\_wellconstrained]
{
  ...
  \vs = \TInt(\WellConstrained(\css, \vptwo))\hva\\
  \vp \eqdef \precisionjoin(\vpone, \vptwo)\\
  \tty \eqdef \TInt(\WellConstrained(\cst \concat \css, \vp))
}{
  \lca(\tenv, \vt, \vs) \typearrow \tty
}
\end{mathpar}
...
\inferrule[t\_bits]{
  \vt = \TBits(\vet, \Ignore)\\
  \vs = \TBits(\ves, \Ignore)\\
  \typeequal(\tenv, \vt, \vs) \typearrow \False\hva\\
  \exprequal(\tenv, \vet, \ves) \typearrow \vbequal\\
  \techeck(\vbequal, \NoLCA) \typearrow \True \OrTypeError
}{
  \lca(\tenv, \vt, \vs) \typearrow \overname{\TBits(\vet, \emptylist)}{\tty}
}
\end{mathpar}

\begin{mathpar}
\inferrule[t\_array]{
  \vt = \TArray(\widtht, \vtyt)\\
  \vs = \TArray(\widths, \vtys)\\
  \typeequal(\tenv, \vt, \vs) \typearrow \False\hva\\
  \arraylengthequal(\tenv, \widtht, \widths) \typearrow \vbequallength \OrTypeError\hva\\\\
  \techeck(\vbequallength, \NoLCA) \typearrow \True \OrTypeError\\\\
  \lca(\tenv, \vtyt, \vtys) \typearrow \vtone \OrTypeError
}{
  \lca(\tenv, \vt, \vs) \typearrow \overname{\TArray(\widtht, \vtone)}{\tty}
}
\end{mathpar}
...
\inferrule[t\_tuple]{
  ...
  \techeck(\vb, \NoLCA) \typearrow \True \OrTypeError\hva\\\\
  {
    \begin{array}{r}
  ...
  \lca(\tenv, \vt, \vs) \typearrow \overname{\TTuple(\vli)}{\tty}
}
\end{mathpar}
...
\RenderRule{lowest_common_ancestor}
\identr{YZHM}
...
\inferrule[bnot\_t\_bool]{
  \checktypesatisfies(\tenv, \vtone, \TBool) \typearrow \True \OrTypeError\\
}{
...
\inferrule[neg\_error]{
  \typesatisfies(\tenv, \vt, \unconstrainedinteger) \typearrow \False \OrTypeError\hva\\\\
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item both $\vtone$ and $\vttwo$ are integer types and at least one of them is the unconstrained integer type;
  ...
  \begin{itemize}
    \item $\op$ is one of $\{\MUL, \DIV, \DIVRM, \MOD, \SHL,  \SHR, \POW, \ADD, \SUB\}$;
    \item both $\vtone$ and $\vttwo$ are integer types, neither is an unconstrained integer type, and at least one of them is a \parameterizedintegertypeterm;
...
\inferrule[named]{
  ...
  \makeanonymous(\tenv, \vtone) \typearrow \vtoneanon \OrTypeError\hva\\\\
  \makeanonymous(\tenv, \vttwo) \typearrow \vttwoanon \OrTypeError\hva\\\\
...
\inferrule[bits\_concat]{
  \vtone = \TBits(\vwone, \Ignore)\\
  \vttwo = \TBits(\vwtwo, \Ignore)\\
  \vw \eqdef \EBinop(\ADD, \vwone, \vwtwo) \\
  \normalize(\tenv, \vw) \typearrow \vwp
}{
  \applybinoptypes(\tenv, \BVCONCAT, \vtone, \vttwo) \typearrow
...
\inferrule[arith\_t\_int\_wellconstrained]{
  \vtone = \TInt(\vcone)\\
  \vttwo = \TInt(\vcstwo)\\
  \op \in  \{\MUL, \POW, \ADD, \SUB, \DIVRM, \DIV, \MOD, \SHL, \SHR\}\hva\\
  \vcone = \WellConstrained(\cstwo, \vpone)\\
  \vctwo = \WellConstrained(\csone, \vptwo)\\
  \annotateconstraintbinop(\tenv, \op, \vcsone, \vcstwo) \typearrow (\cs, \vpthree) \OrTypeError\hva\\\\
  \vp = \precisionjoin(\vpone, \precisionjoin(\vptwo, \vpthree))
}{
      \applybinoptypes(\tenv, \op, \vtone, \vttwo) \typearrow
...
\inferrule[super]{
  \supers(\tenv, \vt) \typearrow \vtsupers\\
  \vs \not\in \vtsupers\\
  \tenv.\staticenvsG.\subtypes(\vs) = \vsp\\
...
\inferrule[none]{
  \supers(\tenv, \vt) \typearrow \vtsupers\\
  \vs \not\in \vtsupers\\
  \tenv.\staticenvsG.\subtypes(\vs) = \bot
}{
  \namedlca(\tenv, \vt, \vs) \typearrow \None
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{named_lowest_common_ancestor}
...
\begin{mathpar}
\inferrule[exploding]{
  \binopfilterrhs(\vapprox, \tenv, \op, \cstwo) \typearrow \cstwof \terminateas \CannotUnderapproximate, \CannotOverapproximate\hva\\\\
  \commonprefixline\hva\\\\
  \binopisexploding(\op) \typearrow \True\\
  \explodeintervals(\tenv, \csone) \typearrow (\csonee, \vpone)\\
  \explodeintervals(\tenv, \cstwof) \typearrow (\cstwoe, \vptwo)\hva\\
  ...
  \ifthenelseop[V]{\vexpectedconstraintlength < \maxconstraintsize}{(\csonee, \cstwoe, \vpzero)}{(\csone, \cstwof, \PrecisionLost)}
  }\hva\\\\
  \commonsuffixline\hva\\\\
  \constraintbinop(\op, \csonearg, \cstwoarg) \typearrow \csvanilla\\
  \refineconstraintfordiv(\vapprox, \op, \csvanilla) \typearrow \refinedcs \terminateas \CannotUnderapproximate,\CannotOverapproximate\hva\\\\
...
\inferrule[non\_exploding]{
  ...
  (\csonearg, \cstwoarg) \eqdef (\csone, \cstwof)\hva\\\\
  \commonsuffixline\hva\\\\
  \constraintbinop(\op, \csonearg, \cstwoarg) \typearrow \csvanilla\\
  \refineconstraintfordiv(\vapprox, \op, \csvanilla) \typearrow \refinedcs \terminateas \CannotUnderapproximate, \CannotOverapproximate\hva\\\\
  \reduceconstraints(\tenv, \refinedcs) \typearrow \annotatedcs
...
\RenderRule{annotate_constraint_binop}
...
\inferrule[div\_non\_empty]{
  \op = \DIV\\
  \vi\in\listrange(\cs): \filterreduceconstraintdiv(\cs[\vi]) \typearrow \vcopt_\vi \OrTypeError\hva\\\\
  \vres \eqdef \filteroptionlist{\vi\in\listrange(\cs): \vcopt_\vi}\\
  \vres \neq \emptylist
}{
...
\inferrule[div\_empty]{
  ...
  \refineconstraintfordiv(\vapprox, \op, \cs) \typearrow \ifthenelseop[V]{\vapprox=\Over}{\CannotOverapproximate}{\CannotUnderapproximate}
}
\end{mathpar}
...
\RenderRule{refine_constraint_for_div}
...
\begin{mathpar}
\inferrule[exact]{
  \getliteraldivopt(\ve) \typearrow \some{(\vzone, \vztwo)} \terminateas \None\hva\\\\
  {
    \vcopt \eqdef
    \begin{cases}
      \None & \text{if }\vztwo > 0 \land \frac{\vzone}{\vztwo} \not\in \Z\\
      \some{\vc} & \text{otherwise}
    \end{cases}
  }
}{
...
\inferrule[range]{
  ...
  \vczfive \eqdef \AbbrevConstraintExact{\ELInt{\vzfive}}\\
  \vczfivesix \eqdef \AbbrevConstraintRange{\ELInt{\vzfive}}{\ELInt{\vzsix}}\\
  \vczfiveetwo \eqdef \AbbrevConstraintRange{\ELInt{\vzfive}}{\vetwo}\\
  \vceonezsix \eqdef \AbbrevConstraintRange{\veone}{\ELInt{\vzsix}}\\
  \vceoneetwo \eqdef \AbbrevConstraintRange{\veone}{\vetwo}\hva\\
  {
    \vcopt \eqdef
    \begin{cases}
     \some{\vczfive} & \text{if }\vzoneopt = \some{\vzfive} \land \vztwoopt = \some{\vzsix} \land \vzfive=\vzsix\\
     \some{\vczfivesix} & \text{if }\vzoneopt = \some{\vzfive} \land \vztwoopt = \some{\vzsix} \land \vzfive<\vzsix\\
     \None & \text{if }\vzoneopt = \some{\vzfive} \land \vztwoopt = \some{\vzsix} \land \vzfive>\vzsix\\
     \some{\vczfiveetwo} & \text{if }\vzoneopt = \some{\vzfive} \land \vztwoopt = \None\\
     \some{\vceonezsix} & \text{if }\vzoneopt = \None \land \vztwoopt = \some{\vzsix}\\
     \some{\vceoneetwo} & \text{if }\vzoneopt = \None \land \vztwoopt = \None\\
    \end{cases}
  }
}{
...
\RenderRule{filter_reduce_constraint_div}
...
\begin{mathpar}
\inferrule{
  \zoneDIVztwo \eqdef \AbbrevEBinop{\DIV}{\ELInt{\vzone}}{\ELInt{\vztwo}}\\
  \rangeopt \eqdef \choice{\ve = \zoneDIVztwo}{\some{(\vzone, \vztwo)}}{\None}
}{
  \getliteraldivopt(\ve) \typearrow \rangeopt
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{get_literal_div_opt}
...
\begin{itemize}
  \item \AllApplyCase{exact}
  \begin{itemize}
    \item $\vza$ and $\vzb$ are the same integer;
    \item define $\vcopt$ as the optional exact constraint for the literal expression for $\vza$.
  \end{itemize}
  \item \AllApplyCase{range}
  \begin{itemize}
    \item $\vza$ is less than $\vzb$;
    \item define $\vcopt$ as the optional range constraint for the literal expressions for $\vza$ and $\vzb$.
  \end{itemize}
  \item \AllApplyCase{invalid}
  \begin{itemize}
    \item $\vza$ is greater than $\vzb$;
    \item define $\vcopt$ as $\None$.
  \end{itemize}
\end{itemize}

\FormallyParagraph
\RenderRule{make_constraint_range_opt}
...
\inferrule[range\_reduced]{
  ...
  \reducetozopt(\tenv, \vb) \typearrow \some{\vzb}\hva\\
  \explodedinterval \eqdef [\vz \in \vza..\vzb: \ConstraintExact(\ELInt{\vz})]\hva\\
...
\begin{mathpar}
\inferrule{
  \vbf \in \bfsone: \membfs(\bfstwo, \vbf) \typearrow \vb_\vbf \OrTypeError\hva\\\\
...
\begin{mathpar}
\inferrule[okay]{
  \tstruct(\vt) \typearrow \vtp \OrTypeError\hva\\\\
  \astlabel(\vtp) = \vl
}
{
...
\RenderRule{check_structure_label}
...
\RenderRule{to_well_constrained}
...
\begin{mathpar}
\inferrule{
  \tstruct(\tenv, \vt) \typearrow \vtone \OrTypeError\hva\\\\
...
\RenderRule{get_well_constrained_structure}
...
\RenderRule{get_bitvector_width}
...
\begin{mathpar}
\inferrule{
  \getbitvectorwidth(\tenv, \vt) \typearrow \ewidth \OrTypeError\hva\\\\
...
\RenderRule{get_bitvector_const_width}
...
\begin{mathpar}
\inferrule{
  \getbitvectorwidth(\tenv, \vtone) \typearrow \vn \OrTypeError\hva\\\\
  \getbitvectorwidth(\tenv, \vttwo) \typearrow \vm \OrTypeError\hva\\\\
...
\RenderRule{check_bits_equal_width}
