...
\begin{itemize}
  ...
  \item applying $\annotatefuncsig$ to each node $\vf$ in $\genv$, where $\DFunc(\vf)$ is a declaration in $\decls$,
        yields $(\tenv_\vf, \vd_\vf, \vses_f)$\ProseOrTypeError;
  ...
  \item for tuple in $\envandfstwo$ consisting of a local static environment, an element of $\func$,
        ...
        $(\genvtwo, \lenvtwo, \vses_\vf)$ and $\vf$ yields $(\vnew_\vf, \vses_\vf)$\ProseOrTypeError;
  \item define $\vds$ as the list of $\DFunc(\vnew_\vf)$ for all $(\lenvtwo, \vf, \vses\_\vf)$ in $\envandfstwo$;
  \item define $\vsess$ as the list of $(\vf, \vses_\vf)$ for all $(\lenvtwo, \vf, \vses\_\vf)$ in $\envandfstwo$;
...
\inferrule{
  ...
  \DFunc(\vf)\in\decls: \annotatefuncsig(\genv, \vf) \typearrow (\tenv_\vf, \vd_\vf, \vses_f)\OrTypeError\\\\
  \envandfs \eqdef [\DFunc(\vf)\in\decls: (L^{\tenv_\vf}, \vd_\vf)]\\
  {
  \setters \eqdef \left[ \begin{array}{r|l}
    (\lenv,\vd, \vses) & (\lenv, \DFunc(\vf), \vses) \in \envandfs\ \land \\
  ...
  {
    \begin{array}{r}
  (\lenvtwo, \vf, \vses\_\vf)\in\envandfstwo: \annotatesubprogram(\genvtwo, \lenvtwo, \vf, \vses\_vf) \typearrow \\
      (\vnew_\vf, \vses_\vf) \OrTypeError
    \end{array}
  }\\\\
  \vds \eqdef [(\lenvtwo, \vf, \vses\_\vf)\in\envandfstwo: \DFunc(\vnew_\vf)]\\
  \vsess \eqdef [(\lenvtwo, \vf, \vses\_\vf)\in\envandfstwo: (\vf, \vses\_\vf)]\\
...
\[
  \addsubprogramdecls(
    ...
    \overname{(\func\times\TSideEffectSet)^*}{\vfuncs} \aslsep
  ) \aslto
...
\subsubsection{TypingRule.PropagateRecursiveCallsSess\label{sec:TypingRule.PropagateRecursiveCallsSess}}
