...
\SectionDef[describes how implementations of ASL
        should handle errors]
  {How Implementations Should Handle Errors}{How Implementations Should Handle Errors}
...
File ../compliance/tests/definition/TypingErrorReporting.asl, line 3,
  characters 11 to 22:
    return 5 + "hello";
           ^^^^^^^^^^^
...
(incorrect) behaviour not following the dynamic semantics, termination of execution or raising an exception.
%
...
\begin{Verbatim}[fontsize=\footnotesize, frame=single]
> aslref ../compliance/tests/definition/DynamicErrorReporting.asl
...
%
Applying \aslrefterm{} in a \linuxbashshellterm{} on the specification in \listingref{dynamic-error-reporting}
...
\begin{Verbatim}[fontsize=\footnotesize, frame=single]
File ../compliance/tests/definition/AssertionStatement.asl, line 5,
  characters 11 to 22:
    assert a + b < 256;
           ^^^^^^^^^^^
...
File ../compliance/tests/definition/UnreachableStatement.asl, line 5,
  characters 8 to 20:
        unreachable;
        ^^^^^^^^^^^^
...
\SectionDef[defines a classification of ASL errors]
...
\SectionDef[summarises the error codes used throughout this reference]
  {ErrorCodesSummary}{Error Codes Summary}
...
\hline
  ...
  $\BinopPriority$               & Binary operation priority    & "             \\
  ...
  $\DynamicAssertionFailure$     & Dynamic assertion failure      & "             \\
...
\begin{description}
  ...
  \hypertarget{def-binoppriority}{}
  \item[$\BinopPriority$]
    \textit{Binary operation priority.}
    A compound \binopexpressionterm{} consisting of two binary operators of the same priority was used without sufficient parentheses.
    See \ASTRuleRef{CheckNotSamePriority}.
...
\SectionDef[details \typingerrorsterm]
  {TypingErrors}{Type Errors}
\begin{description}
...
\item[$\IdentifierAlreadyDeclared$]
  ...
  For example:
  \begin{itemize}
    \item Redefining a local variable (see the use of \TypingRuleRef{CheckVarNotInEnv} in \TypingRuleRef{LDVar}).
    \item Redefining a global variable (see the use of \TypingRuleRef{CheckVarNotInGEnv} in \TypingRuleRef{DeclareGlobalStorage}).
  \end{itemize}
...
\item[$\TypeSatisfactionFailure$]
  ...
  A type satisfaction check failed.
  For example:
  \begin{itemize}
    \item \TypingRuleRef{CheckTypeSatisfaction}.
    \item A value is assigned to a variable of incompatible type \\ (see \TypingRuleRef{CheckCanBeInitializedWith}).
  \end{itemize}
...
\item[$\UnexpectedType$]
  ...
  \begin{itemize}
    ...
    \item Encountered a forbidden \pendingconstrainedintegertypeterm{} (\TypingRuleRef{TInt}).
    \item An anonymous enumeration or \structuredtypeterm{} was used as a type annotation outside of a type declaration (\TypingRuleRef{TNonDecl}).
...
\item[$\BadSlices$]
  ...
  \begin{itemize}
    ...
    \item Bit slices that are \symbolicallyevaluableterm{} overlap on the left-hand side of an assignment (\TypingRuleRef{DisjointSlicesToPositions}).
      Note that if the overlapping slices are not \symbolicallyevaluableterm{}, then this is a \dynamicerrorterm{} ($\OverlappingSliceAssignment$).
...
\item[$\BadField$]
  \textit{Bad field.}
  Invalid usage of a field of a \structuredtypeterm{} or bitfield of a bitvector.
  For example:
  \begin{itemize}
    \item An initialisation expression for a \structuredtypeterm{} is missing a field (\TypingRuleRef{ERecord}).
    \item An access (read or write) is made to a non-existent field for a \structuredtypeterm{} or bitfield of a bitvector type (\TypingRuleRef{LESetStructuredField}).
  \end{itemize}
...
\item[$\SideEffectViolation$]
  ...
  \begin{itemize}
    ...
    \item A non-constant-time initialisation expression was provided for a constant declaration (\TypingRuleRef{SDecl}.\textsc{constant}).
  \end{itemize}
...
\SectionDef[details \dynamicerrorsterm]
...
\item[$\OverlappingSliceAssignment$]
  ...
  Bitvector slices that are not \symbolicallyevaluableterm{} overlap on the left-hand side of an assignment \\
  (\SemanticsRuleRef{CheckNonOverlappingSlices}).
  Note that overlapping \emph{bitfields} are \typingerrorsterm{},
  and that if the overlapping slices are \symbolicallyevaluableterm{} this too is a \typingerrorterm{} ($\BadSlices$).
...
\hypertarget{def-noentrypoint}{}
\item[$\NoEntryPoint$]
  \textit{No entry point.}
  The entry point could not be found\\
  (see \SemanticsRuleRef{EvalSpec}).
\end{description}
