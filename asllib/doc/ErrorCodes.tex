...
\RequirementDef{StaticErrorSafety} \Staticerrorsterm\ must never cause a dynamic error or
cause an exception to be raised.

\RequirementDef{DynamicErrorBehavior} The behaviour of an implementation when a dynamic error
...
\RequirementDef{ErrorAsException}
...
(see \SemanticsRuleRef{SAssert} and error code $\DynamicAssertionFailure$).

\RequirementDef{DynamicErrorUnreachable}
Evaluation of the \unreachablestatementterm\ is a dynamic error
(see \SemanticsRuleRef{SUnreachable} and error code $\UnreachableError$).

\section{Error Kinds}
\RequirementDef{ErrorDescriptors}
...
\section{Typing Errors\label{sec:TypingErrors}}
\begin{description}
...
\item[$\BadOperands$]
  \textit{Bad operands.}
  A primitive operator was provided invalid operands during typechecking.
  For example:
  \begin{itemize}
...
\item[$\BadSubprogramDeclaration$]
  ...
  \begin{itemize}
    ...
    \item A setter is missing its corresponding getter, or a getter with the same name exists but does not have a matching signature \\
          (\TypingRuleRef{CheckSetterHasGetter}).
