...
% These behave as records for sequential semantics, but are viewed as a disjoint collection of fields for concurrent semantics.
% TO DO reference the relevant section
...
\begin{itemize}
  \item \texttt{UInt}:
    \begin{itemize}
      ...
      \item now \verb|UInt{N: integer{1..128}} (x: bits(N)) => integer{0..2^N-1}|
    \end{itemize}
  \item \texttt{SInt}:
    \begin{itemize}
      \item previously \verb|SInt{N} (x: bits(N)) => integer{-(2^(N-1)) .. 2^(N-1)-1}|
      \item now \verb|SInt{N: integer{1..128}} (x: bits(N)) => integer{-(2^(N-1)) .. 2^(N-1)-1}|
    \end{itemize}
\end{itemize}
