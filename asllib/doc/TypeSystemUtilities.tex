...
\begin{itemize}
  ...
  \item \TypingRuleRef{WithEmptyLocal} constructs a static environment out of a global static environment;
  \item \TypingRuleRef{CheckVarNotInEnv} ensures that a variable is not in the static environment;
  \item \TypingRuleRef{CheckVarNotInGEnv} ensures that a variable is not in the global static environment;
  \item \TypingRuleRef{AddLocal} updates a static environment with a local storage element;
  \item \TypingRuleRef{IsUndefined} checks whether an identifier is defined in a static environment;
  \item \TypingRuleRef{IsGlobalUndefined} tests whether an identifier is defined in the global static environment;
  \item \TypingRuleRef{IsLocalUndefined} tests whether an identifier is defined in the local static environment;
  \item \TypingRuleRef{LookupConstant} looks up a static environment for a global constant;
  \item \TypingRuleRef{AddGlobalConstant} updates a static environment for a global storage element;
  \item \TypingRuleRef{LookupImmutableExpr} looks up the initializing expression associated with an immutable storage element;
  \item \TypingRuleRef{AddGlobalImmutableExpr} updates a global static environment by associating a global immutable storage element
        with its initializing expressions;
  \item \TypingRuleRef{AddLocalImmutableExpr} updates a local static environment by associating a local immutable storage element
        with its initializing expressions;
  ...
  \item \TypingRuleRef{AddImmutableExpr} conditionally updates a static environment by associating
        a local immutable storage element with its initializing expressions;
  \item \TypingRuleRef{AddSubprogram} updates a static environment with a subprogram;
  \item \TypingRuleRef{AddType} updates a static environment with a new type.
\end{itemize}
