...
\begin{itemize}
  ...
  \item \TypingRuleRef{FindBitfieldOpt} recursively looks up a bitfield in a list of bitfields;
  \item \TypingRuleRef{TypeOfArrayLength} returns the type associated with an array index;
  \item \TypingRuleRef{WithEmptyLocal} constructs a \staticenvironmentterm{} from a \globalstaticenvironmentterm;
...
\begin{itemize}
  ...
  \begin{itemize}
    \item there do not exist two different positions in the list where the identifier is the same;
...
\begin{mathpar}
\inferrule[okay]{
  \forall{i,j \in \listrange(\ids)}{i \neq j \implies \id_i \neq \id_j}
}{
  \checknoduplicates(\ids) \typearrow \True
}
\end{mathpar}

\begin{mathpar}
\inferrule[error]{
  \neg\forall{i,j \in \listrange(\ids)}{i \neq j \implies \id_i \neq \id_j}
}{
  \checknoduplicates(\ids) \typearrow \TypeErrorVal{\IdentifierAlreadyDeclared}
}
\end{mathpar}
} % END_OF_BACKUP_PROSE_AND_FORMALLY
\RenderProseAndFormally{check_no_duplicates}

\TypingRuleDef{Sort}
\hypertarget{def-sort}{}
The parametric function
\[
\sort(\overname{\KleeneStar{T}}{\vlone}, \overname{(T\cartimes T)\rightarrow \{-1,0,1\}}{\compare}) \typearrow \overname{\KleeneStar{T}}{\vltwo}
\]
...
requires finding the corresponding bitfields, which all exist in the \verb|Packet| \bitvectortypeterm.
...
\RenderProseAndFormally{find_bitfield_opt}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\size$ is an enumeration index over the enumeration $\vs$\\($\ArrayLengthEnum(\vs, \Ignore)$);
    \item $\vt$ is the named type for $\vs$, that is, $\TNamed(\vs)$.
  \end{itemize}

  \item \AllApplyCase{expr}
  \begin{itemize}
    \item $\size$ is an expression for integer-sized arrays ($\ArrayLengthExpr(\Ignore)$);
...
\RenderProseAndFormally{type_of_array_length}
...
\begin{mathpar}
\inferrule{}{
  \withemptylocal(\genv) \typearrow \{ \staticenvsG: \genv, \staticenvsL: \emptytenv \}
}
\end{mathpar}
} % END_OF_BACKUP_PROSE_AND_FORMALLY
\RenderProseAndFormally{with_empty_local}
...
\RenderProseAndFormally{check_var_not_in_env}
...
\RenderProseAndFormally{check_var_not_in_genv}
...
\begin{mathpar}
\inferrule{
  \newlocalstoragetypes \eqdef \tenv.\staticenvsL.\localstoragetypes[\id \mapsto (\tty, \ldk)]\\
  \newtenv \eqdef \{\staticenvsG: \tenv.\staticenvsG, \staticenvsL: \tenv.\staticenvsL[\localstoragetypes \mapsto \newlocalstoragetypes]\}
}{
  \addlocal(\tenv, \id, \tty, \ldk) \typearrow \newtenv
}
\end{mathpar}
} % END_OF_BACKUP_PROSE_AND_FORMALLY
\RenderProseAndFormally{add_local}
...
\begin{mathpar}
\inferrule{
  \isglobalundefined(\tenv.\staticenvsG, \vx) \typearrow \vbone\\
  \islocalundefined(\tenv.\staticenvsL, \vx) \typearrow \vbtwo\\
}{
  \isundefined(\tenv, \vx) \typearrow \overname{\vbone \land \vbtwo}{\vb}
}
\end{mathpar}
} % END_OF_BACKUP_PROSE_AND_FORMALLY
\RenderProseAndFormally{is_undefined}
...
\RenderProseAndFormally{is_global_undefined}
...
\begin{mathpar}
\inferrule{}{
  \islocalundefined(\lenv, \vx) \typearrow \overname{\tenv.\staticenvsL.\localstoragetypes(\vx) = \bot}{\vb}
}
\end{mathpar}
} % END_OF_BACKUP_PROSE_AND_FORMALLY
\RenderProseAndFormally{is_local_undefined}
...
\RenderProseAndFormally{lookup_constant}
...
\RenderProseAndFormally{add_global_constant}
...
\begin{mathpar}
\inferrule[local]{
  \tenv.\staticenvsL.\exprequiv(\vx) = \ve
}{
...
\begin{mathpar}
\inferrule[global]{
  \tenv.\staticenvsL.\exprequiv(\vx) = \bot\\
  \tenv.\staticenvsG.\exprequiv(\vx) = \ve\\
}{
  \lookupimmutableexpr(\tenv, \vx) \typearrow \ve
}
\end{mathpar}
...
\inferrule[none]{
  \tenv.\staticenvsL.\exprequiv(\vx) = \bot\\
  \tenv.\staticenvsG.\exprequiv(\vx) = \bot\\
}{
  \lookupimmutableexpr(\tenv, \vx) \typearrow \bot
}
\end{mathpar}
} % END_OF_BACKUP_PROSE_AND_FORMALLY
\RenderProseAndFormally{lookup_immutable_expr}
...
\BackupProseAndFormally{
\ProseParagraph
\ProseEqdef{$\newtenv$}{the \staticenvironmentterm{} with the same local environment as $\tenv$ and a global environment
...
\inferrule{}{
  {
  \begin{array}{r}
  \addglobalimmutableexpr(\tenv, \vx, \ve) \typearrow \\
  \overname{\{\staticenvsG: \tenv.\staticenvsG.\exprequiv[\vx \mapsto \ve], \staticenvsL: \tenv.\staticenvsL\}}{\newtenv}
  \end{array}
  }
}
\end{mathpar}
} % END_OF_BACKUP_PROSE_AND_FORMALLY
\RenderProseAndFormally{add_global_immutable_expr}
...
\BackupProseAndFormally{
\ProseParagraph
\ProseEqdef{$\newtenv$}{the \staticenvironmentterm{} with the same global environment as $\tenv$ and a local environment
where $\exprequiv$ binds $\vx$ to $\ve$.}
...
\inferrule{}{
  {
  \begin{array}{r}
  \addlocalimmutableexpr(\tenv, \vx, \ve) \typearrow \\
  \overname{\{\staticenvsG: \tenv.\staticenvsG, \staticenvsL: \tenv.\staticenvsL.\exprequiv[\vx \mapsto \ve]\}}{\newtenv}
  \end{array}
  }
}
\end{mathpar}
} % END_OF_BACKUP_PROSE_AND_FORMALLY
\RenderProseAndFormally{add_local_immutable_expr}
...
\RenderProseAndFormally{should_remember_immutable_expression}
...
In \listingref{AddImmutableExpr}, determining that the declaration for \verb|t|
...
\OneApplies
\begin{itemize}
  \item \AllApplyCase{add}
  \begin{itemize}
    \item $\veopt$ contains the expression $\ve$ and \sideeffectsetterm\ $\vsese$;
    \item $\ldk$ is $\LDKLet$;
    \item applying $\shouldrememberimmutableexpression$ to $\vsese$ yields $\True$;
    \item applying $\normalize$ to $\ve$ in $\tenv$ yields $\vep$\ProseOrTypeError;
    \item applying $\addlocalimmutableexpr$ to $\vx$ and $\vep$ yields $\newtenv$.
  \end{itemize}

  \item \AllApplyCase{noop}
  \begin{itemize}
    \item \OneApplies
    \begin{itemize}
      \item $\ldk$ is not $\LDKLet$;
      \item $\veopt$ is $\None$;
      \item $\veopt$ contains the expression $\ve$ and \sideeffectsetterm{} $\vsese$ and
            applying $\shouldrememberimmutableexpression$ to $\vsese$ yields $\False$;
    \end{itemize}
    \item define $\newtenv$ as $\tenv$.
  \end{itemize}
\end{itemize}
...
\inferrule[add]{
  ...
  \normalize(\tenv, \ve) \typearrow \vep \OrTypeError\hva\\\\
...
\inferrule[noop]{
  {
    \begin{array}{ll}
      ...
      \left(\begin{array}{ll}
      \some{\ve, \vsese} & \land\\
      \shouldrememberimmutableexpression(\vsese) \typearrow \False &
      \end{array}\right)
    \end{array}
  }
}{
...
\RenderProseAndFormally{add_immutable_expression}
...
\FIELDargs &:& [(\vx, \unconstrainedinteger)],\\
\funcbody &:& \SReturn(\AbbrevEBinop{+}{\AbbrevEVar{\vx}}{\ELInt{1}}),\\
\FIELDreturntype &:& \some{\unconstrainedinteger},\\
\funcsubprogramtype &:& \STFunction\\
\FIELDrecurselimit    &:& \None\\
\funcbuiltin &:& \False\\
...
\begin{mathpar}
\inferrule{
  \newtenv \eqdef \{\staticenvsG: \tenv.\staticenvsG.\subprograms[\name\mapsto(\funcdef, \vs)], \staticenvsL: \tenv.\staticenvsL\}
}{
  \addsubprogram(\tenv, \name, \funcdef, \vs) \typearrow \newtenv
}
\end{mathpar}
} % END_OF_BACKUP_PROSE_AND_FORMALLY
\RenderProseAndFormally{add_subprogram}
...
\inferrule{}{
  \addtype(\tenv, \name, \tty, \vf) \typearrow
  \overname{\{\staticenvsG: \tenv.\staticenvsG.\declaredtypes[\name\mapsto(\tty, \vf)] , \staticenvsL: \tenv.\staticenvsL\}}{\newtenv}
}
\end{mathpar}
} % END_OF_BACKUP_PROSE_AND_FORMALLY
\RenderProseAndFormally{add_type}
