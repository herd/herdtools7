...
All of the following apply:
\begin{itemize}
  \item \Proseannotateexpr{$\tenv$}{$\ve$}{$(\ve, \vep, \vses)$};
...
\begin{mathpar}
\inferrule{
  \annotateexpr{\tenv, \ve} \typearrow (\ve, \vep, \vses) \OrTypeError\\\\
  \checkstaticallyevaluable(\vses) \typearrow \True \OrTypeError
}{
  \annotatestaticallyevaluableexpr(\tenv, \ve) \typearrow (\ve, \vep, \vses)
}
\end{mathpar}
