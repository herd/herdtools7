...
The dynamic semantics of ASL define all valid behaviours of a given ASL specification.
More precisely, an ASL specification is first parsed into an \emph{abstract syntax tree},
or AST, for short. Second, a typechecker analyses the \emph{\untypedast} to determine whether it
...
based on the definition of the semantics in this reference, to test and analyse a given specification.

We note that the dynamic semantics are only used to define the set of valid behaviours
of a given specification without assigning any other quantification, such as the time
required to evaluate it.

\paragraph{Understanding the Dynamic Semantics Formalisation:}
We assume basic familiarity with the ASL language constructs.
The ASL dynamic semantics are defined in terms of its AST,
...
The mathematical background needed to understand the mathematical formalisation
of the ASL dynamic semantics appears in \chapref{FormalSystem}.

\ChapterOutline

\SectionDef[explains which ASL specifications have a dynamic semantics
        assigned to them;]
  {MeaningfulASLSpecifications}{Meaningful ASL Specifications}
The ASL dynamic semantics defined here assign meaning only to \emph{well-typed specifications}.
More specifically, the semantics are only defined for a \typedast{} produced by the typechecker
...
\SectionDef[explains how this reference defines both the
        sequential semantics and concurrent semantics for ASL;]
  {Sequential Semantics and Dynamic Semantics}{Sequential Semantics and Dynamic Semantics}
\hypertarget{def-semanticconfiguration}{}
The ASL dynamic semantics are given by relations between
\emph{\semanticsconfigurationsterm}~\cite{SemanticsWithApplicationsBook}
(\configurationsterm{} dedicated for the dynamic semantics).
We refer to relations between \semanticsconfigurationsterm{} as \emph{semantic relations}.
\Semanticsconfigurationsterm{} encapsulate information needed to transition into other \semanticsconfigurationsterm{}, such as:
\begin{itemize}
...
\paragraph{Execution:}
A valid execution of an ASL specification transitions from an \emph{initial \semanticsconfigurationterm{}},
which consists of the given specification and the standard library specification, to an output \semanticsconfigurationterm{}
...
Intuitively, these graphs define Read Effects and Write Effects on variables and constraints over those effects.
...
from all \semanticsconfigurationsterm{}.

\SectionDef[defines the configurations used by the dynamic semantics;]
  {Dynamic Semantics Configurations}{Dynamic Semantics Configurations}
...
\Semanticsconfigurationsterm{} express intermediate states related by \emph{semantic relations}.
%
More precisely, semantic relations relate two distinct sets of \semanticsconfigurationsterm{} ---
\emph{input \semanticsconfigurationsterm{}} and \emph{output \semanticsconfigurationsterm{}}.
Input \semanticsconfigurationsterm{} consist of an environment and an AST node.
Output \semanticsconfigurationsterm{} consist of an output environment, values,
and concurrent execution graphs.
%
\Semanticsconfigurationsterm{} wrap together elements such as environments and AST nodes
and associate them with a \emph{configuration domain}. Input \semanticsconfigurationterm{} domains
determine the semantic relation they pertain to, while output \semanticsconfigurationterm{}
...
The rest of this section defines the components comprising \semanticsconfigurationsterm{}:
\begin{itemize}
    \item Native values.
...
\SectionDef[defines the concept of \nativevaluesterm{};]
  {nativevalues}{Native Values}
Semantic evaluation binds values to storage elements when a specification is semantically evaluated.
To formalise this, we define the set of \emph{\nativevaluesterm} as follows
(\texttt{NV} stands for Native Value):
\RenderType{native_value}

Specifically:
\begin{itemize}
  \item $\NVLiteral(\vv)$ represents integer literal, real literal, Boolean literal, string literals, and bitvector literals; and
  \item $\NVVector(\vl)$ represents tuples and arrays.
  \item $\NVRecord(\vr)$ represents records.
\end{itemize}
...
A \emph{sequential dynamic environment}, or \\ \emph{\dynamicenvironmentterm}
$\denv \in \dynamicenvs$
...
\[
\dom(\denv.\dynamicenvsG.\storage) \cap \dom(\denv.\dynamicenvsL) = \emptyset \enspace.
\]
...
\texthypertarget{constant-emptydenv}
...
The concurrent semantics of an ASL specification utilise \emph{concurrent execution graphs}
...
The label $\Read$ stands for a \emph{read effect type}, and the label
$\Write$ stands for a \emph{write effect type}.
%
...
\hypertarget{relation-writeeffect}{}
\hypertarget{relation-readeffect}{}
...
Given two execution graphs $S_1 = (N_1, E_1, O_1)$ and $S_2 = (N_2, E_2, O_2)$ their unordered composition,
...
Given two execution graphs, \\ $S_1 = (N_1, E_1, O_1)$ and $S_2 = (N_2, E_2, O_2)$ and an ordering label $l$,
the ordered composition $\ordered{S_1}{l}{S_2}$ is defined as follows:
\[
  \ordered{S_1}{l}{S_2} \triangleq (N_1 \cup N_2,\ E_1 \cup E_2 \cup (O_1 \cartimes \{l\} \cartimes N_2),\ O_2) \enspace.
\]
...
\texthypertarget{def-parallellist}
We write $\parallelgraphs{L}$ for the parallel composition of all execution graphs in the list $L$.
\end{definition}

\subsection{Concurrent Values\label{sec:ConcurrentValues}}
\hypertarget{def-concurrentnativevalue}{}
The ASL dynamic semantics operate over pairs consisting of \nativevaluesterm{} and \executiongraphsterm,
which we refer to as \concurrentnativevalues.

\subsection{Kinds of Semantics Configurations\label{sec:KindsOfSemanticsConfigurations}}

Recall that the ASL dynamic semantics define a relation between input \semanticsconfigurationsterm{} and output \semanticsconfigurationsterm{}.
Input \semanticsconfigurationterm{} domains are unique to the semantic relation that employs them.
For that reason, we name semantic relations by the name of the corresponding configuration domain of the input \semanticsconfigurationterm{}.
For example, the semantic relation that employs input \semanticsconfigurationsterm{} with the domain $\evalexpr$
is named $\evalexpr$.
%
We will often use the prefix \texttt{eval} for semantic relations with the intuition being that their input \semanticsconfigurationsterm{}
should be semantically evaluated, yielding an output \semanticsconfigurationterm{}.
...
ASL dynamic semantics mainly utilise the following types of output \semanticsconfigurationsterm{}:
\begin{description}
  \item[Normal Values.]\hypertarget{def-normal}{}
  \Semanticsconfigurationsterm{} consisting of different combinations of values,
  ...
  expressions, \sideeffectfreeexpressionsterm{}, \assignableexpressions,
  local declaration items, slices, and expression lists
  (which are evaluated in two different ways, by $\ResultExprList$ and $\ResultExprListM$, for the purpose of concurrency):
\RenderType{TNormal}

  \item[Exceptions.] \Semanticsconfigurationsterm{} in \RenderType{TThrowing} represent thrown exceptions.

  There are two flavours of exceptions:
  ...
  The ASL dynamic semantics propagate these \emph{exceptional \semanticsconfigurationsterm{}} to the nearest \catcherterm{} that matches
  them, and otherwise they are caught at the top-level and reported as errors (see \dynamicerrorsterm{} below).

  \item[Returned Values.] \Semanticsconfigurationsterm{} in \RenderType{TReturning}
  represent (tuples of) values being returned by the currently executing subprogram.
  The ASL dynamic semantics propagate these \emph{early return \semanticsconfigurationsterm{}} to the respective call expression/statement.

  \item[In-flight Subprogram.] \Semanticsconfigurationsterm{} in \RenderType{TContinuing}
  represent the fact that a subprogram has more statements to execute.
  The ASL dynamic semantics treat these \semanticsconfigurationsterm{} as a signal to keep evaluating the remainder
  of the subprogram currently being evaluated.

  \item[Dynamic Errors.] \Semanticsconfigurationsterm{} in
  \RenderTypes{dynamic_error_and_codes}
  represent \dynamicerrorsterm{} (for example, division by zero).
  The ASL dynamic semantics are set up such that when these \emph{error \semanticsconfigurationsterm{}} appear,
  the evaluation of the entire specification terminates by outputting them.

  \item[Diverging Configurations.] The \semanticsconfigurationterm{} $\Diverging$
  represents a diverging evaluation.
  The corresponding type is:
  \RenderType{TDiverging}

\end{description}
Helper relations often have output \semanticsconfigurationsterm{} that are just tuples, without an associated configuration domain.


We will say that a semantic transition
\begin{itemize}
\item \emph{terminates normally} when the output \semanticsconfigurationterm{} is a member of \\
      $\TNormal$,
\item \emph{terminates exceptionally} when the output \semanticsconfigurationterm{} is a member of $\TThrowing$,
\item \emph{terminates erroneously} when the output \semanticsconfigurationterm{} is a member of $\TDynError$,
\item \emph{diverges} when the output \semanticsconfigurationterm{} is a member of $\TDiverging$,
\item is \emph{abnormal} when it either terminates exceptionally, terminates erroneously, or diverges.
\end{itemize}

We introduce the following shorthand notations for \semanticsconfigurationsterm{} where all variables
...
\item $\ReturningConfig \triangleq \Returning((\vvs,\newg), \newenv)$
      is an early return \semanticsconfigurationterm{}.
      \hypertarget{def-divergingconfig}{}
\item $\DivergingConfig \triangleq \Diverging$ is a diverging configuration.
\end{itemize}

\subsection{Extracting and Substituting Elements of Semantics Configurations}

\hypertarget{relation-graphof}{}
\hypertarget{relation-environof}{}
Given a \semanticsconfigurationterm{} $C$, we define the graph component of the \semanticsconfigurationterm{},
$\graphof{C}$, and the environment of the \semanticsconfigurationterm{}, $\environof{C}$, as follows:
\[
\begin{array}{lcc}
  C & \graphof{C} & \environof{C}\\
  \hline
  \ResultExpr((\Ignore, \vg), \env) & \vg & \env\\
  \ResultExprSEF(\Ignore, \vg) & \vg & \text{undefined}\\
  \ResultLexpr(\vg, \env) & \vg & \env \\
  \ResultLDI(\vg, \env) & \vg & \env \\
  \ResultSlices((\Ignore, \vg), \env) & \vg & \env \\
  \ResultExprList((\Ignore, \vg), \env) & \vg & \env \\
  \ResultExprListM(\Ignore, \env) & \text{undefined} & \env\\
  \ResultPattern(\Ignore, \vg) & \vg & \text{undefined}\\
  \ResultCall(\Ignore, \vg) & \vg & \text{undefined}\\
  \Throwing(\Ignore, \Ignore,\vg, \env) & \vg & \env\\
  \Returning((\Ignore,\vg), \env) & \vg & \env\\
  \Continuing(\vg, \env) & \vg & \env\\
  \DynamicError(\Ignore) & \text{undefined} & \text{undefined}\\
  \Diverging & \text{undefined} & \text{undefined}\\
\end{array}
\]

\hypertarget{relation-withgraph}{}
Given a \semanticsconfigurationterm{} $C$, we define $\withgraph{C}{\vgp}$ to be a \semanticsconfigurationterm{}
...
\begin{array}{ll}
  ...
  \ResultCall((\vvs, \gdenv), \vg) & \ResultCall((\vvs, \gdenv), \vgp)\\
...
Similarly, we define the $\withenviron{C}{\envp}$ to be a \semanticsconfigurationterm{}
like $C$ where the environment component, if one exists, is substituted with $\envp$:
\begin{small}
\[
\begin{array}{ll}
  \textbf{\Semanticsconfigurationterm{}} & \withenviron{C}{\envp}\\
  \hline
  \ResultExpr((\vv, \vg), \env) & \ResultExpr((\vv, \vg), \envp)\\
  \ResultExprSEF(\Ignore, \Ignore) & \text{undefined}\\
  ...
  \ResultPattern(\Ignore, \Ignore) & \text{undefined}\\
  \ResultCall(\Ignore, \Ignore) & \text{undefined}\\
  ...
  \DynamicError(\Ignore) & \text{undefined}\\
  \Diverging & \text{undefined} \\
\end{array}
\]
\end{small}

\SectionDef[explains how the dynamic semantics rules define the evaluation of ASL constructs; and]
...
That is, it concludes transitions from \semanticsconfigurationsterm{} starting from
non-leaf AST nodes by concluding transitions from \semanticsconfigurationsterm{} starting from their children nodes.

\subsubsection{No Undefined Behaviours}
When an input \semanticsconfigurationterm{} does not satisfy any semantic rule,
there is no output \semanticsconfigurationterm{} for it to transition to. We say that
the \semanticsconfigurationterm{} is \emph{stuck} and the ASL dynamic semantics are undefined for that
input \semanticsconfigurationterm{}.
...
Otherwise, for every input \semanticsconfigurationterm{} there is at least one rule that can be used to take a semantic transition.

\hypertarget{eval-example}{}
\subsubsection{Evaluation Example}
The following example shows how to utilise the rules for expression literals and binary operator expressions
to derive a transition from an input \semanticsconfigurationterm{} with the expression $(1+2)*(4+5)$,
given by the AST
\[
  \EBinop\left(
    \begin{array}{c}
  \MUL, \\
  \overname{\EBinop\left(
  \begin{array}{l} \ADD, \\ \overname{\elint{1}}{\veone}, \\ \overname{\elint{2}}{\vetwo} \end{array}
                \right)}{\veonetwo}, \\
  \overname{\EBinop\left(
  \begin{array}{l }\ADD, \\ \overname{\elint{4}}{\vefour}, \\ \overname{\elint{5}}{\vefive} \end{array}
                \right)}{\vefourfive}
    \end{array}
          \right)
\]
to an output \semanticsconfigurationterm{} with the value resulting from the calculation of the expression.

We annotate subexpressions to allow referring to them.

\texthypertarget{def-emptyenv}
We define the empty environment as follows:
\RenderConstant{empty_env}
...
\begin{itemize}
  ...
  \item array-indexing (\SemanticsRuleRef{EGetArray}) --- in particular, for \verb|arr[[idx]]| whether \verb|arr| or \verb|idx| is evaluated first;
...
When there are multiple feasible choices for the next evaluation transition, ASL is defined to take the transition
for the construct appearing syntactically first. That is, ASL uses left-to-right evaluation.
...
Evaluation order and parenthesis are separate concepts. More precisely,
parentheses affect how text is parsed into ASL's AST, and then evaluation
order is defined with respect to ASL's AST.

For example, \listingref{evaluation-order-and-parenthesis},
which is followed by output to the console from running the specification,
shows a \verb|main()|
function with three statements, each parenthesised differently.
However, the order of side-effects is the same
across all three statements, due to ASL's defined evaluation order.
...
\SectionDef[defines the semantics of diverging specifications.]
  {Semantics of Diverging Specifications}{Semantics of Diverging Specifications}
...
\begin{description}
  ...
  \item[Co-inductive Rules] The set of \emph{co-inductive rules}~\cite{LeroyG:IC09}
       ...
       are transition judgements, the last, and only the last one has an output configuration in $\TDiverging$.
...
\inferrule{
  ...
  \evalblock(\envone, \vbody) \evalarrow \Continuing(\vgtwo, \envtwo)\hva\\
...
an environment $\env$. That is, consider the rules where \executiongraphsterm{}
and related judgements are omitted.
The infinite \derivationtreeterm{} consists of vertices
\[
\{S_1, L_1, V_1\} \cup \{n \geq 2 \;|\; E_n, B_n, T_n, P_n, Q_n, V_n\} \enspace,
\]
...
\inferrule{
  ...
  \begin{array}{rc}
  L_1:& \evallimit(\env, \none) \evalarrow \none\\
  V_1:& \evalloop(\env, \True, \none, \ETrue, \SPass) \evalarrow \Diverging
  \end{array}
  }
}{
  S_1: \evalstmt(\env, \SWhile(\ETrue, \none, \SPass)) \evalarrow \Diverging
}
\end{mathpar}
...
\inferrule{
  ...
  V_{n+1}:& \evalloop(\env, \True, \none, \ETrue, \SPass) \evalarrow \Diverging
  \end{array}
  }
}{
  V_n: \evalloop(\env, \True, \none, \ETrue, \SPass) \evalarrow \Diverging
}
\end{mathpar}
