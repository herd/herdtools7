...
For example, assignment statements (\secref{AssignmentStatements}) must still evaluate their right-hand side before considering their left-hand side.

\ExampleDef{Left-to-right Evaluation}
\listingref{left-to-right} shows examples of the constructs above, followed by output to the console from running the specification.
\textbf{Note:} the ordering for slicing expressions seems not to be left-to-right because some forms of slice are elaborated during typechecking (see \TypingRuleRef{Slice}).
\ASLListing{Left-to-right evaluation}{left-to-right}{\definitiontests/LeftToRight.asl}
% CONSOLE_BEGIN aslref \definitiontests/LeftToRight.asl
