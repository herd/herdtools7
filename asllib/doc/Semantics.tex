...
An ASL specification is \emph{non-terminating} when \underline{some} of its \derivationtreesterm{} are
infinite.
...
\[
  \begin{array}{rcl}
    \TNormal          & \triangleq & \ExprResult((\vals \times \XGraphs), \envs)\ \cup \ExprSEFResult(\vals, \XGraphs) \cup \LexprResult(\XGraphs, \envs) \cup\\
                      &            & \Normal(((\vals \times \vals)^* \times \XGraphs), \envs) \cup  \StmtResult(\XGraphs, \envs)\ \cup   \\
...
\begin{itemize}
\item \emph{terminates normally} when the output semantic configuration is a member of $\TNormal$,
...
\item $\DivergingConfig \triangleq \Diverging$
is a diverging configuration.
\end{itemize}
...
\section{Semantics for Non-terminating Specifications}

To extend the dynamic semantics to non-terminating specifications,
we introduce configurations of the form
\hypertarget{def-diverging}{}
\[
\Diverging(\XGraphs)
\]

A configuration $\Diverging(g)$ means that the evaluation of a
construct (expression, statement, etc.)
diverges with the (potentially infinite) \executiongraph{} $g$.

\begin{mathpar}
\inferrule{
  \evalstmt{\env, \vsone} \evalarrow \Continuing(\vgone, \envone) \terminateas \ReturningConfig,\ThrowingConfig,\DynErrorConfig,\DivergingConfig\\\\
  \evalstmt{\envone, \vstwo} \evalarrow C \OrAbnormal,\DivergingConfig\\\\
...
The problem with this definition is that if $\vstwo$ diverges
with $\Diverging(\vgtwo)$, the result is $\Diverging(\vgtwo)$
rather than $\Diverging(\ordered{\vgone}{\aslpo}{\vgtwo})$.
