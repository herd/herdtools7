...
That is, given a list of tokens, returned from $\aslscan$, it is possible to apply
...
\begin{itemize}
  ...
  \item \secref{ASLParametricProductions} introduces the parametric productions used for the ASL grammar;
  \item \secref{ASLGrammar} defines the ASL grammar;
  \item \secref{ParseTrees} defines parse trees; and
  \item \secref{PriorityAndAssociativity} defines the priority and associativity of operators.
\end{itemize}
...
but rather the priority level $\Tunops$, as denoted by the notation \\
$\precedence{\Tunops}$
...
$\Tbor$, $\Tband$, $\Tbimpl$, $\Tbeq$, $\Tas$                                  & $\leftassoc$\\
$\Telse$                                                                      & $\nonassoc$
\end{tabular}
\end{center}
...
The \assignmentstatementterm{} \texttt{x = y + 5 * 7;}
is transformed by the lexical analysis into:
\[
\Tidentifier(x) \parsesep \Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon
\]
...
1 & \fbox{$\emptysentence$} $\Tidentifier(x) \parsesep \Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & shift\\
2 & \fbox{$\Tidentifier(x)$} $\Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & reduce $\Naccess$\\
3 & \fbox{$\Tidentifier(x) \parsesep \Naccess$} $\Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & reduce $\Nbasiclexpr$\\
4 & \fbox{$\Nbasiclexpr$} $\Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & reduce $\Nlexpr$\\
5 & \fbox{$\Nlexpr$} $\Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & shift\\
6 & \fbox{$\Nlexpr \parsesep \Teq$} $\Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & shift\\
7 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Tidentifier(y)$} $\Tplus \parsesep \Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & reduce $\Nexpr$\\
8 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr$} $\Tplus \parsesep \Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & shift\\
9 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus$} $\Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & shift\\
10 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \Tintlit(5)$} $\Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & reduce $\Nexpr$\\
11 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \Nexpr$} $\Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & shift\\
12 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \Nexpr \parsesep \Tmul$} $\Tintlit(7) \parsesep \Tsemicolon$ & shift\\
13 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \Nexpr \parsesep \Tmul \parsesep \Tintlit(7)$} $\Tsemicolon$ & reduce $\Nexpr$\\
14 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \Nexpr \parsesep \Tmul \parsesep \Nexpr$} $\Tsemicolon$ & reduce $\Nexpr$\\
15 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \Nexpr$} $\Tsemicolon$ & reduce $\Nexpr$\\
16 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr$} $\Tsemicolon$ & shift\\
17 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tsemicolon$} $\emptysentence$ & reduce $\Nstmt$\\
18 & \fbox{$\Nstmt$} $\emptysentence$ & accept
\end{tabular}
\end{footnotesize}
\end{center}

A corresponding parse tree for the expanded grammar is shown below.
A node is created for each reduction step in the table above
for non-terminal being reduced, using the content stack content
...
\paragraph{Observation:}
Step 11 shows how the parser resolves the potential ambiguity in
$\Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7)$.
At that point the stack contains
$\Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \Nexpr$
...
$\Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7)$ to be formed first.
This is how the priority of $\Tmul$ over $\Tplus$ is enforced.

The table above corresponds to the following rightmost derivation in the expanded grammar
(the underline indicates the rightmost non-terminal that is reduced at each step):
\[
\begin{array}{rl}
\Nstmt & \derives\ \Nlexpr \parsesep \Teq \parsesep \underline{\Nexpr} \parsesep \Tsemicolon\\
       & \derives\ \Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \underline{\Nexpr} \parsesep \Tsemicolon\\
       & \derives\ \Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \Nexpr \parsesep \Tmul \parsesep \underline{\Nexpr} \parsesep \Tsemicolon\\
       & \derives\ \Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \Nexpr \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
       & \derives\ \Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \underline{\Nexpr} \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
       & \derives\ \Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
       & \derives\ \Nlexpr \parsesep \Teq \parsesep \underline{\Nexpr} \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
       & \derives\ \underline{\Nlexpr} \parsesep \Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
       & \derives\ \underline{\Nbasiclexpr} \parsesep \Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
       & \derives\ \Tidentifier(x) \parsesep \underline{\Naccess} \parsesep \Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
       & \derives\ \Tidentifier(x) \parsesep \Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tmul \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
\end{array}
\]
...
is transformed by the lexical analysis into:
\[
\Tidentifier(x) \parsesep \Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon
\]
...
\begin{tabular}{rll}
\textbf{\#} & \fbox{\textbf{Stack}} \textbf{Input} & \textbf{Action}\\
1 & \fbox{$\emptysentence$} $\Tidentifier(x) \parsesep \Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & shift\\
2 & \fbox{$\Tidentifier(x)$} $\Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & reduce $\Naccess$\\
3 & \fbox{$\Tidentifier(x) \parsesep \Naccess$} $\Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & reduce $\Nbasiclexpr$\\
4 & \fbox{$\Nbasiclexpr$} $\Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & reduce $\Nlexpr$\\
5 & \fbox{$\Nlexpr$} $\Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & shift\\
6 & \fbox{$\Nlexpr \parsesep \Teq$} $\Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & shift\\
7 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Tidentifier(y)$} $\Tplus \parsesep \Tintlit(5) \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & reduce $\Nexpr$\\
8 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr$} $\Tplus \parsesep \Tintlit(5) \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & shift\\
9 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus$} $\Tintlit(5) \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & shift\\
10 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \Tintlit(5)$} $\Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & reduce $\Nexpr$\\
11 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \Nexpr$} $\Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & reduce $\Nexpr$\\
12 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr$} $\Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon$ & shift\\
13 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus$} $\Tintlit(7) \parsesep \Tsemicolon$ & shift\\
14 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \Tintlit(7)$} $\Tsemicolon$ & reduce $\Nexpr$\\
15 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \Nexpr$} $\Tsemicolon$ & reduce $\Nexpr$\\
16 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr$} $\Tsemicolon$ & shift\\
17 & \fbox{$\Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tsemicolon$} $\emptysentence$ & reduce $\Nstmt$\\
18 & \fbox{$\Nstmt$} $\emptysentence$ & accept
...
A corresponding parse tree for the expanded grammar is:
\begin{center}
\begin{footnotesize}
\begin{adjustbox}{max width=\textwidth}
\Tree
...
As a result, the input is parsed as $(y + 5) + 7$, not as $y + (5 + 7)$.
...
\Nstmt & \derives\ \Nlexpr \parsesep \Teq \parsesep \underline{\Nexpr} \parsesep \Tsemicolon\\
       & \derives\ \Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \underline{\Nexpr} \parsesep \Tsemicolon\\
       & \derives\ \Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
       & \derives\ \Nlexpr \parsesep \Teq \parsesep \underline{\Nexpr} \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
       & \derives\ \Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \underline{\Nexpr} \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
       & \derives\ \Nlexpr \parsesep \Teq \parsesep \Nexpr \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
       & \derives\ \Nlexpr \parsesep \Teq \parsesep \underline{\Nexpr} \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
       & \derives\ \underline{\Nlexpr} \parsesep \Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
       & \derives\ \underline{\Nbasiclexpr} \parsesep \Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
       & \derives\ \Tidentifier(x) \parsesep \underline{\Naccess} \parsesep \Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
       & \derives\ \Tidentifier(x) \parsesep \Teq \parsesep \Tidentifier(y) \parsesep \Tplus \parsesep \Tintlit(5) \parsesep \Tplus \parsesep \Tintlit(7) \parsesep \Tsemicolon\\
\end{array}
\]
...
This is exactly the precedence rule for unary minus:
...
\ExampleDef{Shift-Reduce Parsing with Parametric Productions}
...
\begin{tabular}{rll}
\textbf{Step} & \fbox{\textbf{Stack}} \textbf{Input} & \textbf{Action}\\
1 & \fbox{$\emptysentence$} $\Tidentifier(b) \parsesep \Tlbracket \parsesep \Tidentifier(c) \parsesep \Tcomma \parsesep \Tidentifier(d) \parsesep \Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket$ & shift\\
2 & \fbox{$\Tidentifier(b)$} $\Tlbracket \parsesep \Tidentifier(c) \parsesep \Tcomma \parsesep \Tidentifier(d) \parsesep \Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket$ & reduce $\Nexpr$\\
3 & \fbox{$\Nexpr$} $\Tlbracket \parsesep \Tidentifier(c) \parsesep \Tcomma \parsesep \Tidentifier(d) \parsesep \Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket$ & shift\\
4 & \fbox{$\Nexpr \parsesep \Tlbracket$} $\Tidentifier(c) \parsesep \Tcomma \parsesep \Tidentifier(d) \parsesep \Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket$ & shift\\
5 & \fbox{$\Nexpr \parsesep \Tlbracket \parsesep \Tidentifier(c)$} $\Tcomma \parsesep \Tidentifier(d) \parsesep \Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket$ & reduce $\Nexpr$\\
6 & \fbox{$\Nexpr \parsesep \Tlbracket \parsesep \Nexpr$} $\Tcomma \parsesep \Tidentifier(d) \parsesep \Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket$ & reduce $\Nslice$\\
7 & \fbox{$\Nexpr \parsesep \Tlbracket \parsesep \Nslice$} $\Tcomma \parsesep \Tidentifier(d) \parsesep \Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket$ & shift\\
8 & \fbox{$\Nexpr \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma$} $\Tidentifier(d) \parsesep \Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket$ & shift\\
9 & \fbox{$\Nexpr \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma \parsesep \Tidentifier(d)$} $\Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket$ & reduce $\Nexpr$\\
10 & \fbox{$\Nexpr \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma \parsesep \Nexpr$} $\Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket$ & reduce $\Nslice$\\
11 & \fbox{$\Nexpr \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma \parsesep \Nslice$} $\Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket$ & shift\\
12 & \fbox{$\Nexpr \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma \parsesep \Nslice \parsesep \Tcomma$} $\Tidentifier(e) \parsesep \Trbracket$ & shift\\
13 & \fbox{$\Nexpr \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma \parsesep \Nslice \parsesep \Tcomma \parsesep \Tidentifier(e)$} $\Trbracket$ & reduce $\Nexpr$\\
14 & \fbox{$\Nexpr \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma \parsesep \Nslice \parsesep \Tcomma \parsesep \Nexpr$} $\Trbracket$ & reduce $\Nslice$\\
15 & \fbox{$\Nexpr \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma \parsesep \Nslice \parsesep \Tcomma \parsesep \Nslice$} $\Trbracket$ & reduce $\clistoneslice$\\
16 & \fbox{$\Nexpr \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma \parsesep \Nslice \parsesep \Tcomma \parsesep \clistoneslice$} $\Trbracket$ & reduce $\clistoneslice$\\
17 & \fbox{$\Nexpr \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma \parsesep \clistoneslice$} $\Trbracket$ & reduce $\clistoneslice$\\
18 & \fbox{$\Nexpr \parsesep \Tlbracket \parsesep \clistoneslice$} $\Trbracket$ & shift\\
19 & \fbox{$\Nexpr \parsesep \Tlbracket \parsesep \clistoneslice \parsesep \Trbracket$} $\emptysentence$ & reduce $\Nslices$\\
20 & \fbox{$\Nexpr \parsesep \Nslices$} $\emptysentence$ & reduce $\Nexpr$\\
21 & \fbox{$\Nexpr$} $\emptysentence$ & accept
\end{tabular}
\end{small}
\end{center}
...
\paragraph{Observation:}
Notice that there is no reduction to $\ClistOne{\Nslice}$ itself in the table above.
This is because parametric productions are expanded before parsing.
The parser therefore reduces to the unique symbol $\clistoneslice$
that stands for this particular instantiation of $\ClistOne{x}$.

The table above corresponds to the following rightmost derivation in the expanded grammar
(the underline indicates the rightmost non-terminal that is reduced at each step):
\[
\begin{array}{rl}
\Nexpr & \derives\ \underline{\Nexpr} \parsesep \Nslices\\
       & \derives\ \underline{\Nexpr} \parsesep \Tlbracket \parsesep \clistoneslice \parsesep \Trbracket\\
       & \derives\ \underline{\Nexpr} \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma \parsesep \underline{\clistoneslice} \parsesep \Trbracket\\
       & \derives\ \underline{\Nexpr} \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma \parsesep \Nslice \parsesep \Tcomma \parsesep \underline{\clistoneslice} \parsesep \Trbracket\\
       & \derives\ \underline{\Nexpr} \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma \parsesep \Nslice \parsesep \Tcomma \parsesep \Nslice \parsesep \Trbracket\\
       & \derives\ \underline{\Nexpr} \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma \parsesep \Nslice \parsesep \Tcomma \parsesep \Nexpr \parsesep \Trbracket\\
       & \derives\ \underline{\Nexpr} \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma \parsesep \Nslice \parsesep \Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket\\
       & \derives\ \underline{\Nexpr} \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma \parsesep \Nexpr \parsesep \Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket\\
       & \derives\ \underline{\Nexpr} \parsesep \Tlbracket \parsesep \Nslice \parsesep \Tcomma \parsesep \Tidentifier(d) \parsesep \Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket\\
       & \derives\ \underline{\Nexpr} \parsesep \Tlbracket \parsesep \Nexpr \parsesep \Tcomma \parsesep \Tidentifier(d) \parsesep \Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket\\
       & \derives\ \underline{\Nexpr} \parsesep \Tlbracket \parsesep \Tidentifier(c) \parsesep \Tcomma \parsesep \Tidentifier(d) \parsesep \Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket\\
       & \derives\ \Tidentifier(b) \parsesep \Tlbracket \parsesep \Tidentifier(c) \parsesep \Tcomma \parsesep \Tidentifier(d) \parsesep \Tcomma \parsesep \Tidentifier(e) \parsesep \Trbracket\\
\end{array}
\]
