...
\begin{itemize}
  \item It cannot be derived from the grammar. The text is therefore invalid
        ASL syntax, and causes a parse error.
...
In some cases, there seem to be ambiguities in \ASLvOneterm{}'s context-free grammar.
...
Use of an LR(1) grammar ensures that the syntax of \ASLvOneterm{}:
\begin{itemize}
  \item Is demonstrably unambiguous.
...
\item Assign the next token a priority (in this case, $\Tmul$).
...
\SectionDef[defines \emph{inlined derivations};]
  {InlinedDerivations}{Inlined Derivations}
...
\SectionDef[defines \emph{parametric productions};]
  {ParametricProductions}{Parametric Productions}
...
\SectionDef[introduces the parametric productions
        used for the ASL grammar;]
  {ASLParametricProductions}{ASL Parametric Productions}
...
\paragraph{Possibly-empty Parenthesised, Comma-separated List}
\hypertarget{def-plistzero}{}
\begin{flalign*}
\PlistZero{x}   \derivesinline \ & \Tlpar \parsesep \ClistZero{x} \parsesep \Trpar &\\
\end{flalign*}

\paragraph{Parenthesised Comma-separated List With At Least Two Elements}
...
\SectionDef[defines the ASL grammar;]
  {ASLGrammar}{ASL Grammar}
...
Notice that $\Nexpr$ uses $\priority{\Tunops}$.
...
|\ & \Ttype \parsesep \Tidentifier \parsesep \Tof \parsesep \Ntydecl \parsesep \Tsemicolon &\\
...
\begin{flalign*}
\Ncasealt \derives \ & \Twhen \parsesep \Ncasepattern \parsesep \option{\Twhere \parsesep \Nexpr} \parsesep \Tarrow \parsesep \Nstmtlist &
\end{flalign*}
...
\hypertarget{def-ncasepattern}{}
\begin{flalign*}
\Ncasepattern \derives \ & \Npatternset &\\
                    |\  & \Npatternlist &
\end{flalign*}
...
\begin{itemize}
  ...
  \item A \emph{non-terminal node} of the form $N(n_{1..k})$ where $N$ is a non-terminal symbol,
        ...
        \Ndecl\left(
         \begin{array}{l}
         \Nqualifier, \Noverride, \Tfunc, \Tidentifier(\id), \Nparamsopt, \Nfuncargs, \\
         \Nrecurselimit, \Nfuncbody
         \end{array}
         \right)
        \]
        is labelled by $\Ndecl$ and has eight child nodes,
        where non-terminal children are shown by their labels, abbreviating their subtrees.
\end{itemize}
\end{definition}
...
\begin{itemize}
   ...
   \item $n_i$ is a token whose label is $l_i$.
\end{itemize}
...
\SectionDef[defines the priority and associativity
        of tokens; and]
  {PriorityAndAssociativity}{Priority and Associativity for Parsing Tokens}
...
Each priority is assigned an associativity.
...
never returned by the parser. Note that the non-associativity of $\Tunops$
...
\SectionDef[presents examples showing how the grammar and
...
\ExampleDef{Failing to Derive an Invalid Expression}
The expression \texttt{1 + * 3}
is transformed by the lexical analysis into:
\[
...
\ExampleDef{Resolving Ambiguities with Binary Operators and Priority}
...
\ExampleDef{Resolving Ambiguity with Left Associativity}
The expression \texttt{1 + 2 + 3}
is transformed by the lexical analysis into:
\[
...
\ExampleDef{Derivation for Nested Unary Operators}
...
The parse tree above corresponds to the following derivation in the expanded grammar
(the underline indicates the non-terminal that is reduced at each step):
\[
\begin{array}{rl}
...
\ExampleDef{Resolving Ambiguity with Unary Operators and Priority}
...
\ExampleDef{Parametric Productions}
...
To emphasise the role of parametric productions, let $\clistoneslice$
