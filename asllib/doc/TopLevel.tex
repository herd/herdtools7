...
\left(
    \begin{array}{cc}
        (\overname{\tint}{\vret} \cartimes \overname{\XGraphs}{\vg})   & \cup\\
...
\inferrule{
    ...
    \aslparse(\vstdtokens) \parsearrow \vstdparse \OrParseError\\\\
    ...
    \aslparse(\vspectokens) \parsearrow \vspecparse \OrParseError\\\\
...
\inferrule{
  ...
  \renamelocalsargs(\vargs) \astarrow \newargs\\
...
The function
\[
\renamelocalsargs(\overname{\KleeneStar{(\Identifier\cartimes\ty)}}{\vargs}) \aslto
\overname{\KleeneStar{(\Identifier\cartimes\ty)}}{\newargs}
\]
renames the local storage elements appearing in the list of arguments
$\vargs$, yielding the list of arguments $\newargs$.
} % END_OF_UNIMPORTED_RELATION
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vargs$ is the empty list;
    \item \Proseeqdef{$\newargs$}{the empty list}.
  \end{itemize}

  \item \AllApplyCase{non\_empty}
  \begin{itemize}
    \item $\vargs$ is the list with \head\ $(\name,\vt)$ and \tail\ $\vargsone$;
...
\begin{mathpar}
\inferrule[empty]{}{
  \renamelocalsargs(\overname{\emptylist}{\vargs}) \astarrow \overname{\emptylist}{\newargs}
}
\end{mathpar}
...
The function
\[
\renamelocalsnamedargs(\overname{\KleeneStar{(\Identifier\cartimes\Option{\ty})}}{\params}) \aslto
\overname{\KleeneStar{(\Identifier\cartimes\Option{\ty})}}{\newparams}
\]
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\tty$ is the \bitvectortypeterm\ with width expression $\ve$ and bitfields $\vbitfields$;
    \item \Proserenamelocalsexpr{$\ve$}{$\vep$};
    \item \Proseeqdef{$\newty$}{the \bitvectortypeterm\ with width expression $\vep$ bitfields\\ $\vbitfields$}.
  \end{itemize}
...
\inferrule[t\_bits]{
  ...
  \renamelocalsty(\overname{\TBits(\ve, \vbitfields)}{\tty}) \astarrow \overname{\TBits(\vep, \vbitfields)}{\newty}
}
\end{mathpar}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vs$ is a \printstatement{$\vargs$}{$\vnewline$};
    \item \Proseeqdef{$\newargs$}{the list obtained by applying $\renamelocalsexpr$ to each expression in $\vargs$};
    \item \Proseeqdef{$\news$}{\printstatement{$\newargs$}{$\vnewline$}}.
  \end{itemize}
...
\begin{mathpar}
\inferrule[s\_print]{
  \newargs = [\ve\in\vargs: \renamelocalsexpr(\ve)]
}{
  {
  \begin{array}{r}
  \renamelocalsstmt(\overname{\SPrint(\vargs, \vnewline)}{\vs}) \astarrow\\
  \overname{\SPrint(\newargs, \vnewline)}{\news}
  \end{array}
  }
}
...
\begin{mathpar}
\inferrule[s\_pragma]{
  \newargs \eqdef [\ve\in\vargs: \renamelocalsexpr(\ve)]
}{
  \renamelocalsstmt(\overname{\SPragma(\name, \vargs)}{\vs}) \astarrow \overname{\SPragma(\name, \newargs)}{\news}
}
\end{mathpar}
...
\begin{mathpar}
\inferrule{
  \becheck(\astlabel(\vdecl) = \DFunc, \BuildBadDeclaration) \typearrow \True \OrBuildError\\\\
