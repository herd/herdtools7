...
%
We emphasise that this is only an example usage of the components listed above.
...
\SectionDef[presents an example of how to construct an ASL interpreter]
...
\SectionDef[explains how local storage
        elements of the standard library can be renamed to allow them to be correctly combined into
        arbitrary specifications]
  {Renaming Local Storage Elements in the Standard Library}{Renaming Local Storage Elements in the Standard Library}
...
\begin{itemize}
  \item view $\vf$ as a subprogram description consisting of a subprogram named $\name$,
        ...
        optional return type $\rettyopt$, optional subprogram type $\vsubprogramtype$,
  ...
  \item define $\vnewf$ as the subprogram description consisting of a subprogram named $\name$,
    ...
    optional return type $\newrettyopt$, optional subprogram type $\vsubprogramtype$,
    optional limit expression $\vlimitexpr$, and builtin flag $\vbuiltin$.
\end{itemize}
...
\inferrule{
  ...
  \left\{
   ...
   \FIELDargs &:& \args,\\
   \funcbody &:& \vbody,\\
   \FIELDreturntype &:& \rettyopt,\\
   \funcsubprogramtype &:& \vsubtype\\
   \FIELDrecurselimit    &:& \vlimitexpr\\
...
The function
\[
\renamelocalsargs(\overname{\KleeneStar{(\Identifier, \ty)}}{\args}) \aslto
\overname{\KleeneStar{(\Identifier, \ty)}}{\newargs}
\]
...
\inferrule[non\_empty]{
  ...
  \renamelocalsargs(\overname{(\name,\vt) \cons \vargsone}{\args}) \astarrow \overname{(\namep, \vtp) \cons \vargsonep}{\newargs}
}
\end{mathpar}
...
The function
\[
\renamelocalsnamedargs(\overname{\KleeneStar{(\Identifier, \Option{\ty})}}{\params}) \aslto
\overname{\KleeneStar{(\Identifier, \Option{\ty})}}{\newparams}
\]
...
\begin{mathpar}
\inferrule[non\_empty]{
  \params \reverseeqdef (\name,\tyopt) \cons \vargsone\\
  \renamelocalsname(\name) \astarrow \namep\hva\\
  \mapopt{\renamelocalsty}(\tyopt) \astarrow \tyoptp\\
  \renamelocalsnamedargs(\paramsone) \astarrow \paramsonep
}{
  \renamelocalsnamedargs(\params) \astarrow \overname{(\namep, \tyoptp) \cons \paramsonep}{\newparams}
}
\end{mathpar}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \Proseeqdef{$\newty$}{is the \parameterizedintegertypeterm{} for the renamed parameter
  ...
  \begin{itemize}
    ...
    \item \Proseeqdef{$\newty$}{the \wellconstrainedintegertypeterm\ with list of constraints $\newcs$}.
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item renaming the local storage elements in the list of types $\vli$ yields the list of types $\newli$;
  ...
  \begin{itemize}
    ...
    \item applying $\renamelocalsexpr$ to $\vindex$ yields $\newindex$;
  ...
  \begin{itemize}
    \item $\tty$ is a \structuredtypeterm{} with AST label $L$ and list of pairs consisting of identifiers and types $\vli$;
    \item \Proseeqdef{$\newli$}{the list of pairs $(\name, \renamelocalsty(\vt))$ for each pair
          $(\name, \vt)$ in $\vli$}
    \item \Proseeqdef{$\newty$}{the \structuredtypeterm\ with AST label $L$ and $\newli$}.
  \end{itemize}
\end{itemize}
...
\inferrule[t\_int\_wellconstrained]{
  \tty = \TInt(\WellConstrained(\vcs))\\
  \newcs \eqdef [\vc\in \vcs: \renamelocalsconstraint(\vcs)]
}{
  \renamelocalsty(\tty) \astarrow \overname{\TInt(\WellConstrained(\newcs))}{\newty}
}
\end{mathpar}
...
\begin{mathpar}
\inferrule[t\_array]{
  \renamelocalsexpr(\vindex) \astarrow \newindex\\
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vs$ is an \assignmentstatement{$\vle$}{$\ve$};
  ...
  \begin{itemize}
    \item $\vs$ is a \trystatementterm{} with statement $\vsone$, list of \catchersterm{} $\catchers$,
          and optional \Totherwise{} statement $\otherwiseopt$;
    \item \Proserenamelocalsstmt{$\vsone$}{$\vsonep$};
    \item \Proseeqdef{$\catchersp$}{the list of catchers obtained by applying \\
          $\renamelocalscatcher$ to each \catchersterm{} in $\catchers$};
    \item \Prosemapopt{$\renamelocalsstmt$}{$\otherwiseopt$}{$\otherwiseoptp$};
    \item \Proseeqdef{$\news$}{\trystatementterm{} with statement $\vsonep$, list of \catchersterm{} $\catchersp$, and optional \Totherwise{} statement  $\otherwiseoptp$}.
  \end{itemize}

  \item \AllApplyCase{s\_print}
  \begin{itemize}
    \item $\vs$ is a \printstatement{$\args$}{$\vaddlinefeed$};
...
\inferrule[s\_for]{
  ...
  \renamelocalsexpr(\eend) \astarrow \eendp\hva\\
  \renamelocalsstmt(\vbody) \astarrow \vbodyp\\
  \mapopt{\renamelocalsexpr}(\velimitopt) \astarrow \velimitoptp\hva\\
}{
  {
  \begin{array}{r}
...
\inferrule[s\_throw]{
  ...
  \renamelocalsstmt(\overname{\SThrow(\ve)}{\vs}) \astarrow
  \overname{\SThrow(\vep)}{\news}
}
\end{mathpar}
...
\inferrule[s\_try]{
  \renamelocalsstmt(\vsone) \astarrow \vsonep\\
  \catchersp \eqdef [\vc\in\catchers: \renamelocalscatcher(\vc)]\hva\\
...
\inferrule[s\_print]{
  ...
  {
  \begin{array}{r}
  \renamelocalsstmt(\overname{\SPrint(\args, \vaddlinefeed)}{\vs}) \astarrow\\
  \overname{\SPrint(\newargs, \vaddlinefeed)}{\news}
  \end{array}
  }
}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\ve$ is an \arbitraryexpression{$\vt$};
    \item \Proserenamelocalsty{$\vt$}{$\vtp$};
  ...
  \begin{itemize}
    \item $\ve$ is an \atcexpression{$\veone$}{$\vt$};
  ...
  \begin{itemize}
    \item $\ve$ is an \getarrayexpression{$\veone$}{$\vetwo$};
    \item \Proserenamelocalsexpr{$\veone$}{$\veonep$};
    \item \Proserenamelocalsexpr{$\vetwo$}{$\vetwop$};
...
\inferrule[e\_getfield]{
  ...
  \renamelocalsexpr(\overname{\EGetField(\veone, \vf)}{\ve}) \astarrow \overname{\EGetField(\veonep, \vf)}{\newe}
}
\end{mathpar}
...
\inferrule[e\_pattern]{
  \renamelocalsexpr(\veone) \astarrow \veonep\\
  \vpatternasts \eqdef [\vi\in\indices(\vps): \renamelocalspattern(\vps[\vi])]
}{
  {
    \begin{array}{r}
  \renamelocalsexpr(\overname{\EPattern(\veone, (\vps, \vpatternkind))}{\ve}) \astarrow\\
  \overname{\EPattern(\veonep, (\vpatternasts, \vpatternkind))}{\newe}
    \end{array}
  }
}
\end{mathpar}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vle$ is an $\LESetField(\vleone, \vf)$;
    \item \Proserenamelocalslexpr{$\vle$}{$\vlep$};
    \item \Proseeqdef{$\newle$}{the $\LESetField(\vleonep, \vf)$}.
  \end{itemize}

  \item \AllApplyCase{le\_setfields}
  \begin{itemize}
    \item $\vle$ is an \setfieldslexpr{$\vleone$}{$\vfl$};
    \item \Proserenamelocalslexpr{$\vle$}{$\vleonep$};
...
\inferrule[le\_slice]{
  ...
  \renamelocalslexpr(\overname{\LESlice(\vleone, \vslices)}{\vle}) \astarrow \overname{\LESlice(\vleonep, \vslicesp)}{\newle}
}
\end{mathpar}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \Proserenamelocalsexpr{$\ve$}{$\vep$};
    \item \Proseeqdef{$\newc$}{the \Proseexactconstraint{$\vep$}}.
  \end{itemize}
...
\inferrule[range]{
  ...
  \renamelocalsconstraint(\vc) \astarrow \overname{\ConstraintRange(\veonep, \vetwop)}{\newc}
}
\end{mathpar}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vslice$ is a slice over two expression $\veone$ and $\vetwo$ with AST label \\
          $S \in \{\SliceLength, \SliceRange\}$;
    \item \Proserenamelocalsexpr{$\veone$}{$\veonep$};
    \item \Proserenamelocalsexpr{$\vetwo$}{$\vetwop$};
    \item \Proseeqdef{$\vnewslice$}{the slice with AST label $S$ over the two expressions $\veonep$ and $\vetwop$};
  \end{itemize}
\end{itemize}
...
\begin{mathpar}
\inferrule{}{
  \renamelocalsname(\name) \astarrow \overname{\stdliblocalprefix\ \concatstrings\ \name}{\newname}
}
\end{mathpar}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vp$ is a pattern with AST label $L$ and expression $\vpe$,
          where $L$ is one of $\PatternGeq$, $\PatternLeq$, and $\PatternSingle$;
    \item \Proserenamelocalsexpr{$\veone$}{$\veonep$};
    \item \Proseeqdef{$\vpe$}{the pattern with AST label $L$ and expression $\vnewpe$}.
  \end{itemize}

  \item \AllApplyCase{range}
  \begin{itemize}
    \item $\vp$ is a range pattern with expressions $\veone$ and $\vetwo$;
    \item \Proserenamelocalsexpr{$\veone$}{$\veonep$};
    \item \Proserenamelocalsexpr{$\vetwo$}{$\vetwop$};
...
\inferrule[single\_expr]{
  \vp = L(\vpe)\\
  \astlabel(\vp) \in \{\PatternGeq, \PatternLeq, \PatternSingle\}\\
...
\]
renames the identifiers corresponding to local storage elements that appear in the
\catcherterm{} $\vc$, yielding the \catcherterm{} $\newc$.
} % END_OF_UNIMPORTED_RELATION
...
\AllApply
\begin{itemize}
  \item $\vc$ is a \catcherterm{} with optional exception name $\optexnname$, exception type $\exnty$,
...
\SectionDef[defines how to mark subprograms in the standard
        library to allow distinguishing them from other subprograms]
  {Marking Standard Library Functions}{Marking Standard Library Functions}
...
\]
sets the $\funcbuiltin$ flag of a top-level function declaration $\decl$,
which is used to identify standard library functions in \TypingRuleRef{InsertStdlibParam}.
It produces a build error when $\decl$ is not a function.
} % END_OF_UNIMPORTED_RELATION

\ProseParagraph
\AllApply
\begin{itemize}
  \item \Prosebecheck{that $\vdecl$ is a function}{\BuildBadDeclaration};
...
\begin{mathpar}
\inferrule{
  \becheck(\astlabel(\vdecl) = \DFunc, \BuildBadDeclaration) \typearrow \True \OrBuildError\hva\\\\
  \vdecl \reverseeqdef \DFunc(\funcsig)\\
