...
\[
  ...
  \left(
  \begin{array}{ll}
  \PatternResult(\overname{\tbool}{\vb}, \overname{\XGraphs}{\newg}) & \cup\\
...
resulting in either $\PatternResult(\vb, \newg)$ or an abnormal configuration.
...
\inferrule{}
{
  \evalpattern{\env, \Ignore, \PatternAll} \evalarrow \PatternResult(\nvbool(\True), \emptygraph)
}
\end{mathpar}
...
\begin{itemize}
  ...
  \item the side-effect-free evaluation of $\ve$ in
    environment $\env$ is \\ $\ExprSEFResult(\vvone, \newg)$\ProseOrError;
...
\begin{mathpar}
\inferrule{
  \evalexprsef{\env, \ve} \evalarrow \ExprSEFResult(\vvone, \newg) \OrDynError\\\\
  \binoprel(\EQOP, \vv, \vvone) \evalarrow \vb
}{
  \evalpattern{\env, \vv, \overname{\PatternSingle(\ve)}{\vp}} \evalarrow \PatternResult(\vb, \newg)
}
\end{mathpar}
...
\begin{itemize}
  ...
  \item the side-effect-free evaluation of $\veone$ in $\env$ is $\ExprSEFResult(\vvone, \vgone)$\ProseOrError;
  \item the side-effect-free evaluation of $\vetwo$ in $\env$ is $\ExprSEFResult(\vvtwo, \vgtwo)$\ProseOrError;
...
\begin{mathpar}
\inferrule{
  \evalexprsef{\env, \veone} \evalarrow \ExprSEFResult(\vvone, \vgone) \OrDynError\\\\
  \binoprel(\GEQ, \vv, \vvone) \evalarrow \vbone\\
  \evalexprsef{\env, \veone} \evalarrow \ExprSEFResult(\vvtwo, \vgtwo) \OrDynError\\\\
  \binoprel(\LEQ, \vv, \vvtwo) \evalarrow \vbtwo\\
  \binoprel(\BAND, \vbone, \vbtwo) \evalarrow \vb\\
  \newg \eqdef \vgone \parallelcomp \vgtwo
}{
  \evalpattern{\env, \vv, \PatternRange(\veone, \vetwo)} \evalarrow \PatternResult(\vb, \newg)
}
\end{mathpar}
...
\begin{mathpar}
\inferrule{
  \evalexprsef{\env, \ve} \evalarrow \ExprSEFResult(\vvone, \newg) \OrDynError, \DivergingConfig\\\\
  \binoprel(\LEQ, \vv, \vvone) \evalarrow \vb
}{
  \evalpattern{\env, \vv, \PatternLeq(\ve)} \evalarrow \PatternResult(\vb, \newg)
}
\end{mathpar}
...
\inferrule{
  ...
  \evalpattern{\env, \vv, \PatternGeq(\ve)} \evalarrow \PatternResult(\vb, \newg)
}
\end{mathpar}
...
\begin{mathpar}
\inferrule[empty]{}{
  \evalpattern{\env, \overname{\nvbitvector(\emptylist)}{\vv}, \PatternMask(\emptylist)} \evalarrow \PatternResult(\True, \emptygraph)
}
\end{mathpar}
...
\inferrule[non\_empty]{
  ...
  \evalpattern{\env, \vv, \PatternMask(\vm)} \evalarrow \PatternResult(\vb, \emptygraph)
}
\end{mathpar}
...
\inferrule{
  ...
  i=1..k: \evalpattern{\env, \vvs_i, \vp_i} \evalarrow \PatternResult(\nvbool(\vbs_i), \vg_i) \OrDynError\\\\
  \vres \eqdef \nvbool(\bigwedge_{i=1..k} \vbs_i)\\
  \vg \eqdef \vg_1 \parallelcomp \ldots \parallelcomp \vg_k
}{
  \evalpattern{\env, \vv, \PatternTuple(\vps)} \evalarrow \PatternResult(\vres, \emptygraph)
}
\end{mathpar}
...
\begin{itemize}
  ...
  \item $\vps$ is $\vp_{1..k}$;
  \item evaluating each pattern $\vp_i$ in $\env$ results in $\PatternResult(\nvbool(\vb_i), \vg_i)$\ProseOrDynErrorDiverging;
...
\inferrule{
  \vps \eqname \vp_{1..k}\\
  i=1..k : \evalpattern{\env, \vv, \vp_i} \evalarrow \PatternResult(\nvbool(\vb_i), \vg_i) \terminateas \OrDynErrorDiverging\\\\
  \vb \eqdef \nvbool(\bigvee_{i=1..k} \vb_i)\\
  \newg \eqdef \vg_1 \parallelcomp \ldots \parallelcomp \vg_k
}{
  \evalpattern{\env, \vv, \PatternAny(\vps)} \evalarrow \PatternResult(\vb, \newg)
}
\end{mathpar}
...
\begin{mathpar}
\inferrule{
  \evalexprsef{\env, \vpone} \evalarrow \ExprSEFResult(\vbone, \newg) \OrDynErrorDiverging\\\\
  \unoprel(\BNOT, \vbone) \evalarrow \vb
}{
  \evalpattern{\env, \vv, \PatternNot(\vpone)} \evalarrow \PatternResult(\vb, \newg)
}
\end{mathpar}
