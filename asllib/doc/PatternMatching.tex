...
\end{mathpar}
\RenderRule{annotate_pattern_all}
...
\AllApply
\begin{itemize}
  \item $\vp$ is the pattern that matches everything, $\PatternAll$, and therefore
    matches $\vv$;
...
\begin{itemize}
  ...
  \item obtaining the \underlyingtypeterm\ of $\vte$ yields $\vtestruct$\ProseOrTypeError;
  \item \OneApplies
  \begin{itemize}
    \item \AllApplyCase{t\_bool, t\_real, t\_int, t\_string}
    \begin{itemize}
      \item the AST label of $\vtstruct$ is one of $\TBool$, $\TReal$, $\TInt$, or $\TString$;
      \item checking that the labels of $\vtstruct$ and $\vtestruct$ are equal yields $\True$\ProseOrTypeError;
    \end{itemize}
    ...
    \begin{itemize}
      ...
      \item checking that the labels of $\vtstruct$ and $\vtestruct$ are equal yields $\True$\ProseOrTypeError;
      \item determining whether the bitwidths of $\vtstruct$ and $\vtestruct$ are equal yields $\True$\ProseOrTypeError;
    \end{itemize}
    ...
    \begin{itemize}
      ...
      \item checking that the labels of $\vtstruct$ and $\vtestruct$ are equal yields $\True$\ProseOrTypeError;
      \item determining whether the lists of enumeration literals of $\vtstruct$ and $\vtestruct$ are equal yields $\True$\ProseOrTypeError;
    \end{itemize}

    \item \AllApplyCase{error}
    \begin{itemize}
      \item determining whether the labels of $\vtstruct$ and $\vtestruct$ are the same yields $\True$\ProseOrTypeError;
...
\inferrule[t\_bool, t\_real, t\_int, t\_string]{
  \annotateexpr(\tenv, \ve) \typearrow (\vte, \vep, \vses) \OrTypeError\hva\\\\
  \checksymbolicallyevaluable(\vses) \typearrow \True \OrTypeError\hva\\\\
  \makeanonymous(\tenv, \vt) \typearrow \vtstruct \OrTypeError\hva\\\\
  \makeanonymous(\tenv, \vte) \typearrow \vtestruct \OrTypeError\hva\\\\
  \commonprefixline\hva\\\\
  \astlabel(\vtstruct) \in \{\TBool, \TReal, \TInt, \TString\}\\
  \techeck(\astlabel(\vtstruct) = \astlabel(\vtestruct), \BadOperands) \typearrow \True \OrTypeError
}{
...
\inferrule[t\_bits]{
  ...
  \techeck(\astlabel(\vtstruct) = \astlabel(\vtestruct), \BadOperands) \typearrow \True \OrTypeError\hva\\
  \bitwidthequal(\tenv, \vtstruct, \vtestruct) \typearrow \vb\\
...
\inferrule[t\_enum]{
  ...
  \techeck(\astlabel(\vtstruct) = \astlabel(\vtestruct), \BadOperands) \typearrow \True \OrTypeError\hva\\
  \vtstruct \reverseeqdef \TEnum(\vlione)\\
  \vtestruct \reverseeqdef \TEnum(\vlitwo)\hva\\
...
\RenderRule{annotate_pattern_single}
...
\RenderRule{eval_pattern_PSingle}
...
\AllApply
\begin{itemize}
  \item $\vp$ is the pattern that matches anything within the range given by
  ...
  \item checking that the AST labels of $\vtstruct$, $\vteonestruct$, and $\vtetwostruct$ are all the same and are either
  ...
  \item $\newp$ is a range pattern with bounds $\veonep$ and $\vetwop$ ($\PatternRange(\veonep, \vetwop)$).
\end{itemize}
\FormallyParagraph
\BackupOriginalRule{
\begin{mathpar}
\inferrule{
  \vp = \PatternRange(\veone, \vetwo)\\
  \annotatesymbolicallyevaluableexpr(\tenv, \veone) \typearrow (\vteone, \veonep, \vsesone) \OrTypeError\hva\\\\
  \annotatesymbolicallyevaluableexpr(\tenv, \vetwo) \typearrow (\vtetwo, \vetwop, \vsestwo) \OrTypeError\hva\\\\
  \vses \eqdef \vsesone \cup \vsestwo\\
  \makeanonymous(\tenv, \vt) \typearrow \vtstruct \OrTypeError\hva\\\\
  \makeanonymous(\tenv, \vteone) \typearrow \vteonestruct \OrTypeError\hva\\\\
  \makeanonymous(\tenv, \vtetwo) \typearrow \vtetwostruct \OrTypeError\hva\\\\
  {
    \begin{array}{rl}
  ...
  \annotatepattern(\tenv, \vt, \vp) \typearrow (\overname{\PatternRange(\veonep, \vetwop)}{\newp}, \vses)
}
\end{mathpar}
} % END_OF_BACKUP_RULE
\RenderRule{annotate_pattern_range}
...
\begin{itemize}
  \item $\vp$ is the condition corresponding to being greater than or equal
    to $\veone$, and less than or equal to $\vetwo$, that is, $\PatternRange(\veone, \vetwo)$;
...
\begin{mathpar}
\inferrule{
  \evalexprsef(\env, \veone) \evalarrow \ResultExprSEF(\vvone, \vgone) \OrDynErrorDiverging\hva\\\\
  \evalbinop(\GE, \vv, \vvone) \evalarrow \vbone\\
  \evalexprsef(\env, \vetwo) \evalarrow \ResultExprSEF(\vvtwo, \vgtwo) \OrDynErrorDiverging\hva\\\\
...
\RenderRule{eval_pattern_PRange}
...
\AllApply
\begin{itemize}
\item $\vp$ is the pattern that matches anything less than or equal to an expression $\ve$,
that is, $\PatternLeq(\ve)$;
\item annotating the expression $\ve$ in $\tenv$ yields $(\vte, \vep, \vses)$\ProseOrTypeError;
\item \Prosechecksymbolicallyevaluable{$\vses$};
\item obtaining the \underlyingtypeterm\ of $\vt$ in $\tenv$ yields $\vtstruct$\ProseOrTypeError;
\item obtaining the \underlyingtypeterm\ of $\vte$ in $\tenv$ yields $\vtestruct$\ProseOrTypeError;
\item $\vb$ is true if and only if $\vtstruct$ and $\vtestruct$ are both integer types or both the \realtypeterm{};
...
\item $\newp$ is the pattern that matches anything less than or equal to $\vep$.
\end{itemize}
...
\inferrule{
  ...
  {
    \begin{array}{rl}
      \vb \eqdef& \astlabel(\vtstruct) = \astlabel(\vtestruct)\ \land\\
                & \astlabel(\vtstruct) \in \{\TInt, \TReal\}
    \end{array}
  }\\
  \techeck(\vb, \BadOperands) \typearrow \True \OrTypeError
...
\RenderRule{annotate_pattern_leq}
...
\AllApply
\begin{itemize}
\item $\vp$ is the pattern that matches anything greater than or equal to an expression $\ve$,
...
\item if $\vb$ is $\False$ a \typingerrorterm{} is returned (indicating that the types of $\vt$ and $\vte$
      ...
      which short-circuits the entire rule;
\item $\newp$ is the pattern that matches anything greater than or equal to $\vep$.
\end{itemize}
\FormallyParagraph
\BackupOriginalRule{
\begin{mathpar}
...
\RenderRule{annotate_pattern_geq}
...
\AllApply
  \begin{itemize}
  \item $\vp$ is the pattern that matches a mask $\vm$, that is, $\PatternMask(\vm)$;
...
\begin{mathpar}
\inferrule{
  \checkstructurelabel(\tenv, \vt, \TBits) \typearrow \True \OrTypeError\hva\\\\
  \vn \eqdef \listlen{\vm}\\
  \checktypesatisfies(\tenv, \vt, \TBits(\vn, \emptylist)) \typearrow \True \OrTypeError
}{
  \annotatepattern(\tenv, \vt, \overname{\PatternMask(\vm)}{\vp}) \typearrow (\overname{\PatternMask(\vm)}{\newp}, \overname{\emptyset}{\vses})
}
\end{mathpar}
} % END_OF_BACKUP_RULE
\RenderRule{annotate_pattern_mask}
...
%
In contrast, the tuple patterns in \listingref{typing-ptuple-bad} and \listingref{typing-ptuple-bad} are ill-typed.
...
\AllApply
\begin{itemize}
  \item $\vp$ is the pattern that matches a tuple $\vli$, that is, $\PatternTuple(\vli)$;
  ...
  \item $\vtstruct$ is a \tupletypeterm{} with list of tuple types $\vts$;
  ...
  \item $\newp$ is the pattern that matches the tuple $\newli$ ($\PatternTuple(\newli)$);
...
\inferrule{
  \vp = \PatternTuple(\vli)\\
  \tstruct(\tenv, \vt) \typearrow \vtstruct \OrTypeError\hva\\\\
  \techeck(\astlabel(\vtstruct) = \TTuple, \UnexpectedType) \typearrow \True \OrTypeError\hva\\\\
  \vtstruct \reverseeqdef \TTuple(\vts)\\
  \techeck(\equallength(\vli, \vts), \UnexpectedType) \typearrow \True \OrTypeError\hva\\\\
  i\in\listrange(\vli): \annotatepattern(\tenv, \vts[i], \vli[i]) \typearrow (\vlip[i], \vxs_i) \OrTypeError\hva\\\\
  \newli \eqdef i\in\listrange(\vli): \vlip[i]\\
  \vses \eqdef \bigcup_{i\in\listrange(\vli)} \vxs_i
}{
  \annotatepattern(\tenv, \vt, \vp) \typearrow (\overname{\PatternTuple(\newli)}{\newp}, \vses)
}
\end{mathpar}
} % END_OF_BACKUP_RULE
\RenderRule{annotate_pattern_tuple}
...
\AllApply
\begin{itemize}
  \item $\vp$ is a list of patterns $\vps$ of length $k$, $\PatternTuple(\vps)$;
  \item obtaining the value at index $\vi$ from $\vv$, for each $\vi=1..k$, yields $\vvs[i]$\ProseOrError;
  \item evaluating the value $\vvs[i]$ against the pattern $\vp_i$ in environment $\env$,
        for each $\vi=1..k$, yields the Boolean $\vb[i]$ and \executiongraphterm{} $\vg[i]$\ProseOrDynErrorDiverging;
  \item $\vb$ is the conjunction of all Boolean values in $\vbs$;
  \item $\vg$ is the parallel composition of all \executiongraphsterm{} in $\vg$.
\end{itemize}
...
\begin{mathpar}
\inferrule{
  i=1..k: \getindex(i, \vv) \evalarrow \vvs[i]\OrDynError\\
  i=1..k: \evalpattern(\env, \vvs[i], \vps[i]) \evalarrow \ResultPattern(\nvbool(\vbs[i]), \vg[i]) \OrDynErrorDiverging\hva\\\\
  \vb \eqdef \nvbool(\bigwedge_{i=1..k} \vbs[i])\\
  \vg \eqdef \vg[1] \parallelcomp \ldots \parallelcomp \vg[k]
}{
  \evalpattern(\env, \vv, \overname{\PatternTuple(\vps)}{\vp}) \evalarrow \ResultPattern(\vb, \overname{\emptygraph}{\newg})
}
\end{mathpar}
} % END_OF_BACKUP_RULE
\RenderRule{eval_pattern_PTuple}
...
\begin{mathpar}
\inferrule{
  \vps \reverseeqdef \vp_{1..k}\\
  i=1..k : \evalpattern(\env, \vv, \vp_i) \evalarrow \ResultPattern(\nvbool(\vb_i), \vg_i) \OrDynErrorDiverging\hva\\\\
...
\AllApply
\begin{itemize}
  \item $\vp$ is the pattern that matches the negation of a pattern $\vq$ ($\PatternNot(\vq)$);
...
\RenderRule{annotate_pattern_neg}
