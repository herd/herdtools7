...
\Subdomains\left(\left[
  \begin{array}{l}
    ...
    \ConstrainedDom(\ConstraintExact(\AbbrevEBinop{\PLUS}{\AbbrevEVar{\vN}}{\AbbrevEVar{\vN}}))\\
  \end{array}
\right]\right)
\]
...
\begin{itemize}
  ...
  \item $\ConstrainedDom(\ConstraintExact(\AbbrevEBinop{\PLUS}{\AbbrevEVar{\vN}}{\AbbrevEVar{\vN}}))$
        is subsumed by \\
        $\ConstrainedDom(\ConstraintExact(\AbbrevEBinop{\MUL}{\AbbrevEVar{\vN}}{\ELInt{2}}))$.
\end{itemize}
...
In \listingref{SymdomOfConstraint}, normalizing the \symbolicdomain{} for the type\\
\verb|integer{1..6, 5..N, 9, N*2}| (see \ExampleRef{The Symbolic Domain of a Type})
merges $\Finite(\{1,2,3,4,5,6\})$ and $\Finite(\{9\})$, yielding the \symbolicdomain{}
\[
\Subdomains\left(\left[
  \begin{array}{l}
    \Finite(\{1,2,3,4,5,6, 9\}),\\
...
In \listingref{SymdomOfConstraint}, the \symbolicdomain{} for the type
...
\begin{itemize}
  \item the \symbolicdomain{} for the constraint \verb|1..6| is $\Finite(\{1,2,3,4,5,6\})$
  \item the \symbolicdomain{} for the constraint \verb|9| is $\Finite(\{9\})$,
  \item the \symbolicdomain{} for the constraint \verb|N*2| is \\
        $\ConstrainedDom(\ConstraintExact(\AbbrevEBinop{\MUL}{\AbbrevEVar{\vN}}{\ELInt{2}}))$,
        since the premise \\
        $\symdomeval(\tenv, \AbbrevEBinop{\MUL}{\AbbrevEVar{\vN}}{\ELInt{2}})$ yields $\Top$,
  \item the \symbolicdomain{} for the constraint \verb|5..N| is \\
        $\ConstrainedDom(\ConstraintRange(\ELInt{5}, \AbbrevEVar{\vN}))$,
        since the premise \\
...
involves test whether the \symbolicdomain{} for \verb|2..4| is a subset of the
\symbolicdomain{} for \verb|1..6|, which $\True$.
Similarly for testing whether the \symbolicdomain{} for \verb|N+N|
is a subset of the \symbolicdomain{} for \verb|N*2| also yields $\True$.
In contrast, testing whether the \symbolicdomain{} for \verb|5..N|
is a subset of the \symbolicdomain{} for \verb|N*2| yields $\False$.
