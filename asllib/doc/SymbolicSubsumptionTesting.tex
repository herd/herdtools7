...
\begin{mathpar}
\inferrule{
  \sdone = \Subdomains(\symdomsone)\\
  \sdtwo = \Subdomains(\symdomstwo)\hva\\
  \symdomnormalize(\symdomsone) \typearrow \symdomsonenorm\\
  \symdomnormalize(\symdomstwo) \typearrow \symdomstwonorm\hva\\
  {
  \begin{array}{rcl}
  ...
  \symdomsubsetunions(\tenv, \sdone, \sdtwo) \typearrow \vb
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{symdom_subset_unions}
...
\inferrule{
  ...
  \vfinitedomains = [\vs \in \symdoms \;|\; \configdomain{\vs} = \Finite]\hva\\
...
\begin{mathpar}
\inferrule{}{
  {\begin{array}{r}\symdomofwidthexpr(\ve) \typearrow \\ \overname{\Subdomains([\ \ConstrainedDom(\ConstraintExact(\ve))\ ])}{\vd}\end{array}}
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{symdom_of_width_expr}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item define $\vd$ as the constrained symbolic integer domain for $\vc$ if either $\vvone$ or $\vvtwo$
          ...
          set integers that are both greater than or equal to $\vvone$ and less than or equal to $\vvtwo$
...
\inferrule[range]{
  ...
  \symdomeval(\tenv, \vetwo) \typearrow \vvtwo\hva\\
...
\RenderRule{symdom_of_constraint}
...
\inferrule[constrained\_constrained\_non\_equal\_sets]{
  \constraintequal(\tenv, \vcone, \vctwo) \typearrow \False\hva\\\\
  \approxconstraint(\tenv, \Over, \vcone) \typearrow \vsone\\
  \approxconstraint(\tenv, \Under, \vctwo) \typearrow \vstwo\hva\\
...
\inferrule[range\_over1]{
  ...
  \approxexprmax(\tenv, \vetwo) \typearrow \vztwo\hva\\
  \vzone \neq \CannotOverapproximate \land \vztwo \neq \CannotOverapproximate\\
  \makeinterval(\Over, \vzone, \vztwo) \typearrow \vsinterval \hva\\
  {
  \vs \eqdef \ifthenelseop[V]{\vsinterval \neq \CannotOverapproximate}{\vsinterval}{\approxbottomtop(\Over)}
  }
}{
...
\inferrule[range\_under1]{
  ...
  \approxexprmin(\tenv, \vetwo) \typearrow \vztwo\hva\\
  \vzone \neq \CannotOverapproximate \land \vztwo \neq \CannotOverapproximate\\
  \makeinterval(\Over, \vzone, \vztwo) \typearrow \vsinterval \hva\\
  {
...
\end{mathpar}
} % END_OF_ORIGINAL_RULE_BACKUP
\RenderRule{make_interval}
...
\inferrule{
  ...
  {
  \cs \eqdef \left[a..b \in \intervals:
  \ifthenelseop[V]{a=b}{\AbbrevConstraintExact{\ELInt{a}}}{\AbbrevConstraintRange{\ELInt{a}}{\ELInt{b}}}
    \right]
  }
}{
...
\RenderRule{intset_to_constraints}
...
\begin{mathpar}
\inferrule[unop]{
  \approxexpr(\tenv, \vapprox, \vep) \typearrow \vsp \terminateas \CannotOverapproximate\hva\\\\
...
\inferrule[binop\_precise]{
  ...
  \approxexpr(\tenv, \vapprox, \vetwo) \typearrow \vstwo \terminateas \CannotOverapproximate\hva\\\\
  \approxconstraintbinop(\tenv, \vapprox, \op, \vsone, \vstwo) \typearrow (\vsp, \plf)\\
  \plf = \PrecisionFull \lor (\plf = \PrecisionLost \land \vapprox = \Under)\hva\\
...
\inferrule[binop\_approx]{
  \approxexpr(\tenv, \vapprox, \veone) \typearrow \vsone \terminateas \CannotOverapproximate\hva\\\\
  \approxexpr(\tenv, \vapprox, \vetwo) \typearrow \vstwo \terminateas \CannotOverapproximate\hva\\\\
  \approxconstraintbinop(\tenv, \vapprox, \op, \vsone, \vstwo) \typearrow (\vsp, \plf)\\
...
\inferrule[cond]{
  \approxexpr(\tenv, \vapprox, \vetwo) \typearrow \vstwo \terminateas \CannotOverapproximate\hva\\\\
  \approxexpr(\tenv, \vapprox, \vethree) \typearrow \vsthree \terminateas \CannotOverapproximate\hva\\\\
...
\RenderRule{approx_constraint_binop}
...
\end{mathpar}
%\RenderRule{apply_binop_extremities}
...
\RenderRule{possible_extremities_left}
...
\RenderRule{possible_extremities_right}
