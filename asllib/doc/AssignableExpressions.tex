...
\begin{itemize}
  ...
  \item annotating the right-hand-side expression corresponding to $\ebase$ in $\tenv$ yields \\ $(\tbase, \Ignore)$\ProseOrTypeError;
  \item obtaining the \structure\ of $\tbase$ in $\tenv$ yields an array type of size $\size$ and element type $\telem$, that is, $\TArray(\size, \telem)$\ProseOrTypeError;
  \item annotating the left-hand-side expression $\ebase$ with type $\tbase$ in $\tenv$ yields $\ebasep$\ProseOrTypeError;
  ...
  \item $\newle$ is an access to array $\ebasep$ at index $\eindexp$, that is, \\ $\LESetArray(\ebasep, \eindexp)$.
\end{itemize}
\subsubsection{Formally}
\begin{mathpar}
\inferrule{
  \annotateexpr{\tenv, \torexpr(\ebase)} \typearrow (\tbase, \Ignore) \OrTypeError\\\\
  \tstruct(\tenv, \tbase) \typearrow \TArray(\size, \telem) \OrTypeError\\\\
  \annotatelexpr{\tenv, \ebase, \tbase} \typearrow \ebasep \OrTypeError\\\\
  \checktypesat(\tenv, \telem, \vt) \typearrow \True \OrTypeError\\\\
