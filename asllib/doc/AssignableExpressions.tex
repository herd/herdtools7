...
\inferrule{
  ...
  \makeanonymous(\tenv, \vtleone) \typearrow \tleoneanon \OrTypeError\\\\
  \astlabel(\tleoneanon) \not\in \{\TException, \TRecord, \TCollection, \TBits\}
}{
...
\[
  \assignbitvectorfields(
    \overname{(\tbitvector\times\XGraphs)}{\vm} \aslsep
...
\begin{itemize}
  ...
  \item $\vvone$ is the value of $\vbase$ in the global component of $\env$ ;
  \item \Proseeqdef{$\vm$}{$(\vv, \vg)$};
  \item \Proseeqdef{$\vrmrecord$}{$(\vvone, \{\})$};
  \item applying $\assignbitvectorfields$ to $\vm$, $\vrmrecord$,
    $\fields$, and $\vslices$, yields $((\vvtwo, \vgtwo), \envone)$\ProseOrError;
  \item \Proseeqdef{$\vgzero$}{the graph containing a Write Effect for each
    $\fieldname$ in $\fields$};
  \item $\newenv$ is $\env$ where $\vbase$ is bound to $\vv$ in the $\storage$ map
    of the global dynamic environment $G^\denv$.
  \item $\newg$ is the ordered composition with the $\asldata$ edge of $\vgtwo$
    and $\vgzero$;
\end{itemize}
...
\inferrule{
  \env \eqname (\tenv, \denv)\\
  \vbase \in \dom(G^\denv)\\
  \vrmrecord \eqdef (G^\denv[\vbase], \{\}) \\
  \vm \eqdef (\vv,\vg) \\
  \assignbitvectorfields(\vm, \vrmrecord, \fields, \vslices) \evalarrow ((\vvtwo, \vgtwo), \envone) \OrDynError\\\\
  \vgzero \eqdef \{\fieldname \in \fieldnames: \WriteEffect(\vbase + \ttdotstr + \fieldname)\}\\
  \newenv \eqdef (\tenv, (G^\denv[\vx \mapsto \vvtwo], L^\denv))\\
  \newg \eqdef \ordered{\vgtwo}{\asldata}{\vgzero} \\
}{
