...
resulting in either a configuration $\LexprResult(\newg, \env)$ or an abnormal configuration.
...
\inferrule{
  ...
  \evallexpr{\env, \LEDiscard, (\vv, \vg)} \evalarrow \LexprResult(\newg, \newenv)
}
\end{mathpar}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item $\newle$ is $\vle$;
    \item \Proseeqdef{$\vses$}{the set containing a \LocalEffectTerm{} with \purity{} $\SEImpure$, and a \ImmutabilityTerm{} with immutability $\False$}.
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item determining whether $\tty$ \typesatisfies\ $\vte$ in $\tenv$ yields $\True$\ProseOrTypeError;
    \item $\newle$ is $\vle$;
    \item \Proseeqdef{$\vses$}{the set containing a \GlobalEffectTerm{} with \purity{} $\SEImpure$, and a \ImmutabilityTerm{} with immutability $\False$}.
  \end{itemize}
...
\inferrule[local]{
  ...
  (\overname{\vle}{\newle}, \{\LocalEffect(\SEImpure), \Immutability(\False)\})
}
\end{mathpar}
...
\inferrule[global]{
  ...
  \annotatelexpr{\tenv, \overname{\LEVar(\vx)}{\vle}, \vte} \typearrow
  (\overname{\vle}{\newle}, \{\GlobalEffect(\SEImpure), \Immutability(\False)\})
}
\end{mathpar}
...
\inferrule[local]{
  ...
  \evallexpr{\env, \LEVar(\vx), (\vv, \vg)} \evalarrow \LexprResult(\newg,\newenv)
}
\end{mathpar}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \Proseevallexpr{$\env$}{$\vle$}{$\vm$}{$\LexprResult(\vgone, \envone)$}\ProseOrAbnormal;
...
\inferrule[empty]{}
{
  ...
  \LexprResult(\overname{\emptygraph}{\newg}, \overname{\env}{\newenv})
}
\end{mathpar}
...
\inferrule[non\_empty]{
  ...
  \evallexpr{\env, \vle, \vm} \evalarrow \LexprResult(\vgone, \envone) \OrAbnormal\\\\
  \evalmultiassignment(\envone, \vlelistone, \vmlistone) \evalarrow \LexprResult(\vgtwo, \newenv) \OrAbnormal\\\\
  \newg \eqdef \ordered{\vgone}{\aslpo}{\vgtwo}
}{
  \evalmultiassignment(\env, \vlelist, \vmlist) \evalarrow \LexprResult(\newg, \newenv)
}
\end{mathpar}
...
\begin{itemize}
  ...
  is \ExprResult(\rmarray, \envone)\ProseOrAbnormal;
  \item evaluating $\eindex$ in $\envone$ is \ExprResult(\mindex, \envtwo)\ProseOrAbnormal;
...
\begin{mathpar}
\inferrule{
  \evalexpr{\env, \torexpr(\rearray)} \evalarrow \ExprResult(\rmarray, \envone) \OrAbnormal\\
  \evalexpr{\envone, \eindex} \evalarrow \ExprResult(\mindex, \envtwo) \OrAbnormal\\
...
\begin{itemize}
  ...
  \item evaluating the right-hand-side expression that corresponds to $\ebv$
        ...
        is $\ExprResult(\mbv,\envone)$\ProseOrAbnormal;
  \item evaluating $\slices$ in $\envone$ is $\SlicesResult(\msliceranges, \envtwo)$\ProseOrAbnormal;
...
\begin{mathpar}
\inferrule{
  \evalexpr{\env, \torexpr(\ebv)} \evalarrow \ExprResult(\mbv,\envone) \OrAbnormal\\
  \evalslices(\envone, \slices) \evalarrow \SlicesResult(\msliceranges, \envtwo) \OrAbnormal\\
...
\inferrule{
  ...
  \evallexpr{\env, \overname{\LESetCollectionFields(\vbase, \fields, \vslices)}{\vle}, (\vv,\vg)} \evalarrow \LexprResult(\newg,\newenv)
}
\end{mathpar}
...
\begin{itemize}
  ...
  in $\env$ is $\ExprResult(\rmrecord, \envone)$\ProseOrAbnormal;
...
\begin{mathpar}
\inferrule{
    \evalexpr{\env, \torexpr(\rerecord)} \evalarrow \ExprResult(\rmrecord, \envone) \OrAbnormal\\
