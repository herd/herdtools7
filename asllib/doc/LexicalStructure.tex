...
Comments and whitespace characters are colloquially referred to as whitespace
and are discarded. Technically, line comments, multi-line comments are treated
...
Formally, whitespace other than comments is defined via the following regular expression:
...
Multi-line comments are defined via $\actionstartcomment$ and in \secref{scanningmultulinecomments}.
...
\begin{center}
\begin{tabular}{rcl}
$\REreallit$ &$\triangleq$& \texttt{\REdigit\ (\Underscore\ | \REdigit)*} $\underbracket{\texttt{.}}$ \texttt{\REdigit\ (\Underscore\ | \REdigit)*}
\end{tabular}
\end{center}
...
\begin{center}
\begin{tabular}{rcl}
$\REletter$ &$\triangleq$& $\underbracket{\texttt{abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ}}$\\
...
\subsection{Scanning Multi-line Comments\label{sec:scanningmultulinecomments}}
The lexeme action
