...
and then formalises the lexical analysis as a function that takes a text and returns
either a list of \emph{tokens} or a lexical error.

\ChapterOutline

\SectionDef[defines what constitutes the text of an ASL specification]
  {ASL Specification Text}{ASL Specification Text}
...
\SectionDef[defines the regular expressions used to define the lexical structure]
  {Lexical Regular Expressions}{Lexical Regular Expressions}
...
\ascii{$a$}                           & The ASCII character with decimal $a$\\
...
\begin{center}
\begin{tabular}{rcl}
$\REasciichar$  &$\triangleq$& \ascii{0-127}
\end{tabular}
\end{center}
...
We distinguish characters within a source line from sequences that terminate a source line:
\hypertarget{def-relinechar}{}
\hypertarget{def-relineterm}{}
\begin{center}
\begin{tabular}{rcl}
$\RElinechar$ &$\triangleq$& $\REchar$ \regexminus{} (\ascii{10} \texttt{|} \ascii{13})\\
$\RElineterm$ &$\triangleq$& \ascii{13} \ascii{10} \texttt{|} \ascii{10}\\
\end{tabular}
\end{center}

The sequence \ascii{13} \ascii{10} is a single line terminator.

\hypertarget{def-lang}{}
The notation $\Lang{e}$ stands for the \emph{formal language} of a regular expression $e$.
That is, the set of strings that match that regular expression.

\SectionDef[defines whitespace]
  {Whitespace}{Whitespace}
\RequirementDef{Whitespace}
A whitespace lexeme is a non-empty sequence of space characters and line terminators.
Whitespace lexemes are discarded during lexical analysis.
...
Formally, whitespace lexemes are defined via the following regular expression:
\hypertarget{def-rewhitespace}{}
\begin{center}
\begin{tabular}{rcl}
$\REwhitespace$ & $\triangleq$ & (\ascii{32} $|$ \RElineterm)+\\
\end{tabular}
\end{center}

\section{Comments\label{sec:Comments}}
ASL supports line comments and multi-line comments in the style of C++.
...
A line comment starts with \texttt{//} and continues up to, but does not include,
the next line terminator or $\eof$.

Line comments are formally defined via the following regular expression:
\hypertarget{def-relinecomment}{}
\begin{center}
\begin{tabular}{rcl}
$\RElinecomment$ &$\triangleq$& \texttt{"//"} \RElinechar{}*\\
\end{tabular}
\end{center}

\subsection{Multi-line Comments}
A multi-line comment starts with \texttt{/*} and ends with the first subsequent \texttt{*/}.
Multi-line comments are defined by \hyperlink{start-multiline-comment}{intercepting \texttt{"/*"}} (see \secref{scanningmultilinecomments}).

\SectionDef[defines integer literals]
...
This is formalised by the following lexical regular expression:
...
\SectionDef[defines rational number literals]
  {Rational Number Literals}{Rational Number Literals}
Rational numbers are written in decimal form and consist of one or more decimal digits, a decimal point, and one
...
\SectionDef[defines Boolean literals]
  {Boolean Literals}{Boolean Literals}
Boolean literals are written using \texttt{TRUE} or \texttt{FALSE}.

\SectionDef[defines bitvector literals]
  {Bitvector Literals}{Bitvector Literals}
...
\RequirementDef{BitvectorOrdering}
ASL bitvectors have the least significant bit (\ProseLSB) at index $0$,
and the most significant bit (\ProseMSB) at the highest index.
In terms of notation, the \ProseMSB{} is the leftmost bit and the
\ProseLSB{} is the rightmost bit.
% INLINED_EXAMPLE
For example, in \verb|'10100'| the \ProseMSB{} is \verb|1| at index $4$,
and the \ProseLSB{} is \verb|0| at index $0$.

\SectionDef[defines bitmasks]
  {Bitmasks}{Bitmasks}
...
Bitmasks follow \RequirementRef{BitvectorOrdering}.

\section{String Literals\label{sec:String Literals}}
String literals consist of string elements enclosed in double quotes and denote string values.
Each string element is either an unescaped string character or an escape sequence
and contributes exactly one character to the string value.

A line terminator cannot occur within a string literal.
Consequently, a string literal cannot span multiple source lines.

The double-quote and backslash characters are syntactically significant within a string literal.
Escape sequences allow them, as well as tab and line feed characters, to be included in the string value.

The escape sequences permitted in string literals appear in \taref{EscapeSequences}.
\begin{table}
...
\hline
\verb|\n| & The line feed character, ASCII code \texttt{10}\\
\verb|\t| & The tab character, ASCII code \texttt{9}\\
...
\hypertarget{def-reunescapedstringchar}{}
\hypertarget{def-restringescape}{}
\hypertarget{def-restringelement}{}
\hypertarget{def-restringlit}{}
\begin{center}
\begin{tabular}{rcl}
$\REunescapedstringchar$ &$\triangleq$& $\RElinechar\ \regexminus{}$ \anycharacter{\texttt{"}\ \backslash\ }\\
$\REstringescape$ &$\triangleq$& \anycharacter{\backslash\ } \anycharacter{\texttt{" n t }\backslash\ }\\
$\REstringelement$ &$\triangleq$& \REunescapedstringchar{} \texttt{|} \REstringescape\\
$\REstringlit$ &$\triangleq$& \anycharacter{\texttt{"}} \REstringelement{}* \anycharacter{\texttt{"}}
\end{tabular}
\end{center}

\SectionDef[defines identifiers]
  {LexicalIdentifiers}{Identifiers}
...
\SectionDef[defines the analysis used to transform text into tokens]
...
\begin{description}
  \item[Value-carrying] Tokens that carry a value have the form $L(v)$ where $L$ is a token label,
...
\Token &\triangleq& \{\ \Tintlit(n) \;|\; n\in\Z\ \} & \cup\\
        ...
        & & \{\ \Tstringlit(s) \;|\; s\in \Strings \} & \cup\\
        & & \{\ \Tstringchar(c) \;|\; c \in \Lang{\REasciichar} \} & \cup\\
        & & \{\ \Tstringend \} & \cup\\
...
\begin{itemize}
  ...
  \item Tokens of the form $\Tstringlit(s)$ carry the string value denoted by a string literal;
  \hypertarget{def-tstringchar}{}
  \item Tokens of the form $\Tstringchar(c)$ represent a single character in a string value;
  \hypertarget{def-tstringend}{}
  \item The token $\Tstringend$ represents the closing double quote of a string literal;
...
The function
\[
\remaxmatch(\overname{\RegExp}{e}, \overname{\Strings}{s}) \aslto (\overname{\Strings}{s_1}, \overname{\Strings}{s_2}) \cup \{\nomatch\}
\]
...
%
For this purpose, most lexical analysers split the analysis into separate ``states'' ---
one for keywords, symbols, single-line comments, and identifiers, one for string literals,
and one for multi-line comments. The lexical analysers switch between the states as
needed, and analysing a string literal involves converting its string elements to characters and
concatenating the resulting characters into a string value token.
...
the string value accumulated so far.
...
is parameterised by a function $f$ that converts strings into corresponding tokens.
...
\]
switches to scanning string literals via $\scanstring$.
...
\hypertarget{relation-decimaltolit}{}
...
$\RElinecomment$                      & $\discard$ \texthypertarget{start-multiline-comment}\\
$\texttt{"/*"}$                       & $\actionstartcomment$\\
\anycharacter{\texttt{"}}             & $\actionstartstring$ \\
$\REintlit$                           & $\actiontoken(\decimaltolitname)$ \\
...
\texttt{"setter"},  \texttt{"string"}                  & $\actiontoken(\tokenid)$ \\
...
\texttt{"when"}, \texttt{"where"}, \texttt{"while"}          & $\actiontoken(\tokenid)$ \\
\end{tabular}
\end{center}
...
(\KleeneStar{\Token} \cup \overname{\TLexicalError}{\LexicalErrorConfig})
\]
scans a string literal with the $\specstring$ specification while building the resulting string value in $\buff$.
...
\begin{mathpar}
\inferrule[char]{
  \maxmatches(\specstring, s) = [(e_1,a_1),\ldots,(e_n,a_n)]\\
  \remaxmatch(e_1, s) = (s_1, s_2)\\
...
token, which is added to the characters that make up the resulting string value.
...
\stringescape(s_1, s_2) \triangleq \begin{cases}
  \Tstringchar(\ascii{10}) & s_1 = \texttt{\textbackslash\;\;n}\\
  \Tstringchar(\ascii{9}) & s_1 = \texttt{\textbackslash\;\;t}\\
  \Tstringchar(\ascii{34}) & s_1 = \texttt{\textbackslash\;\;"}\\
  \Tstringchar(\ascii{92}) & s_1 = \texttt{\textbackslash\;\;\textbackslash}\\
\end{cases}
\]
returns a $\Tstringchar$ token containing the ASCII character for the corresponding escape sequence.
The character is added to those that make up the final string value.
...
\textbf{Lexical Regular Expression} & \textbf{Lexeme Action}\\
\hline
$\REstringescape$          & $\stringescape$\\
\anycharacter{{\color{white}\backslash}\texttt{" }} & $\stringfinish$\\
$\REunescapedstringchar$   & $\stringchar$\\
\end{tabular}
\end{center}

\subsection{Scanning Multi-line Comments\label{sec:scanningmultilinecomments}}
The lexeme action
\hypertarget{def-discardcommentelement}{}
\[
\discardcommentelement(s_1, s_2) \triangleq \aslscan(\speccomment, s_2)
\]
discards the string $s_1$ and continues scanning $s_2$ with $\speccomment$.
...
\hline
\texttt{"*/"} & $\actiondiscard$ \\
$\RElinechar$ & $\discardcommentelement$ \\
$\RElineterm$ & $\discardcommentelement$ \\
\hline
\end{tabular}
\end{center}
