...
\begin{itemize}
\item \texthypertarget{type-N}$\N$ is the set of natural numbers, including $0$.

\item \RenderType{N_pos} is the set of positive natural numbers (that is, excluding $0$).

\item \texthypertarget{type-Z}$\Z$ is the set of integers.

\item \texthypertarget{type-Q}$\Q$ is the set of rational numbers.

\item The set of ASL Boolean literals is defined as follows:
\RenderType{Bool}
...
\texthypertarget{relation-land}\texthypertarget{relation-lor}%
...
\end{array}
\]
\texthypertarget{relation-neg}%
\texthypertarget{relation-IFF}%
\texthypertarget{relation-implies}%
...
\item \RenderType{Bit}
is the set $\{\zerobit,\onebit\}$. That is, a single bit.

\item \texthypertarget{type-Strings}$\Strings$ is the set of all ASCII strings.

\item the set of all ASL identifiers is defined as \RenderType{Identifier}
...
We employ the standard notation for first-order logic quantifiers:
\begin{itemize}
  \item $\forall x \in A.\ P(x)$ stands for ``for all $x$ in set $A$, the property $P(x)$ holds'';
  \item $\exists x \in A.\ P(x)$ stands for ``there exists an $x$ in set $A$ such that the property $P(x)$ holds''.
\end{itemize}

We utilise the notation $\overname{a}{b}$ to enable us to name the mathematical term $a$ as $b$ so that
...
\texthypertarget{def-choice}
We write ``$\ifthenelseop[H]{c}{v}{w}$'' to denote the value of $v$ if the condition $c$ holds, and $w$ otherwise.
...
The function $\sign(\overname{\Q}{\vq}) \aslto \overname{\Sign}{\vs}$ returns the sign of $\vq$:
\[
\sign(\vq) \triangleq \begin{cases}
  \positivesign & \text{if }\vq > 0\\
  \equalsign & \text{if }\vq = 0\\
  \negativesign & \text{if }\vq < 0
\end{cases}
\]
\end{definition}
...
\hypertarget{relation-cardinality}{}
...
The \emph{Cartesian product} of sets $S_1,\ldots,S_n$ denoted $S_1 \cartimes \ldots \cartimes S_n$,
is defined as $S_1 \cartimes \ldots \cartimes S_n \triangleq \{(s_1,\ldots,s_n) \;|\; s_i \in S_i \text{ for } i=1,\ldots,n\}$.
%
In function and relation signatures, we use the more succinct notation $(S_1,\ldots,S_n)$
instead of $S_1 \cartimes \ldots \cartimes S_n$.
\end{definition}
...
Specifically, we write $f(T_1,\ldots,T_n) \aslto R$ to denote a function from
...
\paragraph{Parametric Functions and Relations}
A \emph{parametric function} is a function whose domain is not a priori fixed but rather
parameterised by the type of its arguments. An example is the $\equallength$ function where the
types $A$ and $B$ (of the list elements) are not explicitly specified, but rather inferred from the context where the function is used.

\texthypertarget{def-finfunction}
\begin{definition}[Finite-domain Function]
The notation $\rightarrowfin$ stands for a function whose domain is finite.
\end{definition}

\texthypertarget{def-emptyfunc}
\begin{definition}[Empty Function\label{def:EmptyFunction}]
The function with an empty domain is denoted as $\emptyfunc$.
\end{definition}

\texthypertarget{def-mapupdate}
\begin{definition}[Function Update\label{def:FunctionUpdate}]
  ...
  $\emptyfunc[a_1\mapsto b_1]\ldots[a_k\mapsto b_k]$ (alternatively, \\
  \texthypertarget{def-bindingstomap}
  $\bindingstomap{[i=1..k: a_i\mapsto b_i]}$)
\end{definition}

\begin{definition}[Function Restriction]
\hypertarget{def-restrictfunc}{}
The \emph{restriction} of a function $f(X) \aslto Y$ to a subset of its domain
...
For example,
\[
\sign(\overname{\Q}{\vq}) \aslto \overname{\Sign}{\vs}
\]
...
\paragraph{Integer Range Notation.}
\texthypertarget{def-range}
We use the notation $a..b$, where $a,b\in\Z$ as a shorthand for the interval $[a\ldots b]$,
which counts up when $a \leq b$ and counts down when $a \geq b$.

We write $x_{a..b}$ as a shorthand for the sequence $x_a \ldots x_b$.
%
We write $i=1..k: V(i)$, where $V(i)$ is a mathematical expression parameterised by $i$,
to denote the sequence of expressions $V(1) \ldots V(k)$.
The notation $a \in A: V(a)$, where $A$ is a set and $V$ is an expression parameterised by the free variable $a$,
...
We write $\KleeneStar{T}$ to denote the type of a possibly-empty list of elements of type $T$,
and $\KleenePlus{T}$ for a non-empty list of elements of type $T$.

\hypertarget{def-inlist}{}
We slightly abuse notation by employing the notation for set membership for lists.
Formally, the notation $e \inlist L$ where $e \in T$ and $L \in \KleeneStar{T}$ stands for $\exists i\in\Npos.\ L[i]=e$.

\hypertarget{def-listset}{}
\begin{definition}[Listing a Set]
The parametric relation $\listset{\pow{T}} \;\aslrel\; \KleeneStar{T}$
lists the elements of a set in an arbitrary order:

\[
\listset{X} \triangleq \begin{cases}
  \emptylist & \text{if }X = \emptyset\\
  x_{1..k}\text{ such that }|X| = k\text{ and }\forall x\in X.\ \exists 1 \leq i \leq k.\ x = x_i & \text{otherwise } \enspace.
\end{cases}
\]
\end{definition}

\hypertarget{relation-bindings}{}
\begin{definition}[Listing the Bindings of a Function]
The parametric relation
\[
\bindings{\overtext{A \partialto B}{\vf}} \;\aslrel\; (\KleeneStar{A \times B})
\]
lists the pairs of input-output values for a given partial function $f$ in arbitrary order:
\[
\bindings{f} = [ (a,b) \;|\; a \in \dom(f) \wedge f(a) = b] \enspace.
\]
\end{definition}

\hypertarget{relation-concat}{}
\begin{definition}[List Concatenation]
The parametric function $\concat(\KleeneStar{T}, \KleeneStar{T}) \aslto \KleeneStar{T}$ concatenates two lists:
\[
  L \concat{} M \triangleq
  \begin{cases}
    L & \text{if } M = \emptylist\\
    M & \text{if } L = \emptylist\\
    L_1 \ldots L_k {} M_1 \ldots M_n & \text{if } L = L_1 \ldots L_k \text{ and } M = M_1 \ldots M_n \enspace.
  \end{cases}
\]
\end{definition}

\hypertarget{relation-concatlist}{}
\begin{definition}[Concatenation of a List of Lists]
The parametric function
\[
\concatlist{\KleeneStar{\left(\KleeneStar{T}\right)}} \aslto \KleeneStar{T}
\]
...
The parametric function
\[
  \equallength(\overname{\KleeneStar{A}}{a}, \overname{\KleeneStar{B}}{b}) \aslto \Bool
\]
...
The parametric function $\listprefixname(\overname{\KleeneStar{T}}{\vlone}, \overname{\KleeneStar{T}}{\vltwo}) \aslto \Bool$ checks whether
...
The parametric function $\listrange(\KleeneStar{T}) \aslto \KleeneStar{\Npos}$ returns the ($1$-based) list of indices for a given list:
\[
    \begin{array}{rcl}
...
\texthypertarget{relation-filteroptionlist}
\begin{definition}[Filtering a List of Optional Values]
The parametric function
\[
  \filteroptionlist{\KleeneStar{\Option{ T }}} \aslto \KleeneStar{T}
\]
filters a list of \optionalterm{} elements, removing those which are $\none$ and unwrapping those which are $\some{ v }$ to $v$:
\[
\filteroptionlist{L} \triangleq \begin{cases}
  \emptylist & \text{if }L = \emptylist\\
  \filteroptionlist{L'} & \text{if }L = \none \cons L'\\
  v \cons \filteroptionlist{L'} & \text{if }L = \some{v} \cons L' \enspace.
\end{cases}
\]
\end{definition}

\hypertarget{relation-listfst}{}
\begin{definition}[Extracting the First Components from a List of Pairs]
The\\ parametric function
\[
\listfstname(\KleeneStar{(T_1, T_2)}) \aslto \KleeneStar{T_1}
\]
transforms a list of pairs into a lists of elements containing the first components of each pair:
\[
  \listfst{\pairs} \triangleq \begin{cases}
    \emptylist  & \text{if }\pairs = \emptylist\\
    a_{1..k}    & \text{if } \pairs = ((a_1, b_1) \ldots (a_k,b_k)) \enspace.
  \end{cases}
\]
\end{definition}
...
The parametric function
\[
\unziplist(\KleeneStar{(T_1, T_2)}) \aslto (\KleeneStar{T_1}, \KleeneStar{T_2})
\]
...
\[
  \unziplist(\pairs) \triangleq \begin{cases}
    ...
    (a_{1..k}, b_{1..k})      & \text{if } \pairs = (a_1,b_1) \ldots (a_k,b_k)  \enspace.
  \end{cases}
\]
\end{definition}
\texthypertarget{relation-listcombine}
...
The parametric function
\[
\unziplistthree(\KleeneStar{(T_1, T_2, T_3)}) \aslto (\KleeneStar{T_1}, \KleeneStar{T_2}, \KleeneStar{T_3})
\]
...
\[
  \unziplistthree(\triples) \triangleq \begin{cases}
    ...
    (a_{1..k}, b_{1..k}, c_{1..k})      & \text{if }\triples = (a_1,b_1,c_1) \ldots (a_k,b_k,c_k)  \enspace.
  \end{cases}
\]
\end{definition}
\texthypertarget{relation-listcombinethree}
As a shorthand, we use the notation $\listcombinethree{a}{b}{c} = d$ for $\unziplistthree(d) = (a, b, c)$.

\begin{definition}[Lists Cross-product]
\texthypertarget{relation-listcross}
The parametric function
\[
\listcrossop(\KleeneStar{T_1}, \KleeneStar{T_2}) \aslto \KleeneStar{(T_1, T_2)}
\]
computes the cross-product of two lists:
\[
  \vsone{} \listcrossop{} \vstwo \triangleq [(a,b) \;|\; a \in \vsone, b \in \vstwo] \enspace.
\]
\end{definition}

\begin{definition}[Finding unique elements of a list]
\RenderRelation{unique_list}
\end{definition}

\begin{definition}[Finding a Value in a List of Pairs]
\hypertarget{relation-assocopt}{}
The function
\[
  \assocopt{\overname{\KleeneStar{(\Identifier, T)}}{\vli}}{\overname{\Identifier}{\id}} \aslto \Option{\overname{T}{\vv}}
\]
returns $\some{\vv}$ for the first pair $(\id, \vv)$ in the list of pairs $\vli$,
or $\none$ if no such pair exists.
\end{definition}
...
\[
\unionlist(\KleeneStar{\pow{T}}) \aslto \pow{T}
\]
takes the union of a list of sets:
\[
  \unionlist(S_{1..k}) \triangleq \bigcup_{i=1}^{k} S_i \enspace.
\]
\end{definition}

\begin{definition}[Taking the intersection of a list of sets]
The parametric function
\hypertarget{def-intersectionlist}{}
\[
\intersectionlist(\KleenePlus{\pow{T}}) \aslto \pow{T}
\]
takes the intersection of a list of sets:
\[
  \intersectionlistop{S_{1..k}} \triangleq \bigcap_{i=1}^{k} S_i \enspace.
\]
\end{definition}

\texthypertarget{def-disjointunion}
We write $\disjointunion$ to denote a union of pairwise disjoint sets.

\subsection{Strings}
\hypertarget{relation-stringconcat}{}
The function $\concatstrings(\Strings, \Strings) \aslto \Strings$
...
The function $\stringofintname(\Z) \aslto \Strings$ converts an integer number
...
%
The type $\Option{T} \triangleq \{ \none \} \cup \{ \some{x} \;|\; x\in T\}$ denotes
...
and a \emph{conclusion} judgement.
...
and the rule \TypingRuleRef{EBinop} (somewhat simplified here) has four premises
 (the notation used for the premises is explained further below):
\begin{mathpar}
\inferrule{
  ...
  \annotateexpr(\tenv, \EBinop(\op, \veone, \vetwo)) \typearrow (\vt, \EBinop(\op, \veonep, \vetwop), \vses)
}
\end{mathpar}

Judgements of the form $R(a_1,\ldots,a_n) \longrightarrow r$ are used to uniformly denote
both function applications and relation applications, where $R$ is either a function or a relation.
When $R$ is a function, the judgement $R(a_1,\ldots,a_n) \longrightarrow r$ stands for $R(a_1,\ldots,a_n) = r$ and
when $R$ is a relation, the judgement stands for $((a_1,\ldots,a_n), r) \in R$.
...
Judgements can be \emph{grounded} by substituting their free variables with values.
A \emph{ground rule} is a rule with all its judgements (premises and conclusion) grounded.
\end{definition}
For example,
...
A \emph{\derivationtreeterm} is a tree whose vertices correspond to ground judgements.
...
%
We call such judgements \emph{transition judgements}.
A set of rules $M$ with transition judgements defines the relation
\[
...
We refer to those labels as \emph{\configurationterm{} domains}.

We refer to \configurationsterm{} at the origin of a transition as \emph{input \configurationsterm{}} and to the
\configurationsterm{} at the destination of a transition as \emph{output \configurationsterm{}}.

For example, the conclusion of the rule \TypingRuleRef{ELit} has \\
$\annotateexpr(\tenv, \ELiteral(\vv))$ as its input \configurationterm{}
and \\
$(\vt, \ELiteral(\vv), \emptyset)$ as its output \configurationterm{}.

Our rules always make use of labelled input \configurationsterm{}. This makes it easier to ensure
the mutual exclusion rule principle.

Our rules always define relations whose sets of input \configurationsterm{} and output \configurationsterm{} are disjoint.

\hypertarget{def-freshvariables}{}
\begin{definition}[Fresh Element]
  Premises of the form ``$\freshidentifier$'' mean that in any
...
uses an ignore variable to stand for the value being matched by a \hyperlink{any-pattern}{\Tminus{} pattern}.
...
\inferrule{
  ...
  \mcond \reverseeqdef (\nvbool(\vb), \vgone)\\
  \vep \eqdef \ifthenelseop[H]{\vb}{\veone}{\vetwo}\hva\\\\
...
\begin{description}
  \item[Range:] we write $i=1..k$ to allow listing premises parameterised by $i$ or constructing
  lists from expressions parameterised by $i$.
  ...
  \hypertarget{relation-reverseassign}{}
  \item[Deconstruction / ``View as'':] some values, such as tuples, are compound. In order to refer to the structure
  of compound values, we write $v \reverseeqdef \textit{f}(u_{1..k})$ where the expression on the right-hand side exposes the internal structure of $v$ by introducing the variables
  $u_{1..k}$, allowing us to alias internal components of $v$.
  Intuitively, $v$ is reinterpreted as $\textit{f}(u_{1..k})$.
  For example, suppose we know that $v$ is a pair of values.
  Then, $v \reverseeqdef (a, b)$ allows us to alias $a$ and $b$.
  In \SemanticsRuleRef{ECond}, we know from the definition of $\evalexpr$ that
  $\mcond$ is a pair datatype.
  Therefore, writing $\mcond \reverseeqdef (\nvbool(\vb), \vgone)$ allows us to name each component of this pair
  and then refer to it, while \hyperlink{def-ignore}{ignoring} the \staticenvironmentterm{} component.
  Similarly, if $v$ is a non-empty list, then $v \reverseeqdef h \cons{} t$ deconstructs the list into the
  head of the list $h$ and its tail $t$.
  Given that a variable $v$ represents a list, we write $v \reverseeqdef v_{1..k}$ to list its elements and allow
  referring to them by index.

\hypertarget{relation-assign}{}
...
\inferrule{
  \evalexpr(\env, \econd) \evalarrow \ResultExpr(\mcond, \envone) \OrAbnormal\hva\\\\
  \mcond \reverseeqdef (\nvbool(\vb), \vgone)\\
  \evalexpr(\envone, \ifthenelseop[H]{\vb}{\veone}{\vetwo}) \evalarrow \ResultExpr((\vv, \vgtwo), \newenv) \OrAbnormal
}{
  {
  \begin{array}{r}
  \evalexpr(\env, \overname{\ECond(\econd, \veone, \vetwo)}{\ve}) \evalarrow\\
...
$\FIELDname$, $\FIELDparameters$, $\FIELDargs$, $\funcbody$, $\FIELDreturntype$, $\funcsubprogramtype$,
$\FIELDrecurselimit$, and \\
$\funcbuiltin$.
The notation $\{ \funcbody:\vbody,\ \FIELDargs:\vargdecls, \ldots \}$
allows us to deconstruct a given \func\ node by matching only the $\funcbody$ and $\FIELDargs$ fields.

Recall that a subset of AST nodes are either labels or labelled tuples.\texthypertarget{def-astlabel}
...
\inferrule{\op \not\in \{\BAND, \BOR, \IMPL\}\\\\
  ...
  \vmone \reverseeqdef (\vvone, \vgone) \\
  \vmtwo \reverseeqdef (\vvtwo, \vgtwo) \\
...
In this case, the rule describes a transition from an input \configurationterm{} \\
$\evalexpr( \env, \EBinop(\op, \veone, \vetwo) )$,
whose \configurationterm{} domain is $\evalexpr$, to an output \configurationterm{} $\ResultExpr((\vv, \vg), \newenv)$
whose \configurationterm{} domain is\\$\ResultExpr$.
%
A rule uses the free variables appearing in the input \configurationterm{} of the conclusion
($\env$, $\op$, $\veone$, and $\vetwo$ in our example),
with the goal of assigning values to the free variables in the output \configurationterm{}
...
%
In this case, $\vv$ is assigned as the result of the transition judgement
$\evalbinop(\op, \vvone, \vvtwo) \evalarrow \vv$,
$\vg$ is assigned the expression $\vgone \parallelcomp \vgtwo$,
and $\newenv$ is assigned as the result of the transition judgement
$\evalexpr( \envone, \vetwo ) \evalarrow \ResultExpr(\vmtwo, \newenv)$.
%
...
If the transition results in the first of the alternative output \configurationsterm{}, the following premises are considered.
However, if the result is the second, short-circuit output \configurationterm{}, then the following premises are ignored
and the conclusion transitions into the short-circuit output \configurationterm{}.
These short-circuit output \configurationsterm{} are typically, but not always, due to (type or dynamic) errors.
...
thereby short-circuiting the rule, and the input \configurationterm{}
...
actual \configurationsterm{}, which are properly defined in \chapref{Semantics}.
Intuitively, the alternative \configurationsterm{} $\ThrowingConfig$, $\DynErrorConfig$,
and $\DivergingConfig$
...
One may first read the rule ignoring these alternative \configurationsterm{}, to see how the
goal of transitioning into the output \configurationterm{} appearing in the conclusion ---
$\ResultExpr((\vv, \vg), \newenv)$ --- is achieved.
Then, re-reading the rule would indicate where exceptions, \dynamicerrorsterm,
and diverging evaluation may result in other output \configurationsterm{}.
%
For example, if the first transition judgement results in a throwing \configurationterm{} $\ThrowingConfig$
then the output \configurationterm{} of the conclusion is also $\ThrowingConfig$.
...
Similarly, if the first transition judgement results in a \DynamicErrorConfigurationTerm, the output \configurationterm{} of
...
Finally, if the first transition judgement results in a diverging \configurationterm{}, the output \configurationterm{} of
...
$\ResultExpr(\vmone, \envone)$, but the second transition judgement results in either
...
Expanding the last transition judgement, gives us the case:
\begin{mathpar}
\inferrule{\op \not\in \{\BAND, \BOR, \IMPL\}\\\\
  \evalexpr( \env, \veone) \evalarrow \ResultExpr(\vmone, \envone) \\\\
  \evalexpr( \envone, \vetwo ) \evalarrow \ResultExpr(\vmtwo, \newenv) \hva\\\\
...
\SectionDef[presents commonly used judgements and notations]
...
\RenderProseAndFormally{te_check}

We define a similar function for dynamic errors.
\RenderRelation{de_check}
\RenderProseAndFormally{de_check}

\subsection{Boolean Transition Judgements}
\RenderRelation{bool_transition}
We use this function defined by the following trivial rule to allow us to treat Boolean values as transition judgements,
which in turn allows us to employ \shortcircuitrulemacros{}:
\RenderProseAndFormally{bool_transition}
...
\subsection{Judgements Over Optional Data Types}
\hypertarget{def-mapopt}{}
Optional data types are prevalent in the AST.
To facilitate transition judgements over optional data types,
...
\hva\and
\inferrule[None]{}{
  \mapopt{f}(\overname{\none}{\vvopt}) \longrightarrow \overname{\none}{\vvoptnew}
}
\end{mathpar}
...
The full name of the first case is then \TypingRuleRef{BaseValue}.\textsc{T\_BOOL}
and the full name of the second case is \TypingRuleRef{BaseValue}.\textsc{T\_REAL}.
