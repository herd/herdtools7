...
the set of semantic rules, we must apply rules to form a \emph{\derivationtreeterm}.
...
\inferrule{
  \evalexpr{\env, \econd} \evalarrow \ExprResult(\mcond, \envone) \OrAbnormal\\\\
  \mcond \eqname (\nvbool(\vb), \vgone)\\
  \evalexpr{\envone, \choice{\vb}{\veone}{\vetwo}} \evalarrow \ExprResult((\vv, \vgtwo), \newenv) \OrAbnormal
}{
  \evalexpr{\env, \overname{\ECond(\econd, \veone, \vetwo)}{\ve}} \evalarrow
  \ExprResult((\vv, \ordered{\vgone}{\aslctrl}{\vgtwo}), \newenv)
}
\end{mathpar}
\end{description}
...
\begin{mathpar}
\inferrule{\op \not\in \{\BAND, \BOR, \IMPL\}\\\\
  \evalexpr{ \env, \veone} \evalarrow \ExprResult(\vmone, \envone) \\\\
...
whose configuration domain is \texttt{eval\_expr}, to an output configuration $\ExprResult((\vv, \vg), \newenv)$
whose configuration domain is $\ExprResult$.
%
...
For example, $\evalexpr{ \env, \veone} \evalarrow \ExprResult(\vmone, \envone)$
...
$\ExprResult(\vmone, \envone)$, but the second transition assertion results in either
