...
parameterized by the type of its arguments. An example is the $\mapapplyoptname$ function where the
types $A$ and $B$ are not explicitly specified, but rather inferred from the context where the function is used.

\texthypertarget{relation-map_apply_opt}
\begin{definition}[Safe Function Application]
The parametric function
\[
  \mapapplyoptop{\overname{A \partialto B}{\vf}}{\overname{A}{\va}} \aslto \Option{\overname{B}{\vb}}
\]
safely applies a partial function $\vf$ to an argument $\va$:
\[
  \mapapplyoptop{\vf}{\va} \triangleq
  \begin{cases}
    \some{\vb} & \text{if }\va \in \dom(\vf) \text{ and } \vf(\va) = \vb\\
    \None       & \text{if }\vf(\va) = \bot
  \end{cases}
\]
\end{definition}
...
\[
  \mapapplyoptop{\overname{A \partialto B}{\vf}}{\overname{A}{\va}} \aslto \Option{\overname{B}{\vb}}
\]
defines a function type and lets us refer to the first argument as $\vf$, the second argument as $\va$,
and to the result as $\vb$.
...
The parametric relation $\listset{\pow{T}} \cartimes \KleeneStar{T}$
...
\[
\bindings{\overtext{A \partialto B}{\vf}} \cartimes (\KleeneStar{A \times B})
\]
...
The parametric function
\[
\concatlist{\KleeneStar{\left(\KleeneStar{T}\right)}} \rightarrow \KleeneStar{T}
\]
...
% TODO: Move this to the Type System Utilities chapter
\RenderRelation{unique_list}
\RenderRule{unique_list}

It relies on the following helper function.
\RenderRelation{unique_list_acc}
which is defined as followg:
\RenderRule{unique_list_acc}
\end{definition}
...
The function
\[
  \assocopt{\overname{\KleeneStar{(\Identifier\cartimes T)}}{\vli}}{\overname{\Identifier}{\id}} \aslto \Option{\overname{T}{\vv}}
\]
...
\hypertarget{relation-stringofnat}{}
The function $\stringofnatname : \N \rightarrow \Strings$ converts a natural number
