...
\]
retains a single copy of each element of the list $l$.
...
\uniquep(l, \acc) &\triangleq&
  \begin{cases}
    ...
    \uniquep(t, \acc)& \text{if }l = h \cons{} t \text{ and } h \inlist \acc\\
    \uniquep(t, \acc \concat [h]) & \text{if }l = h \cons{} t \text{ and } h \not\inlist \acc\\
  \end{cases}
\end{array}
\]
\end{definition}
