...
\inferrule[returning]{
  ...
  \evalstmt(\env, \stm) \evalarrow \vres \OrDynErrorDiverging\hva\\\\
  \commonprefixline \hva\\\\
  \vres = \Returning((\vvs, \newg), \envret) \\
  \envret \reverseeqdef (\tenvone, \denvone) \\
...
\inferrule[continuing]{
  ...
  \commonprefixline \hva\\\\
  \vres = \Continuing(\newg, \envcont) \\
  \envcont \reverseeqdef (\tenvone, \denvone) \\
  \newenv \eqdef (\tenv, \poplocalscope(\denv,\denvone))
}{
...
\inferrule[throwing]{
  ...
  \commonprefixline \hva\\\\
  \vres = \Throwing(\vv,\vt,\newg, \envthrow) \\
  \envthrow \reverseeqdef (\tenvone, \denvone) \\
  \newenv \eqdef (\tenv, \poplocalscope(\denv,\denvone))
}{
