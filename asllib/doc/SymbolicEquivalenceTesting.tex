...
In this chapter, we use the special value \RenderConstant{CannotBeTransformed}
...
\RenderRelation{polynomial_of_int} returns the polynomial representation of an integer literal:
\[
\polynomialofint(\vi) \typearrow \overname{\{\emptyfunc \mapsto \vi\}}{\vp} \enspace.
\]

\RenderRelation{polynomial_of_var} returns the polynomial representation of a variable:
\RenderRule{polynomial_of_var}
...
\inferrule[normalizable]{
  \toir(\tenv, \ve) \typearrow \vpone \OrTypeError\hva\\\\
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item define $\newc$ as the \Proseexactconstraint{$\vep$}, that is, \\ $\ConstraintExact(\vep)$.
  \end{itemize}

  \item \AllApplyCase{range}
  \begin{itemize}
    \item $\vc$ is a range integer constraint for $\veone$ and $\vetwo$ ($\ConstraintRange(\veone, \vetwo)$);
    \item applying $\normalize$ to $\veone$ in $\tenv$ yields $\veonep$;
    \item applying $\normalize$ to $\vetwo$ in $\tenv$ yields $\vetwop$;
    \item define $\newc$ as the \Proserangeconstraint{$\veonep$}{$\vetwop$}, that is, \\ $\ConstraintRange(\veonep, \vetwop)$.
  \end{itemize}
\end{itemize}
...
\inferrule[range]{
  ...
  \reduceconstraint(\tenv, \vc) \typearrow \overname{\ConstraintRange(\veonep, \vetwop)}{\newc}
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{reduce_constraint}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\ve$ is a literal expression other than an integer literal;
  ...
  \begin{itemize}
    ...
    \item $\vep$ is the addition expression with operands $\veone$ and the negation of $\vetwo$, that is, \\ $\EBinop(\ADD, \veone, \EUnop(\NEG, \vetwo))$;
  ...
  \begin{itemize}
    ...
    \item neither $\veone$ nor $\vetwo$ is a binary division expression;
    \item converting $\veone$ to a \symbolicexpressionterm{} in $\tenv$ yields $\irone$\ProseOrTypeErrorOrCannotBeTransformed;
    \item converting $\vetwo$ to a \symbolicexpressionterm{} in $\tenv$ yields $\irtwo$\ProseOrTypeErrorOrCannotBeTransformed;
...
\inferrule[evar\_exact\_constraint]{
  ...
  \typeof(\vs) \typearrow \vt \OrTypeError\hva\\\\
  \makeanonymous(\vt) \typearrow \ttyone \OrTypeError\hva\\\\
  \astlabel(\ttyone) = \TInt\\
  \ttyone = \TInt(\WellConstrained([\ConstraintExact(\ve)]))\\
  \toir(\tenv, \ve) \typearrow \vp \terminateas \CannotBeTransformed
}{
  \toir(\tenv, \overname{\EVar(\vs)}{\ve}) \typearrow \vp
}
\end{mathpar}
...
\inferrule[evar\_int]{
  \lookupconstant(\tenv, \vs) \typearrow \bot\\
  \lookupimmutableexpr(\tenv, \vs) \typearrow \bot\hva\\
  \typeof(\vs) \typearrow \vt\\
  \makeanonymous(\vt) \typearrow \ttyone\hva\\
  \astlabel(\ttyone) = \TInt\\
...
\inferrule[ebinop\_plus]{
  \toir(\tenv, \veone) \typearrow \irone \OrTypeError, \CannotBeTransformed\hva\\\\
  \toir(\tenv, \vetwo) \typearrow \irtwo \OrTypeError, \CannotBeTransformed\hva\\\\
  \vp \eqdef \addpolynomials(\irone, \irtwo)
}{
  \toir(\tenv, \overname{\EBinop(\ADD, \veone, \vetwo)}{\ve}) \typearrow \vp
}
\end{mathpar}

\begin{mathpar}
\inferrule[ebinop\_minus]{
  \vep \eqdef \EBinop(\ADD, \veone, \EUnop(\NEG, \vetwo))\\
  \toir(\tenv, \vep) \typearrow \vp \OrTypeError, \CannotBeTransformed\hva\\\\
}{
  \toir(\tenv, \overname{\EBinop(\SUB, \veone, \vetwo)}{\ve}) \typearrow \vp
}
\end{mathpar}

\begin{mathpar}
\inferrule[ebinop\_mul\_div\_left]{
  \toir(\tenv, \EBinop(\DIV, \EBinop(\MUL, \veone, \vethree), \vetwo)) \typearrow \vp \OrTypeError, \CannotBeTransformed\hva\\\\
}{
...
\inferrule[ebinop\_mul\_div\_right]{
  ...
  \toir(\tenv, \EBinop(\DIV, \EBinop(\MUL, \veone, \vetwo), \vethree)) \typearrow \vp \OrTypeError, \CannotBeTransformed\hva\\\\
}{
...
\inferrule[ebinop\_div\_int\_denominator]{
  ...
  \irone \reverseeqdef [i=1..k: \vm_\vi \mapsto \vc_\vi]\\
...
\inferrule[eunop\_neg]{
  \toir(\tenv, \EBinop(\MUL, \ELiteral(\LInt(-1)),\veone )) \typearrow \vp \OrTypeError, \CannotBeTransformed\hva\\\\
}{
...
We make use of the helper relations:
\RenderRelation{is_div_binop}
It defined as follows:
\RenderRule{is_div_binop}
\RenderRelation{scale_polynomial}
It is defined as follows:
\RenderRule{scale_polynomial}
...
\inferrule[all\_supported]{
  \toir(\tenv, \veone) \typearrow \irone \OrTypeError\hva\\\\
  \toir(\tenv, \vetwo) \typearrow \irtwo \OrTypeError
}{
  \exprequalnorm(\tenv, \veone, \vetwo) \typearrow \overname{\irone = \irtwo}{\vb}
}
\hva\and
\inferrule[unsupported1]{
  \toir(\tenv, \veone) \typearrow \CannotBeTransformed
}{
...
\inferrule[unsupported2]{
  ...
  \toir(\tenv, \vetwo) \typearrow \CannotBeTransformed
}{
  \exprequalnorm(\tenv, \veone, \vetwo) \typearrow \overname{\False}{\vb}
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{expr_equal_norm}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item for each index $i$ in the list of indices for $\vargsone$, testing whether $\vargsone[\vi]$ and $\vargstwo[\vi]$
          are \equivalentexprsterm{} in $\tenv$ yields $\vbs[\vi]$\ProseOrTypeError;
    \item $\vb$ is $\True$ if and only if $\vbs[\vi]$ is $\True$ for each index $i$ in the list of indices for $\vargsone$.
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item $\vetwo$ is a slicing expression with expression $\vetwoone$ and list of slices $\slicestwo$,
  ...
  \begin{itemize}
    ...
    \item checking whether the lengths of $\vlone$ and $\vltwo$ are equal yields either $\True$ or $\False$, which short-circuits
          the entire rule;
    \item for each index $i$ in the list of indices for $\vlone$, testing whether $\vlone[\vi]$ is equivalent to $\vltwo[\vi]$
          in $\tenv$ yields $\vbs[\vi]$\ProseOrTypeError;
    \item $\vb$ is $\True$ if and only if $\vbs[\vi]$ is $\True$ for each index $i$ in the list of indices for $\vlone$.
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item applying $\exprequal$ to $\vvone$ and $\vvtwo$ in $\tenv$ yields $\vbtwo$\ProseOrTypeError;
    \item $\vb$ is $\True$ if and only if both $\vbone$ and $\vbtwo$ are $\True$.
  \end{itemize}

  \item \AllApplyCase{e\_unop}
  \begin{itemize}
    \item $\veone$ is a unary operator expression with operator $\opone$ and operand expression $\veoneone$,
          that is, $\EUnop(\opone, \veoneone)$;
    \item $\vetwo$ is a unary operator expression with operator $\optwo$ and operand expression $\vetwoone$,
          that is, $\EUnop(\optwo, \vetwoone)$;
    \item testing whether $\veoneone$ is equivalent to $\vetwoone$ in $\tenv$ yields $\vbone$\ProseOrTypeError;
  ...
  \begin{itemize}
    \item $\veone$ is an \atcexpression{$\veoneone$}{$\vtone$},
          that is, $\EATC(\veoneone, \vtone)$;
    \item $\vetwo$ is an  \atcexpression{$\vetwoone$}{$\vttwo$},
  ...
  \begin{itemize}
    ...
    \item $\vb$ is $\True$ if and only if $\nameone$ is equal to $\nametwo$.
  \end{itemize}
\end{itemize}
...
\inferrule[e\_binop]{
  ...
  \exprequal(\veoneone, \vetwoone) \typearrow \vbone \OrTypeError\hva\\\\
  \exprequal(\veonetwo, \vetwotwo) \typearrow \vbtwo \OrTypeError\hva\\\\
...
\inferrule[e\_call]{
  ...
  i \in \listrange(\vargsone): \exprequal(\tenv, \vargsone[\vi], \vargstwo[\vi]) \typearrow \vbs[\vi] \OrTypeError\\\\
  \vb \eqdef \bigwedge_{i \in \listrange(\vargsone)} \vbs[\vi]
}{
  \exprequalcase(\tenv, \veone, \vetwo) \typearrow \vb
}
\end{mathpar}
...
\inferrule[e\_cond]{
  ...
  \vetwo = \ECond(\vetwoone, \vetwotwo, \vetwothree)\hva\\\\
  \exprequal(\tenv, \veoneone, \vetwoone) \typearrow \vbone \OrTypeError\hva\\\\
  \exprequal(\tenv, \veonetwo, \vetwotwo) \typearrow \vbtwo \OrTypeError\hva\\\\
  \exprequal(\tenv, \veonethree, \vetwothree) \typearrow \vbthree \OrTypeError\hva\\\\
...
\begin{mathpar}
  ...
  \vetwo = \ESlice(\vetwoone, \slicestwo)\hva\\\\
  \exprequal(\tenv, \veoneone, \vetwoone) \typearrow \vbone \OrTypeError\hva\\\\
  \slicesequal(\tenv, \slicesone, \slicestwo) \typearrow \vbtwo \OrTypeError\hva\\\\
...
\begin{mathpar}
  ...
  \vetwo = \EGetArray(\vetwoone, \vetwotwo)\hva\\\\
  \exprequal(\tenv, \veoneone, \vetwoone) \typearrow \vbone \OrTypeError\hva\\\\
  \exprequal(\tenv, \veonetwo, \vetwotwo) \typearrow \vbtwo \OrTypeError\hva\\\\
  \vb \eqdef \vbone \land \vbtwo
}{
...
\begin{mathpar}
  ...
  \vetwo = \EGetField(\vetwoone, \vfieldtwo)\hva\\\\
  \vbone \eqdef \vfieldone = \vfieldtwo\\
  \exprequal(\tenv, \veoneone, \vetwoone) \typearrow \vbtwo \OrTypeError\hva\\\\
  \vb \eqdef \vbone \land \vbtwo
}{
  \exprequalcase(\tenv, \veone, \vetwo) \typearrow \vb
}
...
\begin{mathpar}
  ...
  \vetwo = \EGetFields(\vetwoone, \vfieldstwo)\hva\\\\
...
\begin{mathpar}
  ...
  \vetwo = \EGetItem(\vetwoone, \vitwo)\hva\\\\
...
\inferrule[e\_tuple]{
  ...
  i \in \listrange(\vlone): \exprequal(\tenv, \vlone[\vi], \vltwo[\vi]) \typearrow \vbs[\vi] \OrTypeError\\\\
  \vb \eqdef \bigwedge_{i \in \listrange(\vlone)} \vbs[\vi]
}{
  \exprequalcase(\tenv, \veone, \vetwo) \typearrow \vb
}
\end{mathpar}
...
\inferrule[e\_array]{
  ...
  \vetwo = \EArray\{\EArrayLength: \vltwo, \EArrayValue: \vvtwo\}\hva\\
  \exprequal(\tenv, \vlone, \vltwo) \typearrow \vbone \OrTypeError\\\\
  \exprequal(\tenv, \vvone, \vvtwo) \typearrow \vbtwo \OrTypeError\hva\\\\
  \vb \eqdef \vbone \land \vbtwo
}{
  \exprequalcase(\tenv, \veone, \vetwo) \typearrow \vb
}
...
\inferrule[e\_unop]{
  ...
  \vetwo = \EUnop(\optwo, \vetwoone)\hva\\\\
  \exprequal(\tenv, \veoneone, \vetwoone) \typearrow \vbone \OrTypeError\hva\\\\
...
\inferrule[e\_atc]{
  ...
  \exprequal(\tenv, \veoneone, \vetwoone) \typearrow \vbone \OrTypeError\hva\\\\
  \typeequal(\tenv, \vtone, \vttwo) \typearrow \vbtwo \OrTypeError\hva\\\\
  \vb \eqdef \vbone \land \vbtwo
}{
  \exprequalcase(\tenv, \veone, \vetwo) \typearrow \vb
}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vtone$ and $\vttwo$ are equal and are one of $\TBool$, $\TReal$, or $\TString$;
    \item $\vb$ is $\True$.
  \end{itemize}
  ...
  \begin{itemize}
    \item $\vtone$ is an array type with index $\vlone$ and element type $\vtone$ ($\TArray(\vlone, \vtone)$);
    \item $\vttwo$ is an array type with index $\vltwo$ and element type $\vttwo$ ($\TArray(\vltwo, \vttwo)$);
  ...
  \begin{itemize}
    ...
    \item for each index $i$ in the list $\vtsone$, testing whether $\vtsone[\vi]$ is equivalent to $\vtstwo[\vi]$ in $\tenv$
          yields $\vbs[\vi]$\ProseOrTypeError;
    \item $\vb$ is $\True$ if and only if $\vbs[\vi]$ is $\True$ for each index $i$ in the list $\vtsone$.
  \end{itemize}
\end{itemize}
...
\inferrule[tint\_wellconstrained]{
  \vtone = \TInt(\WellConstrained(\vcone))\\
  \vttwo = \TInt(\WellConstrained(\vctwo))\hva\\\\
  \constraintsequal(\tenv, \vcone, \vctwo) \typearrow \vb \OrTypeError
}{
  \typeequal(\tenv, \vtone, \vttwo) \typearrow \vb
}
\end{mathpar}

\begin{mathpar}
\inferrule[tbits]{
  \bitwidthequal(\tenv, \vwone, \vwtwo) \typearrow \vbone \OrTypeError\hva\\\\
  \bitfieldsequal(\tenv, \bfone, \bftwo) \typearrow \vbtwo \OrTypeError\hva\\\\
  \vb \eqdef \vbone \land \vbtwo
}{
...
\begin{mathpar}
\inferrule[tarray]{
  \exprequal(\tenv, \vlone, \vltwo) \typearrow \vbone \OrTypeError\hva\\\\
  \typeequal(\tenv, \vtone, \vttwo) \typearrow \vbtwo \OrTypeError\hva\\\\
  \vb \eqdef \vbone \land \vbtwo
}{
...
\inferrule[tstructured]{
  ...
  \booltrans(\fieldnames(\vfieldsone) = \fieldnames(\vfieldstwo)) \booltransarrow \True \terminateas \False\hva\\\\
  {
    \begin{array}{l}
...
\inferrule[ttuple]{
  ...
  i \in \listrange(\vtsone): \typeequal(\tenv, \vtsone[\vi], \vtstwo[\vi]) \typearrow \vbs[\vi] \OrTypeError\\\\
  \vb \eqdef \bigwedge_{i \in \listrange(\vtsone)} \vbs[\vi]
}{
  \typeequal(\tenv, \overname{\TTuple(\vtsone)}{\vtone}, \overname{\TTuple(\vtstwo)}{\vttwo}) \typearrow \vb
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{type_equal}
...
\OneApplies
\begin{itemize}
  \item \AllApplyCase{all}
  \begin{itemize}
    \item $\vpone$ and $\vptwo$ are both $\PatternAll$;
    \item $\vb$ is $\True$.
  \end{itemize}

  \item \AllApplyCase{any}
  \begin{itemize}
    \item $\vpone$ is $\PatternAny(\vpsone)$ and $\vptwo$ is $\PatternAny(\vpstwo)$;
    \item checking whether $\vpsone$ and $\vpstwo$ have equal length yields $\True$ or $\False$, which short-circuits the entire rule;
    \item for each index $i$ in $\vpsone$, testing whether $\vpsone[\vi]$ is equivalent to $\vpstwo[\vi]$ in $\tenv$
          yields $\vbs[\vi]$\ProseOrTypeError;
    \item $\vb$ is $\True$ if and only if $\vbs[\vi]$ is $\True$ for each index $i$ in $\vpsone$.
  \end{itemize}

  \item \AllApplyCase{tuple}
  \begin{itemize}
    \item $\vpone$ is $\PatternTuple(\vpsone)$ and $\vptwo$ is $\PatternTuple(\vpstwo)$;
    \item checking whether $\vpsone$ and $\vpstwo$ have equal length yields $\True$ or $\False$, which short-circuits the entire rule;
    \item for each index $i$ in $\vpsone$, testing whether $\vpsone[\vi]$ is equivalent to $\vpstwo[\vi]$ in $\tenv$
          yields $\vbs[\vi]$\ProseOrTypeError;
    \item $\vb$ is $\True$ if and only if $\vbs[\vi]$ is $\True$ for each index $i$ in $\vpsone$.
  ...
  \item \AllApplyCase{geq\_leq\_single}
  \begin{itemize}
    \item $\vpone$ and $\vptwo$ are both $\PatternGeq(\veone)$ and $\PatternGeq(\vetwo)$, or both $\PatternLeq(\veone)$ and $\PatternLeq(\vetwo)$, or both $\PatternSingle(\veone)$ and $\PatternSingle(\vetwo)$;
    \item testing whether $\veone$ is equivalent to $\vetwo$ in $\tenv$ yields $\vb$\ProseOrTypeError.
  \end{itemize}

  \item \AllApplyCase{mask}
  \begin{itemize}
    \item $\vpone$ is $\PatternMask(\vmone)$ and $\vptwo$ is $\PatternMask(\vmtwo)$;
    \item $\vb$ is $\True$ if and only if $\vmone$ is equal to $\vmtwo$.
  \end{itemize}

  \item \AllApplyCase{not}
  \begin{itemize}
    \item $\vpone$ is $\PatternNot(\vpone')$ and $\vptwo$ is $\PatternNot(\vptwo')$;
    \item testing whether $\vpone'$ is equivalent to $\vptwo'$ in $\tenv$ yields $\vb$\ProseOrTypeError.
  \end{itemize}

  \item \AllApplyCase{range}
  \begin{itemize}
    \item $\vpone$ is $\PatternRange(\veoneone, \veonetwo)$ and $\vptwo$ is $\PatternRange(\vetwoone, \vetwotwo)$;
    \item testing whether $\veoneone$ is equivalent to $\vetwoone$ in $\tenv$ yields $\vbone$\ProseOrTypeError;
    \item testing whether $\veonetwo$ is equivalent to $\vetwotwo$ in $\tenv$ yields $\vbtwo$\ProseOrTypeError;
    \item $\vb$ is $\True$ if and only if both $\vbone$ and $\vbtwo$ are $\True$.
  \end{itemize}

  \item \AllApplyCase{other}
  \begin{itemize}
    \item $\vpone$ and $\vptwo$ have different forms;
    \item $\vb$ is $\False$.
  \end{itemize}
\end{itemize}

\FormallyParagraph
\RenderRule{pattern_equal}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item testing whether the bitfield $\bfone[\vi]$ is equivalent to $\bftwo[\vi]$ in $\tenv$ for every index
          of $\bfone$ yields $\vbs[\vi]$\ProseOrTypeError;
    \item $\vb$ is $\True$ if and only if $\vbs[\vi]$ is $\True$ for every index of $\bfone$.
  \end{itemize}
\end{itemize}
...
\inferrule[same\_lengths]{
  ...
  i\in\listrange(\bfone): \bitfieldequal(\tenv, \bfone[\vi], \bftwo[\vi]) \typearrow \vbs[\vi]\\\\
  \vb \eqdef \bigwedge_{i\in\listrange(\bfone)} \vbs[\vi]
}{
  \bitfieldsequal(\tenv, \bfone, \bftwo) \typearrow \vb
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{bitfields_equal}
...
\inferrule[bitfield\_nested]{
  ...
  \bftwo = \BitFieldNested(\nametwo, \slicestwo, \bftwoone)\hva\\
  \booltrans(\nameone = \nametwo) \booltransarrow \True \terminateas \False\\\\
  \slicesequal(\tenv, \slicesone, \slicestwo) \typearrow \vbone \terminateas \TypeErrorConfig,\hva\\\\
...
\inferrule[bitfield\_typed]{
  \bfone = \BitFieldType(\nameone, \slicesone, \vtone)\\
  \bftwo = \BitFieldType(\nametwo, \slicestwo, \vttwo)\\
  \booltrans(\nameone = \nametwo) \booltransarrow \True \terminateas \False \hva\\\\
  \slicesequal(\tenv, \slicesone, \slicestwo) \typearrow \vbone \OrTypeError\hva\\\\
  \typeequal(\tenv, \vtone, \vttwo) \typearrow \vbtwo \OrTypeError\hva\\\\
  \vb \eqdef \vbone \land \vbtwo
}{
  \bitfieldequal(\tenv, \bfone, \bftwo) \typearrow \vb
...
\end{mathpar}
} % BACKUP_ORIGINAL_RULE
\RenderRule{bitfield_equal}
...
\begin{itemize}
  ...
  \item testing whether $\csone[\vi]$ and $\cstwo[\vi]$ are \equivalentconstraintsterm{} in $\tenv$
        yields $\vbs[\vi]$ for each index in the index in the indices for $\csone$ ($i\in\listrange(\csone)$)\ProseOrTypeError;
  \item $\vb$ is $\True$ if and only if all $\vbs[\vi]$ are $\True$ for each index in the indices for $\csone$.
\end{itemize}
...
\inferrule{
  ...
  i\in\listrange(\csone): \constraintequal(\tenv, \csone[\vi], \cstwo[\vi]) \typearrow \vbs[\vi] \OrTypeError\\\\
  \vb \eqdef \bigwedge_{i\in\listrange(\csone)} \vbs[\vi]
}{
  \constraintsequal(\tenv, \csone, \cstwo) \typearrow \vb
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{constraints_equal}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vcone$ is an exact constraint with subexpression $\veone$ ($\ConstraintExact(\veone)$);
    \item $\vctwo$ is an exact constraint with subexpression $\vetwo$ ($\ConstraintExact(\vetwo)$);
...
\begin{itemize}
  \item \AllApplyCase{different\_lengths}
  \begin{itemize}
    \item checking whether the number of slices in $\slicesone$ is equal to the number of slices in $\slicestwo$ yields $\False$;
    \item $\vb$ is $\False$.
  \end{itemize}

  \item \AllApplyCase{same\_lengths}
  \begin{itemize}
    \item checking whether the number of slices in $\slicesone$ is equal to the number of slices in $\slicestwo$ yields $\True$;
    \item determining whether the expression $\slicesone[\vi]$ is equivalent to $\slicestwo[\vi]$ in $\tenv$
          for each index in the indices for $\slicesone$ ($i \in \listrange(\slicesone)$), yields $\vbs[\vi]$\ProseOrTypeError;
    \item $\vb$ is $\True$ if and only if all $\vbs[\vi]$ are $\True$ for each index in the indices for $\slicesone$.
  \end{itemize}
\end{itemize}
...
\inferrule[same\_lengths]{
  ...
  i\in\listrange(\slicesone): \sliceequal(\tenv, \slicesone[\vi], \slicestwo[\vi]) \typearrow \vbs[\vi] \OrTypeError\\\\
  \vb \eqdef \bigwedge_{i\in\listrange(\slicesone)} \vbs[\vi]
}{
  \slicesequal(\tenv, \slicesone, \slicestwo) \typearrow \vb
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{slices_equal}
...
\inferrule[two\_exprs]{
  ...
  \vLone = \vLtwo\\
  \exprequal(\tenv, \veoneone, \veonetwo) \typearrow \vbone \OrTypeError\hva\\\\
  \exprequal(\tenv, \vetwoone, \vetwotwo) \typearrow \vbtwo \OrTypeError\hva\\\\
  \vb \eqdef \vbone \land \vbtwo
}{
...
\inferrule[expr\_expr]{
  \vlone = \ArrayLengthExpr(\veoneone)\\
  \vltwo = \ArrayLengthExpr(\vetwoone)\\
  \exprequal(\veoneone, \vetwoone) \typearrow \vb \OrTypeError
}{
  \arraylengthequal(\vlone, \vltwo) \typearrow \vb
}
\end{mathpar}

\begin{mathpar}
\inferrule[enum\_enum]{
  \vlone = \ArrayLengthEnum(\venumone, \Ignore)\\
  \vltwo = \ArrayLengthEnum(\venumtwo, \Ignore)\\
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item sorting (see $\sort$ for details) the list of monomial-to-factor bindings in $f$,
          that is, the set of pairs $(\vm, \vq)$ such that $\vm$ is in the domain of $f$
          and $f(\vm) = \vq$,
...
The function $\sortmonomialbindings{f}$ sorts the list of monomial-to-factor bindings in $f$
...
\inferrule[non\_empty]{
  \monomsandfactors \eqdef \{ (\vm,\vq) \;|\; \vm\in\dom(f) \land f(\vm) = \vq \}\\
  \monomsandfactorslist \eqdef \listset{\monomsandfactors}\\
  \monoms \eqdef \sort(\monomsandfactorslist, \comparemonomialbindings)\\
  \monomialstoexpr(\monoms) \typearrow (\veone, \vsone)\hva\\
  {
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item $s$ is determined as follows:
    \begin{itemize}
      \item $1$ if $\vv$ is only in the domain of $g$;
      \item $-1$ if $\vv$ is only in the domain of $f$;
      \item otherwise ($\vv$ is in the domains of both $f$ and $g$) it is the sign of $g(\vv)-f(\vv)$.
    \end{itemize}
  \end{itemize}
\end{itemize}
...
\inferrule[different\_monomials]{
  ...
  \ids \reverseeqdef \idsone \concat \idstwo\hva\\
  \vi\in\listrange(\idsone): f(\idsone[\vi]) = g(\idsone[\vi])\\
  \vv \eqdef \idstwo[1]\hva\\
  {
  \vs \eqdef
    \begin{cases}
...
\RenderRule{compare_monomial_bindings}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $q$ is a strictly positive natural number;
  ...
  \begin{itemize}
    \item $q$ is a strictly positive number that is not an integer;
    \item the reduced representation of the fraction $q$ is $\frac{d}{n}$;
    \item symbolically multiplying the literal expression for $d$ and $\ve$ via $\symmulexpr$ yields $\vetwo$;
...
\inferrule[q\_positive\_fraction]{
  q > 0\\
  q \not\in \N\hva\\\\
  q \reverseeqdef \frac{d}{n} \;\;\;\;\text{ is the reduced fraction for }q\hva\\\\
...
\inferrule[same\_sign]{
  ...
  \symaddexpr(\veone, \vsone, \vetwo, \vstwo) \typearrow (\overname{\EBinop(\ADD, \veone, \vetwo)}{\ve}, \overname{\vsone}{\vs})
}
\hva\and
\inferrule[different\_signs]{
  ...
  \symaddexpr(\veone, \vsone, \vetwo, \vstwo) \typearrow (\overname{\EBinop(\SUB, \veone, \vetwo)}{\ve}, \overname{\vsone}{\vs})
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{sym_add_expr}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\monoms$ is a list where the first element is $(\vv, 0)$ and its tail is $\monomsone$;
    \item transforming $\monomsone$ to an expression yields $\ve$.
  \end{itemize}

  \item \AllApplyCase{exp\_one}
  \begin{itemize}
    \item $\monoms$ is a list where the first element is $(\vv, 1)$ and its tail is $\monomsone$;
  ...
  \begin{itemize}
    \item $\monoms$ is a list where the first element is $(\vv, 2)$ and its tail is $\monomsone$;
  ...
  \begin{itemize}
    \item $\monoms$ is a list where the first element is $(\vv, n)$ and its tail is $\monomsone$;
...
\RenderRule{unitary_monomials_to_expr}
...
\begin{mathpar}
\inferrule[local]{
  \tenv.\staticenvsL.\localstoragetypes(\vs) = \tty
}{
  \typeof(\tenv, \vs) \typearrow \tty
}
\hva\and
\inferrule[global]{
  \tenv.\staticenvsL.\localstoragetypes(\vs) = \bot\\
  \tenv.\staticenvsG.\globalstoragetypes(\vs) = \tty
}{
  \typeof(\tenv, \vs) \typearrow \tty
}
\hva\and
\inferrule[error]{
  \tenv.\staticenvsL.\localstoragetypes(\vs) = \bot\\
  \tenv.\staticenvsG.\globalstoragetypes(\vs) = \bot
}{
...
\inferrule[normalizable]{
  ...
  \normalize(\tenv, \ve) \typearrow \overname{\some{\newe}}{\neweopt}
}
\hva\and
\inferrule[not\_normalizable]{
  \toir(\tenv, \ve) \typearrow \CannotBeTransformed
