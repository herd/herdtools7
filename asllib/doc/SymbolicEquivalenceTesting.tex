...
\[
  \begin{array}{rcl}
    ...
    \unitarymonomial   &\triangleq& \Identifier \partialto \Npos\\
  \end{array}
\]
}
...
is examples similar to the one in \listingref{IR}, which requires representing the
...
The function
\[
  \addpolynomials(\polynomial, \polynomial) \rightarrow \polynomial
\]
...
The function
\hypertarget{relation-mulpolynomials}{}
\[
...
The function
\hypertarget{relation-polynomialdividebyterm}{}
\[
...
The function
\[
\normalize(\overname{\staticenvs}{\tenv} \aslsep \overname{\expr}{\ve}) \aslto \overname{\expr}{\newe} \cup \overname{\typeerror}{\TypeErrorConfig}
\]
\hypertarget{def-symbolicallysimplifies}{symbolically simplifies} an expression $\ve$ in the \staticenvironmentterm{} $\tenv$,
...
\]
\symbolicallysimplifiesterm\ an integer constraint $\vc$, yielding the integer constraint $\newc$
} % END_OF_BACKUP_RELATION
...
\]
\symbolicallysimplifiesterm\ a list of integer constraints $\cs$, yielding a list of integer constraints $\newcs$
} % END_OF_BACKUP_RELATION
...
\[
  ...
  \overname{\polynomial}{\vp}\ \cup\ \{\CannotBeTransformed\}\ \cup\ \overname{\typeerror}{\TypeErrorConfig}
\]
transforms a subset of ASL expressions into \symbolicexpressionsterm{}. If an ASL expression cannot be represented
by a symbolic expression (because, for example, it contains operations that are not available in \symbolicexpressionsterm{}),
...
\inferrule[ebinop\_minus]{
  \vep \eqdef \EBinop(\ADD, \veone, \EUnop(\SUB, \vetwo))\\
...
\]
conservatively checks whether $\veone$ and $\vetwo$ are \equivalentexprsterm{} in
the \staticenvironmentterm{} $\tenv$.
...
\]
conservatively tests whether the $\veone$ and $\vetwo$ are \equivalentexprsterm{} in the \staticenvironmentterm{} $\tenv$
by attempting to transform both expressions to their \symbolicexpressionterm{} form
and, if successful, comparing the resulting \symbolicexpressionsterm{} for equality.
The result is given in $\vb$ or a \typingerrorterm{}, if one is detected.
} % END_OF_BACKUP_RELATION
...
The function
\[
  \typeequal(\overname{\staticenvs}{\tenv} \aslsep \overname{\ty}{\vtone} \aslsep \overname{\ty}{\vttwo}) \aslto
   \overname{\Bool}{\vb} \cup \overname{\typeerror}{\TypeErrorConfig}
\]
conservatively tests whether $\vtone$ and $\vttwo$ are \equivalenttypesterm{} in
the \staticenvironmentterm{} $\tenv$
...
The function
\[
  \bitfieldsequal(\overname{\staticenvs}{\tenv} \aslsep \overname{\KleeneStar{\bitfield}}{\bfone} \aslsep \overname{\KleeneStar{\bitfield}}{\bftwo})
  \aslto \overname{\Bool}{\vb} \cup \overname{\typeerror}{\TypeErrorConfig}
\]
...
The function
\[
  \constraintsequal(\overname{\staticenvs}{\tenv} \aslsep \overname{\KleeneStar{\intconstraint}}{\csone} \aslsep \overname{\KleeneStar{\intconstraint}}{\cstwo})
  \aslto \overname{\Bool}{\vb} \cup \overname{\typeerror}{\TypeErrorConfig}
\]
...
The function
\[
  \slicesequal(\overname{\staticenvs}{\tenv} \aslsep \overname{\KleeneStar{\slice}}{\slicesone} \aslsep \overname{\KleeneStar{\slice}}{\slicestwo})
  \aslto \overname{\Bool}{\vb} \cup \overname{\typeerror}{\TypeErrorConfig}
\]
...
The function
\[
\monomialstoexpr(\overname{\KleeneStar{(\overname{\unitarymonomial}{\vm}\times\overname{\Q}{q})}}{\monoms}) \aslto
(\overname{\expr}{\ve},\overname{\Sign}{\vs})
\]
...
\symaddexpr(
  ...
  \overname{\Sign}{\vsone} \aslsep
  \overname{\expr}{\vetwo} \aslsep
  \overname{\Sign}{\vstwo}
  )
  \typearrow (\overname{\expr}{\ve}, \overname{\Sign}{\vs})
\]
...
\inferrule[same\_sign]{
  ...
  \symaddexpr(\veone, \vetwo) \typearrow (\overname{\EBinop(\ADD, \veone, \vetwo)}{\ve}, \overname{\vsone}{\vs})
}
\and
\inferrule[different\_signs]{
  ...
  \symaddexpr(\veone, \vetwo) \typearrow (\overname{\EBinop(\SUB, \veone, \vetwo)}{\ve}, \overname{\vsone}{\vs})
}
\end{mathpar}
...
The function
\[
\unitarymonomialstoexpr(\overname{\KleeneStar{(\Identifier\times\N)}}{\monoms}) \aslto \overname{\expr}{\ve}
\]
...
\overname{\Option{\expr}}{\neweopt} \cup \overname{\typeerror}{\TypeErrorConfig}
\]
