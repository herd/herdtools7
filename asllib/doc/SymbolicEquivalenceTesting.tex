...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vtone$ is the bitvector type with width expression $\vwone$ and list of bitfields $\vbfone$, that is, $\TBits(\vwone, \vbfone)$;
    \item $\vttwo$ is the bitvector type with width expression $\vwtwo$ and list of bitfields $\vbftwo$, that is, $\TBits(\vwtwo, \vbftwo)$;
    \item testing whether $\vwone$ and $\vwtwo$ are equivalent bitwidths in $\tenv$ yields $\vbone$\ProseOrTypeError;
    \item testing whether $\vbfone$ and $\vbftwo$ are equivalent lists of bitfields in $\tenv$ yields $\vbtwo$\ProseOrTypeError;
    \item $\vb$ is $\True$ if and only if both $\vbone$ and $\vbtwo$ are $\True$.
  \end{itemize}
...
\inferrule[tbits]{
  ...
  \bitfieldsequal(\tenv, \vbfone, \vbftwo) \typearrow \vbtwo \OrTypeError\\\\
  \vb \eqdef \vbone \land \vbtwo
}{
  \typeequal(\tenv, \overname{\TBits(\vwone, \vbfone)}{\vtone}, \overname{\TBits(\vwtwo, \vbftwo)}{\vttwo}) \typearrow \vb
}
\end{mathpar}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item the number of bitfields in $\vbfone$ is different from the number of bitfields in $\vbftwo$;
    \item $\vb$ is $\False$.
  \end{itemize}

  \item \AllApplyCase{same\_lengths}
  \begin{itemize}
    \item the number of bitfields in $\vbfone$ is the same as the number of bitfields in $\vbftwo$;
    \item testing whether the bitfield $\vbfone[i]$ is equivalent to $\vbftwo[i]$ in $\tenv$ for every index
          of $\vbfone$ yields $\vb_i$\ProseOrTypeError;
    \item $\vb$ is $\True$ if and only if $\vb_i$ is $\True$ for every index of $\vbfone$.
  \end{itemize}
\end{itemize}
...
\inferrule[different\_lengths]{
  \equallength(\vbfone, \vbftwo) \typearrow \False
}{
  \bitfieldsequal(\tenv, \vbfone, \vbftwo) \typearrow \False
}
\and
\inferrule[same\_lengths]{
  \equallength(\vbfone, \vbftwo) \typearrow \True\\
  i\in\listrange(\vbfone): \bitfieldequal(\tenv, \vbfone[i], \vbftwo[i]) \typearrow \vb_i\\\\
  \vb \eqdef \bigwedge_{i\in\listrange(\vbfone)} \vb_i
}{
...
\inferrule[different\_labels]{
  ...
  \bitfieldequal(\tenv, \vbfone, \vbftwo) \typearrow \False
}
\and
\inferrule[bitfield\_simple]{
  \vbfone = \BitFieldSimple(\nameone, \slicesone)\\
  \vbftwo = \BitFieldSimple(\nametwo, \slicestwo)\\
  \booltrans(\nameone = \nametwo) \booltransarrow \vbone\\
  \slicesequal(\tenv, \slicesone, \slicestwo) \typearrow \vbtwo \OrTypeError\\\\
  \vb \eqdef \vbone \land \vbtwo
}{
  \bitfieldequal(\tenv, \vbfone, \vbftwo) \typearrow \vb
}
\and
...
\inferrule[constraint\_range]{
  \vbfone = \ConstraintRange(\veoneone, \veonetwo)\\
  \vbftwo = \ConstraintRange(\vetwoone, \vetwotwo)\\
  \exprequal(\tenv, \veoneone, \vetwoone) \typearrow \vbone \OrTypeError\\\\
  \exprequal(\tenv, \veonetwo, \vetwotwo) \typearrow \vbtwo \OrTypeError\\\\
  \vb \eqdef \vbone \land \vbtwo
}{
  \constraintequal(\tenv, \vbfone, \vbftwo) \typearrow \vb
}
\end{mathpar}
