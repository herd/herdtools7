...
desired form varies according to the functions utilising this value).

\ChapterOutline

\SectionDef[formally defines equivalence for expressions, types, constraints, and bitfields;]
  {Equivalence Definitions}{Equivalence Definitions}
...
\SectionDef[defines \symbolicexpressionsterm{} and operations over \symbolicexpressionsterm{}; and]
  {symbolicexpressions}{Symbolic Expressions}
...
\end{mathpar}
Notice that if $\vfone$ and $\vftwo$ map the same monomial $\vm$ to factors that sum to zero
(that is, $\vm \in \dom(\vfone) \cap \dom(\vftwo)$ and $\vfone(\vm) + \vftwo(\vm) = 0$),
then $\vf$ does not map $\vm$ to any factor, which corresponds to the fact that the monomial
$\vm$ does not appear in the resulting polynomial.

\TypingRuleDef{AddPolynomialList}
The overloaded function
\[
  \addpolynomials(\KleeneStar{\polynomial}) \aslto \polynomial
\]
...
\SectionDef[defines rules for symbolic reduction and equivalence testing.]
  {SymbolicReductionAndEquivalenceTestingRules}{Symbolic Reduction and Equivalence Testing Rules}
We employ the following rules:
\begin{TypingRuleList}{SymbolicEquivalenceTesting}

\TypingRuleDef[normalizes an expression.]{Normalize}
\RenderRelation{normalize}

\hypertarget{def-normalizedterm}{}
We refer to an expression in the image of $\normalize$ as a \emph{\normalizedexpressionterm}.

\ExampleDef{Normalise}
...
and their corresponding normalised expressions.
\ASLListing{Normalising expressions}{Normalize}{\typingtests/TypingRule.Normalize.asl}


\RenderProseAndFormally{normalize}
\CodeSubsection{\NormalizeBegin}{\NormalizeEnd}{../StaticModel.ml}

\TypingRuleDef[symbolically simplifies a constraint.]{ReduceConstraint}
...
\TypingRuleDef[symbolically simplifies a list of constraints.]{ReduceConstraints}
...
\TypingRuleDef[transforms an expression into a symbolic representation.]{ToIR}
...
\TypingRuleDef[checks symbolic equivalence between expressions.]{ExprEqual}
...
\TypingRuleDef[checks symbolic equivalence between normalized expressions.]{ExprEqualNorm}
...
\TypingRuleDef[checks symbolic equivalence for one expression form.]{ExprEqualCase}
...
\TypingRuleDef[checks symbolic equivalence between types.]{TypeEqual}
...
\TypingRuleDef[checks symbolic equivalence between patterns.]{PatternEqual}
...
\TypingRuleDef[checks symbolic equivalence between pattern matchers.]{PatternMatcherEqual}
\RenderRelation{pattern_matcher_equal}

See \ExampleRef{Expression Equivalence}.

\RenderProseAndFormally{pattern_matcher_equal}

\TypingRuleDef[checks symbolic equivalence between bitwidth expressions.]{BitwidthEqual}
...
\TypingRuleDef[checks symbolic equivalence between lists of bitfields.]{BitFieldsEqual}
...
\TypingRuleDef[checks symbolic equivalence between bitfields.]{BitFieldEqual}
...
\TypingRuleDef[checks symbolic equivalence between lists of constraints.]{ConstraintsEqual}
...
\TypingRuleDef[checks symbolic equivalence between constraints.]{ConstraintEqual}
...
\TypingRuleDef[checks symbolic equivalence between lists of slices.]{SlicesEqual}
...
\TypingRuleDef[checks symbolic equivalence between slices.]{SliceEqual}
...
\TypingRuleDef[transforms a polynomial into an expression.]{PolynomialToExpr}
...
\TypingRuleDef[orders monomial bindings.]{CompareMonomialBindings}
...
\TypingRuleDef[transforms a list of monomials into an expression.]{MonomialsToExpr}
...
\TypingRuleDef[transforms a monomial into an expression.]{MonomialToExpr}
...
\TypingRuleDef[symbolically adds signed expressions.]{SymAddExpr}
\RenderRelation{sym_add_expr}

The effect of the function can be summarised by the following table:
\begin{center}
...
\TypingRuleDef[transforms unitary monomials into expressions.]{UnitaryMonomialsToExpr}
...
\TypingRuleDef[symbolically multiplies expressions.]{SymMulExpr}
...
\TypingRuleDef[retrieves the type bound to an identifier.]{TypeOf}
...
\TypingRuleDef[normalizes an optional expression.]{NormalizeOpt}
\RenderRelation{normalize_opt}

See \ExampleRef{Normalise}.
