...
% BEGIN_GENERATED_ELEMENT(static_envs, global_static_envs, local_static_envs)
\hypertarget{type-staticenvs}{}
\begin{flalign*}
\staticenvs \triangleq\ & (\overtext{\globalstaticenvs}{G}, \overtext{\localstaticenvs}{L})
\hypertarget{type-globalstaticenvs}{}
\\
\globalstaticenvs \triangleq\ & \left\{\begin{array}{lll}
\text{declared\_types} &:& \Identifier \partialto (\overtext{\ty}{element type}, \overtext{\TPurity}{element purity})\\
\text{global\_storage\_types} &:& \Identifier \partialto (\overtext{\ty}{element type}, \overtext{\globaldeclkeyword}{declared keyword})\\
\text{expr\_equiv} &:& \Identifier \partialto \overtext{\expr}{initializer}\\
\text{subtypes} &:& \overtext{\Identifier}{sub type} \partialto \overtext{\Identifier}{super type}\\
\text{subprogram} &:& \Identifier \partialto (\func, \overtext{\pow{\TSideEffect}}{side effects})\\
\text{overloaded\_subprogram} &:& \Identifier \partialto \pow{\Strings}
\end{array}\right\}
\hypertarget{type-localstaticenvs}{}
\\
\localstaticenvs \triangleq\ & \left\{\begin{array}{lll}
\text{local\_storage\_types} &:& \Identifier \partialto (\overtext{\ty}{element type}, \overtext{\localdeclkeyword}{declared keyword})\\
\text{expr\_equiv} &:& \Identifier \partialto \expr\\
...
\begin{description}
  ...
  % BEGIN_GENERATED_ELEMENT(type_error)
\hypertarget{type-typeerror}{}\hypertarget{type-TypeError}{}
\begin{flalign*}
\typeerror \triangleq\ & \TypeError(\overtext{\Strings}{error code})
\end{flalign*}
  % END_GENERATED_ELEMENT
