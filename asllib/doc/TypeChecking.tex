...
\SectionDef[defines static environments; and]
  {StaticEnvironments}{Static Environments}
...
A \staticenvironmentterm{} $\tenv$ consists of two
distinct components: the \emph{\globalstaticenvironmentterm} $\tenv.\staticenvsG \in \globalstaticenvs$ --- pertaining to AST nodes
appearing outside of a given subprogram, and the \emph{\localstaticenvironmentterm}
$\tenv.\staticenvsL \in \localstaticenvs$ --- pertaining to AST nodes appearing inside a given subprogram.
...
\begin{itemize}
  ...
  \item $\declaredtypes$ assigns to names of declared types and enumeration labels their inferred types and \purity{};
  ...
  of their initialising expression;
  ...
  \item $\returntype$ contains the optional return type that a subprogram declares,
        if it is a function or a getter.
\end{itemize}

\texthypertarget{constant-emptytenv}
...
\SectionDef[defines the configurations used in typing rules.]
  {Typing Rule Configurations}{Typing Rule Configurations}
The output configurations of type system assertions have two flavours:
\begin{description}
  \item[Normal Outputs.]
  ...
  the category of error (see \chapref{Errors} for the list of all error codes organised by category).
\end{description}
