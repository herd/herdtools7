...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vc$ is a range constraint consisting of two side-effect-free expressions $\veone$ and $\vetwo$, that is, $\ConstraintRange(\veone, \vetwo)$;
    ...
    \item $\vd$ is the set containing all native integer values for integers greater than or equal to $a$ and less than or equal to $b$.
  \end{itemize}
  ...
  \item \AllApply
  \begin{itemize}
    \item $\vt$ is an integer-indexed array type with length expression $\ve$ and element type $\telem$, $\TArray(\ve, \telem)$;
    \item \OneApplies
      \begin{itemize}
...
\inferrule[t\_array\_abnormal]{
  ...
  \dynamicdomain(\env, \vt) = \overname{\emptyset}{\vd}
}
\end{mathpar}
...
\inferrule[t\_array\_negative\_length\_error]{
  ...
  \dynamicdomain(\env, \overname{\TArray(\ve, \telem)}{\vt}) = \overname{\emptyset}{\vd}
}
\end{mathpar}
...
\begin{mathpar}
\inferrule[t\_named]{
  \env \reverseeqdef (\tenv, \Ignore)\\
  \tenv.\staticenvsG.\declaredtypes(\id) = (\tty, \Ignore)
}{
