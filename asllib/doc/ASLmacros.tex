...
% XeTeX passes Unicode input directly to the selected T1 font. Map the Unicode
% punctuation used in the source to T1-safe TeX forms.
% pdfLaTeX already performs equivalent UTF-8 input mappings.
\ifx\XeTeXrevision\undefined
\else
  % XeTeX does not provide inputenc's \DeclareUnicodeCharacter.  Define the
  % equivalent here, so the mappings name their code points explicitly.
  ...
  \DeclareUnicodeCharacter{201C}{``}                 % U+201C “ LEFT DOUBLE QUOTATION MARK
  \DeclareUnicodeCharacter{201D}{''}                 % U+201D ” RIGHT DOUBLE QUOTATION MARK
  \DeclareUnicodeCharacter{2019}{'}                  % U+2019 ’ RIGHT SINGLE QUOTATION MARK
  \DeclareUnicodeCharacter{00AE}{\textregistered{}}  % U+00AE ® REGISTERED SIGN
  \DeclareUnicodeCharacter{2122}{\texttrademark{}}   % U+2122 ™ TRADE MARK SIGN
  \DeclareUnicodeCharacter{00A9}{\textcopyright{}}   % U+00A9 © COPYRIGHT SIGN
  \DeclareUnicodeCharacter{2013}{--}                 % U+2013 – EN DASH
  \DeclareUnicodeCharacter{2014}{---}                % U+2014 — EM DASH
\fi
\usepackage{ifthen} % High-level statements for LaTeX
...
\newcommand\TERM[1]{#1} % escape hatch for spellchecking
...
\newcommand\RenderProseAndFormally[1]{\ProseParagraph \RenderProse{\detokenize{#1}} \FormallyParagraph \RenderRule{\detokenize{#1}}}
% Use this macro to highlight notes in prose sections that should be preserved
% rather than overwritten during automatic prose generation.
...
\newcommand\SingleCase[1]{(\CaseDef{#1}):}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Rule list generation macros
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% These environments generate rule-reference lists using the same multi-run
% approach as the table of contents. Place the rules to include inside the
% corresponding ...RuleList environment and run LaTeX twice.
\makeatletter
\let\ASLCurrentASTRuleList\@empty
\let\ASLCurrentTypingRuleList\@empty
\let\ASLCurrentSemanticsRuleList\@empty

\newcommand\ASLRuleListEntry[5]{%
  \@ifundefined{ASLRuleList@#1@#2}{%
    \expandafter\gdef\csname ASLRuleList@#1@#2\endcsname{}%
  }{}%
  \def\ASLRuleListDescription{#5}%
  \ifx\ASLRuleListDescription\@empty
    \expandafter\g@addto@macro\csname ASLRuleList@#1@#2\endcsname{%
...
\newcommand\ASLPrintRuleList[2]{%
  \begin{itemize}
  \@ifundefined{ASLRuleList@#1@#2}{%
    \item \emph{Run LaTeX again to generate this rule list.}%
  }{%
    \csname ASLRuleList@#1@#2\endcsname
  }%
  \end{itemize}%
}

\newenvironment{ASTRuleList}[1]{%
  \ASLPrintRuleList{AST}{#1}%
  \gdef\ASLCurrentASTRuleList{#1}%
}{%
  \global\let\ASLCurrentASTRuleList\@empty
}

\newenvironment{TypingRuleList}[1]{%
  \ASLPrintRuleList{Typing}{#1}%
  \gdef\ASLCurrentTypingRuleList{#1}%
}{%
  \global\let\ASLCurrentTypingRuleList\@empty
}

\newenvironment{SemanticsRuleList}[1]{%
  \ASLPrintRuleList{Semantics}{#1}%
  \gdef\ASLCurrentSemanticsRuleList{#1}%
}{%
  \global\let\ASLCurrentSemanticsRuleList\@empty
}

\newcommand\ASLRecordASTRule[2]{%
  \ifx\ASLCurrentASTRuleList\@empty
  \else
    \immediate\write\@auxout{%
      \string\ASLRuleListEntry{AST}{\ASLCurrentASTRuleList}{ASTRuleRef}{#2}{\unexpanded{#1}}%
    }%
  \fi
}

\newcommand\ASLRecordTypingRule[2]{%
  \ifx\ASLCurrentTypingRuleList\@empty
  \else
    \immediate\write\@auxout{%
      \string\ASLRuleListEntry{Typing}{\ASLCurrentTypingRuleList}{TypingRuleRef}{#2}{\unexpanded{#1}}%
    }%
  \fi
}

\newcommand\ASLRecordSemanticsRule[2]{%
  \ifx\ASLCurrentSemanticsRuleList\@empty
  \else
    \immediate\write\@auxout{%
      \string\ASLRuleListEntry{Semantics}{\ASLCurrentSemanticsRuleList}{SemanticsRuleRef}{#2}{\unexpanded{#1}}%
    }%
  \fi
}
...
% in the outline.  Descriptions are sentence fragments and should not end with
% punctuation; the outline adds a final period.  The second argument is a stable
% label key: the section receives the label
...
\newcommand\ASTRuleDef[2][]{\subsubsection{ASTRule.#2\label{sec:ASTRule.#2}}\ASLRecordASTRule{#1}{#2}}
\newcommand\ASTRuleDefExample[1]{\emph{ASTRule.#1}}
\newcommand\TypingRuleDef[2][]{\subsubsection{TypingRule.#2\label{sec:TypingRule.#2}}\ASLRecordTypingRule{#1}{#2}}
\newcommand\TypingRuleDefExample[1]{\emph{TypingRule.#1}}
\newcommand\SemanticsRuleDef[2][]{\subsubsection{SemanticsRule.#2\label{sec:SemanticsRule.#2}}\ASLRecordSemanticsRule{#1}{#2}}
...
\newcommand\SyntacticSugarRef[1]{\nameref{Guide.SyntacticSugar.#1}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Operator macros
\newcommand\eqdef[0]{\hyperlink{relation-assign}{:=}}
\newcommand\reverseeqdef[0]{\hyperlink{relation-reverseassign}{=:}}
\newcommand\freshidentifier[0]{\text{fresh from }\Identifier}
\newcommand\astlabelop[1]{\astlabel(#1)}
\newcommand\makestructured[3][H]{
    {#2}\left({#3}\right)
}
\newcommand\mapupdate[4][H]{ {#2}\left[{#3} \mapsto {#4}\right] }

\newcommand\mapapplyop[3][H]{
    {#2}\left({#3}\right)
}

\newcommand\sort[0]{\hyperlink{def-sort}{\textfunc{sort}}} % SPECIFICATION_DEFERRED
\newcommand\sortop[3][H]{
...
\newcommand\sortmonomialbindings[1]{\hyperlink{relation-sortmonomialbindings}{\textfunc{sort\_monomial\_bindings}}\left(#1\right)} % SPECIFICATION_DEFERRED

\newcommand\equal[0]{\mathrel{=}}
\newcommand\notequal[0]{\mathrel{\neq}}

\newcommand\condcase[3][H]{
  \ifthenelse{\equal{#1}{H}}{
    \textbf{if }{#2}\textbf{ then }{#3}
  }{
    \begin{array}{ll}
    \textbf{if }{#2}\\
    \textbf{ then }{#3}\\
...
\newcommand\condop[1]{\left\{{#1}\right.} % A list of cases

\newcommand\forallop[4][H]{
  \ifthenelse{\equal{#1}{H}}
    {\forall {#2} \in {#3} : {#4}}
    {\forall
    \begin{array}{ll}
...
\newcommand\listmapunion[4][H]{%
  \bigcup_{#2 \in #3} #4
}
\newcommand\listfilter[4][H]{%
  \ifthenelse{\equal{#1}{H}}{
    \left[#2 \in #3 : #4\right]
  }{
    \left[
...
\newcommand\filteroptionlist[1]{\hyperlink{relation-filteroptionlist}{\textfunc{filter\_option\_list}}\left(#1\right)}

\newcommand\listprefixname[0]{\hyperlink{def-listprefix}{\textfunc{list\_prefix}}}
\newcommand\listprefix[3][H]{%
  \ifthenelse{\equal{#1}{H}}{
    \listprefixname(#2, #3)
  }{
    \listprefixname\left(
...
\newcommand\ELInt[1]{\hyperlink{def-elint}{\textsf{ELint}}(#1)}
\newcommand\AbbrevEBinop[4][H]{
  \ifthenelse{\equal{#1}{H}}{
  \overbracket{#3\ #2\ #4}^{\hyperlink{def-abbrevebinop}{\abbrevfont{E\_Binop}}}
  }{
  \overbracket{\begin{array}{c}#3\\#2\\#4\end{array}}^{\hyperlink{def-abbrevebinop}{\abbrevfont{E\_Binop}}}
  }
}
\newcommand\AbbrevECond[4][H]{
  \ifthenelse{\equal{#1}{H}}{
  \ECond\left(#2, #3, #4\right)
  }{
    \ECond\left(
    \begin{array}{c}
      #2,\\
...
\let\OldLand\land
\renewcommand\land[0]{\hyperlink{relation-land}{\OldLand}}
\let\OldLor\lor
\renewcommand\lor[0]{\hyperlink{relation-lor}{\OldLor}}
\let\OldNeg\neg
\newcommand\listand[0]{\hyperlink{relation-land}{\bigwedge}}
\newcommand\listor[0]{\hyperlink{relation-lor}{\bigvee}}
\renewcommand\neg[0]{\hyperlink{relation-neg}{\OldNeg}}
...
\newcommand\listassetop[1]{\left\{{#1}\right\}}
\newcommand\cardinality[1]{\hyperlink{relation-cardinality}{|}#1\hyperlink{relation-cardinality}{|}}
\newcommand\rangeop[3][H]{
  \ifthenelse{\equal{#1}{H}}{
    \left[{#2} \ldots {#3}\right]
  }{
    \left[
    \begin{array}{l}
    {#2} \ldots\\
...
\newcommand\listmax[1]{\max(#1)}
\newcommand\setmin[1]{\min(#1)}
\newcommand\setmax[1]{\max(#1)}
\newcommand\member[0]{\mathrel{\in}}
\newcommand\notmember[0]{\mathrel{\not\in}}
\newcommand\disjointunion[0]{\hyperlink{def-disjointunion}{\mathbin{\mathaccent\cdot\cup}}}
\newcommand\unionlist[0]{\hyperlink{def-unionlist}{\textfunc{union\_list}}}
\newcommand\unionlistop[1]{\unionlist\left({#1}\right)} % A union of a list of sets
\newcommand\intersectionlist[0]{\hyperlink{def-intersectionlist}{\textfunc{intersection\_list}}}
\newcommand\intersectionlistop[1]{\intersectionlist\left({#1}\right)} % An intersection of a list of sets
\newcommand\assocopt[3][H]{
  \ifthenelse{\equal{#1}{H}}
    {\hyperlink{relation-assocopt}{\textfunc{assoc\_opt}}({#2}, {#3})}
    {\hyperlink{relation-assocopt}{\textfunc{assoc\_opt}}
    \left(\begin{array}{l}
    ({#2},\\
    {#3})\\
    \end{array}\right)}
...
\newcommand\identityop[1]{#1}
\newcommand\numplus[0]{\mathbin{+}}
\newcommand\numminus[0]{\mathbin{-}}
\newcommand\numnegate[1]{\mathbin{-}{#1}}
\newcommand\numtimes[0]{\mathbin{\times}}
\newcommand\numdivide[0]{\mathbin{\div}}
\newcommand\numexponent[3][H]{
    {#2}^{#3}
}

\newcommand\lessthan[0]{\mathbin{<}}
\newcommand\lessorequal[0]{\mathbin{\leq}}
\newcommand\greaterthan[0]{\mathbin{>}}
\newcommand\greaterorequal[0]{\mathbin{\geq}}

\newcommand\roundup[1]{\left\lceil {#1} \right\rceil}
\newcommand\rounddown[1]{\left\lfloor {#1} \right\rfloor}
\newcommand\fractionop[3][H]{\frac{#2}{#3}}
\newcommand\isintegerop[1]{{#1}\in\Z}
\newcommand\isnotintegerop[1]{{#1}\notin\Z}
\newcommand\absvalueop[1]{\left| {#1} \right|}
...
\newcommand\numberedidentifierop[3][H]{
  \ifthenelse{\equal{#1}{H}}{
    {#2} \concat \stringofint{#3}
  }{
    \left(\begin{array}{l}
    {#2} \concat\\
    \stringofint{#3}\\
...
\newcommand\orderedctrl[0]{\hyperlink{def-ordered}{\xrightarrow{\aslctrl}}}
\newcommand\ordereddata[0]{\hyperlink{def-ordered}{\xrightarrow{\asldata}}}
\newcommand\orderedpo[0]{\hyperlink{def-ordered}{\xrightarrow{\aslpo}}}
\newcommand\graphof[1]{\hyperlink{relation-graphof}{\textfunc{graph}}({#1})} % NO_SPECIFICATION_REQUIRED
\newcommand\environof[1]{\hyperlink{relation-environof}{\textfunc{environ}}({#1})} % NO_SPECIFICATION_REQUIRED
\newcommand\WriteEffectop[1]{\WriteEffect\langle{#1}\rangle}
\newcommand\ReadEffectop[1]{\ReadEffect\langle{#1}\rangle}

\newcommand\withgraph[3][H]{ {#2}(\hyperlink{relation-withgraph}{\textfunc{graph}}\mapsto{#3}) } % NO_SPECIFICATION_REQUIRED
\newcommand\withenviron[3][H]{ {#2}(\hyperlink{def-withenviron}{\textfunc{environ}}\mapsto{#3}) } % NO_SPECIFICATION_REQUIRED

\newcommand\concatstrings[0]{\hyperlink{relation-stringconcat}{\mathbin{+}}}
\newcommand\decimaltolitname[0]{\hyperlink{relation-decimaltolit}{\textfunc{dec\_to\_lit}}} % NO_SPECIFICATION_REQUIRED
\newcommand\decimaltolit[1]{\decimaltolitname\left(#1\right)} % NO_SPECIFICATION_REQUIRED

\newcommand\nvintop[1]{\nvint\left(#1\right)}
\newcommand\nvboolop[1]{\nvbool\left(#1\right)}
\newcommand\nvstringop[1]{\nvstring\left(#1\right)}
\newcommand\sccop[3][H]{
  \ifthenelse{\equal{#1}{H}}{
    \hyperlink{relation-scc}{\textfunc{SCC}}\left({#2}, {#3}\right)
  }{
    \hyperlink{relation-scc}{\textfunc{SCC}}
    \left(\begin{array}{l}
    {#2},\\
    {#3}\\
    \end{array}\right)
...
\newcommand\topologicalorderingcomps[3][H]{
  \ifthenelse{\equal{#1}{H}}{
    \hyperlink{relation-topologicalorderingcomps}{\textfunc{topological\_ordering\_comps}}\left({#2}, {#3}\right)
  }{
    \hyperlink{relation-topologicalorderingcomps}{\textfunc{topological\_ordering\_comps}}
    \left(\begin{array}{l}
    {#2},\\
    {#3}\\
    \end{array}\right)
...
\newcommand\emptyfunc[0]{\hyperlink{def-emptyfunc}{{\OldEmptyset}_\lambda}}
\newcommand\COLON[0]{:}
\newcommand\PIPE[0]{|}
...
\newcommand\indicesop[1]{\hyperlink{def-listrange}{\textfunc{indices}}(#1)}
...
\newcommand\listset[1]{\hyperlink{def-listset}{\textfunc{list\_set}}\left({#1}\right)}
\newcommand\bindings[1]{\hyperlink{relation-bindings}{\textfunc{bindings}}\left({#1}\right)}
\newcommand\bindingstomap[1]{\hyperlink{def-bindingstomap}{\textfunc{bindings\_to\_map}}\left({#1}\right)}
\newcommand\stringofintname[0]{\hyperlink{relation-stringofint}{\texttt{string\_of\_int}}}
\newcommand\stringofint[1]{\stringofintname(#1)}
\newcommand\restrictmapop[3][H]{
  {#2}\hyperlink{def-restrictfunc}{|}_{#3}
}
\newcommand\restrictfunc[2]{
  {#1}\hyperlink{def-restrictfunc}{|}_{#2}
}
...
\newcommand\Npos[0]{\hyperlink{type-Npos}{\mathbb{N}^{\text{pos}}}}
...
\newcommand\domop[1]{\dom\left({#1}\right)}
\newcommand\sign[0]{\hyperlink{def-sign}{\texttt{sign}}}
\newcommand\signop[1]{\sign\left({#1}\right)}
...
\newcommand\Tbimpl[0]{\verbatimterminal{BIMPL}{==>}}
...
\newcommand\priority[1]{\textsf{priority: }#1}
...
\newcommand\Ncasepattern[0]{\hyperlink{def-ncasepattern}{\nonterminal{case\_pattern}}}
...
\newcommand\nonassoc[0]{\textsf{non-associative}}
...
\newcommand\typedLESetFields[0]{\hyperlink{ast-typedLESetFields}{\textsc{LE\_Set\_Fields}}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\newcommand\BuildError[0]{\hyperlink{type-builderror}{\textsf{BuildError}}}
...
\newcommand\buildcasepattern[0]{\hyperlink{build-casepattern}{\textfunc{build\_case\_pattern}}} % SPECIFICATION_DEFERRED
...
\newcommand\binoppriority[0]{\hyperlink{build-binopprec}{\textfunc{binop\_priority}}} % SPECIFICATION_DEFERRED
\newcommand\checknotsamepriority[0]{\hyperlink{build-checknotsamepriority}{\textfunc{check\_not\_same\_priority}}} % SPECIFICATION_DEFERRED
...
\newcommand\RElinechar[0]{\hyperlink{def-relinechar}{\texttt{<}\textsf{line\_char}\texttt{>}}}
\newcommand\RElineterm[0]{\hyperlink{def-relineterm}{\texttt{<}\textsf{line\_term}\texttt{>}}}
...
\newcommand\REunescapedstringchar[0]{\hyperlink{def-reunescapedstringchar}{\texttt{<}\textsf{unescaped\_string\_char}\texttt{>}}}
\newcommand\REstringescape[0]{\hyperlink{def-restringescape}{\texttt{<}\textsf{string\_escape}\texttt{>}}}
\newcommand\REstringelement[0]{\hyperlink{def-restringelement}{\texttt{<}\textsf{string\_element}\texttt{>}}}
...
\newcommand\truetolit[0]{\hyperlink{def-truetolit}{\textfunc{true\_lit}}} % NO_SPECIFICATION_REQUIRED
\newcommand\falsetolit[0]{\hyperlink{def-falsetolit}{\textfunc{false\_lit}}} % NO_SPECIFICATION_REQUIRED
...
\newcommand\vaddlinefeed[0]{\texttt{add\_line\_feed}}
\newcommand\linefeed[0]{\makelist{\ascii{10}}}
...
\newcommand\discardcommentelement[0]{\hyperlink{def-discardcommentelement}{\textfunc{discard\_comment\_element}}} % NO_SPECIFICATION_REQUIRED
...
% Fields
% Macros for uniquely-named record fields
\newcommand\staticenvsG[0]{\hyperlink{recordfield-staticenvsG}{\textsf{G}}}
\newcommand\staticenvsL[0]{\hyperlink{recordfield-staticenvsL}{\textsf{L}}}
\newcommand\dynamicenvsG[0]{\hyperlink{recordfield-dynamicenvsG}{\textsf{G}}}
\newcommand\dynamicenvsL[0]{\hyperlink{recordfield-dynamicenvsL}{\textsf{L}}}
\newcommand\arrayvalue[0]{\hyperlink{recordfield-arrayvalue}{\textsf{value}}}
\newcommand\casealtpattern[0]{\hyperlink{recordfield-casealtpattern}{\textsf{pattern}}}
\newcommand\casealtstmt[0]{\hyperlink{recordfield-casealtstmt}{\textsf{stmt}}}
\newcommand\globaldeclname[0]{\hyperlink{recordfield-globaldeclname}{\textsf{name}}}
\newcommand\globaldeclty[0]{\hyperlink{recordfield-globaldeclty}{\textsf{ty}}}
\newcommand\globalstaticenvsexprequiv[0]{\hyperlink{recordfield-globalexprequiv}{\textsf{expr\_equiv}}}
\newcommand\localstaticenvsexprequiv[0]{\hyperlink{recordfield-localexprequiv}{\textsf{expr\_equiv}}}
\newcommand\localstaticenvsreturntype[0]{\hyperlink{recordfield-localreturntype}{\textsf{return\_type}}}
\newcommand\funcbody[0]{\hyperlink{recordfield-funcbody}{\textsf{body}}}
\newcommand\funcsubprogramtype[0]{\hyperlink{recordfield-funcsubprogramtype}{\textsf{subprogram\_type}}}
\newcommand\funcbuiltin[0]{\textsf{builtin}}
\newcommand\funcqualifierfield[0]{\textsf{qualifier}}
\newcommand\funcoverride[0]{\textsf{override}}

\newcommand\GDkeyword[0]{\textsf{keyword}}
\newcommand\GDname[0]{\textsf{name}}
\newcommand\GDty[0]{\textsf{ty}}
\newcommand\GDinitialvalue[0]{\textsf{initial\_value}}
\newcommand\CasePattern[0]{\textsf{pattern}}
\newcommand\CaseWhere[0]{\textsf{where}}
\newcommand\CaseStmt[0]{\textsf{stmt}}
\newcommand\Forindexname[0]{\textsf{index\_name}}
\newcommand\Forstarte[0]{\textsf{start\_e}}
\newcommand\Fordir[0]{\textsf{dir}}
\newcommand\Forende[0]{\textsf{end\_e}}
\newcommand\Forbody[0]{\textsf{body}}
\newcommand\Forlimit[0]{\textsf{limit}}
\newcommand\EArrayLength[0]{\textsf{length}}
\newcommand\EArrayValue[0]{\textsf{value}}
\newcommand\callname[0]{\hyperlink{recordfield-callname}{\textsf{name}}}
\newcommand\callparams[0]{\textsf{params}}
\newcommand\callargs[0]{\hyperlink{recordfield-callargs}{\textsf{args}}}
\newcommand\callcalltype[0]{\textsf{call\_type}}
\newcommand\lhsaccessaccess[0]{\textsf{access}}
\newcommand\lhsaccessslices[0]{\textsf{slices}}
\newcommand\accessorpairisreadonly[0]{\textsf{is\_readonly}}
\newcommand\accessorpairgetter[0]{\textsf{getter}}
\newcommand\accessorpairsetter[0]{\textsf{setter}}

\newcommand\itemprefix[0]{\texttt{item}}

\newcommand\True[0]{\hyperlink{type-True}{\texttt{TRUE}}}
\newcommand\False[0]{\hyperlink{type-False}{\texttt{FALSE}}}
\newcommand\CheckResult[0]{\{\True\}}
...
\newcommand\emptytenv[0]{\hyperlink{constant-emptytenv}{\OldEmptyset_{\mathbb{SE}}}}
...
\newcommand\Effect[0]{\hyperlink{def-effect}{\textsf{Effect}}}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Semantics macros
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand\dynamicenvironmentterm[0]{\hyperlink{type-dynamicenvs}{dynamic environment}}
\newcommand\globaldynamicenvironmentterm[0]{\hyperlink{type-globaldynamicenvs}{global dynamic environment}}
\newcommand\localdynamicenvironmentterm[0]{\hyperlink{type-localdynamicenvs}{local dynamic environment}}
\newcommand\environmentterm[0]{\hyperlink{type-envs}{environment}}
...
\newcommand\bitvectordomainop[1]{\hyperlink{relation-bitvectordomain}{\mathsf{BitvectorDomain}}\left(#1\right)}
\newcommand\vectordomainop[1]{\hyperlink{relation-vectordomain}{\mathsf{VectorDomain}}\left(#1\right)}
\newcommand\arraydomainname[0]{\hyperlink{relation-arraydomain}{\mathsf{ArrayDomain}}}
\newcommand\arraydomainop[3][H]{
  \ifthenelse{\equal{#1}{H}}{
    \arraydomainname\left({#2}, {#3}\right)
  }{
    \arraydomainname
    \left(\begin{array}{l}
    {#2},\\
...
\newcommand\recorddomainname[0]{\hyperlink{relation-recorddomain}{\mathsf{RecordDomain}}}
\newcommand\recorddomainop[3][H]{
  \ifthenelse{\equal{#1}{H}}{
    \recorddomainname\left({#2}, {#3}\right)
  }{
    \recorddomainname
    \left(\begin{array}{l}
    {#2},\\
...
\newcommand\emptydenv[0]{\hyperlink{constant-emptydenv}{\OldEmptyset_{\mathbb{DE}}}}
...
\newcommand\emptygraph[0]{{\hyperlink{constant-emptygraph}{\OldEmptyset_\xgraph}}}
\newcommand\asldata[0]{\hyperlink{type-asldata}{\mathtt{asl\_data}}}
\newcommand\aslctrl[0]{\hyperlink{type-aslctrl}{\mathtt{asl\_ctrl}}}
\newcommand\aslpo[0]{\hyperlink{type-aslpo}{\mathtt{asl\_po}}}

\newcommand\Nodes[0]{\hyperlink{type-Nodes}{\mathcal{N}}}
\newcommand\Read[0]{\hyperlink{type-Read}{\text{Read}}}
\newcommand\Write[0]{\hyperlink{type-Write}{\text{Write}}}
\newcommand\Labels[0]{\hyperlink{type-Labels}{\mathcal{L}}}
\newcommand\XGraphs[0]{\hyperlink{type-XGraphs}{\mathcal{G}}}
\newcommand\WriteEffect[0]{\hyperlink{relation-writeeffect}{\textsf{WriteEffect}}}
\newcommand\ReadEffect[0]{\hyperlink{relation-readeffect}{\textsf{ReadEffect}}}
\newcommand\returnvarprefix[0]{\texttt{"return-"}}
\newcommand\dotstr[0]{\texttt{"."}}
\newcommand\emptyenv[0]{\hyperlink{def-emptyenv}{\OldEmptyset_{\mathbb{E}}}}
...
\newcommand\checktypesatisfies[0]{\hyperlink{relation-checktypesatisfies}{\textfunc{check\_type\_satisfies}}} % SPECIFIED
...
\newcommand\puritygeq[0]{\hyperlink{relation-gepure}{\geq_{\text{pure}}}}
...
\newcommand\ConstrainedDom[0]{\hyperlink{ast-ConstrainedDom}{\textsc{CDom}}}
...
\newcommand\AbbrevConstraintRangeOp[3][H]{\overbracket{#2..#3}^{\hyperlink{def-abbrevconstraintrange}{\abbrevfont{Constraint\_Range}}}}
\newcommand\AbbrevConstraintExactOp[1]{\overbracket{#1}^{\hyperlink{def-abbrevconstraintexact}{\abbrevfont{Constraint\_Exact}}}}
...
\newcommand\TIntWellConstrainedInt[1]{\TInt(\WellConstrained(#1))}
...
\newcommand\Proserenamelocalsname[2]{\hyperlink{def-renamelocalsname}{renaming}
  the identifier #1 yields the identifier #2}
...
\newcommand\Proserenamelocalsargs[2]{\hyperlink{def-renamelocalsargs}{renaming}
  the list of local arguments #1 yields the list of arguments #2}
...
%% Glossary
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand\shiftreduceconflictterm[0]{\hyperlink{def-shiftreduceconflict}{shift-reduce conflict}}
\newcommand\shiftreduceconflictsterm[0]{\hyperlink{def-shiftreduceconflict}{shift-reduce conflicts}}
\newcommand\ContinuingTerm[0]{\hyperlink{type-Continuing}{continuing result configuration}}
\newcommand\ProseMSB[0]{MSB}
\newcommand\ProseLSB[0]{LSB}
\newcommand\configurationterm[0]{\hyperlink{def-configuration}{configuration}}
\newcommand\configurationsterm[0]{\hyperlink{def-configuration}{configurations}}
\newcommand\semanticsconfigurationterm[0]{\hyperlink{def-semanticconfiguration}{semantics configuration}}
\newcommand\Semanticsconfigurationterm[0]{\hyperlink{def-semanticconfiguration}{Semantics configuration}}
\newcommand\semanticsconfigurationsterm[0]{\hyperlink{def-semanticconfiguration}{semantics configurations}}
\newcommand\Semanticsconfigurationsterm[0]{\hyperlink{def-semanticconfiguration}{Semantics configurations}}
...
\newcommand\astlabelterm[0]{\hyperlink{type-ASTLabels}{AST label}}
...
\newcommand\enumerationtypesterm[0]{\hyperlink{enumerationtypeterm}{enumeration types}} % DO NOT LINT
...
\newcommand\symbolicallysimplifyingterm[0]{\hyperlink{def-symbolicallysimplifies}{symbolically simplifying}}
...
\newcommand\executiongraphsterm[0]{\hyperlink{def-executiongraph}{execution graphs}}
...
\newcommand\catcherterm[0]{\hyperlink{def-catcherterm}{catcher}}
\newcommand\catchersterm[0]{\hyperlink{def-catcherterm}{catchers}}
...
\newcommand\slicelexpr[2]{\hyperlink{def-slicelexprterm}{assignable slicing expression} for the assignable expression #1 and list of slices #2}
...
\newcommand\setfieldslexpr[2]{\hyperlink{def-setfieldlexprterm}{assignable multi-field expression} for the assignable record base expression #1 and list of field identifiers #2}
...
\newcommand\printstatement[2]{\hyperlink{def-printstatementterm}{print statement} with list of expressions #1 and add-line-feed flag #2}
...
\newcommand\sideeffectfreeterm[0]{\hyperlink{def-sideeffectfreeexpressionterm}{side-effect-free}}
\newcommand\sideeffectfreeexpressionsterm[0]{\hyperlink{def-sideeffectfreeexpressionterm}{side-effect-free expressions}}
...
\newcommand\CannotOverapproximate[0]{\hyperlink{type-CannotOverapproximate}{\overline{\textsf{Approx}}}}
\newcommand\CannotUnderapproximate[0]{\hyperlink{type-CannotUnderapproximate}{\underline{\textsf{Approx}}}}
% TODO: define a nicer approximation with a hashtag-style notation.
