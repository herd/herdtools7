...
\begin{mathpar}
\inferrule{
  \annotateexprlist(\tenv, \vcall.\callargs) \typearrow \vargs \OrTypeError\\\\
  \annotateexprlist(\tenv, \vcall.\callparams) \typearrow \params \OrTypeError\\\\
  {
    \begin{array}{r}
  \annotatecallactualstyped(\tenv, \vcall.\callname, \params, \vargs, \vcall.\callcalltype) \typearrow \\
...
\begin{itemize}
  \item applying $\unziplistthree$ to $\typedargs$ yields the corresponding list of types $\argtypes$,
        list of expressions $\vargs$, and a list of \sideeffectdescriptorsetsterm\ $\vsessargs$;
  ...
  \item checking that the lengths of $\funcsig.\FIELDargs$ and $\vargs$ are the same yields $\True$\ProseTerminateAs{\BadCall};
...
\begin{mathpar}
\inferrule{
  \unziplistthree(\typedargs) = (\argtypes, \vargs, \vsessargs)\\
  ...
  \equallength(\funcsig.\FIELDparameters, \paramsone) \typearrow \paramaritymatch \\\\
  \techeck(\paramaritymatch, \BadCall) \typearrow \True \OrTypeError\\\\
  \equallength(\funcsig.\FIELDargs, \vargs) \typearrow \aritymatch\\
  \techeck(\aritymatch, \BadCall) \typearrow \True \OrTypeError\\\\
  \checkparamstypesat(\tenv, \funcsig.\FIELDparameters, \paramsone) \typearrow \True \OrTypeError\\\\
  \eqs \eqdef [(\vx_i, \Ignore) \in \funcsig.\funcsigparams_i ,\; (\Ignore, \vv_i, \Ignore) \in \paramsone: (\vx_i, \vv_i) ] \\
  \checkargstypesat(\tenv, \funcsig.\FIELDargs, \argtypes, \eqs) \typearrow \True \OrTypeError\\\\
...
\inferrule[other]{
  ...
  \checktypesatisfies(\tenv, \tyactual, \tydecl) \typearrow \True \OrTypeError\\\\
  \checkparamstypesat(\tenv, \funcsigparamsone, \paramsone) \typearrow \True \OrTypeError
}{
  \checkparamstypesat(\tenv, \funcsigparams, \params) \typearrow \True
}
...
\inferrule[non\_empty]{
  ...
  \checktypesatisfies(\tenv, \tyactual, \tydeclp) \typearrow \True\OrTypeError\\\\
...
\inferrule[candidates\_exist]
{
  ...
  \hasargclash(\tenv, \formaltypes, \funcdef.\FIELDargs) \typearrow \vbone \OrTypeError\\\\
...
\begin{itemize}
  ...
  \item evaluating each expression in $\vargs$ separately in $\envone$ as per \\
...
\inferrule[normal]{
  ...
  \evalexprlistm(\envone, \vargs) \evalarrow \ResultExprListM(\vvargs, \envtwo)\OrAbnormal\\\\
  \envtwo \eqname (\tenv, \denvtwo)\\
  \incrpendingcalls(\denvtwo.\dynamicenvsG, \name) \evalarrow \genv\\\\
  \envtwo' \eqdef (\tenv, (\genv, \emptyfunc))\\\\
  \commonprefixline\\\\
  {\begin{array}{r}\evalsubprogram(\envtwo', \name, \vvparams, \vvargs) \evalarrow \\ \ResultCall(\vms, (\vglobal, \Ignore)) \OrDynErrorDiverging\end{array}}\\\\
  ...
  \evalcall(\env, \name, \params, \vargs) \evalarrow \ResultCall(\vmstwo, \newenv)
}
\end{mathpar}

\begin{mathpar}
\inferrule[throwing]{
  \evalexprlistm(\env, \vargs) \evalarrow \ResultExprListM(\vvargs, \envone)\OrAbnormal\\\\
  \evalexprlistm(\envone, \params) \evalarrow \ResultExprListM(\vvparams, \envtwo)\OrAbnormal\\\\
  \envtwo \eqname (\tenv, \denvtwo)\\
  \incrpendingcalls(\denvtwo.\dynamicenvsG, \name) \evalarrow \genv\\\\
  \envtwo' \eqdef (\tenv, (\genv, \emptyfunc))\\\\
  \commonprefixline\\\\
  {\begin{array}{r}\evalsubprogram(\envtwo', \name, \vvparams, \vvargs) \evalarrow \\ \Throwing(\vv, \vt, \vg, \envthrow) \OrDynErrorDiverging\end{array}}\\\\
  \envthrow \eqname (\tenv, (\vglobal, \Ignore))\\
  \decrpendingcalls(\vglobal, \name) \evalarrow \genvtwo\\
  \newenv \eqdef (\tenv, \{ \dynamicenvsG: \genvtwo, \dynamicenvsL: \denvtwo.\dynamicenvsL\})
}{
  \evalcall(\env, \name, \params, \vargs) \evalarrow \Throwing(\vv, \vt, \vg, \newenv)
}
\end{mathpar}
...
\begin{itemize}
  ...
  \item finding the function named $\name$ in $\tenv$ (via the $\subprograms$ component of the static global environment of $\tenv$)
        ...
        $\vbody$, parameters $\paramdecls$, arguments $\vargdecls$, and optional recursion limit expression $\vrecurselimit$;
  ...
  \item \Proseeqdef{$\argnames$}{the identifiers appearing as the first component of each pair in $\vargdecls$};
  \item assigning the actual arguments with $((\envone, \emptygraph), \argnames, \vargs)$
        as per \SemanticsRuleRef{AssignArgs} gives $(\envtwo, \vgtwo)$ and ensures that each
        formal argument in $\vargdecls$ is
        locally bound to the corresponding actual value in $\vargs$;
...
\inferrule{
  ...
  \argnames \eqdef [(\vx, \Ignore) \in \vargdecls: \vx]\\
