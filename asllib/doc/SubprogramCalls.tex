...
\begin{itemize}
  \item $\vcallp$ --- the updated call, with all arguments/parameters annotated and $\vcall.\callname$ updated to uniquely identify the call among the set of overloading subprograms declared with the same name;
...
\begin{itemize}
  ...
  \item applying $\annotatecallactualstyped$ to $\vcall.\callname$, $\params$, $\args$, and $\vcall.\callcalltype$ in $\tenv$ yields
        $(\vcallp, \retty)$\ProseOrTypeError.
\end{itemize}
...
All of the following apply:
\begin{itemize}
  \item applying $\unziplist$ to $\typedargs$ yields the corresponding lists of types
        and expressions $\argtypes$ and $\args$;
  \item applying $\subprogramforname$ to match $\name$ and $\argtypes$ in $\tenv$
        yields \\ $(\namep, \funcsig)$\ProseOrTypeError;
  ...
  \item define $\vcall$ as the call with name $\namep$, parameters taken from $\paramsone$, arguments $\vargs$, and call type $\funcsig.\funcsubprogramtype$.
\end{itemize}
\subsubsection{Formally}
\begin{mathpar}
\inferrule{
  \unziplist(\typedargs) = (\argtypes, \args)\\
    \subprogramforname(\tenv, \name, \argtypes) \typearrow
  (\nameone, \funcsig) \OrTypeError \\
  {
    \vb \eqdef
    \left(
    \begin{array}{l}
  ...
  \eqs \eqdef [(\vx_i, \Ignore) \in \funcsig.\funcargs_i ,\; (\Ignore, \vv_i) \in \paramsone: (\vx_i, \vv_i) ] \\
  ...
  \left(\overname{\left\{
      \begin{array}{rcl}
        \callname &:& \namep,\\
        \callparams &:& [(\Ignore, \vv_i) \in \paramsone: \vv_i],\\
        \callargs &:& \args,\\
...
\insertstdlibparam(
  ...
  \overname{(\ty\times\expr)^*}{\paramsone}
\]
...
\begin{itemize}
  ...
  \item $\paramsone$ is the \constrainedinteger{} for $\ve$ appended to $\params$\ProseTerminateAs{\params}.
\end{itemize}
...
\inferrule{
  ...
  \paramsone \eqdef \params \concat [(\TInt(\wellconstrained(\AbbrevConstraintExact{\ve})), \ve)] \; \terminateas \params
}{
...
\checkparamstypesat(
  ...
  \overname{(\ty\times\expr)^*}{\params}
  ) \aslto \\
  \{\True\}\ \cup \overname{\TTypeError}{\TypeErrorConfig}
\]
checks that the typed actual parameters $\params$ are correct with respect to the declared parameters $\funcsigparams$.
\ProseOtherwiseTypeError
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\funcsigparams$ is a non-empty list with \head\ $(\vx, \tydeclopt)$ and \tail\ \\
          $\funcsigparamsone$, and
          $\params$ is a non-empty list with \head\ $(\tyactual, \eactual)$ and \tail\ $\paramsone$;
    \item checking that $\eactual$ is \staticallyevaluable{} yields $\True$\ProseOrTypeError;
...
\inferrule[parameterized]{
  ...
  \params \eqname [(\tyactual, \eactual)] \concat \paramsone\\
  \checkstaticallyevaluable(\tenv, \eactual) \typearrow \True\OrTypeError\\\\
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item define $\newenv$ as the environment where the static environment $\tenv$ and the dynamic environment consists
    ...
    \begin{itemize}
      ...
      \item view $\envthrow$ as $(\tenv, \denvthrow)$;
      \item define $\newenv$ as the environment where the static environment is $\tenv$
            and the global dynamic environment is the global dynamic environment of $\denvthrow$ and the
            local dynamic environment is the local dynamic environment of $\denvtwo$;
      \item the entire evaluation results in $\Throwing(\vv, \newenv)$.
    \end{itemize}
  \end{itemize}
\end{itemize}
...
\inferrule[throwing]{
  ...
  \newenv \eqdef (\tenv, (G^\denvthrow, L^{\denvtwo}))
}{
...
\begin{itemize}
  ...
  \item finding the function named $\name$ in $\tenv$ (via the $\subprograms$ component of the static global environment of $\tenv$)
        ...
        $\SBASL(\vbody)$, parameters $\paramdecls$, and arguments $\argdecls$;
