...
\begin{mathpar}
\inferrule{
  \annotateexprlist(\tenv, \vcall.\callargs) \typearrow \args \OrTypeError\hva\\\\
  \annotateexprlist(\tenv, \vcall.\callparams) \typearrow \params \OrTypeError\hva\\\\
  {
    \begin{array}{r}
...
\begin{itemize}
  ...
  \item checking that the lengths of $\funcsig.\FIELDparameters$ and $\paramsone$ are the same yields $\True$\ProseTerminateAs{\BadCall};
  \item checking that the lengths of $\funcsig.\FIELDargs$ and $\args$ are the same yields $\True$\ProseTerminateAs{\BadCall};
  \item applying $\checkparamstypesat$ to $\paramsone$
        to check that the actual parameters have correct types with respect to $\funcsig.\FIELDparameters$
        in $\tenv$ yields $\True$\ProseOrTypeError;
  \item define $\eqs$ as the association of declared parameter names in $\funcsig.\FIELDparameters$ with actual parameters $\paramsone$;
  \item applying $\checkargstypesat$ to $\argtypes$ and $\eqs$
        to check that the actual arguments have correct types with respect to $\funcsig.\FIELDargs$
        in $\tenv$ yields $\True$\ProseOrTypeError;
  \item applying $\annotateretty$ to $\eqs$, $\calltype$, and $\funcsig.\FIELDreturntype$
...
\inferrule{
  ...
  \vsesargs \eqdef \bigcup \vsessargs \hva\\
  {
    \begin{array}{r}
  ...
  \insertstdlibparam(\funcsig, \params, \argtypes) \typearrow \paramsone\hva\\\\
  \equallength(\funcsig.\FIELDparameters, \paramsone) \typearrow \paramaritymatch \hva\\\\
  \techeck(\paramaritymatch, \BadCall) \typearrow \True \OrTypeError\hva\\\\
  \equallength(\funcsig.\FIELDargs, \args) \typearrow \aritymatch\\
  \techeck(\aritymatch, \BadCall) \typearrow \True \OrTypeError\hva\\\\
  \checkparamstypesat(\tenv, \funcsig.\FIELDparameters, \paramsone) \typearrow \True \OrTypeError\hva\\\\
  \eqs \eqdef [(\vx_i, \Ignore) \in \funcsig.\funcsigparams_i ,\; (\Ignore, \vv_i, \Ignore) \in \paramsone: (\vx_i, \vv_i) ] \\
  \checkargstypesat(\tenv, \funcsig.\FIELDargs, \argtypes, \eqs) \typearrow \True \OrTypeError\hva\\\\
  \annotateretty(\tenv, \calltype, \funcsig.\FIELDreturntype, \eqs) \typearrow \rettyopt \OrTypeError
}{
  {
    \begin{array}{r}
...
\RenderRule{annotate_call_actuals_typed}
...
\inferrule[can\_insert]{
  {
    \caninsertstdlibparam \eqdef \left(\begin{array}{ll}
      ...
      \listlen{\params} < \listlen{\funcsig.\FIELDparameters} & \land\\
      \argtypes \neq \emptylist &
    \end{array}\right)
  }\\
  \caninsertstdlibparam = \True\hva\\\\
  \argtypes \reverseeqdef [\vt] \concat \Ignore\\
  \getbitvectorwidth(\tenv, \vt) \typearrow \vwidth \OrTypeError\hva\\\\
...
\RenderRule{insert_stdlib_param}
...
\begin{mathpar}
\inferrule{
  \funcsig.\funcbuiltin\hva\\\\
  {
    \declaredparam \eqdef \begin{cases}
      \some{\vn} & \text{if }\funcsig.\FIELDparameters = [(\vn, \Ignore)]\\
      \some{\vn} & \text{if }\funcsig.\FIELDparameters = [\Ignore, (\vn, \Ignore)]\\
      \None      & \text{otherwise}
    \end{cases}
  }\\
  \vb \eqdef \declaredparam = \some{\vn} \land
  \funcsig.\FIELDargs = (\Ignore, \TBits(\EVar(\vn), \Ignore))
}{
...
\inferrule[parameterized]{
  ...
  \params = [(\tyactual, \eactual, \vsesactual)] \concat \paramsone\hva\\
  \checksymbolicallyevaluable(\vsesactual) \typearrow \True\OrTypeError\hva\\\\
  \checkconstrainedinteger(\tenv, \tyactual) \typearrow \True\OrTypeError\hva\\\\
  \commonprefixline\hva\\\\
  \tydeclopt = \some{\TInt(\Parameterized(\vx))}\hva\\\\
...
\inferrule[other]{
  ...
  \checkconstrainedinteger(\tenv, \tyactual) \typearrow \True\OrTypeError\hva\\\\
  \commonprefixline\hva\\\\
  \tydeclopt \reverseeqdef \some{\tydecl}\\
  \tydecl \neq \TInt(\Parameterized(\vx))\hva\\
  \checktypesatisfies(\tenv, \tyactual, \tydecl) \typearrow \True \OrTypeError\hva\\\\
  \checkparamstypesat(\tenv, \funcsigparamsone, \paramsone) \typearrow \True \OrTypeError
}{
  \checkparamstypesat(\tenv, \funcsigparams, \params) \typearrow \True
}
...
\RenderRule{check_params_typesat}
...
\RenderRule{subst_expr_normalize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item applying $\assocoptname$ to $\vs$ and $\substs$ yields the expression $\newe$.
  ...
  \begin{itemize}
    ...
    \item applying $\assocoptname$ to $\vs$ and $\substs$ yields $\None$.
  ...
  \begin{itemize}
    ...
    \item define $\newe$ as the tuple expression of expressions $\vesp$ ($\ETuple(\vesp)$).
  \end{itemize}
...
\inferrule[e\_getenumarray]{
  \ve = \EGetEnumArray(\veone, \vetwo)\\
  \substexpr(\tenv, \substs, \veone) \typearrow \veonep\\
  \substexpr(\tenv, \substs, \vetwop) \typearrow \vetwop
}{
  \substexpr(\tenv, \substs, \ve) \typearrow \overname{\EGetEnumArray(\veonep, \vetwop)}{\newe}
}
\end{mathpar}
...
\inferrule[e\_getfields]{
  \ve = \EGetFields(\veone, \fields)\\
  \substexpr(\tenv, \substs, \veone) \typearrow \veonep
}{
  \substexpr(\tenv, \substs, \ve) \typearrow \overname{\EGetFields(\veonep, \fields)}{\newe}
}
\end{mathpar}
...
\inferrule[exact]{
  ...
  \substconstraint(\tenv, \eqs, \vc) \typearrow \overname{\ConstraintExact(\newe)}{\newc}
}
\hva\and
\inferrule[range]{
  ...
  \substconstraint(\tenv, \eqs, \vc) \typearrow \overname{\ConstraintRange(\veonep, \vetwop)}{\newc}
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{subst_constraint}
...
\inferrule[non\_empty]{
  ...
  \renametyeqs(\tenv, \eqs, \tydecl) \typearrow \tydeclp\OrTypeError\hva\\\\
  \checktypesatisfies(\tenv, \tyactual, \tydeclp) \typearrow \True\OrTypeError\hva\\\\
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\tenv$ does not contain a binding for $\name$ in the $\overloadedsubprograms$ map
          ($\tenv.\staticenvsG.\overloadedsubprograms$);
...
\inferrule[undefined]{
  ...
  {
  \subprogramforsignature\left( \begin{array}{l}\tenv,\\ \name,\\ \callerargtypes,\\ \calltype\end{array} \right) \typearrow \TypeErrorVal{\UndefinedIdentifier}
  }
}
\end{mathpar}
...
\inferrule[no\_candidates]{
  ...
  {
  \subprogramforsignature\left( \begin{array}{l}\tenv,\\ \name,\\ \callerargtypes,\\ \calltype\end{array} \right) \typearrow \TypeErrorVal{\BadCall}
  }
}
\end{mathpar}
...
\inferrule[one\_candidate]{
  ...
  {
  \subprogramforsignature\left( \begin{array}{l}\tenv,\\ \name,\\ \callerargtypes,\\ \calltype\end{array} \right) \typearrow (\namep, \callee, \vses)
  }
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{subprogram_for_signature}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item determining whether there is an argument clash between $\formaltypes$ and the arguments in $\funcdef$
          (that is, $\funcdef.\FIELDargs$) yields $\vbone$\ProseOrTypeError;
...
{
  \funcdef \eqdef \tenv.\staticenvsG.\subprograms(\name) \\
  \hasargclash(\tenv, \formaltypes, \funcdef.\FIELDargs) \typearrow \vbone \OrTypeError\hva\\\\
  \calltypematches(\funcdef, \calltype) \typearrow \vbtwo \hva\\\\
  {
  \begin{array}{r}
...
\RenderRule{filter_call_candidates}
...
\begin{mathpar}
\inferrule{
  \equallength(\formaltypes, \vargs) \typearrow \True \terminateas \False\hva\\\\
...
\inferrule[simple]{
  ...
  \issubtype(\tenv, \vt, \vs) \typearrow \False\hva\\\\
  \tstruct(\tenv, \vt) \typearrow \vtstruct \OrTypeError \\
  \tstruct(\tenv, \vs) \typearrow \vsstruct \OrTypeError \hva\\
...
\inferrule[t\_tuple]{
  ...
  \tstruct(\tenv, \vs) \typearrow \TTuple(\vs_{1..n}) \hva\\
...
\begin{mathpar}
\inferrule[non\_empty]{
  \annotateexpr(\tenv, \ve) \typearrow \vtypedexpr \OrTypeError\hva\\\\
...
\begin{itemize}
  ...
  \item applying $\incrpendingcalls$ to $\denvtwo.\dynamicenvsG$ and $\name$ yields $\genv$;
...
\begin{mathpar}
\inferrule[normal]{
  \evalexprlistm(\env, \params) \evalarrow \ResultExprListM(\vvparams, \envone)\OrAbnormal\hva\\\\
  \evalexprlistm(\envone, \args) \evalarrow \ResultExprListM(\vvargs, \envtwo)\OrAbnormal\hva\\\\
  \envtwo \reverseeqdef (\tenv, \denvtwo)\\
  \incrpendingcalls(\denvtwo.\dynamicenvsG, \name) \evalarrow \genv\hva\\\\
  \envtwo' \eqdef (\tenv, (\genv, \emptyfunc))\hva\\\\
  \commonprefixline\hva\\\\
  {\begin{array}{r}\evalsubprogram(\envtwo', \name, \vvparams, \vvargs) \evalarrow \\ \ResultCall(\vms, (\vglobal, \Ignore)) \OrDynErrorDiverging\end{array}}\hva\\\\
  \vmstwo \eqdef [\vm \in \vms: \readvaluefrom(\vm)]\\
  \decrpendingcalls(\vglobal, \name) \evalarrow \genvtwo\\
  \newenv \eqdef (\tenv, \{\dynamicenvsG: \genvtwo, \dynamicenvsL: \denvtwo.\dynamicenvsL\})
}{
...
\begin{mathpar}
\inferrule[throwing]{
  \evalexprlistm(\env, \args) \evalarrow \ResultExprListM(\vvargs, \envone)\OrAbnormal\hva\\\\
  \evalexprlistm(\envone, \params) \evalarrow \ResultExprListM(\vvparams, \envtwo)\OrAbnormal\hva\\\\
  \envtwo \reverseeqdef (\tenv, \denvtwo)\\
  \incrpendingcalls(\denvtwo.\dynamicenvsG, \name) \evalarrow \genv\hva\\\\
  \envtwo' \eqdef (\tenv, (\genv, \emptyfunc))\hva\\\\
  \commonprefixline\hva\\\\
  {\begin{array}{r}\evalsubprogram(\envtwo', \name, \vvparams, \vvargs) \evalarrow \\ \Throwing(\vv, \vt, \vg, \envthrow) \OrDynErrorDiverging\end{array}}\hva\\\\
  \envthrow \reverseeqdef (\tenv, (\vglobal, \Ignore))\\
  \decrpendingcalls(\vglobal, \name) \evalarrow \genvtwo\\
  \newenv \eqdef (\tenv, \{ \dynamicenvsG: \genvtwo, \dynamicenvsL: \denvtwo.\dynamicenvsL\})
}{
...
\begin{mathpar}
\inferrule{
  \env \reverseeqdef (\tenv, \denv)\\
  {
    \tenv.\staticenvsG.\subprograms(\name) \reverseeqdef \left\{
      \begin{array}{rcl}
      \funcbody &:& \vbody,\\
      \FIELDargs &:& \argdecls,\\
      \FIELDparameters &:& \paramdecls,\\
      \FIELDrecurselimit &:& \vrecurselimit, \\
      &&\ldots
      \end{array}
      \right\}
  }\hva\\
  \envone \eqdef (\tenv, \{\dynamicenvsG: \denv.\dynamicenvsG, \dynamicenvsL: \emptyfunc\})\\
  \checkrecurselimit(\envone, \name, \vrecurselimit) \evalarrow \vgone \OrDynErrorDiverging\hva\\\\
  \argnames \eqdef [(\vx, \Ignore) \in \argdecls: \vx]\\
  \assignargs((\envone, \emptygraph), \argnames, \actualargs) \evalarrow (\envtwo, \vgtwo)\hva\\
  \paramnames \eqdef [(\vx, \Ignore) \in \params: \vx]\\
  \assignargs((\envtwo, \vgtwo), \paramnames, \paramdecls) \evalarrow (\envthree, \vgthree)\hva\\
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item view $\env$ as $(\tenv, \denv)$;
    \item applying $\getpendingcalls$ to $\name$ in $\denv$ yields $\vpendingcalls$;
    \item $\vpendingcalls$ is greater than or equal to $\vlimit$;
...
\begin{mathpar}
\inferrule[some\_ok]{
  \evallimit(\env, \velimitopt) \evalarrow (\some{\vlimit}, \vg) \OrDynErrorDiverging\hva\\\\
  \env \reverseeqdef (\tenv, \denv)\\
...
\RenderRule{check_recurse_limit}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item the given configuration is $\Returning(\xs, \retenv)$. \ProseNote{This is the case of a function};
    \item define the list of strings $\ids$ as the concatenation of \returnvarprefix{} and the decimal string
          for every index of $\xs$;
    \item define $\vvs$ to be the list of pairs obtained by the element-wise pairing of the elements of $\xs$ and $\ids$;
...
\begin{mathpar}
  ...
  \hva\and
  \inferrule[returning]{
    \xs \reverseeqdef [i=1..k: \vv_i]\\
    i \in \listrange(\xs): \ids[i] = \returnvarprefix \concatstrings \stringofint{i-1}\\
