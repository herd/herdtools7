...
\hypertarget{def-assertionerror}{}
\[
\assertionerror(C) \triangleq C \in \{ \DynamicErrorVal{\DynamicAssertionFailure}, \DynamicErrorVal{\DynamicTypeAssertionFailure} \}
\]
...
and $\vetwo$ second, leaving throwing configurations and dynamic errors unchanged:
\[
...
\abstractdynerror(C) \triangleq \begin{cases}
    ...
    \top    & \text{if }\assertionerror(C)\\
    \bot    & \text{else}\\
\end{cases}
\]
That is, evaluating the expressions results with the same set of configurations,
up to the order of values, and ignoring dynamic errors that are not due to assertions.\footnote{
Ignoring dynamic errors that are not due to assertions is a limitation that will be lifted
in a subsequent version of this reference.
}
\end{definition}
...
Intuitively, a \emph{pure} \sideeffectdescriptorsterm\ helps to establish that
...
% Abstraction: This function is not reflected in the implementation as it checks conflicts directly on sets of side effects.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item define $\vb$ as $\True$ if and only if $\vstwo$ is either
              ...
              a \ThrowExceptionTerm, or
              a \RecursiveCallTerm.
    \end{itemize}

    \item All of the following apply (\textsc{exception}):
    \begin{itemize}
        \item $\vsone$ is a \ThrowExceptionTerm;
        \item define $\vb$ as $\True$ if and only if $\vstwo$ is either
                a \ThrowExceptionTerm,
                a \WriteLocalTerm,
    ...
    \begin{itemize}
        ...
        \item define $\vb$ as $\True$ if and only if $\vstwo$ is either
              a \ReadLocal\ for a storage element named $\id$,
              a \WriteLocal\ for a storage element named $\id$, or a
                \RecursiveCallTerm.
    \end{itemize}
...
\subsubsection{Prose}
Define $\vb$ if for every \sideeffectdescriptorterm\ $\vsone$ in $\vsesone$ and
...
\begin{itemize}
    ...
    \begin{itemize}
        \item there exists a \sideeffectdescriptorterm\ in $\vses$ that is either
            a \WriteLocalTerm, a \WriteGlobalTerm, a \ThrowExceptionTerm, a \RecursiveCallTerm, or
    ...
    \begin{itemize}
        ...
        \item define $\vtimeframes$ as the \timeframesterm\ appearing in the \sideeffectdescriptorsterm\
            in $\vreads$;
        \item define $\vf$ as the maximal time frame in $\vtimeframes$ and \timeframeconstant,
                with respect to $\timeframeleq$.
    \end{itemize}
\end{itemize}
...
\inferrule[reads]{
    ...
    \vses = \vreads\\
    \vtimeframes \eqdef \{ \vfp \;|\; \ReadLocal(\Ignore, \vfp, \Ignore) \} \cup \{ \vfp \;|\; \ReadLocal(\Ignore, \vfp, \Ignore) \}\\
...
are \staticallyevaluable.

\subsubsection{Formally}
\begin{mathpar}
\inferrule{
  \vb \eqdef \bigwedge_{\vx\in\vses} \sideeffectisstaticallyevaluable(\vs)
}{
...
\]
tests whether the \NonDeterministic\ \sideeffectdescriptorsterm\ is not included in $\vses$,
yielding the result in $\vb$.
...
The function
\[
  \sesisbefore(\overname{\TSideEffectSet}{\vses} \aslsep \overname{\TTimeFrame}{\vt}) \aslto \overname{\Bool}{\bv}
\]
...
% This abstracts both leq_config_time and leq_constant_time
