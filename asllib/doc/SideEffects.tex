...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\SectionDef[defines the concept of expression \purities{};]
  {Purity}{Purity}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{description}
\item[\SEPure] describes the evaluation of a \pureterm{} construct --- one that can be evaluated during typechecking.

\item[\SEReadonly] describes the evaluation of a \readonlyterm{} construct --- one that can read mutable global storage elements but not modify them.

\item[\SEImpure] describes the evaluation of a construct that is neither \pureterm{} nor \readonlyterm{}.
\end{description}

Formally, \purities{} are totally ordered via $\purityless$ as follows:
\hypertarget{def-purityless}{}
\[
\SEImpure \purityless \SEReadonly \purityless \SEPure
\]
Additionally, we define the greater-than-or-equal ordering as follows:
\hypertarget{relation-gepure}{}
\[
f \puritygeq f' \triangleq \lnot(f \purityless f')
\]

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\SectionDef[defines \sideeffectdescriptorsetsterm, which the type system infers
        for each kind of ASL construct;]
  {SideEffectDescriptors}{Side Effect Descriptors}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{description}
    \item[$\LocalEffect$] is a \LocalEffectTerm{} describing an evaluation of a construct whose minimum \purity{} for local reads/writes is $\vpurity$.
    \hypertarget{def-term-GlobalEffect}{}
    \item[$\GlobalEffect$] is a \GlobalEffectTerm{} describing an evaluation of a construct whose minimum \purity{} for global reads/writes is $\vpurity$.
    \hypertarget{def-term-Immutability}{}
    \item[$\Immutability$] is an \ImmutabilityTerm{} describing an evaluation of a construct that accesses storage elements whose immutability is given by $\vb$ (that is, if $\vb$ is $\True$ then accesses are only to \texttt{constant}, \texttt{config}, or \texttt{let} elements).
\end{description}
...
an expression can be evaluated during typechecking.
...
\RenderProseAndFormally{side_effect_is_pure}
...
\RenderProseAndFormally{side_effect_is_readonly}
...
\RenderProseAndFormally{side_effect_is_symbolically_evaluable}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\SectionDef[defines functions used to construct, modify, and query sets
  ...
  {SideEffectSets}{Side Effect Sets}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\RenderProseAndFormally{ses_ldk}
...
\RenderProseAndFormally{ses_gdk}
...
\RenderProseAndFormally{check_symbolically_evaluable}
...
\RenderProseAndFormally{ses_is_readonly}
...
\RenderProseAndFormally{ses_is_pure}
...
\RenderProseAndFormally{ses_for_subprogram}
