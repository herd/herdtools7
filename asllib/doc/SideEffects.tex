...
\begin{description}
\item[\SEPure] describes the evaluation of a \pureterm{} construct --- one that can be evaluated during type-checking.
...
\AllApply
\begin{itemize}
  \item \Proseeqdef{$\vb$}{$\True$ if and only if $\ldk$ is $\LDKLet$};
...
\begin{itemize}
  ...
  \item \Proseeqdef{$\vb$}{$\False$ if and only if $\gdk$ is $\GDKVar$};
