...
A \symbolicallyevaluable{} expression is compatible with symbolic reduction and equivalence testing (see \chapref{SymbolicEquivalenceTesting}).
...
% BEGIN_GENERATED_ELEMENT(TPurity)
\hypertarget{type-TPurity}{}
\begin{flalign*}
...
\begin{description}
    \item[
    % BEGIN_GENERATED_ELEMENT(SE_Pure)
\hypertarget{constant-SEPure}{} $\SEPure$
    % END_GENERATED_ELEMENT
] describes evaluation of a \pure{} construct, one that can be evaluated at type-checking.

\item[
  % BEGIN_GENERATED_ELEMENT(SE_Readonly)
\hypertarget{constant-SEReadonly}{} $\SEReadonly$
  % END_GENERATED_ELEMENT
] describes evaluation of a \readonly{} construct, one that can read mutable global storage elements but not modify them.

\item[
  % BEGIN_GENERATED_ELEMENT(SE_Impure)
\hypertarget{constant-SEImpure}{} $\SEImpure$
  % END_GENERATED_ELEMENT
] describes evaluation of a construct that is neither \pure{} nor \readonly{}.
\end{description}
...
% BEGIN_GENERATED_ELEMENT(TSideEffect)
\hypertarget{type-TSideEffect}{}\hypertarget{type-LocalEffect}{}
\begin{flalign*}
\TSideEffect \triangleq\ & \LocalEffect(\overtext{\TPurity}{purity})\hypertarget{type-GlobalEffect}{}\\
\cup\ & \GlobalEffect(\overtext{\TPurity}{purity})\hypertarget{type-Immutability}{}\\
\cup\ & \Immutability(\overtext{\Bool}{immutable})
\end{flalign*}
% END_GENERATED_ELEMENT
