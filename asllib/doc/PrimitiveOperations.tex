...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vv$ is a literal native value, that is, $\nvliteral{l}$;
    \item statically evaluating $\op$ on $l$ yields a type error with message $m$;
...
\inferrule[static\_error]{
  \unopliterals(\op, l) \typearrow \TypeErrorVal{m} \\
}{
  \unoprel(\op, \overname{\nvliteral{l}}{\vv}) \evalarrow \TypeErrorVal{m}
}
\end{mathpar}
...
\inferrule[non\_literal]{
  ...
  \unoprel(\op, \vv) \evalarrow \DynamicErrorVal{TypeMismatch}
}
\end{mathpar}

\subsubsection{SemanticsRule.BinopValues \label{sec:SemanticsRule.BinopValues}}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vvone$ is a literal native value, that is, $\nvliteral{l_1}$;
    \item $\vvtwo$ is a literal native value, that is, $\nvliteral{l_2}$;
    \item statically evaluating $\op$ on the literals $l_1$ and $l_2$ yields a type error with message $m$;
    \item the result is a dynamic error with message $m$.
  \end{itemize}
...
\begin{mathpar}
\inferrule[static\_error]{
  \binopliterals(\op, l_1, l_2) \typearrow \TypeErrorVal{m} \\
}{
  \binoprel(\op, \overname{\nvliteral{l_1}}{\vvone}, \overname{\nvliteral{l_2}}{\vvtwo}) \evalarrow \TypeErrorVal{m}
}
\end{mathpar}
...
\inferrule[non\_literal]{
  ...
  \binoprel(\op, \vvone, \vvtwo) \evalarrow \DynamicErrorVal{TypeMismatch}
}
\end{mathpar}
