...
\begin{itemize}
  ...
  and the list of catchers $\catchers$ gives a catcher that declares the name $\name$ and gives $\None$
  (that is, neither of the \texttt{catch} clauses matches the raised exception);
  \item since there is no \texttt{otherwise} clause, the result is $\sm$.
\end{itemize}
\FormallyParagraph
\BackupOriginalRule{
\begin{mathpar}
...
In \listingref{catcher-not-found}, no catch clause is matched for the type of the exception value thrown by \verb|update_and_throw|,
