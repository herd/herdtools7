...
\texthypertarget{def-catcherterm}
Catchers can be used to process thrown exceptions.
ASL allows specifying a list of \emph{\catchersterm{}}.
Each \catcherterm{} starts with the \Twhen{} keyword,
...
A final optional \Totherwise{} \catcherterm{}
catches all exceptions that have not been caught
by a preceding \catcherterm{}.
...
\ExampleDef{Annotating Catchers}
\listingref{annotate-catcher} shows a \trystatementterm{}
with \catchersterm{} for unnamed exception values for the exception types \verb|ExceptionType1| and \verb|ExceptionType3|,
and a \catcherterm{} for the named exception value for the exception type \verb|ExceptionType2|.
\ASLListing{Annotating catcher}{annotate-catcher}{\typingtests/TypingRule.Catcher.asl}
...
\inferrule[none]{
  \vc = (\overname{\None}{\nameopt}, \tty, \vstmt)\\
  \annotatetype(\tenv, \vt) \typearrow (\ttyp, \vsesty) \OrTypeError\hva\\\\
  \checkstructurelabel(\tenv, \ttyp, \TException) \typearrow \True \OrTypeError\hva\\\\
  \annotateblock(\tenv, \vstmt) \typearrow (\newstmt, \vsesblock) \OrTypeError\hva\\\\
...
\inferrule[some]{
  ...
  \annotatetype(\tenv, \vt) \typearrow (\ttyp, \vsesty) \OrTypeError\hva\\\\
  \checkstructurelabel(\tenv, \ttyp, \TException) \typearrow \True \OrTypeError\hva\\\\
  \checkvarnotinenv(\tenv, \name) \typearrow \True \OrTypeError\hva\\\\
...
either by a \catcherterm{} specified via \texttt{catch} or a \catcherterm{} specified via \texttt{otherwise},
the statement associated with that \catcherterm{}, which we will refer to as the \catcherterm{} statement, is evaluated.
It is important to evaluate the \catcherterm{} statement in an environment that includes any updates
...
When the \catcherterm{} is evaluated, the semantics takes the global environment from $\envthrow$
to account for the update to \texttt{g} and the local environment from $\env$ to account for the
updates to the local environment in \texttt{main}, which binds \texttt{x} to $2$.
The evaluation uses this environment to evaluate \texttt{print(x, g)}, resulting in the output \texttt{2 1}.
...
\begin{mathpar}
\inferrule{
  \sm \reverseeqdef \Throwing(\vv, \vvty, \sg, \envthrow)\\
  \env \reverseeqdef (\tenv, \denv)\\
  \envthrow \reverseeqdef (\tenv, \denvthrow)\hva\\
  \findcatcher(\tenv, \vvty, \catchers) \reverseeqdef \some{ (\None, \Ignore, \vs) }\hva\\
...
\RenderRule{eval_catchers_catch}
...
\inferrule{
  ...
  \env \reverseeqdef (\tenv, \denv)\\
  \envthrow \reverseeqdef (\tenv, \denvthrow)\hva\\
  \findcatcher(\tenv, \vvty, \catchers) \reverseeqdef \some{ (\some{\name}, \Ignore, \vs) }\\
  \readvaluefrom(\vv) \evalarrow (\vv, \vgone)\hva\\
  \declarelocalidentifierm(\envthrow, \name, (\vv, \vgone)) \evalarrow (\envtwo, \vgtwo)\\
  \evalblock(\envtwo, \vs) \evalarrow C \OrDynErrorDiverging\hva\\\\
  \envthree \eqdef \environof{C}\\
  \removelocal(\envthree, \name) \evalarrow \envfour\\
  D \eqdef \withenviron{C}{\envfour} \hva\\
...
\RenderRule{eval_catchers_catch_named}
...
\begin{itemize}
  ...
  \item finding the first catcher with the \staticenvironmentterm{} $\tenv$, the exception type $\vvty$,
        and the list of catchers $\catchers$ gives a catcher that declares the name $\name$ and gives $\None$
        (that is, neither of the \catchersterm{} matches the raised exception);
  \item evaluating the \texttt{otherwise} statement $\vs$ in $\envtwo$ as a block (\SemanticsRuleRef{Block})
        does not yield an error configuration $C$\ProseOrDynErrorDiverging;
  \item $\newg$ is the ordered composition of $\sg$ and the graph of $C$,
        with the $\aslpo$ edge;
  \item the result of the entire evaluation is $C$ with its graph substituted with $\newg$.
\end{itemize}
...
\inferrule{
  ...
  \evalblock(\envthrow, \vs) \evalarrow C \OrDynErrorDiverging\hva\\\\
...
\RenderRule{eval_catchers_catch_otherwise}
...
It shows how a \texttt{try} statement (the inner one) may not have a \catcherterm{}
that matches the exception type (\texttt{MyExceptionType1}).
\ASLListing{A catcher that does not match a thrown exception type}{semantics-catchnone}{\semanticstests/SemanticsRule.CatchNone.asl}
...
\begin{itemize}
  ...
  and the list of catchers $\catchers$ gives a catcher that declares the name $\name$ and gives $\None$
  (that is, neither of the \catchersterm{} matches the raised exception);
  \item since there is no \texttt{otherwise} \catcherterm{}, the result is $\sm$.
\end{itemize}
\FormallyParagraph
\BackupOriginalRule{
\begin{mathpar}
...
\RenderRule{eval_catchers_catch_none}
...
\RenderRule{eval_catchers_no_throw}
...
\ExampleDef{Finding a Catcher}
In \listingref{annotate-catcher}, the second \catcherterm{} --- for the exception type ExceptionType2
is matched for the type of the exception value thrown by \verb|update_and_throw|.

In \listingref{catcher-not-found}, no \catcherterm{} is matched for the type of the exception value thrown by \verb|update_and_throw|,
resulting in a \dynamicerrorterm{}.
\ASLListing{Looking for a catcher and failing to find one}{catcher-not-found}{\typingtests/TypingRule.FindCatcher.None.asl}
