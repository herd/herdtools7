         ...
         (new_s: stmt, new_tenv: static_envs, ses: powerset(TSideEffect)) | type_error
{
  ...
  case SAssert {
    s = S_Assert(e);
    annotate_expr(tenv, e) -> (t_e', e', ses_e);
    check( ses_is_readonly(ses_e) ) -> True;
  ...
  case SFor {
    ...
    for_constraints(tenv, start_struct, end_struct, start_e', end_e', die, cs) -> cs;
    ty := T_Int(cs);
  ...
  case SReturn {
    case Error {
      s = S_Return(e_opt);
      b := equal(tenv.L.return_type = None, e_opt = None);
      b = False;
      --
      type_error(TE_BSPD);
    }
...
// render annotate_stmt_SPass = annotate_stmt(SPass);
// render annotate_stmt_SDecl = annotate_stmt(SDecl);
// render annotate_stmt_SDecl_Some = annotate_stmt(SDecl/Some);
