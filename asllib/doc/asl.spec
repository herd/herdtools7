...
=
  ...
  case STry {
    ...
    ses_catchers := UNION_LIST(catchers'); // This is a union of a list of sets
  ...
  case SPrint {
    ...
    // MAP(arg, args): annotate_expr(tenv, arg) -> tys * _ * xs;
    INDEX(i, args): annotate_expr(tenv, args[i]) -> (tys[i], _, xs[i]);
...
render rule annotate_stmt = annotate_stmt(-); // For testing purposes
render rule annotate_stmt_SDecl_Some = annotate_stmt(SDecl.Some); // For testing purposes
