...
constant is_positive_integer_func : (fun Z -> Bool)
{
  "positivity test function",
  math_macro = \ispositiveintegerfunc
};

constant is_non_negative_integer_func : (fun Z -> Bool)
{
  "non-negative test function",
  math_macro = \isnonnegativeintegerfunc
};
...
operator swap_pair[A,B]((A, B)) -> (B, A)
{
  "swaps the elements of a pair",
};

operator decl_func(d: decl) -> func
{
  "extracts the function descriptor from a subprogram declaration",
};

operator local_env_of(tenv: static_envs) -> local_static_envs
{
  "local static environment of {tenv}",
};
...
operator maybe_empty_list_to_non_empty[T](list0(T)) -> list1(T)
{
...
operator list_to_set[T](s: list0(T)) -> powerset(T)
{
...
operator set_minus[T](set1: powerset(T), set2: powerset(T)) -> powerset(T)
{
  "the subtraction of {set2} from {set1}",
  math_macro = \setminusop,
};
...
operator intersection_list[T](list0(powerset(T))) -> powerset(T)
{
  math_macro = \intersectionlistop,
};
...
operator n_pos_to_n(n: N_pos) -> N
{
  "casts {n} to a natural number",
  math_macro = \identityop,
};
...
constant label_D_GlobalStorage : ASTLabels { math_macro = \DGlobalStorage };
constant label_D_TypeDecl : ASTLabels { math_macro = \DTypeDecl };
...
constant discard_var_str : Strings
{
  math_macro = \discardvarstr,
};
...
typedef TNormal
{
    ...
    | ResultCall((values_and_env: list0(value_read_from), gdenv: global_dynamic_envs), g: XGraphs)
    { "call result with value-read effects {values_and_env}, a global dynamic environment {gdenv}, and a an \executiongraphterm{} {g}" }
;
...
typing function list_get_z(indices: list0(Z), idx: Z) ->
         (v: Z) | type_error
{
  "returns the element at {idx} in {indices}, assuming
  {idx} is within bounds.",
  prose_application = "",
} =
  case zero {
    idx = zero;
    indices =: match_cons(v, _);
    --
    v;
  }
  ...
  case positive {
    idx > zero;
    indices =: match_cons(_, rest);
    list_get_z(rest, idx - one) -> v;
    --
    v;
  }
...
typing relation lowest_common_ancestor(tenv: static_envs, t: ty, s: ty) -> (ty: ty) | type_error
{
  ...
  case t_int_wellconstrained {
    ...
    ty := T_Int(typed_WellConstrained(maybe_empty_list_to_non_empty(concat(cs_t, cs_s)), p));
    --
    ty;
  }
         ...
         (s: ty) | type_error
{
  ...
  case neg_t_int_well_constrained {
    ...
    --
    T_Int(typed_WellConstrained(maybe_empty_list_to_non_empty(neg_cs), p));
  }
         ...
         (t: ty) | type_error
{
  ...
  case arith_t_int_wellconstrained {
    ...
    --
    T_Int(typed_WellConstrained(maybe_empty_list_to_non_empty(cs), p));
  }
...
//   op in make_set(SHL, SHR, POW);
//   refine_constraints(approx, tenv, refine_constraint_by_sign(tenv, is_non_negative_integer_func, bot), cs) -> new_cs;
//   te_check(new_cs != empty_list, TE_BO) -> True;
//   --
//   maybe_empty_list_to_non_empty(new_cs);
// }
//
// case greater_than {
//   op in make_set(MOD, DIV, DIVRM);
//   refine_constraints(approx, tenv, refine_constraint_by_sign(tenv, is_positive_integer_func, bot), cs) -> new_cs;
//   te_check(new_cs != empty_list, TE_BO) -> True;
//   --
//   maybe_empty_list_to_non_empty(new_cs);
// }
//
// case no_filtering {
//   op in make_set(SUB, MUL, ADD);
//   --
//   maybe_empty_list_to_non_empty(cs);
// }
;
...
{
  "strongly-connected components of the graph (nodes, edges)",
  math_macro = \SCC,
};
         ...
         (new_decls: list0(decl), new_tenv: static_envs) | type_error
{
  ...
  pragma_lists :=
    list_map(d, decls',
      if d =: D_Pragma(_, _) then make_singleton_list(d) else empty_list
    );
  pragmas := list_flatten(pragma_lists);
  other_lists :=
    list_map(d, decls',
      if d =: D_Pragma(_, _) then empty_list else make_singleton_list(d)
    );
  others := list_flatten(other_lists);
  build_dependencies(others) -> (defs, dependencies);
  reversed_dependencies := list_map(dep, dependencies, swap_pair(dep));
  comps := scc(defs, reversed_dependencies);
  ordered_comps := topological_ordering_comps(comps, reversed_dependencies);
  comp_decls :=
    list_map(c, ordered_comps,
      list_flatten(
        list_map(d, others,
          if member(def_decl(d), c) then make_singleton_list(d) else empty_list
        )
      )
    );
         ...
         (new_decls: list0(decl), new_genv: global_static_envs) | type_error
{
  ...
  INDEX(i, decls: te_check(ast_label(decls[i]) = label_D_Func, TE_BD) -> True);
  funcs := list_map(i, indices(decls), decl_func(decls[i]));
  INDEX(i, funcs: annotate_func_sig(genv, funcs[i]) -> (tenvs[i], fs[i], ses_fs[i]));
  lenvs := list_map(i, indices(funcs), local_env_of(tenvs[i]));
  env_and_fs := list_combine_three(lenvs, fs, ses_fs);
  declare_subprograms(genv, env_and_fs) -> (genv2, env_and_fs2);
  ...
  new_decls := list_map(i, indices(fs2), D_Func(new_fs[i]));
  vsess := list_combine(new_fs, ses_ps);
  with_empty_local(genv2) -> tenv2;
  add_subprogram_decls(tenv2, vsess) -> tenv3;
         ...
         (ids: powerset(def_use_name))
{
  ...
  case decl_func {
    ...
    param_ids := list_map(param_ty, param_types, use_ty(param_ty));
         ...
         (ids: powerset(def_use_name))
{
  ...
  case s_try {
    s =: S_Try(s1, catchers, s2);
    catcher_ids := list_map(c, catchers, use_catcher(c));
    --
    union(use_stmt(s1), union(union_list(catcher_ids), use_stmt(s2)));
  }
         ...
         (env: envs)
{
  ...
  bindings(tenv.static_envs_G.constant_values) =: list_combine(ids, lits);
  storage_values := list_map(lit, lits, NV_Literal(lit));
  storage_bindings := list_combine(ids, storage_values);
  // TODO: use bindings_to_map. This currently fails - probably because the
  // subsumption test uses equality for both domain and range.
  bindings_to_storage(storage_bindings) -> storage;
  global := [storage: storage, pending_calls: bot];
  local := bot;
  denv := [dynamic_envs_G: global, dynamic_envs_L: local];
  --
  (tenv, denv);
;

// Helper for static_env_to_env.
semantics function bindings_to_storage(bindings: list0((Identifier, native_value))) ->
         (storage: partial Identifier -> native_value)
{
  "converts binding pairs into a storage map.",
  prose_application = "",
} =
  case empty {
    bindings = empty_list;
  ...
  case non_empty {
    bindings =: match_cons(binding, binding_tail);
    binding =: (id, v);
    bindings_to_storage(binding_tail) -> storage_for_tail;
    --
    map_update(storage_for_tail, id, v);
  }
;
         ...
         (new_ty: ty) | type_error
{
  ...
  case t_int_wellconstrained {
    ...
    { ([_]) };
    new_constraints1 := maybe_empty_list_to_non_empty(new_constraints);
    --
         ...
         (new_symdoms: list1(symdom))
{
  ...
  new_list := if_then_else(
    ...
    others);
  new_symdoms := maybe_empty_list_to_non_empty(new_list);
  --
  new_symdoms;
;
         ...
         (d: symdom_or_top)
{
  ...
  case int_well_constrained {
    ...
    --
    Subdomains(maybe_empty_list_to_non_empty(ds));
  }
         ...
         (s_opt: option(powerset(Z)))
{
  ...
  case e_cond {
    ...
    s := if approx = Over then union(s2, s3) else intersect(s2, s3);
    --
    some(s);
  }
         ...
         (new_cs: list0(int_constraint))
{
  ...
  case exact_exact {
    ...
    --
    make_singleton_list(AbbrevConstraintExact(AbbrevEBinop(op, a, c)));
  }
  ...
  case range_exact {
    ...
    extpairs =: list_combine(ext_lhs, ext_rhs);
    new_cs :=
      list_map(
        i,
        indices(ext_lhs),
        AbbrevConstraintRange(
          AbbrevEBinop(op, ext_lhs[i], c),
          AbbrevEBinop(op, ext_rhs[i], c)));
    --
    new_cs;
  }
  ...
  case exact_range {
    ...
    new_cs :=
      list_map(
        ...
        indices(ext_lhs),
        AbbrevConstraintRange(
          AbbrevEBinop(op, a, ext_lhs[i]),
          AbbrevEBinop(op, a, ext_rhs[i])));
    --
    new_cs;
  }

  // TODO: finish when list_map allows decomposing its bound element
  //case range_range {
  //  c1 =: Constraint_Range(a, b);
  //  c2 =: Constraint_Range(c, d);
  //  possible_extremities_left(op, a, b) -> extpairs_ab;
  //  possible_extremities_right(op, c, d) -> extpairs_cd;
  //  extpairs_ab =: list_combine(ext_ab_lhs, ext_ab_rhs);
  //  extpairs_cd =: list_combine(ext_cd_lhs, ext_cd_rhs);
  //  extpairs_ab := list_combine(ext_a, ext_b);
  //  extpairs_cd := list_combine(ext_c, ext_d);
//
  //  list_cross(ext_a, ext_c)
//
  //  new_cs :=
  //    list_map(
  //      i,
  //      indices(lhs_exts),
  //      AbbrevConstraintRange(
  //        AbbrevEBinop(op, lhs_ab[i], lhs_cd[i]),
  //        AbbrevEBinop(op, rhs_ab[i], rhs_cd[i])));
  //  --
  //  new_cs;
  //}
;
         ...
         (new_tenv: static_envs) | type_error
{
  ...
  case non_empty {
    ...
    declare_enum_labels(tenv1, name, maybe_empty_list_to_non_empty(ids1)) -> new_tenv;
    --
    new_tenv;
  }
;
         ...
         (new_ty: ty, ses: powerset(TSideEffect)) | type_error
{
  ...
  case t_int {
    ...
    case well_constrained {
      ...
      ses := union_list(sess);
      --
      (T_Int(typed_WellConstrained(constraints1, Precision_Full)), ses)
      { [_] };
    }
