...
\begin{itemize}
  ...
  \item symbolically simplifying $\ewidth$ in $\tenv$ via $\staticeval$ yields the literal integer for $\vwidth$\ProseOrTypeError;
  \item annotating each bitfield $\vf$ in $\fields$ with width $\vwidth$ in $\tenv$ yields the corresponding annotated
...
\inferrule{
  ...
  \checknoduplicates(\names) \typearrow \True \OrTypeError\hva\\\\
  \staticeval(\tenv, \ewidth) \typearrow \LInt(\vwidth) \OrTypeError\hva\\\\
  \vf\in\fields: \annotatebitfield(\tenv, \vwidth, \vfield) \typearrow (\vfp, \vxs_\vf) \OrTypeError\hva\\\\
...
\begin{itemize}
  \item \OneApplies
  \begin{itemize}
      \item $\vbf$ is a simple bitfield with list of slices $\vslices$\\ ($\BitFieldSimple(\Ignore, \vslices)$);
      \item $\vbf$ is a nested bitfield with list of slices $\vslices$\\ ($\BitFieldNested(\Ignore, \vslices, \Ignore)$);
      \item $\vbf$ is a typed bitfield with list of slices $\vslices$\\ ($\BitFieldType(\Ignore, \vslices, \Ignore)$).
  \end{itemize}
  \item the result is $\vslices$.
\end{itemize}
...
\RenderRule{bitfield_get_slices}
...
\ProseParagraph
\ProseEqdef{$\vnested$}{the bitfields nested within the bitfield $\vbf$, if there are any, and the empty list otherwise.}
...
\RenderRule{bitfield_get_nested}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item checking whether the range of positions in $\slicesone$ fits inside $0..\vwidth-1$ yields $\True$\ProseOrTypeError;
      \item define $\newfield$ as the bitfield named $\name$ with list of slices $\slicesone$, that is, $\BitFieldSimple(\name, \slicesone)$.
    \end{itemize}

    \item \AllApplyCase{nested}
    \begin{itemize}
      \item applying $\disjointslicestopositions$ to $\tenv$, $\True$ (indicating that the expressions comprising $\slicesone$ must be \staticallyevaluableterm), and $\slicesone$ yields the list of positions $\positions$\ProseOrTypeError;
      \item checking that all positions in $\positions$ fit inside $0..\vwidth$ yields $\True$\ProseOrTypeError;
      ...
      \item define $\newfields$ as the nested bitfield named $\name$ with $\slicesone$ and bitfields $\bitfieldspp$, that is,\\ $\BitFieldNested(\name, \slicesone, \bitfieldspp)$;
      \item define $\vses$ as the union of $\vsesslices$ and $\vsesbitfields$.
    \end{itemize}

    \item \AllApplyCase{type}
    \begin{itemize}
      \item Annotating the type $\vt$ as a non-declaration yields $(\vtp, \vsesty)$\ProseOrTypeError;
      \item checking whether the range of positions in $\slicesone$ fit inside $0..\vwidth$ yields $\True$\ProseOrTypeError;
      \item applying $\disjointslicestopositions$ to $\tenv$, $\True$ (indicating that the expressions comprising $\slicesone$ must be \staticallyevaluableterm), and $\slicesone$ yields the list of positions $\positions$\ProseOrTypeError;
      \item checking that all positions in $\positions$ fit inside $0..\vwidth$ yields $\True$\ProseOrTypeError;
      \item let $\widthp$ be the length of the list $\positions$;
...
\begin{mathpar}
\inferrule[simple]{
  \annotateslices(\tenv, \vslices) \typearrow (\slicesone, \vsesslices) \OrTypeError\hva\\\\
  \commonprefixline\hva\\\\
  \checkslicesinwidth(\tenv, \vwidth, \slicesone) \typearrow \True \OrTypeError
}{
  \annotatebitfield(\tenv, \vwidth, \overname{\BitFieldSimple(\name, \vslices)}{\vfield}) \typearrow \\
...
\inferrule[nested]{
  \annotateslices(\tenv, \vslices) \typearrow (\slicesone, \vsesslices) \OrTypeError\hva\\\\
  \commonprefixline\hva\\\\
  \disjointslicestopositions(\tenv, \True, \slicesone) \typearrow \positions \OrTypeError\hva\\\\
  \checkpositionsinwidth(\tenv, \vwidth, \positions) \typearrow \True \OrTypeError\hva\\\\
  ...
  \annotatebitfield(\tenv, \vwidth, \overname{\BitFieldNested(\name, \vslices, \bitfieldsp)}{\vfield}) \typearrow \\
...
\inferrule[type]{
  \annotateslices(\tenv, \vslices) \typearrow (\slicesone, \vsesslices) \OrTypeError\hva\\\\
  \commonprefixline\hva\\\\
  \annotatetype(\tenv, \vt) \typearrow (\vtp, \vsesty) \OrTypeError\hva\\\\
  \checkslicesinwidth(\tenv, \vwidth, \slicesone) \typearrow \True \OrTypeError\hva\\\\
  \disjointslicestopositions(\tenv, \True, \slicesone) \typearrow \positions \OrTypeError\hva\\\\
  \checkpositionsinwidth(\tenv, \slicesone, \vwidth, \positions) \typearrow \True \OrTypeError\hva\\\\
  \widthp \eqdef \listlen{\positions}\\
  \checkbitsequalwidth(\TBits(\widthp, \emptylist), \vt) \typearrow \True \OrTypeError\hva\\\\
  \vses \eqdef \vsesslices \cup \vsesty
}{
  \annotatebitfield(\tenv, \vwidth, \overname{\BitFieldType(\name, \vslices, \vt)}{\vfield}) \typearrow \\
  (\overname{\BitFieldType(\name, \slicesone, \vtp)}{\newfield}, \vses)
}
\end{mathpar}
} % END_OF_BACKUP_RULE
\RenderRule{annotate_bitfield}
...
\begin{mathpar}
\inferrule{
    \disjointslicestopositions(\tenv, \True, \vslices) \typearrow \positions \OrTypeError\hva\\\\
...
\RenderRule{check_slices_in_width}
...
\begin{itemize}
    ...
    \item define $\maxpos$ as the maximum position in $\positions$;
...
\RenderRule{check_positions_in_width}

\TypingRuleDef{DisjointSlicesToPositions}
\RenderRelation{disjoint_slices_to_positions}

This function assumes that all expressions in {slices} are \pureterm{} and thus deterministic.
...
\inferrule[non\_empty]{
  ...
  \bitfieldslicetopositions(\tenv, \isstatic, \vs) \typearrow \positionsoneopt \OrTypeError\hva\\\\
  \positionsone \eqdef \choice{\positionsoneopt = \some{\vsone}}{\vsone}{\emptyset}\\
  \disjointslicestopositions(\tenv, \isstatic, \vslicesone) \typearrow \positionstwoopt \OrTypeError\hva\\\\
...
\RenderRule{disjoint_slices_to_positions}
...
\begin{mathpar}
\inferrule{
  \evalsliceexpr(\tenv, \isstatic, \veone) \typearrow \some{\offset} \terminateas\TypeErrorConfig, \None\hva\\\\
  \evalsliceexpr(\tenv, \isstatic, \vetwo) \typearrow \some{\length} \terminateas\TypeErrorConfig, \None\hva\\\\
...
\RenderRule{bitfield_slice_to_positions}
...
\begin{itemize}
  ...
  \item define $\vtopabsolute$ as an \absolutebitfield{} with the empty list
        for a name and the interval $\vlastindex..0$ (that is, the entire range
...
\RenderRule{check_common_bitfields_align}
...
\RenderRule{bitfields_to_absolute}
...
\RenderRule{bitfield_to_absolute}
...
\inferrule{
  ...
  \vsamescope \eqdef \listprefixname(\vscopeone, \vscopetwo) \lor \listprefixname(\vscopetwo, \vscopeone)\\
...
\RenderRule{absolute_bitfields_align}
...
\begin{itemize}
  ...
  \item \Prosestaticeval{$\tenv$}{$\vi$}{the literal for the integer $\vzi$};
  \item \Prosestaticeval{$\tenv$}{$\vw$}{the literal for the integer $\vzw$};
  \item define $\vstart$ as $\vzi$;
  \item define $\vend$ as $\vzi + \vzw - 1$;
