...
In contrast, the pair of bitfields with \absolutenames{} \texttt{fmt0.common}
and \texttt{fmt1.common} are not in the same scope, since neither \texttt{fmt0} nor \texttt{fmt1} is a prefix of the other.
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item applying $\disjointslicestopositions$ to $\tenv$,
            $\True$ (indicating that the expressions comprising $\slicesone$ must be \staticallyevaluableterm), \\
            and $\slicesone$
      ...
      \item define $\newfields$ as the nested bitfield named $\name$ with $\slicesone$ and bitfields \\
            $\bitfieldspp$, that is, $\BitFieldNested(\name, \slicesone, \bitfieldspp)$;
