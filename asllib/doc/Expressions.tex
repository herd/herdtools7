...
\[
  ...
  \left(
  \begin{array}{ll}
  \ExprResult((\overname{\vals}{\vv} \times \overname{\XGraphs}{\vg}) \aslsep \overname{\envs}{\newenv}) & \cup\\
...
\begin{mathpar}
\inferrule{}{
  \evalexpr{\env, \overname{\ELiteral(\vl)}{\ve}} \evalarrow \ExprResult((\overname{\nvliteral{\vl}}{\vv},\overname{\emptygraph}{\vg}), \overname{\env}{\newenv})
}
\end{mathpar}
...
\inferrule[local]{
  ...
  \evalexpr{\env, \EVar(\vx)} \evalarrow \ExprResult((\overname{L^\denv(\vx)}{\vv}, \overname{\ReadEffect(\vx)}{\vg}), \overname{\env}{\newenv})
}
\end{mathpar}
...
\inferrule[global]{
  ...
  \evalexpr{\env, \EVar(\vx)} \evalarrow \ExprResult((\overname{G^\denv.\storage(\vx)}{\vv}, \overname{\ReadEffect(\vx)}{\vg}), \overname{\env}{\newenv})
}
\end{mathpar}
...
\begin{itemize}
  ...
  \item the evaluation of the expression $\veone$ in $\env$ is the configuration \\
        $\ExprResult(\vmone, \envone)$\ProseOrAbnormal;
...
\begin{mathpar}
\inferrule{\op \not\in \{\BAND, \BOR, \IMPL\}\\
  \evalexpr{ \env, \veone} \evalarrow \ExprResult(\vmone, \envone) \OrAbnormal \\\\
  \evalexpr{ \envone, \vetwo } \evalarrow \ExprResult(\vmtwo, \newenv) \OrAbnormal \\\\
...
\item the evaluation of the expression $\veone$ in $\env$ yields \\ $\ExprResult((\vvone, \vg), \newenv)$\ProseOrAbnormal;
\item applying the unary operator $\op$ to $\vvone$ is $\vv$.
\end{itemize}
\FormallyParagraph
\begin{mathpar}
\inferrule{
  \evalexpr{ \env, \veone} \evalarrow \ExprResult((\vvone,\vg), \newenv) \OrAbnormal\\\\
...
\begin{itemize}
  ...
  \item the evaluation of the conditional expression $\econd$ in $\env$ yields \\
        $\ExprResult(\mcond, \envone)$\ProseOrAbnormal;
  ...
  \item the evaluation of $\vep$ in $\envone$ yields $\ExprResult((\vvtwo, \vgtwo), \newenv)$\ProseOrAbnormal;
  \item $\vg$ is the parallel composition of $\vgone$ and $\vgtwo$.
\end{itemize}
...
\begin{mathpar}
\inferrule{
  \evalexpr{o\plusdata, \env, \econd} \evalarrow \ExprResult(o_1, \nvbool(\vb), \envone) \OrAbnormal\\\\
  \vep \eqdef \choice{\vb}{\veone}{\vetwo}\\\\
  \evalexpr{o_1\plusctrl, \envone, \vep} \evalarrow \ExprResult(o_2, \vv, \newenv)  \OrAbnormal
}{
  \evalexpr{o, \env, \overname{\ECond(\econd, \veone, \vetwo)}{\ve}} \evalarrow
  \ExprResult(o_2, \vv, \newenv)
}
\end{mathpar}
}
...
\begin{mathpar}
\inferrule{
  \evalexpr{\env, \econd} \evalarrow \ExprResult(\mcond, \envone) \OrAbnormal\\\\
  \mcond \eqname (\nvbool(\vb), \vgone)\\
  \vep \eqdef \choice{\vb}{\veone}{\vetwo}\\\\
  \evalexpr{\envone, \vep} \evalarrow \ExprResult((\vv, \vgtwo), \newenv)  \OrAbnormal\\\\
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      which goes into the output configuration $\ExprResult((\vv, \vg), \newenv)$.
    \end{itemize}
    ...
    \begin{itemize}
      ...
      \item the resulting configuration is $\ExprResult((\vv, \vg), \newenv)$.
    \end{itemize}
  \end{itemize}
\end{itemize}
...
\inferrule[single\_returned\_value]{
  \evalcall(\env, \vcall.\name, \vcall.\params, \vcall.\args) \evalarrow \ExprResult(\vms, \newenv) \OrAbnormal\\
  \vms \eqname [(\vv, \vg)]
}{
  \evalexpr{\env, \ECall(\vcall)} \evalarrow \ExprResult((\vv, \vg), \newenv)
}
\end{mathpar}

\begin{mathpar}
\inferrule[multiple\_returned\_values]{
  \evalcall(\env, \vcall.\callname, \vcall.\callparams, \vcall.\callargs) \evalarrow \ExprResult(\vms, \newenv) \OrAbnormal\\
...
\item the evaluation of $\ebv$ in $\env$ yields $\ExprResult(\mbv, \envone)$\ProseOrAbnormal;
\item the evaluation of $\slices$ in $\env$ yields $\ExprResult(\mpositions, \newenv)$\ProseOrAbnormal;
...
\begin{mathpar}
\inferrule{
  \evalexpr{\env, \ebv} \evalarrow \ExprResult(\mbv, \envone)  \OrAbnormal\\\\
  \mbv \eqname (\vbv,\vgone) \\
  \evalslices(\envone, \slices) \evalarrow \ExprResult(\mpositions, \newenv)  \OrAbnormal \\
  \mpositions \eqname (\positions, \vgtwo) \\
  \readfrombitvector(\vbv, \positions) \evalarrow \vv \OrDynError\\\\
  \vg \eqdef \vgone \parallelcomp \vgtwo
}{
  \evalexpr{\env, \ESlice(\ebv, \slices)} \evalarrow \ExprResult((\vv, \vg), \newenv)
}
\end{mathpar}
...
\begin{itemize}
  ...
  \item the evaluation of $\earray$ in $\env$ is $\ExprResult(\marray, \envone)$\ProseOrAbnormal;
  \item the evaluation of $\eindex$ in $\env$ is  $\ExprResult(\mindex, \newenv)$\ProseOrAbnormal
...
\begin{mathpar}
\inferrule{
  \evalexpr{\env, \earray} \evalarrow \ExprResult(\marray, \envone)  \OrAbnormal\\
  \evalexpr{\envone, \eindex} \evalarrow \ExprResult(\mindex, \newenv)  \OrAbnormal\\
  ...
  \evalexpr{\env, \EGetArray(\earray, \eindex)} \evalarrow \ExprResult((\vv, \vg), \newenv)
}
\end{mathpar}
...
\inferrule{
  ...
  \evalexpr{\env, \EGetEnumArray(\earray, \eindex)} \evalarrow \ExprResult((\vv, \vg), \newenv)
}
\end{mathpar}
...
\item the evaluation of $\erecord$ in $\env$ is $\ExprResult((\vrecord, \vg), \newenv)$\ProseOrAbnormal;
\item $\vv$ is the value mapped by $\fieldname$ in the native record $\vrecord$.
\end{itemize}
\FormallyParagraph
\begin{mathpar}
\inferrule{
  \evalexpr{\env, \erecord} \evalarrow \ExprResult((\vrecord, \vg), \newenv)  \OrAbnormal\\
  \getfield(\fieldname, \vrecord) \evalarrow \vv
}{
  \evalexpr{\env, \EGetField(\erecord, \fieldname)} \evalarrow \ExprResult((\vv, \vg), \newenv)
}
\end{mathpar}
...
\begin{itemize}
  ...
  \item evaluating the expression $\etuple$ yields $\ExprResult((\vvtuple, \vg), \newenv)$\ProseOrAbnormal;
...
\begin{mathpar}
\inferrule{
  \evalexpr{\env, \etuple} \evalarrow \ExprResult((\vvtuple, \vg), \newenv) \OrAbnormal\\\\
  \getindex(\vvtuple, \vindex) \evalarrow \vv
}{
  \evalexpr{\env, \overname{\EGetItem(\etuple, \vindex)}{\ve}} \evalarrow \ExprResult((\vv, \vg), \newenv)
}
\end{mathpar}
...
\item evaluating $\veone$ in $\env$ results in $\ExprResult((\vv, \vgone), \newenv)$\ProseOrAbnormal;
...
\begin{mathpar}
\inferrule[okay]{
  \evalexpr{\env, \veone} \evalarrow \ExprResult((\vv, \vgone), \newenv) \OrAbnormal\\\\
  \isvaloftype(\env, \vv, \vt) \evalarrow (\vb, \vgtwo) \OrDynError\\\\
  \vb \eqname \nvbool(\True)\\
  \vg \eqdef \ordered{\vgone}{\asldata}{\vgtwo}
}{
  \evalexpr{\env, \EATC(\veone, \vt)} \evalarrow \ExprResult((\vv, \vg), \newenv)
}
\and
\inferrule[error]{
  \evalexpr{\env, \veone} \evalarrow \ExprResult((\vv, \Ignore), \Ignore)\\
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item evaluating the side-effect-free expression $\ve$ in $\env$ yields $\ExprResult(\vvp, \vg)$\ProseOrDynErrorDiverging;
...
\begin{mathpar}
\inferrule[bits]{
  \evalexprsef{\env, \ve} \evalarrow \ExprSEFResult(\vvp, \vg) \OrDynErrorDiverging
}{
...
\inferrule[Constraint\_Exact\_Sat]{
  \evalexprsef{\env, \ve} \evalarrow \ExprSEFResult(\nvint(m), \vg) \OrDynError
}{
...
\inferrule[Constraint\_Range\_Sat]{
  \evalexprsef{\env, \veone} \evalarrow \ExprSEFResult(\nvint(a), \vgone) \OrDynError\\\\
  \evalexprsef{\env, \vetwo} \evalarrow \ExprSEFResult(\nvint(b), \vgtwo) \OrDynError\\\\
...
\begin{itemize}
  ...
  $\ExprResult((\vvone, \vgone), \newenv)$\ProseOrAbnormal;
  \item evaluating whether the pattern $\vp$ matches the value $\vvone$ in $\env$
  results in $\PatternResult(\vv, \vgtwo)$ where is a native Boolean that determines
...
\begin{mathpar}
\inferrule{
  \evalexpr{\env, \ve} \evalarrow \ExprResult((\vvone, \vgone), \newenv) \OrAbnormal\\
  \evalpattern{\env, \vvone, \vp} \evalarrow \PatternResult(\vv, \vgtwo)\\
  \vg \eqdef \ordered{\vgone}{\asldata}{\vgtwo}
}{
  \evalexpr{\env, \EPattern(\ve, \vp)} \evalarrow \ExprResult((\vv, \vg), \newenv)
}
\end{mathpar}
...
\inferrule[okay]{
  ...
  \evalexpr{\env, \overname{\EArbitrary (\vt)}{\ve}} \evalarrow \ExprResult((\vv, \overname{\emptygraph}{\vg}), \overname{\env}{\newenv})
}
\end{mathpar}
...
\inferrule{
  ...
  \evalexprlist{\env, \fields} \evalarrow \ExprListResult((\vvfields,\vg), \newenv)  \OrAbnormal\\
  \vvfields \eqname \vv_{1..k}\\
  \vv \eqdef \nvrecord{\{i=1..k: \id_i\mapsto \vv_i\}}
}{
  \evalexpr{\env, \ERecord(\Ignore, \efields)} \evalarrow \ExprResult((\vv, \vg), \newenv)
}
\end{mathpar}
...
\begin{itemize}
  ...
  \item the evaluation of $\elist$ in $\env$ is $\ExprListResult((\vlist, \vg), \newenv)$\ProseOrAbnormal;
...
\begin{mathpar}
\inferrule{
  \evalexprlist{\env, \elist} \evalarrow \ExprListResult((\vlist, \vg), \newenv) \OrAbnormal\\
  \vv \eqdef \nvvector{\vlist}
}{
  \evalexpr{\env, \ETuple(\elist)} \evalarrow \ExprResult((\vv, \vg), \newenv)
}
\end{mathpar}
...
\begin{itemize}
  ...
  \item evaluating the expression $\evalue$ in $\env$ yields $\ExprResult((\vvalue, \vgone), \newenv)$\ProseOrAbnormal;
  \item evaluating the side-effect-free expression $\elength$ in $\env$ yields \\
        $\ExprSEFResult((\vlength, \vgtwo))$\ProseOrError;
...
\begin{mathpar}
\inferrule{
  \evalexpr{\env, \evalue} \evalarrow \ExprResult((\vvalue, \vgone), \newenv) \OrAbnormal\\\\
  \evalexprsef{\env, \elength} \evalarrow \ExprSEFResult((\vlength, \vgtwo)) \OrDynError\\\\
  \vlength \eqname \nvint(\nlength)\\
...
\begin{itemize}
  ...
  \item evaluating the expression $\evalue$ in $\env$ yields $\ExprResult((\vvalue, \vg), \newenv)$\ProseOrAbnormal;
...
\begin{mathpar}
\inferrule{
  \evalexpr{\env, \evalue} \evalarrow \ExprResult((\vvalue, \vg), \newenv) \OrAbnormal\\\\
...
\[
  ...
  \left(
  \begin{array}{ll}
  \ExprSEFResult(\overname{\vals}{\vv}\aslsep\overname{\XGraphs}{\vg}) & \cup \\
  \overname{\TDynError}{\DynErrorConfig} & \cup \\
  \overname{\TDiverging}{\DivergingConfig} & \\
  \end{array}
  \right)
...
\begin{mathpar}
\inferrule{
  \evalexpr{\env, \ve} \evalarrow \ExprResult((\vv,\vg), \env) \OrDynError
}{
  \evalexprsef{\env, \ve} \evalarrow \ExprSEFResult(\vv, \vg)
}
\end{mathpar}
...
\[
  ...
  \left(
  \begin{array}{ll}
  \ExprListResult((\overname{\vals^*}{\vv} \times \overname{\XGraphs}{\vg})\aslsep \overname{\envs}{\newenv}) & \cup\\
  \overname{\TThrowing}{\ThrowingConfig}    & \cup\\
  \overname{\TDynError}{\DynErrorConfig}    & \cup\\
  \overname{\TDiverging}{\DivergingConfig}  & \\
  \end{array}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \Proseevalexpr{$\env$}{$\ve$}{\\ $\ExprResult((\vvone, \vgone), \envone)$}\ProseOrAbnormal;
    \item evaluating the list of expressions $\vle1$ in the environment $\envone$ yields \\
          $\ExprListResult((\vvs, \vgtwo), \envtwo)$\ProseOrAbnormal;
    \item \Proseeqdef{$\vg$}{the parallel composition of $\vgone$ and $\vgtwo$};
...
\inferrule[empty]{}
{
  ...
  \ExprListResult((\overname{\emptylist}{\vv}, \overname{\emptygraph}{\vg}), \overname{\env}{\newenv})
}
\end{mathpar}
...
\inferrule[non\_empty]{
  \vle = [\ve] \concat \vle1\\
  \evalexpr{\env, \ve} \evalarrow \ExprResult((\vvone, \vgone), \envone) \OrAbnormal\\\\
  \evalexprlist{\envone, \vle1} \evalarrow \ExprListResult((\vvs, \vgtwo), \newenv) \OrAbnormal\\\\
  \vg \eqdef \vgone \parallelcomp \vgtwo \\
  \vv \eqdef [\vvone] \concat \vvs
}{
  \evalexprlist{\env, \vle} \evalarrow \ExprListResult((\vv, \vg), \newenv)
}
\end{mathpar}
