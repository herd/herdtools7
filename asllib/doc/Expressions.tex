...
\begin{itemize}
  ...
  \item Bitvector concatenation expressions (see \secref{BitvectorConcatenationExpressions})
...
\begin{itemize}
  ...
  \begin{itemize}
    \item All of the following apply (\textsc{empty\_getter}):
    \begin{itemize}
    \item applying $\shouldreducetocall$ to $\vx$ and $\STEmptyGetter$ in $\tenv$ yields $\True$\ProseOrTypeError;
    \item applying $\annotatecall$ to $\vx$, an empty list of arguments $\emptylist$, and \\
          $\STEmptyGetter$ yields $(\name, \vargs, \eqs, \langle\vt\rangle)$\ProseOrTypeError;
    \item define $\newe$ as the call expression for $\name$, list of arguemnts $\vargs$, and list of parameter equalities $\eqs$,
          that is, $\ECall(\name, \vargs, \eqs)$.
    \end{itemize}

    \item All of the following apply (\textsc{local\_constant}):
    \begin{itemize}
    \item applying $\shouldreducetocall$ to $\vx$ and $\STEmptyGetter$ in $\tenv$ yields $\False$;
...
\subsubsection{Formally}
\begin{mathpar}
\inferrule[empty\_getter]{
  \shouldreducetocall(\tenv, \vx, \STEmptyGetter) \typearrow \True\OrTypeError\\\\
  \annotatecall(\tenv, \vx, \emptylist, \STEmptyGetter) \typearrow (\name, \vargs, \eqs, \langle\vt\rangle) \OrTypeError\\
}{
  \annotateexpr{\tenv, \overname{\EVar(\vx)}{\ve}} \typearrow (\vt, \overname{\ECall(\name, \vargs, \eqs)}{\newe})
}
\end{mathpar}

\begin{mathpar}
\inferrule[local\_constant]{
  \shouldreducetocall(\tenv, \vx, \STEmptyGetter) \typearrow \False\\
  L^\tenv.\localstoragetypes(\vx) = (\vt, \LDKConstant)\\
  \lookupconstants(\tenv, \vx) \typearrow \vv
}{
...
\inferrule[local\_non\_constant]{
  ...
  \lookupconstants(\tenv, \vx) \typearrow \bot \lor k \neq \LDKConstant
}{
...
\subsection{Typing}
\subsubsection{TypingRule.ESetter \label{sec:TypingRule.ESetter}}
\subsubsection{Prose}
All of the following apply:
\begin{itemize}
...
\subsubsection{TypingRule.ReduceSlicesToCall \label{sec:TypingRule.ReduceSlicesToCall}}
...
\ASLExample{\typingtests/TypingRule.EConcatUnresolvableToInteger.asl}
...
\subsubsection{Example}
In the specification:
\ASLExample{\semanticstests/SemanticsRule.EConcat.asl}
the expression \texttt{['10', '11']} evaluates to the value \texttt{'1011'}.

\subsubsection{Example}
In the specification:
\ASLExample{\semanticstests/SemanticsRule.EConcat2.asl}
...
\inferrule[pattern\_set]{}{
  \buildpatternormask(\overname{\Npatternormask(\Npatternset)}{\vparsednode}) \astarrow
...
\begin{mathpar}
\inferrule[bitvector\_lit]{}{
  \buildpatternormask(\overname{\Npatternormask(\Tbitvectorlit(\vb))}{\vparsednode}) \astarrow
  \overname{\PatternMask(\vb)}{\vastnode}
}
\end{mathpar}
