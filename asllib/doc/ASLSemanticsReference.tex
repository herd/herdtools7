...
in a rule).
Similarly, $\ReturningConfig \triangleq \Returning((\vvs,\newg), \newenv)$
is an ``early return'' where $\vvs$, $\newg$, and $\newenv$ are fresh
variables,
and $\ContinuingConfig \triangleq \Continuing(\newg, \newenv)$ is a
continuing configuration where $\newg$, and $\newenv$ are fresh variables.
...
\subsection{Prose}
  Evaluation of a literal expression $\texttt{E\_Literal}(\vv)$,
...
\section{SemanticsRule.BinopAnd \label{sec:SemanticsRule.BinopAnd}}
  ...
  $\texttt{E\_Binop}(\BAND, \veone, \vetwo)$;
...
\section{SemanticsRule.BinopOr \label{sec:SemanticsRule.BinopOr}}
  ...
  \begin{itemize}
  \item $\ve$ denotes a disjunction of two expressions, $\texttt{E\_Binop}(\BOR, \veone, \vetwo)$;
...
\section{SemanticsRule.BinopImpl \label{sec:SemanticsRule.BinopImpl}}
  ...
  the configuration $C$ and all of the following apply:
  \begin{itemize}
  \item $\ve$ denotes an implication over two expressions, $\texttt{E\_Binop}(\IMPL, \veone, \vetwo)$;
...
\section{SemanticsRule.ERecord \label{sec:SemanticsRule.ERecord}}
  ...
  $\Normal((\vv, \vg), \newenv)$, or an abnormal configuration, and all of the following apply:
  \begin{itemize}
  \item $\ve$ denotes a record creation expression, $\texttt{E\_Record}(\names, \efields)$;
...
\section{SemanticsRule.EGetField \label{sec:SemanticsRule.EGetField}}
  ...
  $\Normal((\vv, \vg), \newenv)$, or an abnormal configuration, and all of the following apply:
  \begin{itemize}
  \item $\ve$ denotes a field access expression, $\texttt{E\_GetField}(\erecord, \fieldname)$;
...
\section{SemanticsRule.EConcat \label{sec:SemanticsRule.EConcat}}
  ...
  $\Normal((\vv, \vg), \newenv)$, or an abnormal configuration, and all of the following apply:
  \begin{itemize}
  \item $\ve$ denotes a concatenation of bitvector expressions, $\texttt{E\_Concat}(\elist)$;
...
\section{SemanticsRule.ETuple \label{sec:SemanticsRule.ETuple}}
  ...
  $\Normal((\vv, \vg), \newenv)$, or an abnormal configuration, and all of the following apply:
  \begin{itemize}
  \item $\ve$ denotes a tuple expression, $\texttt{E\_Tuple}(\elist)$;
  \item the evaluation of $\elist$ in $\env$ is either $\Normal((\vlist, \vg), \newenv)$,
  \ProseOrAbnormal;
...
\begin{mathpar}
  ...
  {
    \evalexpr{\env, \texttt{E\_CTC}(\veone, \vt)} \evalarrow \Normal((\vv, \vg), \newenv)
  }
\and
\inferrule[error]{
...
{
  \evalexpr{\env, \texttt{E\_CTC}(\veone, \vt)} \evalarrow \ErrorVal{CTC\_TypeMismatch}
}
\end{mathpar}
\end{emptyformal}
...
\section{SemanticsRule.LELocalVar \label{sec:SemanticsRule.LELocalVar}}
  ...
  and all of the following apply:
  \begin{itemize}
  \item $\vle$ denotes a variable, $\texttt{LE\_Var}(\vx)$;
  ...
  SemanticsRule.LELocalVar is (only) used to assign the value $42$ to the the left-hand-side expression
...
\section{SemanticsRule.LEGlobalVar \label{sec:SemanticsRule.LEGlobalVar}}
  ...
  and all of the following apply:
  \begin{itemize}
    \item $\vle$ denotes a variable $\vx$, $\texttt{LE\_Var}(\vx)$;
  ...
  SemanticsRule.LEGlobalVar is (only) used to assign the value $42$ to the the left-hand-side expression
  \texttt{x} within \texttt{x = 42;}.
...
\section{SemanticsRule.LEDestructuring \label{sec:SemanticsRule.LEDestructuring}}
    ...
    and all of the following apply:
    \begin{itemize}
    \item $\vle$ denotes a list of left-hand-side expressions, $\texttt{LE\_Destructuring}(\vlelist)$;
...
\section{SemanticsRule.PGeq \label{sec:SemanticsRule.PGeq}}
    ...
    \begin{itemize}
      \item $\vp$ is the condition corresponding to being greater than or equal
        than the side-effect-free expression $\ve$, $\texttt{Pattern\_Geq}(\ve)$;
...
\section{SemanticsRule.PLeq \label{sec:SemanticsRule.PLeq}}
  ...
  \begin{itemize}
    \item $\vp$ is the condition corresponding to being less than or equal
      than the side-effect-free expression $\ve$, $\texttt{Pattern\_Leq}(\ve)$;
    \item the side-effect-free evaluation of $\ve$ is either
    $\Normal(\vvone, \newg)$ \ProseOrError;
    \item $\vb$ is the Boolean value corresponding to whether $\vv$
...
\section{SemanticsRule.PNot \label{sec:SemanticsRule.PNot}}
    ...
    and all of the following apply:
      \begin{itemize}
      \item $\vp$ is a negation pattern, $\texttt{Pattern\_Not}(\vpone)$;
      \item evaluating tha pattern $\vpone$ in an environment $\env$ is either \\
      $\Normal(\vbone, \newg)$ \ProseOrError;
...
\section{SemanticsRule.PSingle \label{sec:SemanticsRule.PSingle}}
    ...
    \begin{itemize}
      \item $\vp$ is the condition corresponding to being equal to the
        side-effect-free expression $\ve$, $\texttt{Pattern\_Single}(\ve)$;
...
\section{SemanticsRule.LDTyped \label{sec:SemanticsRule.LDTyped}}
    ...
    and all of the following apply:
    \begin{itemize}
      \item $\ldi$ is a typed declaration, $\texttt{LDI\_Typed}(\ldione, \vt)$;
      \item $\minitopt$ is $\vm$;
...
\section{SemanticsRule.LDUninitialisedTyped\label{sec:SemanticsRule.LDUninitialisedTyped}}
    ...
    and all of the following apply:
    \begin{itemize}
      \item $\ldi$ gives a local declaration with a type, but no initial value, $\texttt{LDI\_Ttyped}(\ldione, \vt)$;
...
\section{SemanticsRule.SAssignTuple \label{sec:SemanticsRule.SAssignTuple}}
  ...
  $\texttt{S\_Assign}(\texttt{LE\_Destructuring}(\vles), \texttt{E\_Tuple}(\exprs))$;
  \item $\les$ is a list of left-hand-side expressions, each of which is either a variable \\ ($\texttt{LE\_Var}(\Ignore)$)
  or a discarded variable (\texttt{LE\_Discard});
  \item evaluating $\exprs$ is either $\Normal(\vms, \envone)$ (where $\vms$ is a list of value-execution graph pairs)
  \ProseOrAbnormal;
  \item assigning each of the value-execution graph pairs to the respective expression in $\les$ in $\envone$ results in
  either $\Normal(\newg, \newenv)$ \ProseOrAbnormal.
  \end{itemize}
...
\section{SemanticsRule.SReturnSome \label{sec:SemanticsRule.SReturnSome}}
    ...
    $\Returning((\vvs, \newg), \newenv)$ and all of the following apply:
    \begin{itemize}
    \item $\vs$ is a \texttt{return} statemenet for a list of expressions, $\texttt{S\_Return}(\langle\texttt{E\_Tuple}(\es)\rangle)$;
...
\begin{mathpar}
  ...
  {
    \evalstmt{\env, \texttt{S\_Seq}(\vsone, \vstwo)} \evalarrow D
  }
\end{mathpar}
\end{emptyformal}
...
\section{SemanticsRule.SCall \label{sec:SemanticsRule.SCall}}
    ...
    \item $\vs$ is a call statement, $\texttt{S\_Call}(\name, \args, \namedargs)$;
...
\section{SemanticsRule.SCond \label{sec:SemanticsRule.SCond}}
    ...
    \begin{itemize}
    \item $\vs$ is a condition statement, $\texttt{S\_Cond}(\ve, \vsone, \vstwo)$;
    ...
    \item evaluating $\vsp$ in $\envone$ as per Chapter~\ref{chap:eval_block} is either
...
\section{SemanticsRule.SCase \label{sec:SemanticsRule.SCase}}
    ...
    and all of the following apply:
    \begin{itemize}
    \item $\vs$ is a case statement, $\texttt{S\_Case}(\ve, \caselist)$;
...
\section{SemanticsRule.SWhile \label{sec:SemanticsRule.SWhile}}
    ...
    \begin{itemize}
    \item $\vs$ is a \texttt{while} statement, $\texttt{S\_While}(\ve, \vbody)$;
...
\begin{mathpar}
  ...
  {
    \evalstmt{\env, \texttt{S\_While}(\ve, \vbody)} \evalarrow C
  }
\end{mathpar}
\end{emptyformal}
...
\section{SemanticsRule.SRepeat \label{sec:SemanticsRule.SRepeat}}
    ...
    \begin{itemize}
    \item $\vs$ is a \texttt{repeat} statement, $\texttt{S\_Repeat}(\ve, \vbody)$;
    \item evaluating $\vbody$ in $\env$ as per Chapter~\ref{chap:eval_block} \\
    \ProseOrAbnormal,
...
\begin{mathpar}
  ...
  {
    \evalstmt{\env, \texttt{S\_Repeat}(\ve, \vbody)} \evalarrow D
  }
\end{mathpar}
\end{emptyformal}
...
\section{SemanticsRule.SThrowNone \label{sec:SemanticsRule.SThrowNone}}
    ...
    and all of the following apply:
    \begin{itemize}
    \item $\vs$ is a \texttt{throw} statement that does not provide an expression, $\texttt{S\_Throw}(\None)$;
    \item $\newenv$ is $\env$;
    \item $\vex$ is $\None$;
    \item $\newg$ is the empty graph;
...
\section{SemanticsRule.STry \label{sec:SemanticsRule.STry}}
    ...
    and all of the following apply:
    \begin{itemize}
    \item $\vs$ is a \texttt{try} statement, $\texttt{S\_Try}(\vs, \catchers, \otherwiseopt)$;
    \item evaluating $\vsone$ in $\env$ as per Chapter~\ref{chap:eval_block}
...
\section{SemanticsRule.SDeclSome \label{sec:SemanticsRule.SDeclSome}}
    ...
    value in $\envone$ as per Chapter~\ref{chap:eval_local_decl} is $\Normal(\newg, \newenv)$;
    \item the result of the entire evaluation is $\Continuing(\newg, \newenv)$.
    \end{itemize}
...
\section{SemanticsRule.SDeclNone \label{sec:SemanticsRule.SDeclNone}}
    ...
    and all of the following apply:
    \begin{itemize}
    \item $\vs$ is a declaration without an initial value, $\texttt{S\_Decl}(\Ignore, \ldi, \None)$;
...
\begin{mathpar}
  ...
  {
    \evalstmt{\env, \texttt{S\_Decl}(\Ignore, \ldi, \None)} \evalarrow \Continuing(\newg, \newenv)
  }
\end{mathpar}
\end{emptyformal}
...
\[
  ...
  \overname{\TError}{\ErrorConfig}
\]
evaluates a statement $\vs$ as a \emph{block}. That is, $\vs$ is evaluated in a fresh local environment,
...
\section{SemanticsRule.Block \label{sec:SemanticsRule.Block}}
    ...
    a returning configuration, or an abnoraml configuration,
    and all of the following apply:
    \begin{itemize}
      ...
      \item evaluating $\stm$ in $\blockenv$, as per Chapter~\ref{chap:eval_stmt},
...
The negation of the condition is used to decide on continuing the loop iteration.
...
Advances the loop counter one step towards the end of its range is achieved via the following rule:
\begin{mathpar}
  \inferrule{
    \opfordir \eqdef \choice(\dir = \UP, \PLUS, \MINUS)\\
    \readidentifier(\vindexname, \vstart) \evalarrow \vgone\\
    \binop(\opfordir, \vstart, \nvint(1)) \evalarrow \vstep\\
...
\begin{mathpar}
  \inferrule[return]{
    ...
    \readidentifier(\vindexname, \vstart) \evalarrow \vgone\\
    \binop(\compfordir, \vend, \vstart) \evalarrow \nvbool(\True)\\
    \newg \eqdef \vgone\\
    \newenv = \env
  }
  {
...
%   \texttt{Up\_for\_body}  &\triangleq& \texttt{S\_Seq}(\vbody, \texttt{plus\_one})\\
...
%   \texttt{Down\_for\_body}  &\triangleq& \texttt{S\_Seq}(\vbody, \texttt{minus\_one})\\
...
\subsection{Code}
\VerbatimInput[firstline=\FindCatcherBegin, lastline=\FindCatcherEnd]{../Interpreter.ml}
...
\and
\inferrule[match]{
  \catchers \eqname [\vc] + \catchersone\\
  \vc \eqname (\nameopt, \ety, \vs) \\
...
\subsection{Code}
\VerbatimInput[firstline=\RethrowImplicitBegin, lastline=\RethrowImplicitEnd]{../Interpreter.ml}
...
\subsection{Code}
\VerbatimInput[firstline=\ReadValueFromBegin, lastline=\ReadValueFromEnd]{../Interpreter.ml}
...
\]
generates write events for the values
...
% \subsection{Code}
%   \VerbatimInput[firstline=\WriteRetValsBegin, lastline=\WriteRetValsEnd]{../Interpreter.ml}
...
\begin{emptyformal}
  ...
  \[
    \evalprimitive{\overname{\identifier}{\name} \aslsep (\vals\times\XGraphs)^*} \bigtimes
    \Normal((\vals\times\XGraphs)^* \aslsep \XGraphs) \cup \overname{\TError}{\ErrorConfig} \enspace,
  \]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
We first introduce the following helper rules:
\begin{itemize}
...
\end{itemize}
and finally the main rule:
SemanticsRule.TopLevel (see \secref{SemanticsRule.TopLevel}).
...
\subsection{Code}
% \VerbatimInput[firstline=\EvalGlobalsBegin, lastline=\EvalGlobalsEnd]{../Interpreter.ml}
...
All of the following apply:
\begin{itemize}
  \item sorting the declarations of the global storage elemnts in topological order with respect to the dependency order
  gives $\vdecls$;
...
\subsection{Code}
\VerbatimInput[firstline=\BuildGlobalEnvBegin, lastline=\BuildGlobalEnvEnd]{../Interpreter.ml}
