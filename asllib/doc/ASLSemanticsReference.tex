...
\section{SemanticsRule.CTC \label{sec:SemanticsRule.CTC}}
  ...
  $\Normal((\vv, \vg), \newenv)$ and all of the following apply:
  \begin{itemize}
  \item $\ve$ denotes a type conversion expression, $\ECTC(\veone, \vt)$;
...
\begin{mathpar}
  ...
  {
    \evalexpr{\env, \ECTC(\veone, \vt)} \evalarrow \Normal((\vv, \vg), \newenv)
  }
\and
\inferrule[error]{
...
{
  \evalexpr{\env, \ECTC(\veone, \vt)} \evalarrow \ErrorVal{CTC\_TypeMismatch}
}
\end{mathpar}
\end{emptyformal}
...
\section{SemanticsRule.PLeq \label{sec:SemanticsRule.PLeq}}
  ...
  \begin{itemize}
    \item $\vp$ is the condition corresponding to being less than or equal
      than the side-effect-free expression $\ve$, $\PatternLeq(\ve)$;
    \item the side-effect-free evaluation of $\ve$ is either
    $\Normal(\vvone, \newg)$ \ProseOrError;
    \item $\vb$ is the Boolean value corresponding to whether $\vv$
...
\section{SemanticsRule.SReturnSome \label{sec:SemanticsRule.SReturnSome}}
    ...
    $\Returning((\vvs, \newg), \newenv)$ and all of the following apply:
    \begin{itemize}
    \item $\vs$ is a \texttt{return} statemenet for a list of expressions, $\SReturn(\langle\ETuple(\es)\rangle)$;
