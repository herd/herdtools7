...
\inferrule{\env \eqname (\tenv, \denv)\\
  ...
  \evalexpr{\env, \overname{\EUnknown}{\ve}} \evalarrow \Normal(\overname{\env}{\newenv}, \vv, \emptygraph)
}
\end{mathpar}
...
\begin{mathpar}
  ...
  }
  {
    ...
    \SFor\left\{\begin{array}{rcl}
      ...
      \Forbody &:& \vbody\\
      \Forlimit &:& \Ignore\\
    \end{array}\right\}} \evalarrow \\ \Continuing(\newg, \newenv)
    \end{array}
    }
  }
\end{mathpar}
...
\]
evaluates a binary operator $\op$ over a pair of native values --- $\vvone$ and $\vvtwo$ --- and returns the
native value $\vw$ or an error.
