...
\newcommand\vals[0]{\hyperlink{def-vals}{\mathcal{V}}}
...
\newcommand\ErrorConfig[0]{\hyperlink{def-errorconfig}{\texttt{\#E}}}
...
\newcommand\TError[0]{\hyperlink{def-terror}{\textsf{TError}}}
...
\newcommand\Error[0]{\hyperlink{def-error}{\textsf{Error}}}
...
\begin{itemize}
  \item The ASL Language Reference Manual~\cite{LRM} introduces the concrete syntax and intention of all ASL language constructs.
...
The ASL semantics defined here assigns meaning only to \emph{well-typed specifications}.
...
The ASL semantics assigns meaning only to terminating executions.

\section{Basic Semantic Concepts}
The ASL semantics is given by \emph{semantic relations} between \emph{semantic configurations},
...
The concurrent semantics operates over concurrent execution graphs.
...
The sequential semantics corresponds to executing an ASL specification in the context of a single thread
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
This chapter defines the mathematical types over which our semantics is defined.
...
appearing are fresh (that is, different from all other variables appearing
in a rule):
\begin{itemize}
...
constructively.
%
% The semantics of the different types of AST nodes is given by a set of \emph{semantic relations} between
...
\[
  \begin{array}{rl}
  \EBinop(\MUL, &\overname{\texttt{E\_Binop}(\PLUS, \overname{\elint{1}}{\veone}, \overname{\elint{2}}{\vetwo})}{\veonetwo}, \\
                &\overname{\texttt{E\_Binop}(\PLUS, \overname{\elint{4}}{\vefour}, \overname{\elint{5}}{\vefive})}{\vefourfive})
  \end{array}
\]
...
\begin{mathpar}
  \inferrule{
    \falsep \eqdef \texttt{E\_Literal}(\texttt{L\_Bool}(\texttt{False}))\\
    \evalexpr{\env, \texttt{E\_Cond}(\veone, \vetwo, \falsep)} \evalarrow C
  }
  {
...
\begin{emptyformal}
  \begin{mathpar}
    \inferrule{
      \truep \eqdef \texttt{E\_Literal}(\texttt{L\_Bool}(\texttt{True}))\\
      \evalexpr{\env, \texttt{E\_Cond}(\veone, \truep, \texttt{e2})} \evalarrow C
    }
    {
...
\begin{emptyformal}
  \begin{mathpar}
    \inferrule{
      \truep \eqdef \texttt{E\_Literal}(\texttt{L\_Bool}(\texttt{True}))\\
      \evalexpr{\env, \texttt{E\_Cond}(\veone, \vetwo, \truep)} \evalarrow C
    }
    {
...
\section{SemanticsRule.CTC \label{sec:SemanticsRule.CTC}}
  ...
  \CodeSubsection{\CTCBegin}{\CTCEnd}{../Interpreter.ml}
...
\begin{emptyformal}
\subsection{Formally}
Recall that $\concat$ stands for list concatenation.
\begin{mathpar}
  \inferrule[empty]{}
  {
...
\begin{mathpar}
  ...
  {
    \casetoconds(\texttt{E\_Var}(\vx), [(\vp, \vs)] \concat \caselist) \evalarrow \\
...
by invoking $\typesatisfies$ with the static environment $\tenv$.
...
\subsection{Formally}
$\astlabel(n)$ is the label of the AST node $n$.
