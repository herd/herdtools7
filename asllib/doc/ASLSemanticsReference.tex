...
\begin{mathpar}
  ...
  \and
  \inferrule{  b \leq 0 }{
    \binop(\texttt{"/"}, Z(a), Z(b)) \evalarrow \Error(\texttt{"/ denominator error!"})
  }
  \and
  \inferrule{  b > 0 \\
  n = a \div b \\
...
Finally, we employ the definition of $\binop$ to define the rule for a binary operator expression:
  \begin{mathpar}
    \inferrule{
      ...
      \evalexpr{ \envp, \vetwo } \evalarrow \Normal((\vmtwo, \vgtwo), \newenv) \OrAbnormal \\
      \vmone = (\vvone, \vgone) \\
      \vmtwo = (\vvtwo, \vgtwo) \\
...
\begin{mathpar}
  \inferrule{
    \evalexpr{\env, \ve} \evalarrow \Normal((\vvone, \vgone), \envp)  \OrAbnormal\\
    \evalpattern{\env, \ve, \vvone, \vp} \evalarrow \Normal(\vv, \vgtwo)\\
    \newenv = \envp\\
