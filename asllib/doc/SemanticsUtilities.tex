...
\begin{itemize}
  ...
  with the same global component as $\denv$ --- $\denv.\dynamicenvsG$, and local component $\denv.\dynamicenvsL$,
...
\begin{mathpar}
  \inferrule{
    ...
    \newenv \eqdef (\tenv, \{\dynamicenvsG: \denv.\dynamicenvsG, \dynamicenvsL:\denv.\dynamicenvsL[\name \mapsto \bot]\})
  }
  {
    \removelocal(\env, \name) \evalarrow \newenv
  }
...
\ProseParagraph
\ProseEqdef{the result \executiongraphterm{}}{a Read Effect on $\name$}.
\ProseNote{The value $\vv$ is ignored, as execution graphs do not contain values.}
...
\inferrule{}
{
  \readidentifier(\name, \vv) \evalarrow ( \overtext{\{\ReadEffect(\name)\}}{\vnodes}, \overtext{\emptyset}{\vedges}, \overtext{\{\ReadEffect(\name)\}}{\voutputnodes} )
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{read_identifier}
...
\ProseParagraph
\ProseEqdef{the result \executiongraphterm{}}{a Write Effect on $\name$}.
\ProseNote{The value $\vv$ is ignored, as execution graphs do not contain values.}
...
\inferrule{}
{
  \writeidentifier(\name, \vv) \evalarrow ( \overtext{\{\WriteEffect(\name)\}}{\vnodes}, \overtext{\emptyset}{\vedges}, \overtext{\{\WriteEffect(\name)\}}{\voutputnodes} )
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{write_identifier}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \Proseeqdef{$\newvs$}{the \nativevalueterm{} bitvector for the sequence of bits $\vres$}.
  \end{itemize}
\end{itemize}
...
\RenderRule{slices_to_positions}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \Proseeqdef{$\maxpos$}{the maximum position in $\slices$};
  ...
  \begin{itemize}
    ...
    \item applying $\slicestopositions$ to $\slices$ yields the list $\positions$\ProseOrError;
    \item \Proseeqdef{$\maxpos$}{the maximum position in $\slices$};
    \item \Proseeqdef{$\bits$}{$\vi$  given as two's complement form of $\maxpos + 1$ bits};
    \item \Proseeqdef{$\newbits$}{the list of bits $\bits[j]$ for every \Proselistrange{$j$}{$\bits$}};
    \item \Proseeqdef{$\vv$}{the native bitvector for the list of bits $\newbits$}.
  \end{itemize}
\end{itemize}
...
\begin{mathpar}
\inferrule[bitvector]{
  \slicestopositions(\slices) \evalarrow \positions \OrDynError\hva\\\\
  \vi \in \listrange(\slices): \maxposofslice(\slice[\vi]) \evalarrow \vp_\vi\hva\\\\
  \maxpos \eqdef \max(\ \{ \vi\in\listrange(\slices) : \vp_\vi \}\ )\hva\\\\
  \commonprefixline\hva\\\\
  \bits \reverseeqdef \bv[n] ... \bv[0]\\
  \decheck(\maxpos \leq n, \BadIndex) \evalarrow \OrDynError\hva\\\\
...
\inferrule[integer]{
  ...
  \maxpos \eqdef \max(\ \{ \vi\in\listrange(\slices) : \vp_\vi \}\ )\hva\\\\
  \commonprefixline\hva\\\\
  \bits \eqdef \vi\text{ given as two's complement form of }(\maxpos + 1)\text{ bits}\\
  \commonsuffixline\hva\\\\
  % The following premise is essentially Bitvector.extract_slice
  \newbits \eqdef [\ \vj \in \listrange(\positions): \bits[\vj]\ ]
}{
...
\RenderRule{read_from_bitvector}
...
\inferrule{
  ...
  \slicestopositions(\slices) \evalarrow \positions \OrDynError\hva\\\\
  \decheck(\listlen{\positions} = m + 1, \BadIndex) \evalarrow \OrDynError\hva\\\\
  \vi \in \listrange(\slices): \maxposofslice(\slice[\vi]) \evalarrow \vp_\vi\hva\\\\
  \maxpos \eqdef \max(\ \{ \vi\in\listrange(\slices) : \vp_\vi \}\ )\hva\\\\
  \decheck(\maxpos < n + 1, \BadIndex) \evalarrow \OrDynError\hva\\\\
  \positions \reverseeqdef I_m \ldots I_0 \\
  {\bitfunc = \lambda i \in n..0.\left\{ \begin{array}{ll}
    \vs_j & \exists j\in m..0.\ i = I_j\\
    \vd_i & \text{otherwise}
  \end{array} \right.}\hva\\\\
...
\ProseParagraph
\ProseEqdef{$\vv$}{the value of field $\name$ in record $\record$}.

\FormallyParagraph
\BackupOriginalRule{
\begin{mathpar}
\inferrule{
  \record \reverseeqdef \NVRecord(\fieldmap)
}{
  \getfield(\name, \record) \evalarrow \fieldmap(\name)
}
\end{mathpar}
} % END_OF_BACKUP_ORIGINAL_RULE
\RenderRule{get_field}
...
\ProseParagraph
\ProseEqdef{$\fieldmapp$}{the record value with the field map of $\record$ updated to bind $\name$ to $\vv$}.
...
\AllApply
\begin{itemize}
  \item \Proseeqdef{\vg}{the execution graph consisting of a Write Effect on $\name$};
  \item $\env$ is the environment consisting of the static environment $\tenv$
        and the dynamic environment $\denv$;
  \item \Proseeqdef{\newenv}{the environment $\env$ modified to declare the variable $\name$
        as a local storage element bound to the value $\vv$}.
\end{itemize}
...
\RenderRule{declare_local_identifier}
...
\RenderRule{declare_local_identifier_m}
...
\RenderRule{declare_local_identifier_mm}
