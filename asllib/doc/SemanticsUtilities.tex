...
\begin{itemize}
  \item \SemanticsRuleRef{GetStackSize} retrieves the value of the $\stacksize$ value from an environment;
  \item \SemanticsRuleRef{SetStackSize} updates the value of the $\stacksize$ value in an environment;
  \item \SemanticsRuleRef{IncrStackSize} updates the value of the $\stacksize$ value in an environment by incrementing it by one;
  \item \SemanticsRuleRef{DecrStackSize} updates the value of the $\stacksize$ value in an environment by decrementing it by one;
  \item \SemanticsRuleRef{RemoveLocal} removes a local storage element from a given environment;
  \item \SemanticsRuleRef{ReadIdentifier} creates an \executiongraph{} representing a Read Effect for a given identifier;
  \item \SemanticsRuleRef{WriteIdentifier} creates an \executiongraph{} representing a Write Effect for a given identifier;
