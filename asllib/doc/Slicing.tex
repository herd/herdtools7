...
\[
  ...
  \left(
  \begin{array}{ll}
  \SlicesResult((\overname{(\vals \times \vals)^*}{\ranges} \times \overname{\XGraphs}{\newg}), \overname{\envs}{\newenv}) & \cup\\
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item evaluating the slice $\vslice$ in $\env$ results in \\
          $\SlicesResult((\range, \vgone), \envone)$\ProseOrAbnormal;
    \item evaluating the tail list $\slicesone$ in $\envone$ results in \\
          $\SlicesResult((\rangesone, \vgtwo), \newenv)$\ProseOrAbnormal;
...
\inferrule[empty]{}
{
  \evalslices(\env, \emptylist) \evalarrow \SlicesResult((\emptylist, \emptygraph), \env)
}
\end{mathpar}
\begin{mathpar}
\inferrule[nonempty]{
  \slices \eqname [\vslice] \concat \slicesone\\
  \evalslice(\env, \vslice) \evalarrow \SlicesResult((\range, \vgone), \envone) \OrAbnormal\\\\
  \evalslices(\envone, \slicesone) \evalarrow \SlicesResult((\rangesone, \vgtwo), \newenv) \OrAbnormal\\\\
  \ranges \eqdef [\range] \concat \rangesone\\
  \newg \eqdef \vgone \parallelcomp \vgtwo
}{
  \evalslices(\env, \slices) \evalarrow \SlicesResult((\ranges, \newg), \newenv)
}
\end{mathpar}
...
\[
  ...
  \left(
  \begin{array}{ll}
  \SlicesResult(((\overname{\tint}{\vstart} \times \overname{\tint}{\vlength}) \times \overname{\XGraphs}{\newg}), \overname{\envs}{\newenv}) & \cup\\
  \overname{\TThrowing}{\ThrowingConfig}    & \cup\\
  \overname{\TDynError}{\DynErrorConfig}    & \cup\\
  \overname{\TDiverging}{\DivergingConfig}  & \\
  \end{array}
...
resulting either in \\
$\SlicesResult(((\vstart, \vlength), \vg), \newenv)$.
\ProseOtherwiseAbnormal
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item evaluating $\ve$ in $\env$ results in $\ExprResult((\vstart, \newg), \newenv)$\ProseOrAbnormal;
  ...
  \begin{itemize}
    ...
    \item evaluating $\etop$ in $\env$ is $\ExprResult(\mtop, \envone)$\ProseOrAbnormal;
    \item $\mtop$ is a pair consisting of the native integer $\vvsubtop$ and execution graph $\vgone$;
    \item evaluating $\estart$ in $\envone$ is $\ExprResult(\mstart, \newenv)$\ProseOrAbnormal;
  ...
  \begin{itemize}
    ...
    \item evaluating $\estart$ in $\env$ is $\ExprResult(\mstart, \envone)$\ProseOrAbnormal;
    \item evaluating $\elength$ in $\envone$ is $\ExprResult(\mlength, \newenv)$\ProseOrAbnormal;
  ...
  \begin{itemize}
    ...
    \item evaluating $\efactor$ in $\env$ is $\ExprResult(\mfactor, \envone)$\ProseOrAbnormal;
    \item $\mfactor$ is a pair consisting of the native integer $\vfactor$ and execution graph $\vgone$;
    \item evaluating $\elength$ in $\env$ is $\ExprResult(\mlength, \newenv)$\ProseOrAbnormal;
    \item $\mlength$ is a pair consisting of the native integer $\vlength$ and execution graph $\vgtwo$;
...
\begin{mathpar}
\inferrule[single]{
  \evalexpr{\env, \ve} \evalarrow \ExprResult((\vstart, \newg), \newenv) \OrAbnormal\\
  \vlength \eqdef \nvint(1)
}{
  \evalslice(\env, \SliceSingle(\ve)) \evalarrow \SlicesResult(((\vstart, \vlength), \newg), \newenv)
}
\end{mathpar}

\begin{mathpar}
\inferrule[range]{
  \evalexpr{\env, \etop} \evalarrow \ExprResult(\mtop, \envone) \OrAbnormal\\\\
  \mtop \eqname (\vvsubtop, \vgone)\\
  \evalexpr{\envone, \estart} \evalarrow \ExprResult(\mstart, \newenv) \OrAbnormal\\\\
  \mstart \eqname (\vstart, \vgtwo)\\
  \binoprel(\MINUS, \vvsubtop, \vstart) \evalarrow \vdiff\\
  \binoprel(\PLUS, \nvint(1), \vdiff) \evalarrow \vlength\\
  \newg \eqdef \vgone \parallelcomp \vgtwo
}{
  \evalslice(\env, \SliceRange(\etop, \estart)) \evalarrow \\ \SlicesResult(((\vstart, \vlength), \newg), \newenv)
}
\end{mathpar}

\begin{mathpar}
\inferrule[length]{
  \evalexpr{\env, \estart} \evalarrow \ExprResult(\mstart, \envone) \OrAbnormal\\
  \evalexpr{\envone, \elength} \evalarrow \ExprResult(\mlength, \newenv) \OrAbnormal\\
  \mstart \eqname (\vstart, \vgone)\\
  \mlength \eqname (\vlength, \vgtwo)\\
  \newg \eqdef \vgone \parallelcomp \vgtwo
}{
  \evalslice(\env, \SliceLength(\estart, \elength)) \evalarrow \\ \SlicesResult(((\vstart, \vlength), \newg), \newenv)
}
\end{mathpar}

\begin{mathpar}
\inferrule[scaled]{
  \evalexpr{\env, \efactor} \evalarrow \ExprResult(\mfactor, \envone) \OrAbnormal\\
  ...
  \evalslice(\env, \SliceStar(\efactor, \elength)) \evalarrow \\ \SlicesResult(((\vstart, \vlength), \newg), \newenv)
}
\end{mathpar}
