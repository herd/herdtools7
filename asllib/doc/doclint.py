...
def check_macro_arity(latex_files: list[str]) -> int:
    ...
    macro_def_with_optional = re.compile(r"\\newcommand\\([a-zA-Z]+)\[(\d+)\]\[[^\]]*\]")
...
def _find_macro_invocations(line: str) -> list[tuple[str, list[str]]]:
    ...
    latex_primitives = {
        'left', 'right', 'big', 'Big', 'bigg', 'Bigg',  # Delimiter sizing
        'choose', 'atop', 'over',  # Infix operators
    }
    ...
    while i < len(file_str):
        ...
        if file_str[i] == '\\' and i + 1 < len(file_str) and file_str[i + 1].isalpha():
            ...
            while k < len(file_str):
                ...
                # Extract the content of this brace group, handling nesting
                arg, end_pos = _extract_brace_content_with_linecount(file_str, k, line_number)
                if arg is not None:
...
def _extract_brace_content_with_linecount(file_str: str, start_pos: int, current_line: int) -> tuple[tuple[str, int] | None, int]:
    r"""
