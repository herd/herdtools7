...
We use $\tenv$ and similar variable names to range over static environments.
...
All of the following apply:
\begin{itemize}
  \item applying $\splitlist$ to $\callerargtyped$ yields into the corresponding lists of types
...
\begin{itemize}
  ...
  \item All of the following apply:
  \begin{itemize}
    \item $\calleeparams$ is a non-empty with \head\ $\calleeparam$ and \tail\ \\
          $\calleeparamsone$;
    \item One of the following applies:
    \begin{itemize}
...
\begin{itemize}
  ...
  \item All of the following apply (\textsc{other}):
  \begin{itemize}
    \item $\tty$ is not one of the types in the previous cases: bitvector type, integer type, or a tuple type;
    \item $\newty$ is $\tty$.
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\ve$ is the array access expression for base expression $\veone$ and index expression $\vetwo$,
          that is, $\EGetArray(\veone, \vetwo)$;
    \item applying $\substexpr$ to $\substs$ and $\veone$ in $\tenv$ yields $\veonep$;
    \item applying $\substexpr$ to $\substs$ and $\vetwo$ in $\tenv$ yields $\vetwop$;
    \item define $\newe$ as the array access expression for base expression $\veonep$ and index expression $\vetwop$,
...
formal arguments $\calleeargs$ with the parameters substituted by their corresponding
...
\inferrule[arg\_is\_param]{
  ...
  \calleeargisparam \eqdef \exists \vi\in\listrange(\calleeparams).\ \calleeparams[\vi]=(\calleex, \Ignore)\\
...
\]
returns the set of identifiers $\ids$ on which the subprogram signature given
by $\vf$ depends on.
...
The parsed AST of an ASL specification consists of a list of declaration.
...
We say that a list of subset of $V$ --- $\compstwo$ --- respects the topological ordering of $\comps$
...
\begin{itemize}
  ...
  \item $\compdecls$ applies $\declsofcomp$ to each component $\vc$ in $\compstwo$ to transform it into a list,
        yielding a list of lists where each sub-list corresponds to one strongly-connected component;
...
\]
return the set of dependent pairs of identifiers $\dependencies$ induced by the declaration $\vd$.
...
\begin{itemize}
  ...
  \item splitting $\envandfs$ into two sub-lists by testing each pair to check whether the subprogram declaration
...
\]
returns set of identifiers $\ids$ on which the declaration $\vd$ depends on.
...
\]
returns set of identifiers $\ids$ on which the type or optional type $\vt$ depends on.
...
\]
returns set of identifiers $\ids$ on which the optional pair consisting of
...
\]
returns set of identifiers $\ids$ on which the expression or optional expression $\ve$ depends on.
...
\begin{itemize}
  ...
  \item All of the following apply (\textsc{e\_call}):
  \begin{itemize}
    \item $\ve$ is the call expression of subprogram named $\vx$ with argument expressions $\vargs$ and parameter expressions $\namedargs$;
  ...
  \item All of the following apply (\textsc{e\_tuple}):
  \begin{itemize}
    \item $\ve$ is the tuple construction expression of with expressions $\ves$;
    \item define $\ids$ as the union of applying of $\useexpr$ to each expression in $\ves$.
  \end{itemize}
  ...
  \item All of the following apply (\textsc{e\_pattern}):
  \begin{itemize}
    \item $\ve$ is the pattern testing expression for sub-expression $\veone$ and pattern $\vp$;
...
\]
returns set of identifiers $\ids$ on which the left-hand-side expression $\vle$ depends on.
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vle$ is a left-hand-side multi-expression with the list of expressions $\vles$;
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vp$ is a pattern for testing less-thean-or-equal with respect to the expression $\ve$;
    \item define $\ids$ as the application of $\useexpr$ to $\ve$.
  \end{itemize}
...
\]
returns set of identifiers $\ids$ on which the slice $\vs$ depends on.
...
\]
returns set of identifiers $\ids$ on which the bitfield $\vbf$ depends on.
...
\]
returns set of identifiers $\ids$ on which the integer constraint $\vc$ depends on.
...
\]
returns set of identifiers $\ids$ on which the statement $\vs$ depends on.
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vs$ is a try statement with statement $\vsone$, cathcer list $\catchers$, and otherwise statement $\vstwo$;
...
\]
returns set of identifiers $\ids$ on which the local declaration item $\ldi$ depends on.
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\ldi$ is a discarding decalaration;
    \item define $\ids$ as the empty set.
  \end{itemize}

  \item All of the following apply (\textsc{typed}):
  \begin{itemize}
    \item $\ldi$ is a typed decalaration for the local declaration item $\ldione$ and type $\vt$;
    \item define $\ids$ as the union of applying $\useldi$ to $\ldione$ and $\usety$ to $\vt$.
  \end{itemize}

  \item All of the following apply (\textsc{tuple}):
  \begin{itemize}
    \item $\ldi$ is a multi-variable decalaration for the list of local declarations $\ldis$;
...
\]
returns set of identifiers $\ids$ on which the case alternative $\vc$ depends on.
...
\]
returns set of identifiers $\ids$ on which the try statement catcher $\vc$ depends on.
...
This error indicates that at least one declarations in a given list of mutually-recursive declarations
is not a subprogram
(see \secref{sec:TypingRule.TypeCheckMutuallyRec}).
