...
An \emph{abstract syntax tree} (AST, for short) represents an ASL program via a labelled tree. 

\section{Environments}
A \emph{type judgement} associates a node in an abstract syntax tree (which intuitively corresponds to an ASL construct) with a certain type. We also say that the node is \emph{annotated} with that type.
%
