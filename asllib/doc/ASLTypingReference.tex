...
to opernads of type $\vtone$ and $\vttwo$ in the static environment $\tenv$,
...
Typing.ExpressionList (\secref{TypingRule.ExpressionList}).
...
\begin{emptyformal}
  ...
  \begin{mathpar}
    ...
    {\annotateliteral{\vl}\typearrow \TBits(\texttt{Bitwidth\_SingleExpr}(\Elit{n}), \emptylist)}
  \end{mathpar}
\end{emptyformal}
...
\texttt{t,new\_e} and all of the following apply:
  ...
  \item $\vx$ is bound to a global constant $\vv$ of type $\tty$ in the global environment given by $\tenv$;
  \item $\vt$ is $\tty$;
  \item $\newe$ is the Literal $\vv$.
  \end{itemize}
...
\texttt{t,new\_e} and all of the following apply:
  ...
  \item $\vt$ is the result of checking compatibility of $\op$ with $\vtone$ and $\vttwo$ as per \secref{TypingRule.CheckBinop};
...
\texttt{t,new\_e} and all of the following apply:
  \begin{itemize}
  \item $\ve$ denotes a unary operation $\op$ over an expression $\vep$;
  \item \texttt{t'',e''} is the result of annotating $\vep$ in $\tenv$;
  \item $\vt$ is the result of checking compatibility of $\op$ with \texttt{t''} as per \secref{TypingRule.CheckUnop};
  \item $\newe$ denotes $\op$ over \texttt{e''}.
  \end{itemize}
...
\begin{mathpar}
\inferrule{
  \annotateexpr{\tenv, \ve'} \typearrow (\vt'', \ve'')\\
  \CheckUnop(\tenv, \op, \vt'') \typearrow \vt
  }
{\annotateexpr{\tenv, \EUnop(\op, \ve')} \typearrow (\vt, \EUnop(\op, \ve''))}
\end{mathpar}
\end{emptyformal}
...
\texttt{t,new\_e} and all of the following apply:
  ...
  \item \texttt{t\_cond, e'\_cond} is the result of annotating \texttt{e\_cond} in $\tenv$;
  \item \texttt{t\_true, e'\_true} is the result of annotating \texttt{e\_true} in $\tenv$;
  \item \texttt{t\_false, e'\_false} is the result of annotating \texttt{e\_false} in $\tenv$;
  \item One of the following applies:
    \begin{itemize}
    \item All of the following apply:
      \begin{itemize}
      \item $\vt$ is the lowest common ancestor of \texttt{t\_true} and \texttt{t\_false};
      \item $\newe$ is the condition \texttt{e'\_cond} with two options \texttt{e'\_true} and \texttt{e'\_false}.
      \end{itemize}
    \item All of the following apply:
      \begin{itemize}
...
\begin{mathpar}
\inferrule{
  \annotateexpr{\tenv, \econd} \typearrow (\tcond, \econd')\\
  \annotateexpr{\tenv, \etrue} \typearrow (\ttrue, \etrue')\\
  \annotateexpr{\tenv, \efalse} \typearrow (\tfalse, \efalse')\\
  \lca(\ttrue, \tfalse) \typearrow \vt
  }
{\annotateexpr{\ECond(\econd, \etrue, \efalse)} \typearrow (\vt, \ECond(\econd', \etrue', \efalse'))}
\end{mathpar}
\end{emptyformal}
...
\inferrule{
  ...
  \sliceswidth(\tenv, \texttt{slices}) \typearrow \vw\\
  \annotateslices(\tenv, \texttt{slices}) \typearrow \texttt{slices}'
  }
{\annotateexpr{\tenv, \ESlice(\ve, \texttt{slices})} \typearrow \\\
...
\texttt{t,new\_e} and all of the following apply:
  \begin{itemize}
  \item $\ve$ denotes a call to a subprogram named $\name$ with arguments \texttt{args} and
    parameters \texttt{eqs};
  \item \texttt{name', args', eqs', ty} is the result of annotating the call of
    that subprogram in $\tenv$ (annotating calls is defined in Chapter~\ref{chap:TypingSubprogramCalls});
  \item $\vt$ is $\tty$;
  \item $\newe$ is the call to the subprogram named \texttt{name'} with arguments \texttt{args'}
...
\texttt{t,new\_e} or an error and all of the following apply:
  ...
  \item an error ``\texttt{Conflicting Types}'' or \texttt{t\_index'} \typesatisfies\  \texttt{wanted\_t\_index} as per \secref{TypingRule.TypeSatisfaction} and all of the following apply:
  \item $\newe$ is an access to array \texttt{e''} at index \texttt{e\_index'}.
  \end{itemize}
...
{\annotatearrayindex(\tenv, \EVar(n)) \typearrow \vt}
\and
\inferrule{G^\tenv.\declaredtypes(n) = \bot\\
l_f = \Elit{0} \\
\vlt = \EBinop(\MINUS, \EVar(n), \Elit{1})
}
{\annotatearrayindex(\tenv, \EVar(n)) \typearrow \TInt(\langle[\ConstraintRange(l_f, \vlt)]\rangle)}
\and
\inferrule{\astlabel(\ve) \neq \EVar \\
l_f = \Elit{0} \\
\vlt = \EBinop(\MINUS, \ve, \Elit{1})
}
{\annotatearrayindex(\tenv, \ve) \typearrow \TInt(\langle[\ConstraintRange(l_f, \vlt)]\rangle)}
\end{mathpar}

And now define the rule to annotate the array access expression:
\begin{mathpar}
\inferrule{
  \annotateexpr{\tenv, \texttt{e}} \typearrow (\texttt{t\_e}', \texttt{e}'')\\
  \tstruct(\tenv, \texttt{t\_e}') \typearrow \TArray(\texttt{size}, \tty')\\
  \annotatearrayindex(\tenv, \texttt{size}) \typearrow \texttt{wanted\_t\_index}\\
  \annotateexpr{\tenv, \eindex} \typearrow (\texttt{t\_{index}}', \texttt{e\_{index}}')\\
  \typesat(\tenv, \texttt{t\_{index}}', \texttt{wanted\_t\_index})\\
  }
{\annotateexpr{\tenv, \ESlice(\texttt{e}', [\SliceSingle(\eindex)])} \typearrow\\\\
...
\texttt{t,new\_e} and all of the following apply:
  ...
  \item For each field named $\name$ associated with the expression $\vep$ in
    ...
    \item \texttt{t'} \typesatisfies\  \texttt{t\_spec'} as per \secref{TypingRule.TypeSatisfaction};
...
\inferrule{
L \in \{\TRecord, \TException\}\\
i=1..k: \annotateexpr{\vt_i, \ve_i} \typearrow (\vt_i',\ve_i')\\
i=1..k: \typesat(\tenv, \vt_i', \vt_i)
}
{\annotateexpr{\tenv, L(\tty, [i=1..k: (f_i,\ve_i)])} \typearrow (\tty, L(\tty, [i=1..k: (f_i,\ve_i')]))}
\end{mathpar}
\end{emptyformal}
...
\texttt{t,new\_e} and all of the following apply:
  ...
  \item \texttt{ty'} is the structure of $\tty$ in $\tenv$;
  \item $\vt$ is $\tty$;
  \item $\newe$ is an expression \UNKNOWN\  of type \texttt{ty'}.
  \end{itemize}
...
\inferrule[type\_equal]{
  \annotateexpr{\tenv, \ve'} \typearrow (\vt, \ve'') \OrTypeError\\
  \tstruct(\tenv, \vt) \typearrow \vtstruct \OrTypeError\\
...
{
  \annotateexpr{\tenv, \EATC(\ve', \tty)} \typearrow (\tty', \ve'')
}
\and
\inferrule[dynamic]{
  \annotateexpr{\tenv, \ve'} \typearrow (\vt, \ve'') \OrTypeError\\
...
{
  \annotateexpr{\tenv, \EATC(\ve', \tty)} \typearrow (\vt', \EATC(\ve'', \vtystruct))
}
\and
\inferrule[error]{
  \annotateexpr{\tenv, \ve'} \typearrow (\vt, \ve'') \OrTypeError\\
...
{
  \annotateexpr{\tenv, \EATC(\ve', \tty)} \typearrow \TypeErrorVal{ATC}
}
\end{mathpar}
\end{emptyformal}
...
\inferrule{
  ...
  \les = [\ve_{1..k}]\\
  \subtys = [\vt_{1..k}]\\
  i=1..k: \annotateexpr{\tenv, \ve_i,\vt_i} \typearrow \ve'_i\\
  \les' = [i=1..k: \ve'_i]\\
...
\begin{mathpar}
  \inferrule{
    ...
    \annotateexpr{\tenv, \ve} \typearrow (\vte', \ve')\\
    \typesat(\tenv, \vte', \vt)\\
...
environment $\newenv$ and all of the following apply:
   ...
   \item \texttt{new\_s} is a case statement with expression $\veone$ and cases \texttt{cases1};
   \item $\newenv$ is \texttt{env1}.
   \end{itemize}
...
environment $\newenv$ and all of the following apply:
   ...
   \item One of the following applies:
     \item All of the following apply:
       ...
       \item \texttt{new\_s} is a \texttt{while} statement with expression \vetwo and statement block \texttt{s2};
       \item $\newenv$ is $\tenv$.
       \end{itemize}
     \item an error ``\texttt{Conflicting Types}'' is raised.
   \end{itemize}
...
environment $\newenv$ and all of the following apply:
   ...
   \item One of the following applies:
     ...
     \item All of the following apply:
       \begin{itemize}
       \item $\vt$ \typesatisfies\  \texttt{t\_bool};
       \item \texttt{new\_s} is a \texttt{repeat} statement with expression \vetwo and statement block \texttt{s2};
       \item $\newenv$ is $\tenv$.
       \end{itemize}
     \item an error ``\texttt{Conflicting Types}'' is raised.
     \end{itemize}
...
%   }
%   {
%     \annotatestmt{\tenv, \texttt{S\_For}(\id, \dir, \veone, \vetwo, \vs')} \typearrow (\news, \newtenv)
%   }
% \]
\end{emptyformal}
...
\begin{mathpar}
  \inferrule{
    ...
    \annotatestmt{\tenv, \vs'} \typearrow \vs''\\
...
\begin{mathpar}
  \inferrule{
    \vs = \texttt{S\_Decl}(\LDKConstant, \ldi, \langle\ve\rangle)\\
    \annotateexpr{\tenv, \ve} \typearrow (\vte, \ve')\\
    \reduceconstants{\tenv, \ve} \typearrow \vv\\
    \declarelocalconstant{\tenv, \ldi} \typearrow (\tenv', \vte, \vv, \ldi')\\
    \news = \texttt{S\_Decl}(\LDKConstant, \ldi', \langle\ve'\rangle)\\
    \newtenv = \tenv'
  }
  {
    \annotatestmt{\tenv, \vs} \typearrow (\news, \newtenv)
...
\begin{mathpar}
  \inferrule{
    \vs = \texttt{S\_Decl}(\LDKConstant, \ldi, \None)\\
    \annotatelocaldeclitemuninit{\tenv, \ldi} \typearrow (\tenv', \vs')\\
    \news = \vs'\\
    \newtenv = \tenv'
  }
  {
...
\begin{mathpar}
  \inferrule{
    \annotatestmt{\tenv, \vs} \typearrow (\vs', \Ignore)
  }
  {
    \annotateblock{\tenv, \vs, \texttt{return\_type}} \typearrow \vs'
  }
\end{mathpar}
\end{emptyformal}
...
\[
  ...
  \overname{(\identifier\times\expr)^*}{\eqstwo} \aslsep
  \overname{\subprogramtype}{\rettyone}
\end{array}
\]

Annotates the call to subprogram $\name$ with arguments $\vargs$,
parameters $\eqs$, and call type $\calltype$, resulting in values for a rewritten call ---
$\nameone$, $\vargsone$, $\eqstwo$, $\rettyone$ --- or a type error if one is detected.

One of the following applies:
\begin{itemize}
\item TypingRule.FCallBadArity (see \secref{TypingRule.FCallBadArity}),
\item TypingRule.FCallGetter (see \secref{TypingRule.FCallGetter}),
\item TypingRule.FCallSetter (see \secref{TypingRule.FCallSetter}),
\item TypingRule.FCallMismatch (see \secref{TypingRule.FCallMismatch}).
\end{itemize}
...
\begin{itemize}
  ...
  \item TypingRule.FindSubprogram1 (see \secref{TypingRule.FindSubprogram1}),
  \item TypingRule.FindSubprogram2 (see \secref{TypingRule.FindSubprogram2}),
...
All of the following apply:
\begin{itemize}
\item $\name$ is bound in $\tenv$ to a function with argument types \texttt{callee\_arg\_types};
...
\section{TypingRule.FCallGetter \label{sec:TypingRule.FCallGetter}}
  ...
  \subsection{Prose}
  Annotating the call to subprogram $\name$ with arguments \texttt{args},
  ...
  or an error is raised and all of the following apply:
   \begin{itemize}
   \item \texttt{caller\_arg\_types, arg1} is the result of annotating \texttt{args} in $\tenv$;
   \item $\name$ is bound in $\tenv$ to a subprogram with argument types
...
\section{TypingRule.FCallSetter \label{sec:TypingRule.FCallSetter}}
  ...
  \begin{itemize}
    \item \texttt{caller\_arg\_types, arg1} is the result of annotating
      \texttt{args} in $\tenv$;
    \item $\name$ is bound in $\tenv$ to a subprogram with a unique
...
\subsection{Prose}
Annotating an ASL specification $\decls$ in an environment $\tenv$ results in a rewritten specification $\declsp$
