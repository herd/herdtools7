...
The purpose of this document is to describe, in a definitive way, which ASL programs are considered \emph{valid}.
Validity is defined in terms of a \emph{type system}~\cite{TypeSystemsLucaCardelli}. That is, a set of \emph{type rules}.
...
A \emph{static environment} (also called a \emph{type environment}) is what the type rules operate over:
...
\section{TypingRule.CatcherSome \label{sec:TypingRule.CatcherSome}}
  ...
  \subsection{Prose}
   Annotating catcher \texttt{(name\_opt, ty, stmt)} in an environment
\texttt{env} given a type \texttt{return\_type} (\texttt{annotate\_catchers env return\_type (name\_opt, ty, stmt)})results in \texttt{(name\_opt, ty, new\_stmt)} and all of the following apply:
   \begin{itemize}
   \item \texttt{ty} has the structure of an exception type;
...
\newcommand\TTypeError[0]{\textsf{TTypeError}}
\newcommand\TypeErrorConfig[0]{\texttt{\#TE}}
...
\begin{itemize}
  ...
  is not enough to prove it, and so we ``pessimistically'' answer negatively.
\end{itemize}
In proof-theoretic terms, we say that such tests are \emph{sound} but \emph{incomplete}.

Notice that for a conservative test, it is always correct to return $\False$.

\begin{definition}[\hypertarget{def-equal}{Equal Rule}]
  The meta function $\equate$ equates to values of the same mathematical type and returns
  the respective Boolean value:
  \begin{mathpar}
      \inferrule[equate-true]{a = b}
      {
          \equate(a, b) \rightarrow \True
      }
      \and
      \inferrule[equate-false]{a \neq b}
      {
          \equate(a, b) \rightarrow \False
      }
  \end{mathpar}
  \end{definition}
  The reason we define such a function for equality is to allow using it as a short-circuiting premise.
...
\begin{itemize}
  \item All of the following applies (\textsc{all\_supported}):
  \begin{itemize}
  ...
  \item All of the following applies (\textsc{unsopported1}):
  \begin{itemize}
    \item $\veone$ can not be transformed into an intremediate representation.
    \item $\vb$ is $\False$;
  \end{itemize}

  \item All of the following applies (\textsc{unsupported2}):
  \begin{itemize}
    \item $\vetwo$ can not be transformed into an intremediate representation.
    \item $\vb$ is $\False$;
  \end{itemize}
\end{itemize}
...
\begin{mathpar}
  \inferrule[trecord]{
    \equate(\dom(\vfone), \dom(\vftwo)) \equalarrow \True \terminateas \False\\\\
...
\[
  ...
  \rightarrow \overname{\{\True, \False\}}{\vb}
\]
conservatively tests whether the constraint list $\csone$ is equivalent to the constraint list $\cstwo$ in envieonment $\tenv$.
...
\[
  ...
  \rightarrow \overname{\{\True, \False\}}{\vb}
\]
conservatively tests whether the constraint $\vcone$ is equivalent to the constraint $\vctwo$ in environemnt $\tenv$.
