...
\item All of the following apply (\textsc{t\_tuple}):
  ...
  \item checking at each index $\vi$ of the list $\vlis$ whether the type $\vlit[\vi]$ type-satisfies the type $\vlis[\vi]$
...
{ \makeanonymous(\tenv, \vs) \typearrow\TTuple(\vlis)\\
  ...
  \vi\in\listrange(\vlis): \typesatisfies(\tenv, \vlit[\vi], \vlis[\vi]) \typearrow \vb_i \terminateas \TTypeError\\
...
{
\annotateexpr{\tenv, \ECTC(\ve', \vt')} = (\vt', \ECTC(\ve'', \vt'))
}
\end{mathpar}
\end{emptyformal}
