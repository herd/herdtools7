...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item obtaining the indices of the slice list $\vslices$ in $\tenv$ via $\tododefine{slices\_to\_positions}$
          yields $\positions$;
    ...
    \item converting the first $\posmax+1$ digits of the binary representation of $\vi$ into a bitvector
          via \tododefine{bitvector\_of\_z} yields $\bvone$;
...
\inferrule[e\_slice\_int]{
  ...
  \bvone \eqdef \tododefine{bitvector\_of\_z}(\posmax + 1, \vi)\\
  \tododefine{extract\_slice}(\bvone, \positions) \typearrow \bvtwo \OrTypeError
}{
...
\inferrule[e\_slice\_bitvector]{
  ...
  \tododefine{extract\_slice}(\bv, \positions) \typearrow \bvtwo \OrTypeError
}{
