...
\section{Annotation}
\TODOComment{ come back to this section.}
...
\TODOComment{ROMAN: define expression equality $\equiv_e$ and type equality $\equiv_t$.}
...
Variables of buitin singular types \texttt{integer}, \texttt{real}, \texttt{boolean}, \texttt{bits(4)} and \texttt{bits(2)} are defined.
...
\[
\begin{array}{c}
\inferrule{\typelabel(T) \in \{\TReal, \TString, \TBool, \TBits, \TEnum, \TInt\} }{\entails T : \isbuiltinsingular} \\ 
\end{array}
\]
\end{formal}
...
\\
\RuleComment{\TODOComment{ I think it makes sence to sort the enum identifiers.}}\\
...
\TODOComment{ consider simplifying/elaborating on this.}
\end{emptyformal}

\TODOComment{ Shouldn't we define the domain of tuples, records, arrays, etc. ?}

\TODOComment{ Should we formalize static evaluation of constraints?}
...
\]     
    ...
    \TODOComment{ROMAN: see note about doma}
\end{emptyformal}
...
\TODOComment{ROMAN: it is not obvious to me why sometimes we check for type satisfaction and other times we check the structure of a type.}
...
\section{TypingRule.Subtype}
  ...
  \begin{itemize}
    ...
    \item All of the following apply:
      ...
      \end{itemize}
  \end{itemize}
    The subtype relation is a partial order over named types. \TODOComment{ROMAN: I think the syntax allows subtyping anonymous types as well. Perhaps we should fix the syntax?}
...
\\
\RuleComment{\TODOComment{ consider adding the following rule.}}\\
...
\]
  ...
  This aims to encompass LRM Section 4.3.1 R\_NXRX, I\_KGKS, I\_MTML, I\_JVRM \TODOComment{ I don't see how, in the code}, I\_CHMP.
...
\TODOComment{ This rule doesn't exist in the code. Also, it is redundant, since it just applies to the same arguments in reverse order. I recommend removing this section.}
...
We define the following relation between two types as an intermediate step to defining the type satisfaction relation, later in this chapter.
  ...
  \subsection{Code}
\TODOComment{Why does the code use anonimization instead of getting the structure of types?}
...
\\
\RuleComment{\TODOComment{ define bitwidth equality $E \entails c_s \equiv_b c_t$}}\\
\RuleComment{\TODOComment{ define bitfield list subsumption $E \entails l_s \subseteq_{\ty} l_t$.}}\\
\RuleComment{\TODOComment{ It isn't obvious how these two rules match the prose.}}\\
...
\\
\RuleComment{\TODOComment{ROMAN: Why do we use type equivalence rather than type satisfaction?}}\\
\inferrule{
...
\TODOComment{ why don't we directly define rules for inferring whether a bitvector type has determined/undetermined width and just use that as a premise? Same for constrained/fixed width.}

\[
\begin{array}{c}
\RuleComment{\TODOComment{ I model the following rule based on the code.}}\\
\RuleComment{\TODOComment{ However, I don't understand why they are independent of $t$.}}\\
...
\TODOComment{ define the evaluation of bitwidth length, which is used above.}
...
\subsection{Comments}
  ...
  type-satisfies itself. \TODOComment{ROMAN: I disagree --- we defined subtyping to hold *only* on named types.}
...
\subsection{Prose}
  Annotating a given conditional expression, requires finding a single type that matches both the true sub-expression and the false sub-expression. This requires the notion of a \emph{lowest common ancestor} of two types. \TODOComment{Perhaps we should move this to the next chapter, right before typing ECond?}
...
\TODOComment{Roman: consider defining an ancestor relation and then having the \lca\ be naturally defined as the least common ancestor.}
...
\[
\begin{array}{c}
\RuleComment{\TODOComment{ROMAN: Why do we use type/expression equivalence for arrays}}\\
\RuleComment{\TODOComment{and type satisfiaction for tuples?}}\\
...
{E \entails S \lcasat \lca(T, S)}\\
\\
\RuleComment{\TODOComment{ROMAN: Why do we element-wise \lca\ here but not before?}}\\
\inferrule{E \entails \tstruct(T) = \TTuple(t_1,\ldots,t_k) \\
  E \entails \tstruct(S) = \TTuple(s_1,\ldots,s_k) \\
  E \entails s_1 \typesat t_1 \ldots E \entails s_k \typesat t_k \\
  E \entails t_1 \typesat s_1 \ldots E \entails t_k \typesat s_k \\
...
\TODOComment{ROMAN: I separated the real case.}

%\TODOComment{ROMAN: it seems to me that we are losing the information of named types. For example, \texttt{type b subtypes boolean = TRUE; boolean c = b;} would not type, but \texttt{type b subtypes boolean = TRUE; boolean c = !b;} would (checkop.asl).}
...
\\
\TODOComment{ROMAN: why don't we check that the type satisfies a bitvector type?}\\
  ...
  \item All of the following apply:
    ...
    \item One of the following applies:
      ...
      \item All of the following apply:
        ...
        \item \texttt{t} is an unconstrained integer;
        \end{itemize}
      \item One of the following applies: \TODOComment{ROMAN: I think the next two cases are subsumed by the case of two under/well-constrained integers.}
       \begin{itemize} 
       \item All of the following apply:
          \begin{itemize}
...
\\
\TODOComment{ROMAN: I think our lexical analyzes misses XOR.}\\
\inferrule{
...
\[
\begin{array}{c}
\TODOComment{ROMAN: I think this captures the case of enums.}\\
...
\TODOComment{ROMAN: define \instantiate, which accepts a list of integer constraints and returns a set of integers.}
...
\TODOComment{ROMAN: the environment here is actually an expression. We should probably explain why we returned a transformed expression.}
\begin{itemize}
...
\TODOComment{ROMAN: we seem to be missing TypinRule.EGlobalVarConstantNoVal}
...
\TODOComment{ROMAN: This rule is problematic since it uses a negative premise. Using negative premises requires specific rule ordering strategies, which we probably don't want. Also, I don't understand the rationale for this rule.}
\[
\inferrule{
  G.\constantsvalues \not\entails x : V\\
  G.\storagetypes \entails x : (T, \texttt{GDK\_Constant})
...
\begin{emptyformal}
  \subsection{Formally}
\TODOComment{ROMAN: I don't think we need this, since errors are implicit when typing fails.}
\end{emptyformal}
...
\begin{emptyformal}
    \subsection{Formally}
\TODOComment{ROMAN: define $\textsf{width}$.}
\TODOComment{ROMAN: define $s_1',\ldots,s_k'$.}
\TODOComment{ROMAN: I think that the language is too powerful when it comes to determining bitvector/slices lengths, which requires proving that expressions are equivalent.}    
...
%  E \entails e : (T, e')\\
%  }
%{E \entails \texttt{E\_Call}(\textit{name}, [p_1,\ldots,p_k], [a_1,\ldots,a_n]) : \\\ 
%(, \texttt{E\_Call}(\textit{name}', [p_1',\ldots,p_k'], [a_1',\ldots,a_n']))}
%\]
\end{emptyformal}
