...
All of the following apply:
\begin{itemize}
  \item $\tty$ is the a named type $\vx$, that is $\TNamed(\vx)$;
  \item retrieving the type associated with $\vx$ yields $\vt$ \ProseOrTypeError;
  \item $\newty$ is $\tty$.
\end{itemize}
...
\begin{mathpar}
\inferrule{
  \tododefine{check\_no\_duplicates}(\vli) \typearrow \True \OrTypeError\\
...
\begin{itemize}
  ...
  \item this is the declaration of $\tty$;
  \item $\fields$ is a list of identifier-type pairs $(\vx_i, \vt_i)$, for $i=1..k$;
  \item checking that the list of field identifiers $\vx_{1..k}$ does not contain duplicates
  yields $\True$ \ProseOrTypeError;
  \item annotating each field type $\vt_i$, for $i=1..k$, yields a rewritten type $\vtp_i$
  \ProseOrTypeError;
...
\inferrule{
  ...
  \tododefine{check\_no\_duplicates}(\vx_{1..k}) \typearrow \True \OrTypeError\\
...
\inferrule[nested]{
  ...
  \widthp \eqdef \listrange(\positions)\\
  \tododefine{annotate\_bit\_fields}(\tenv, \widthp, \bitfieldsp) \typearrow \bitfieldspp \OrTypeError\\
}{
  \annotatebitfield(\tenv, \width, \BitFieldNested(\slices, \bitfieldsp)) \typearrow \\
...
\inferrule[type]{
  ...
  \BitFieldType(\name, \slices, \vt) \typearrow \BitFieldType(\name, \sliceone, \vtp)
}
\end{mathpar}
\end{emptyformal}
