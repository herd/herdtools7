...
A \emph{type judgement} associates a node in an abstract syntax tree (which intuitively corresponds to an ASL construct) with a certain type. We then say that the node is \emph{annotated} with that type.
%
A \emph{typing environment} (environment, for short) maps AST nodes to types.

Defining the type annotation requires inferring various auxiliary attributes for AST nodes. We consider these auxiliary attributes as part of the environment as well. Technically, we partition the environment into two distinct components: the global environment---pertaining to AST nodes appearing outside of a given subprogram, and the local environment---pertaining to AST nodes appearing inside a given subprogram.

\section{Type System}
A \emph{typing rule} specifies a conjunction of conditions that must hold in order to annotate an AST node with a given type. These conditions typically inspect the label of node and the type annotations on the children of the node (in a given environment).

Not all conditions are type judgements. Some conditions are expressed in terms of the auxiliary attributes alluded to above. We define these in the next chapter. 

A \emph{typing system} is a set of typing rules. More than one rule can be associated with a given node label, but they are exclusive---at most one rule holds in a given environment.

\section{Annotation}
Typing a program consists of annotating the root of its AST. This is typically done by traversing the AST bottom-up.
To annotate a node, the typing algorithm finds a rule that matches the node---that is a rule whose all conditions are satisfied. If one such rule is found, the node is annotated by the  result type specified by the rule, essentially adding the type judgement to the environment.
If no such rule is found, it is considered a \emph{typing error} and the algorithm exists.

Sometimes it is to define \emph{error rules}---rules that result in an error and provide extra information to help understand the reason for the error.
...
Components can be (possibly-empty) lists of nodes, shown as $n^{*}$, and optional nodes (lists of 0 or 1 elements), shown as $n?$.
...
\[
\begin{array}{rcll}
\textbf{node} & & \textbf{components} & \textbf{description} \\
\hline
\unop &::=& \texttt{"!"} \;|\; \texttt{"-"} \;|\; \texttt{"NOT"} & \\
\binop &::=& \texttt{"\&\&"} \;|\; \texttt{"||"} \;|\; \texttt{"-->"} \;|\;  \texttt{"<->"} & \texttt{binop\_boolean} \\
          &|& \texttt{"=="} \;|\; \texttt{"!="}  \;|\; \texttt{">"}  \;|\; \texttt{">="} \;|\; \texttt{"<"} \;|\; \texttt{"<="} & \texttt{binop\_comparison} \\
          &|& \texttt{"+"} \;|\; \texttt{"-"}  \;|\; \texttt{"OR"}  \;|\; \texttt{"XOR"} \;|\; \texttt{"AND"} & \texttt{binop\_add\_sub\_logic} \\
          &|& \texttt{"*"} \;|\; \texttt{"/"}  \;|\; \texttt{"DIV"}  \;|\; \texttt{"DIVRM"} \;|\; \texttt{"MOD"}  \;|\; \texttt{"<<"}  \;|\; \texttt{"\>\>"} &  \texttt{binop\_mul\_div\_shift} \\
          &|& \texttt{"\^{}"} & \texttt{binop\_pow} \\
	  &|& \texttt{"IN"} & \texttt{binop\_in} \\
	  &|& \texttt{"EOR"} & \texttt{XOR}\text{ in v0}\\
\literal &=& \texttt{<int\_lit>} & \\
 &|&\texttt{<hex\_lit>} & \textbf{missing/int\_lit?}\\
 &|&\texttt{<boolean\_lit>} & \\
 &|& \texttt{<real\_lit>} & \\
 &|& \texttt{<bitvector\_lit>} & \\
 &|& \texttt{<bitmask\_lit>} & \textbf{merged with bitvector\_lit} \\
 &|&\texttt{<string\_lit>} & \\
\end{array}
\]
...
\expr &::=& \texttt{E\_Literal}(literal) & \\
	...
	&|& \texttt{E\_CTC}(\texttt{\expr, \ty}) & \textrm{checked type constraint}\\
	...
	&|& \texttt{E\_Record}(\ty, (\identifier, \expr)^{*}) & \text{Exception construction}\\
...
\pattern &::=& \texttt{Pattern\_All} & \\
  ...
  &|& \texttt{Pattern\_Range}(\expr, \expr)& \text{lower to upper, included}\\
...
\ty &::=& \texttt{T\_Int}(int\_constraints?)& \\
  ...
  &|& \texttt{T\_String} & \\
  &|& \TBool & \\
  &|& \texttt{T\_Bits}(bits\_constraint, bitfield^{*}) & \\
  &|& \texttt{T\_Enum}(\identifier^{*})  & \\
  &|& \texttt{T\_Tuple}(\ty^{*})  & \\
  &|& \texttt{T\_Array}(\expr, \ty)  & \\
  &|& \texttt{T\_Record}(\Field^{*})  & \\
  &|& \texttt{T\_Exception}(\Field^{*})  & \\
  &|& \texttt{T\_Named}(\identifier)  & \text{A type variable} \\
\end{array}
\]
...
The type \texttt{color} consists in two different constants \texttt{RED} and~\texttt{BLACK}.
    ...
    \[
      \begin{array}{rl}
   & \texttt{is\_builtin\_singular}(\texttt{T\_Real}) \\
   &\texttt{is\_builtin\_singular}(\texttt{T\_String}) \\
   &\texttt{is\_builtin\_singular}(\TBool) \\
   fl \in \textsf{bit\_field}^{*}  \vdash& \texttt{is\_builtin\_singular}(\texttt{T\_Bits}(fl)) \\
   il \in \textsf{\identifier}^{*}  \vdash& \texttt{is\_builtin\_singular}(\texttt{T\_Enum}(il)) \\
   c \in \textsf{int\_constraints}?  \vdash& \texttt{is\_builtin\_singular}(\texttt{T\_Int}(c)) \\
      \end{array}
    \]
...
omitted in type declarations, as it is the case for \texttt{Not\_found}.
    ...
    \[
      \begin{array}{rl}
   tl \in \ty^{*}  \vdash& \texttt{is\_builtin\_aggregate}(\texttt{T\_Tuple}(tl)) \\
   t \in \ty  \vdash& \texttt{is\_builtin\_aggregate}(\texttt{T\_Array}(t)) \\
   fl \in \Field^{*}  \vdash& \texttt{is\_builtin\_aggregate}(\texttt{T\_Record}(fl)) \\
   fl \in \Field^{*}  \vdash& \texttt{is\_builtin\_aggregate}(\texttt{T\_Exception}(fl)) \\
      \end{array}
    \]
...
\section{TypingRule.BuiltinSingularOrAggregate \label{sec:TypingRule.BuiltinSingularOrAggregate}}
   ...
   \[
   \begin{array}{rl}
   t \in \ty \texttt{is\_builtin\_singular}(t) \vdash& \texttt{is\_builtin}(t)\\
   t \in \ty \texttt{is\_builtin\_aggregate}(t) \vdash& \texttt{is\_builtin}(t)\\
   \end{array}
   \]
...
\section{TypingRule.NamedType \label{sec:TypingRule.NamedType} } 
    ...
    \[
      \begin{array}{rl}
      i \in \identifier \vdash& \texttt{is\_named}(\texttt{T\_Named}(i)\\
      \end{array}
    \]
    \subsection{Comments}
...
\section{TypingRule.AnonymousType \label{sec:TypingRule.AnonymousType}}
    ...
    \[
      \begin{array}{rl}
      t \in \ty \rulespace \neg\texttt{is\_named}(t) \vdash& \texttt{is\_anonymous}(t)\\
      \end{array}
    \]
...
\section{TypingRule.AggregateType}
    ...
    \[
      \begin{array}{rl}
      t \in \ty \rulespace \texttt{is\_builtin\_aggregate}(t) \vdash& \texttt{is\_aggregate}(t)\\
      t \in \ty \rulespace \texttt{is\_named}(t) \rulespace \texttt{is\_builtin\_aggregate}(\texttt{t\_struct}(t)) \vdash& \texttt{is\_aggregate}(t)\\
      \end{array}
    \]
...
\item TypingRule.ESliceOrEGetArrayError (see Section~\ref{sec:TypingRule.ESliceOrEGetArrayError})
...
\texttt{t,new\_env} or an error and all of the following apply:
  ...
  \item One of the following applies:
    ...
    \item All of the following apply:
      ...
      \end{itemize}
    \item an error ``\texttt{Typing Assumption Failed}''
    \end{itemize}
  \end{itemize}
...
\section{TypingRule.ESliceOrEGetArrayError \label{sec:TypingRule.ESliceOrEGetArrayError}}
  ...
  \subsection{Prose}
  The result of annotating the expression \texttt{e} in \texttt{env} is an error and all of the following apply:
  \begin{itemize}
  \item \texttt{e} denotes the slicing of expression \texttt{e'} by the slices \texttt{slices};
  \item \texttt{t\_e',e'} is the result of annotating the expression \texttt{e'} in \texttt{env};
  \item \texttt{t\_e'} does not have the structure of an integer or a bitvector;
  ...
  \subsection{Code}
    \VerbatimInput[firstline=\ESliceOrEGetArrayErrorBegin, lastline=\ESliceOrEGetArrayErrorEnd]{../Typing.ml}
   ...
   \item One of the following applies:
     \begin{itemize}
       ...
       \item All of the following applies:
         ...
         \end{itemize}
     \end{itemize}
   \item \texttt{id} is not bound in \texttt{env};
   \item \texttt{env'} is \texttt{env} is \texttt{env} modified so that \texttt{id} is locally declared of type \texttt{ty};
...
\texttt{(name1, args, eqs2, ret\_ty1)} or an error is raised and all of the following apply:
   ...
   \item \texttt{eqs2} is \texttt{eqs1} modified to add all the type equations of \texttt{caller\_arg\_types} and \texttt{callee\_arg\_types};
...
Annotating a subprogram \texttt{f} in an environment \texttt{env} (\texttt{annotate\_func}) results in \texttt{f},
...
\section{TypingRule.Subprogram \label{sec:TypingRule.Subprogram}}
  ...
  \subsection{Prose}
Annotating a subprogram \texttt{f} in an environment \texttt{env} (\texttt{annotate\_func}) results in \texttt{f},
\texttt{new\_body} and all of the following apply:
 \begin{itemize}
