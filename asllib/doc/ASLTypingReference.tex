...
\newcommand\annotaterel[0]{\hyperlink{def-annotaterel}{\textsf{annotate}}}
...
Environments $\tenv \in \staticenvs$ are defined as follows (referring to symbols defined in the abstract syntax):
}
...
\and
\inferrule[false]{
  \astlabel(\tty) \not\in \{\TReal, \TString, \TBool, \TBits, \TEnum, \TInt\}
}{
  \isbuiltinsingular(\tty) \typearrow \False
}
\end{mathpar}
...
\begin{formal}
    \subsection{Formally}
\begin{mathpar}
\inferrule[true]{\astlabel(\tty) \in \{\TTuple, \TArray, \TRecord, \TException\}}
{\isbuiltinaggregate(\tty)}
\and
\inferrule[false]{\astlabel(\tty) \not\in \{\TTuple, \TArray, \TRecord, \TException\}}
{\isbuiltinaggregate(\tty)}
\end{mathpar}
\end{formal}
