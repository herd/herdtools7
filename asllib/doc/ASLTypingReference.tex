...
The domain of \verb|integer{10..1}| is empty set as there are no integers that are
...
\begin{itemize}
  ...
  \item symbolically reducing (symbolic simplification) the expression $\vep$ yields $\vepp$.
\end{itemize}
...
\begin{itemize}
  ...
  \item symbolically reducing (symbolic simplification) $\ewidth$ to a literal yields $\vl$ \ProseOrTypeError;
...
\begin{itemize}
  ...
  \item a check the AST labels of $\vtstruct$, $\vteonestruct$, and $\vtetwostruct$ are all the same and are either
        ...
        $\veone$, $\vetwo$ and the type $\vt$ are inappropriate for a range pattern, which short-circuits the entire rule.
...
\begin{itemize}
  ...
  \item checking, for each $\vi$ in the list of indices of $\formaltys$, whether
        the types $\formaltys[\vi]$ and $\argtys[\vi]$ \typeclash\ yields $\True$
        or $\False$ or a type error each of which short-circuit the entire rule;
  \item $\vb$ is $\True$ (unless the rule short-circuitted with $\False$ or a type error).
\end{itemize}
...
\inferrule{
  ...
  \vi\in\listrange(\formaltys): \typeclashes(\formaltys[\vi], \argtys[\vi]) \typearrow \True \terminateas \False,\TypeErrorConfig
}{
...
yielding the type $\newtenv$.
It is assumed that $\tenv$ is an environment to which all explictly-defined parameters of the subprogram
...
We use the following functions to construct a unique string for for each subprogram:
\begin{itemize}
...
\inferrule{
  ...
  \matchsettertype(\funcsig.\funcsubprogramtype) \typearrow \watendsettertype\\
  \checktrans{\watendsettertype = \funcsigp.\funcsubprogramtype}{\SetterWithoutGetter} \typearrow \True \OrTypeError\\\\
  ...
  \checksetterhasgetter(\tenv, \funcsig) \typearrow \True
}
\end{mathpar}
\end{emptyformal}
...
\[
  ...
  \cup \overname{\TTypeError}{\TypeErrorConfig}
\]
annotates the optional expression $\expropt$ in $\tenv$ and returns a pair of optionals
...
\inferrule[e\_var\_non\_constant]{
  ...
  \staticeval(\tenv, \overname{\EVar(\vx)}{\ve}) \typearrow \UnsupportedExpression
}
\end{mathpar}

\begin{mathpar}
\inferrule[e\_binop]{
  \staticeval(\tenv, \veone) \typearrow \vvone \terminateas \TypeErrorConfig, \UnsupportedExpression\\\\
  \staticeval(\tenv, \vetwo) \typearrow \vvtwo \terminateas \TypeErrorConfig, \UnsupportedExpression\\\\
...
\inferrule[e\_slice\_bitvector]{
  \slicestopositions(\tenv, \slices) \typearrow \positions \OrTypeError\\\\
  \posmax \eqdef \max(\positions)\\
  \checktrans{|\bv| > \posmax}{SliceOutOfRange} \checktransarrow \True \OrTypeError\\\\
...
\begin{mathpar}
\inferrule[e\_cond]{
  \staticeval(\tenv, \econd) \typearrow \vcond \terminateas \TypeErrorConfig, \UnsupportedExpression\\\\
  \vcond \eqname \lbool(\vb)\\
  \vep \eqdef \choice{\vb}{\veone}{\vetwo}\\
  \staticeval(\tenv, \vep) \typearrow \vv \terminateas \TypeErrorConfig, \UnsupportedExpression
}{
...
\inferrule[unsupported]{
  ...
  \staticeval(\tenv, \ve) \typearrow \UnsupportedExpression
}
\end{mathpar}
\end{emptyformal}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item $\vc$ is the sequence of bits of the same length as $\bits$ where in each position
          the bit in $\vr$ is the negation of the corresponding bit in $\bits$;
    \item $\vr$ is the bitvector literal for $\vc$.
  \end{itemize}
\end{itemize}
...
or a type error, if it is considered illegal to apply the operator
to the given values .
...
\inferrule[add\_bits\_int]{
  y \eqname \bintounsigned(a)\\
  c \eqdef \inttobits(y + b, |a|)
}{
...
\[
  ...
  \overname{\Z^*}{\positions} \cup\ \TTypeError
\]
...
\begin{itemize}
  ...
  \item All of the following apply (\textsc{non\_empty}):
  \begin{itemize}
    \item $\slices$ is the list with $\vs$ as its head and $\slicesone$ as its tail;
    \item transforming $\vs$ to a list of positions in $\tenv$ via $\slicetopositions$ yields $\positionsone$ \ProseOrTypeError;
    \item transforming $\slicesone$ to a list of positions in $\tenv$ via $\slicestopositions$ yields $\positionstwo$ \ProseOrTypeError;
...
\and
\inferrule[non\_empty]{
  \slicetopositions(\tenv, \vs) \typearrow \positionsone \OrTypeError\\\\
  \slicestopositions(\tenv, \slicesone) \typearrow \positionstwo \OrTypeError
}{
...
\begin{mathpar}
\inferrule[single]{
  \evaltoint(\tenv, \ve) \typearrow n \OrTypeError\\\\
...
\and
\inferrule[range]{
  \evaltoint(\tenv, \ebot) \typearrow b \OrTypeError\\\\
  \evaltoint(\tenv, \etop) \typearrow t \OrTypeError\\\\
...
\inferrule[length]{
  \evaltoint(\tenv, \ebot) \typearrow b \OrTypeError\\\\
  \evaltoint(\tenv, \elength) \typearrow l \OrTypeError\\\\
  t \eqdef b + l - 1
  \checktrans{t \geq b \geq 0}{BadSlice} \checktransarrow \True \OrTypeError
}{
...
\and
\inferrule[star]{
  \evaltoint(\tenv, \efactor) \typearrow f \OrTypeError\\\\
  \evaltoint(\tenv, \elength) \typearrow l \OrTypeError\\\\
...
\aslto \overname{\Z}{n} \cup \TTypeError
\]

\subsection{Prose}
All of the following apply:
\begin{itemize}
  \item statically evaluating $\ve$ in $\tenv$ yields a literal $\vl$ \ProseOrTypeError;
  \item checking that $\vl$ is an integer literal yields $\True$ \ProseOrTypeError;
  \item $\vl$ is a literal integer for $n$.
\end{itemize}
...
\]
extracts from the list of bits $\bits$ the list of bits $\vr$ containing the values of $\bits$
at the positions given by $\positions$.
The result is a type error, if one is detected.

\subsection{Prose}
All of the following apply:
\begin{itemize}
  \item $\vr$ is the list of bits given by taking $\bits[\vi]$ for every value given in $\positions$ (in the order they appear).
\end{itemize}
...
\begin{mathpar}
\inferrule{
  \vr \eqdef [ \vi\in\positions: \bits[\vi] ]
}{
  \extractslice(\tenv, \bits, \positions) \typearrow \vr
}
\end{mathpar}
\end{emptyformal}
...
\newcommand\ProseOrTypeErrorOrBot[0]{or a type error or $\bot$, which short-circuits the entire rule}
...
% \and
% \inferrule{
%   \sort(\funcgraph(f), \comparemonomialbindings) = [(\vm,c)] \concat \monoms\\
%   \monomialtoexpr(\sort(\funcgraph(\vm), \comparemonomebindings)) \typearrow \veone\\
%   \polynomialtoexpr(\monoms) \typearrow \vetwo\\
%   \symaddexpr(\veone, \vetwo) \typearrow \ve
% }{
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item $\ve$ is the literal expression for the integer $0$ and $\vc$ is $0$.
  \end{itemize}

  \item All of the following apply (\textsc{non\_empty}):
  \begin{itemize}
    \item $\monoms$ is a list with $(\vm,\vcone)$ as its head and $\monomsone$ as its tail;
    \item transforming the unitary monomial $\vm$ to an expression via \\ $\unitarymonomialstoexpr$ yields $\veone$;
    \item transforming $\monoms$ to an expression via $\monomialstoexpr$ yields $(\vetwo, \vctwo)$;
    \item symbolically adding $\veone, \vcone, \vetwo, \vctwo$ via $\symaddexpr$ yields $(\ve, \vc)$.
  \end{itemize}
\end{itemize}
...
\inferrule[empty]{}
{
  \monomialstoexpr(\overname{\emptylist}{\monoms}) \typearrow (\overname{\ELiteral(\lint(0))}{\ve}, \overname{0}{\vc})
}
\and
\inferrule[non\_empty]{
  \unitarymonomialstoexpr(\vm) \typearrow \veone\\
  \monomialstoexpr(\monoms) \typearrow (\vetwo, \vctwo)\\
  \symaddexpr(\veone, \vcone, \vetwo, \vctwo) \typearrow (\ve, \vc)
}{
  \monomialstoexpr(\overname{[(\vm,\vcone)] \concat \monomsone}{\monoms}) \typearrow (\ve, \vc)
}
\end{mathpar}
\end{emptyformal}
...
The function
\[
\symaddexpr(\overname{\expr}{\veone} \aslsep \overname{\Q}{\vcone} \aslsep \overname{\expr}{\vetwo})
  \typearrow \overname{\expr}{\ve}
\]
produces an expression representing the addtion of expressions $\veone$ and $\vetwo$,
making some simplifications to yield $\ve$.
...
\begin{itemize}
  ...
  \begin{itemize}
    \item either $\veone$ or $\vetwo$ are the literal expression for $0$;
    \item $\ve$ is $\veone$ if $\vetwo$ is the literal expression for $0$ and $\veone$, otherwise.
  \end{itemize}
\end{itemize}
...
\begin{mathpar}
\inferrule[zero]{
  (\veone = \ELiteral(\lint(0)) \lor \vetwo = \ELiteral(\lint(0)))\\
  \ve \eqdef \choice{\veone = \ELiteral(\lint(0))}{\vetwo}{\veone}
}{
  \symaddexpr(\veone, \vetwo) \typearrow \ve
}
\and
\inferrule[non\_zero]{
  \ve \eqdef \EBinop(\PLUS, \veone, \vetwo)
}{
  \symaddexpr(\veone, \vetwo) \typearrow \ve
}
\end{mathpar}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item symbolically reducing (symbolic simplification) the expression corresponding to $\ve - 1$ via \tododefine{reduce\_expr} in $\tenv$
...
\begin{itemize}
  \item symbolically reducing (symbolic simplification) $\ve$ in $\tenv$ yields $\veone$;
