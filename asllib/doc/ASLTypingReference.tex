...
\[
  ...
  \aslto \overname{\Bool}{\vb} \cup \overname{\TTypeError}{\TypeErrorConfig}
\]
tests whether a type $\vt$ \emph{type-satisfies} a type $\vs$ is environment $\tenv$,
returning the result $\vb$ or a type error, if one is detected.
...
One of the following applies:
 ...
 \item All of the following apply:
  ...
  \item All of the following apply:
    \begin{itemize}
    \item $\vt$ is an anonymous bitvector type with no bitfields;
...
\begin{mathpar}
\inferrule[subtypes]{
  \subtypes(\tenv, \vt, \vs) \typearrow \True
}{
  \typesatisfies(\tenv, \vt, \vs) \typearrow \True
}
\and
\inferrule[anonymous1]{
  \subtypes(\tenv, \vt, \vs) \typearrow \False\\
