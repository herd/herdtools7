...
in the sequel of this document.
...
Arrays are declared with indices that are either integer-typed of enumeration-typed.
In the example above, \texttt{T} is declared as an array with an integer-typed index (as indicated by the
used of the integer-typed constant \texttt{3}) whereas \texttt{PointArray} is declared with the
index of \texttt{Coord}, which is an enumeration type.
...
\begin{itemize}
  ...
  \item $\news$ is the AST node for the seuqnece of statements $\newsone$ and $\newstwo$, that is, $\SSeq(\newsone, \newstwo)$.
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Symbolic Reudction and Equivalence Testing \label{chap:symbolicequivalencetesting}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\section{Symbolic Expression AST}
Our symbolic reduction and equivalence testing rules use a \emph{symbolic expression AST}.
We now define the symbolic expression AST and the operations associated with it.
...
\[
  \begin{array}{rcl}
    ...
    \polynomial &::=& \Sum(\monomial \partialto \Z)                  \hypertarget{def-monomial}{}\hypertarget{def-prod}{}\\
    \monomial   &::=& \Prod(\Identifiers \partialto \Npos)\\
  \end{array}
\]

The meaning of each node in the symbolic expression AST can be interpreted as a mathematical formula,
...
\[
  \Sum\left(\left\{
    ...
    -1\cdot x^3 \cdot y \cdot z^2 + 3 \cdot \vx^2\cdot \vy \enspace.
\]
...
\[
  ...
  \overname{\symexpr}{\vp}\ \cup\ \{\bot\}\ \cup\ \overname{\TTypeError}{\TypeErrorConfig}
\]
...
The function
\[
  \toircase(\overname{\staticenvs}{\tenv} \aslsep \overname{\expr}{\ve}) \aslto \overname{\symexpr}{\vp} \cup \{\bot\}
\]
transforms a subset of ASL expressions into symbolic expressions. If an expression cannot be represented
by a symbolic expression, the special value $\bot$ is returned.
...
\inferrule[literal\_other]{
  ...
  \toircase(\tenv, \ELiteral(\vv)) \typearrow \bot
}
\and
...
\inferrule[e\_unknown]{
  (\astlabel(\veone) = \EUnknown \lor \astlabel(\vetwo) = \EUnknown)
}{
  \exprequalcase(\tenv, \veone, \vetwo) \typearrow \False
}
\end{mathpar}
...
\begin{mathpar}
\inferrule[trecord]{
  \booltrans(\dom(\vfone) = \dom(\vftwo)) \equalarrow \True \terminateas \False\\\\
