...
\texttt{annotate\_lexpr version env le t\_e} is \texttt{new\_le} and one of the following
applies:
\begin{itemize}
...
\item TypingRule.LETuple (see Section~\ref{sec:TypingRule.LETuple}),
...
\item TypingRule.LESetRecordField (see Section~\ref{sec:TypingRule.LESetRecordField}),
...
\section{TypingRule.LELocalVar \label{sec:TypingRule.LELocalVar}}
   ...
   All of the following apply:
   \begin{itemize}
   \item \texttt{le} denotes a local variable \texttt{x} of type \texttt{ty};
...
\section{TypingRule.LETuple \label{sec:TypingRule.LETuple}}

    \subsection{Prose}
    All of the following apply:
   \begin{itemize}
   \item \texttt{le} denotes a tuple \texttt{les};
   \item \texttt{t\_e} denotes a tuple type \texttt{sub\_tys};
   \item  One of the following applies:
     \begin{itemize}
     \item  All of the following apply:
       \begin{itemize}
...
\section{TypingRule.LESetArray \label{sec:TypingRule.LESetArray}}
   ...
   \item \texttt{slices} is a single expression \texttt{e\_index};
   \item \texttt{t\_index', e\_index'} is the result of annotating \texttt{e\_index} in \texttt{env};
   \item \texttt{t\_index'} type-satisfies \texttt{wanted\_t\_index};
...
\section{TypingRule.LESetBadRecordField \label{sec:TypingRule.LESetBadRecordField}}
...
\section{TypingRule.LESetRecordField \label{sec:TypingRule.LESetRecordField}}
...
\section{TypingRule.LESetFields \label{sec:TypingRule.LESetFields}}
...
\texttt{annotate\_local\_decl\_item loc env ty ldk ldi} is \texttt{new\_env, new\_ldi} and one
...
\section{TypingRule.LDUninitialisedTypedTuple \label{sec:TypingRule.LDUninitialisedTypedTuple}}

    \subsection{Prose}
    All of the following apply:
   \begin{itemize}
   \item \texttt{ldi} denotes a singleton list \texttt{ld};
   \item \texttt{new\_env, new\_ldi} is the result of annotating the local declaration \texttt{ld} with \texttt{ty} in \texttt{env}.
   \end{itemize}
...
\texttt{annotate\_stmt env s} is a statement \texttt{new\_s} and an environment \texttt{new\_env} and
...
\section{TypingRule.SAssign \label{sec:TypingRule.SAssign}}
  ...
  \subsection{Prose}
   ...
   \item \texttt{reduced} is the result of potential result of inlining a setter call in \texttt{le};
   \item One of the following applies:
     \begin{itemize}
     \item All of the following apply:
       \begin{itemize}
     ...
     \item All of the following apply:
       ...
       \item \texttt{le1} is the result of annotating \texttt{le} in \texttt{env1};
...
\section{TypingRule.SReturnNone \label{sec:TypingRule.SReturnNone}}
  ...
  \subsection{Prose}
    All of the following apply:
   \begin{itemize}
   \item \texttt{s} is a return statement with no value and no return type; 
   \item \texttt{new\_s} is a return statement with no value;
   \item the enclosing subprogram does not have a return type (it is either a setter
      or a procedure);
   \item \texttt{new\_env} is \texttt{env}.
   \end{itemize}
...
\section{TypingRule.SReturnOne \label{sec:TypingRule.SReturnOne}}
  ...
  \subsection{Prose}
   ...
   \item One of the following applies:
     ...
     \item All of the following apply:
       \begin{itemize}
       \item \texttt{s} is a return statement with some value;
       \item the enclosing subprogram does not have a return type;
       \end{itemize}
     \item All of the following apply:
       \begin{itemize}
       \item \texttt{s} is a return statement with no value;
       \item the enclosing subprogram has a return type;
       \end{itemize}
     \end{itemize}
...
\section{TypingRule.SReturnSome \label{sec:TypingRule.SReturnSome}}
  ...
  \subsection{Prose}
    All of the following apply:
   \begin{itemize}
   \item \texttt{s} is a return statement with some value \texttt{e};
   \item the enclosing subprogram has a return type \texttt{t};
   \item \texttt{t\_e',e'} is the result of annotating \texttt{e} in \texttt{env};
   \item \texttt{t\_e'} type-satisfies \texttt{t};
   \item \texttt{new\_s} is a return statement with value \texttt{e'};
   \item \texttt{new\_env} is \texttt{env}. 
   \end{itemize}
...
\section{TypingRule.SCase \label{sec:TypingRule.SCase}}
  ...
  \subsection{Prose}
   ...
   \item \texttt{t\_e, e1} is the result of annotating \texttt{e} in \texttt{env};
   \item \texttt{cases1, env1} is the result of annotating each case in \texttt{cases};
   \item \texttt{new\_s} is a case statement with expression \texttt{e1} and cases \texttt{cases1};
   \item \texttt{new\_env} is \texttt{env1}.
   \end{itemize}
    (* do we not check that \texttt{e} has the structure of a boolean? *)
...
\section{TypingRule.SWhile \label{sec:TypingRule.SWhile}}
  ...
  \subsection{Prose}
    All of the following apply:
   \begin{itemize}
   \item \texttt{s} is a while statement with expression \texttt{e1} and statement \texttt{s1};
   \item \texttt{t, e2} is the result of annotating \texttt{e1} in \texttt{env};
   \item \texttt{t} type-satisfies \texttt{t\_bool}; (* is this the same as \texttt{e1} having the structure of a boolean? *)
   \item \texttt{s2} is the result of annotating \texttt{s1} in \texttt{env};
   \item \texttt{new\_s} is a while statement with expression \texttt{e2} and statement \texttt{s2};
   \item \texttt{new\_env} is \texttt{env}.
   \end{itemize}
...
\section{TypingRule.SRepeat \label{sec:TypingRule.SRepeat}}
  ...
  \subsection{Prose}
    All of the following apply:
   \begin{itemize}
   \item \texttt{s} is a repeat statement with expression \texttt{e1} and statement \texttt{s1};
   \item \texttt{s2} is the result of annotating \texttt{s1} in \texttt{env};
   \item \texttt{t, e2} is the result of annotating \texttt{e1} in \texttt{env};
   \item \texttt{t} type-satisfies \texttt{t\_bool}; (* is this the same as \texttt{e1} having the structure of a boolean? *)
   \item \texttt{new\_s} is a repeat statement with expression \texttt{e2} and statement \texttt{s2};
   \item \texttt{new\_env} is \texttt{env}.
   \end{itemize}
...
\section{TypingRule.SFor \label{sec:TypingRule.SFor}}
  ...
  \subsection{Prose}
    All of the following apply:
   \begin{itemize}
   \item \texttt{s} is a for statement with index \texttt{id}, direction \texttt{dir}, two expressions
      \texttt{e1} and \texttt{e2} and a statement \texttt{s'};
   \item \texttt{t1,e1'} is the result of annotating \texttt{e1} in \texttt{env};
   \item \texttt{t2,e2'} is the result of annotating \texttt{e2} in \texttt{env};
   ...
   \item \texttt{new\_s} is a for statement with index \texttt{id}, direction \texttt{dir}, two expressions
      \texttt{e1'} and \texttt{e2'} and statement \texttt{s''};
   \item \texttt{new\_env} is \texttt{env}.
   \end{itemize}
...
\section{TypingRule.SDeclSome \label{sec:TypingRule.SDeclSome}}
  ...
  \subsection{Prose}
    All of the following apply:
   \begin{itemize}
   \item \texttt{s} is a declaration with \texttt{ldk}, \texttt{ldi} and an expression \texttt{e};
   \item \texttt{t\_e,e'} is the result of annotating \texttt{e} in \texttt{env};
...
\section{TypingRule.SDeclNone \label{sec:TypingRule.SDeclNone}}
  ...
  \subsection{Prose}
    All of the following apply:
   \begin{itemize}
   \item \texttt{s} is a declaration statement with \texttt{ldk}, \texttt{ldi} and no initial expression;
