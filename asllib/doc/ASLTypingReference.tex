...
\newcommand\annotateconstraintbinop[0]{\hyperlink{def-annotateconstraintbinop}{\texttt{constraint\_binop}}}
...
\inferrule[arith\_t\_int\_wellconstrained1]{
  ...
  \tododefine{annotate\_constraints\_binop}(\tenv, \op, \vcsone, \vcstwo) \typearrow \vcs
}{
...
The function
\[
  \constraintsisstrictpositive(\overname{\intconstraints}{\cs}) \aslto \overname{\Bool}{\vb}
\]
Conservatively tests whether the set of integer constraints $\cs$ always represent positive integers,
...
The function
\[
  \constraintsisnonnegative(\overname{\intconstraints}{\cs}) \aslto \overname{\Bool}{\vb}
\]
Conservatively tests whether the set of integer constraints $\cs$ always represent positive integers,
yielding the result in $\vb$.
...
\inferrule{
  ...
  {
    \begin{array}{r}
  \annotatetype{\Ignore, \tenv, \TBits(\ewidth, \bitfields)} \typearrow \\
...
\inferrule[expr\_is\_enum]{
  ...
  {
    \begin{array}{r}
  \annotatetype{\Ignore, \tenv, \TArray(\ArrayLengthExpr(\ve), \vt)} \typearrow \\
...
$\news$ and a modified environment $\newenv$. Otherwise, the result is a type error.
...
\inferrule{
  ...
  \makeanonymous(\tenv, \vtone) \typearrow \structone \OrTypeError\\\\
  \makeanonymous(\tenv, \vttwo) \typearrow \structtwo \OrTypeError\\\\
  \getforconstraints(\tenv, \structone, \structtwo, \veonep, \vetwop, \dir) \typearrow \cs \OrTypeError\\\\
  \tty \eqdef \TInt(\cs)\\
...
\]
infers the list of constraints --- $\vis$ --- for a \texttt{for} loop index variable from the following:
\begin{itemize}
...
\inferrule[unconstrained]{
  {
    \begin{array}{cccl}
    \structone = \TInt(\unconstrained) &\land& \astlabel(\structtwo) = \TInt & \lor\\
    \astlabel(\structone) = \TInt &\land& \structtwo = \TInt(\unconstrained) & \\
    \end{array}
  }
}{
...
\begin{itemize}
  \item TypingRule.AnnotateCallArgTyped (see \secref{TypingRule.AnnotateCallArgTyped}),
        which finds annotates the call with the annotated list of arguments.
...
\end{array}
\]
is same as $\annotatecall$, except that the argument expressions are replaced by
...
\begin{mathpar}
\inferrule{
  \splitlist(\callerargstyped) = (\callerargtypes, \vargsone)\\
  {
    \begin{array}{r}
  ...
  \equallength(\calleeargtypes, \vargsone) \typearrow \aritymatch\\
...
by substituting parameter names by their corresponding expressions in
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item define $\newty$ as the well-constraint integer type with constraints \\
          $\newconstraints$.
  \end{itemize}
...
\inferrule[arg\_is\_not\_param]{
  ...
  {
    \begin{array}{r}
    \annotateparameterdefining(\tenv, \eqsone, \vargs, \callerargstyped, \calleeparams) \\
    \typearrow \eqs \OrTypeError
    \end{array}
  }
}
...
\inferrule[ret\_type\_mismatch]{
  \neg(\calltype \in \{\STProcedure, \STSetter, \STEmptySetter\} \leftrightarrow \calleerettyopt=\None)
}{
...
It is assumed that $\callerargtypes$ and $\vargs$ have the same length.
...
\]
This ordering is not total. That is, there may very well exist to strongly connected components
...
returns a graph whose set of nodes are identifiers are used to name declarations
...
All of the following apply:
\begin{itemize}
  \item define $\defs$ the union of two sets:
  \begin{enumerate}
  \item set of identifiers given by applying $\defdecl$ to each declaration in $\decls$;
...
\]
returns the set of identifiers $\ids$ on which the declaration $\vd$ depends on.
...
\]
returns the set of identifiers $\ids$ on which the type or \optional\ type $\vt$ depends on.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item define $\ids$ as the union of the singleton set for $\vx$ and the union of applying $\usety$
          to each type of a field in $\subfields$.
  \end{itemize}
\end{itemize}
...
\]
returns the set of identifiers $\ids$ on which the expression or \optional\ expression $\ve$ depends on.
...
\begin{itemize}
  ...
  \item All of the following apply (\textsc{e\_unop}):
  \begin{itemize}
    \item $\ve$ is the unary operation expression over expression $\veone$;
...
\]
returns the set of identifiers $\ids$ on which the left-hand-side expression $\vle$ depends on.
...
\]
returns the set of identifiers $\ids$ on which the slice $\vs$ depends on.
...
\]
returns the set of identifiers $\ids$ on which the bitfield $\vbf$ depends on.
...
\]
returns the set of identifiers $\ids$ on which the integer constraint $\vc$ depends on.
...
\]
returns the set of identifiers $\ids$ on which the statement $\vs$ depends on.
...
\begin{mathpar}
\inferrule[s\_for]{
  \ids \eqdef \useexpr(\veone) \cup \useexpr(\vetwo) \cup \usestmt(\vsone)
}{
  \usestmt(\overname{\SFor(\Ignore, \veone, \Ignore, \vetwo, \vsone)}{\vs}) \typearrow \ids
}
\and
\inferrule[while\_repeat]{
  \vs = \SWhile(\ve, \vsone) \lor \vs = \SRepeat(\ve, \vsone)
}{
  \usestmt(\vs) \typearrow \overname{\useexpr(\ve) \cup \usestmt(\vsone)}{\ids}
}
\end{mathpar}
...
\]
returns the set of identifiers $\ids$ on which the local declaration item $\ldi$ depends on.
...
\]
returns the set of identifiers $\ids$ on which the case alternative $\vc$ depends on.
...
\]
returns the set of identifiers $\ids$ on which the try statement catcher $\vc$ depends on.
...
\begin{mathpar}
\inferrule[int\_unconstrained]{}{ \symdomoftype(\tenv, \overname{\TInt(\unconstrained)}{\vt}) \typearrow \overname{\DInt(\Top)}{\vd} }
\end{mathpar}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\ve$ is a binary operation expression for an operation that is one of $\PLUS$, $\MINUS$, or $\MUL$
          and subexpression $\veone$ and $\vetwo$;
  ...
  \item All of the following apply (\textsc{unsupported}):
  \begin{itemize}
    \item $\ve$ is not one of the expression types above: a literal expression, a variable expression, a unary operation
...
\aslto \overname{\intconstraints}{\cs}
\]
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item define $\vis$ as the symbolic constrained integer domain for $\vcs$, that is, $\FromSyntax(\vcs)$.
  \end{itemize}
\end{itemize}
...
\and
\inferrule[symbolic]{
  \exists \vi\in\listrange(\vcs): \constrainttointset(\tenv, \vcs[\vi]) \typearrow \visone \land \astlabel(\visone) \neq \Finite
}{
...
\]
symbolically simplifies the \underline{integer-typed} expression $\ve$ and returns the resulting integer or $\Top$ if
...
\constraintsbinop(
  ...
  \overname{\intconstraints}{\csone} \aslsep
  \overname{\intconstraints}{\cstwo}
)
\aslto \overname{\intconstraints}{\cs} \cup \{\top\}
\]
...
Otherwise, the result is $\top$, which indicates a failure of representing the result by a list of integer constraints.
...
Otherwise, the result is $\top$, which indicates a failure of representing the result of
by an integer constraints.
...
\]
symbolically simplifies an expression $\ve$ into the literal $\vl$.
Otherwise, the result is a type error.
...
\begin{itemize}
  ...
  \item All of the following apply (\textsc{e\_tuple}):
  \begin{itemize}
    \item $\veone$ is a tuple expression with subexpressions list $\vlone$,
          that is, $\ETuple(\vlone)$;
    \item $\vetwo$ is a tuple expression with subexpressions list $\vltwo$,
...
and yields the result in $\vb$.  Otherwise, the result is a type error.
...
in $\vb$. Otherwise, the result is a type error.
