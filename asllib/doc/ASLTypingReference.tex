...
\newcommand\staticallyevaluable[0]{\textit{statically-evaluable}}
...
\[
\begin{array}{c}
\RuleComment{Annotating slices of the form \texttt{$a$+:$b$}.}\\
\RuleComment{Other slices kinds are syntactic sugar and we skip their treatment here.}\\
\inferrule{
E \entails e_o : (T_o, e_o')\\
E \entails e_l : (T_l, e_l')\\
E \entails \typelabel(\tstruct(T_o)) = \TInt\\
E \entails \typelabel(\tstruct(T_l)) = \TInt\\
E \entails \staticallyevaluable(e_l)
}
{E \entails \texttt{Slice\_Length}(e_o, e_l) : \texttt{Slice\_Length}(e_o', e_l')}\\
\\
\inferrule{
  E \entails e : (T, e')\\
  E \entails \typelabel(\tstruct(T)) \in \{\TInt, \TBits\}\\
  E \entails \textsf{width}(s_{1..k}) = w\\
  E \entails s_1 : (\Ignore, s_1')
  }
{E \entails \texttt{E\_Slice}(e, [s_{1..k}]) : \\\ (\TBits(\texttt{BitWidth\_SingleExpr}(w), []), \texttt{E\_Slice}(e', [s'_{1..k}])}
\end{array}
\]
\end{emptyformal}
...
\[
\inferrule{
  E \entails a_1 : (T_1,a_1') \ldots a_n : (T_n,a_n')\\
  E^G.\subprograms \entails \name : \{ \text{args}: [(n_1,T_1),\ldots,(n_n,T_n)], \text{return\_type}: T_r, \ldots\}\\
  }
{E \entails \texttt{E\_Call}(\textit{name}, [a_{1..n}], [p_{1..k}]) : 
...
\[
\inferrule{}{E \entails \texttt{E\_GetArray}()}
\]
\end{emptyformal}
...
L \in \{\TRecord, \TException\}\\
E \entails e_1 : (T_1',e_1') \ldots e_k : (T_k',e_k')\\
E \entails T_1' \typesat T_1 \ldots T_k' \typesat T_k
}
{E \entails L(T, [(f_1,e_1),\ldots,(f_k,e_k)]) : (T, L(T, [(f_1,e_1'),\ldots,(f_k,e_k')]))}
\end{array}
\]
\end{emptyformal}
...
{E \entails \texttt{E\_GetField}(e, f_i) : (T_i, \texttt{E\_GetField}(e', f_i))}
\]
\end{emptyformal}
...
E \entails \texttt{E\_Slice}(e, [s_{1..n}]) : (T, e'')
}
{E \entails \texttt{E\_GetField}(e, f_i) : (T, e''))}
\]
\end{emptyformal}
...
{E \entails \texttt{E\_GetField}(e, f_i) : (T_f, e''))}
\]
\end{emptyformal}
...
{
E \entails  e : (\TBits(\texttt{BitWidth\_SingleExpr(\eliteral{0}}, [])), e)
}
\]
\end{emptyformal}
...
\begin{emptyformal}
    \subsection{Formally}
\[
\inferrule{E \entails e_1 : (T_1, e_1') \ldots e_k : (T_k, e_k')}
{
E \entails  \texttt{E\_Tuple}([e_{1..k}]) : (\TTuple(T_{1..k}), \texttt{E\_Tuple}([e'_{1..k}]))
}
\]
\end{emptyformal}
...
\begin{emptyformal}
    \subsection{Formally}
\[
\inferrule{}{E \entails \texttt{E\_Unkown}(T) : (T, \texttt{E\_Unkown}(\tstruct(T)))}
\]
\end{emptyformal}
...
{
E \entails \texttt{E\_CTC}(e, T) : (T, \texttt{E\_CTC}(e, T))
}
\]
\end{emptyformal}
