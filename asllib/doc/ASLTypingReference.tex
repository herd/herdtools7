...
The purpose of this document is to describe, in a formal and authoritative way, which ASL programs are considered \emph{well-typed}.
Well-typedness is defined in terms of a \emph{type system}~\cite{TypeSystemsLucaCardelli}. That is, a set of \emph{typing rules}.
...
\paragraph{Undestanding the Typing Formalization:}
...
This seperation allows us to typecheck subprograms by using an empty local environment.
...
right hand side type is contained in the dynamic domain of the left hand side type~\secref{TypingRule.DomainSubtypeSatisfaction},
...
\begin{description}
  ...
  represent type errors (for example, an integer type as a condition expression).
...
\begin{itemize}
  \item Builtin singular types (\ref{sec:TypingRule.BuiltinSingularType})
  \item Builtin aggregate types (\ref{sec:TypingRule.BuiltinAggregateType})
  \item Buitin types (\ref{sec:TypingRule.BuiltinSingularOrAggregate})
  \item Named types (\ref{sec:TypingRule.NamedType})
  \item Anonymous types (\ref{sec:TypingRule.AnonymousType})
  \item Singular types (\ref{sec:TypingRule.SingularType})
  \item Aggregate types (\ref{sec:TypingRule.AggregateType})
  \item Non-primitive types (\ref{sec:TypingRule.NonPrimitiveType})
  \item Primitive types (\ref{sec:TypingRule.PrimitiveType})
  \item The structure of a type (\ref{sec:structure})
  \item The underlying type of a type (\ref{sec:anonymize})
\end{itemize}
...
\subsection{Prose}
$\tty$ is a builtin type and one of the following apply:
\begin{itemize}
\item $\tty$ is singular;
...
\subsection{Prose}
A type $\tty$ is singular if one of the following apply:
\begin{itemize}
...
\subsection{Prose}
A type $\tty$ is aggregate in an environment $\tenv$ if one of the following apply:
\begin{itemize}
...
The predicate
\[
  \isnonprimitive(\overname{\ty}{\tty}) \;\aslto\; \Bool
\]
tests whether the type $\tty$ is a \emph{non-primitive type}.

\subsection{Prose}
A type $\tty$ is non-primitive if and only if one of the following applies:
\begin{itemize}
  \item (\textsc{named}) $\tty$ is a named type;
  \item All of the following apply (\textsc{tuple}):
    \begin{itemize}
    \item $\tty$ is a tuple type $\vli$;
    \item there exists a non-primitive type in $\vli$;
    \end{itemize}
...
in the static environent $\tenv$.
If a named type is not associated with a declared type in the static environent $\tenv$, a type error is returned.
...
\]
returns the \emph{\underlyingtype} --- $\vt$ --- of the type $\tty$ in the static environent $\tenv$ or a type error.
...
\section{TypingRule.Supertype}
  ...
  \subsection{Prose}
  $\vt$ is a supertype of $\vs$ if and only if $\vs$ is a subtype of $\vt$.


\todocomment{
ROMAN: This rule doesn't exist in the code. Also, it is redundant, since it just apply to the same arguments in reverse order. I recommend removing this section.
}
...
\subsection{Prose}
$\vt$ structurally-subtype-satisfies $\vs$ in environment \tenv, if one of the following apply:
  \begin{itemize}
  \item All of the following apply:
    \begin{itemize}
  ...
  \item All of the following apply:
    \begin{itemize}
    \item $\vs$ has the underlying type of a bitvector type with determined width \texttt{w};
    \item One of the following apply:
      \begin{itemize}
      \item $\vt$ has the underlying type of a bitvector type of determined width \texttt{w};
  ...
  \item All of the following apply:
    ...
    \item for every bitfield \texttt{f} in \texttt{bitfields} there is a bitfield \texttt{f'} in $\vt$ and
  ...
  \item All of the following apply:
    ...
    \item for each element $\ve$ in $\vs$ there is an element \texttt{e'} in $\vt$ and \texttt{e'}
      type-satisfies $\ve$.
    \end{itemize}
  ...
  \item All of the following apply:
    ...
    \item for each field \texttt{f} in $\vs$ there is an element \texttt{f'} in $\vt$ and \texttt{f'} has
...
\subsection{Prose}
 $\vt$ domain-subtype-satisfies $\vs$ in environment \tenv\ if one of the following apply:
 \begin{itemize}
 \item All of the following apply:
    \begin{itemize}
...
$\vt$ \hypertarget{def-typesatisfies}{type-satisfies} $\vs$ if one of the following apply:
...
\section{TypingRule.TypeClash\label{sec:TypingRule.TypeClash}}
  ...
  \subsection{Prose}
  $\vt$ type-clashes with $\vs$ if one of the following apply:
  \begin{itemize}
...
\subsection{Prose}
  ...
  The lowest common ancestor of types $\vs$ and $\vt$ is $\tty$ and one of the following apply:
  \begin{itemize}
  \item All of the following apply:
    \begin{itemize}
...
\subsection{Prose}
  ...
  type \texttt{t1} and one of the following apply:
  \begin{itemize}
  \item All of the following apply:
    \begin{itemize}
...
\subsection{Prose}
  ...
  types \texttt{t1} and \texttt{t2} and one of the following apply:
\begin{itemize}
  \item All of the following apply:
    \begin{itemize}
  ...
  \item All of the following apply:
    ...
    \item $\vt$ is the bitvector type of width \texttt{w}.
    \end{itemize}
...
rewritten type~\texttt{new\_ty} and one of the following apply:
\begin{itemize}
...
\texttt{new\_e} is a rewritten expression, or an error, and one of the following apply:
\begin{itemize}
...
\subsection{Prose}
The result of annotating a literal \texttt{l} is $\vt$ and one of the following apply:
\begin{itemize}
...
\texttt{t,new\_e} and all of the following apply:
  ...
  \item $\vt$ is the result of checking compatibility of \texttt{op} with \texttt{t1} and \texttt{t2} as per Section~\ref{sec:TypingRule.CheckBinop};
...
\texttt{t,new\_e} and all of the following apply:
  \begin{itemize}
  \item $\ve$ denotes a unary operation \texttt{op} over an expression \texttt{e'};
  \item \texttt{t'',e''} is the result of annotating \texttt{e'} in \texttt{env};
  \item $\vt$ is the result of checking compatibility of \texttt{op} with \texttt{t''} as per Section~\ref{sec:TypingRule.CheckUnop};
...
\texttt{t, new\_env} and all of the following apply:
  \begin{itemize}
  \item $\ve$ denotes the slicing of expression \texttt{e'} by the slices \texttt{slices};
...
\texttt{t,new\_e} and all of the following apply:
  \begin{itemize}
  \item $\ve$ denotes whether the expression \texttt{e'} matches \texttt{patterns};
...
\texttt{t,new\_e} and all of the following apply:
  \begin{itemize}
  \item $\ve$ denotes an expression \texttt{e'} and a type \texttt{t'};
...
results in a rewritten expression \texttt{new\_le} and one of the following apply:
\begin{itemize}
...
all of the following apply:
   ...
   \item $\vt$ is the bitvector type of width \texttt{w} and bitfields \texttt{bitfields'};
   \item $\vt$ type-satisfies \texttt{t\_e};
   \item \texttt{le2} is the slicing of \texttt{le1} by \texttt{slices};
   \item \texttt{new\_le} is the result of annotating \texttt{le2} in \texttt{env}.
   \end{itemize}
...
(\texttt{annotate\_slice env slice}) results in the slicing of the pair \texttt{(offset,length)} and one of the following apply:
\begin{itemize}
...
Annotating a pattern $\vt$ in an environment~\texttt{env} given a type $\vt$ (\texttt{annotate\_pattern}) results in a pattern \texttt{new\_p} and one of the following apply:
\begin{itemize}
...
environment~\texttt{env} (\annotatelocaldeclitem{\ldk, \ldi, \tty}) results in \texttt{new\_env, new\_ldi} and one of the following apply:
\begin{itemize}
...
environment \texttt{new\_env} and one of the following apply:
\begin{itemize}
...
new\_stmt)} and one of the following apply:
\begin{itemize}
...
args, eqs2, ret\_ty1)} or an error is raised and one of the following apply:
\begin{itemize}
...
results in a new environment \texttt{env'}, which contains the declared element, and one of the following apply:
\begin{itemize}
...
in \texttt{new\_env} and one of the following apply:
\begin{itemize}
  \item All of the following apply:
  \begin{itemize}
...
in \texttt{env'} and one of the following apply:
\begin{enumerate}
  \item All of the following apply:
  \begin{itemize}
...
%   }
%   {
%     \substparams(\tenv, \denv, \ECTC(\ve, \vt)) \typearrow \ECTC(\ve', \vt')
%   }
% \end{mathpar}
