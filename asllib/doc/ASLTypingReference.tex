...
\subsection{Updating the Environment}

\newcommand\declarelocal[1]{\texttt{declare\_local}(#1)}
\newcommand\declareglobal[1]{\texttt{declare\_global}(#1)}

\begin{mathpar}
  \inferrule{}{ \declarelocal{\tenv, x, t, k} =  (G^\tenv, L^\tenv.\storagetypes[x \mapsto (t, k)])}
  \inferrule{}{ \declareglobal{\tenv, x, t, k} =  (G^\tenv.\storagetypes[x \mapsto (t, k)], L^\tenv)}
\end{mathpar}

\section{Type System}
A \emph{type rule} has the form $\inferrule{P_1 \\ \ldots \\ P_k}{S}$. 
...
\begin{mathpar}
  \inferrule{
    ...
    \tyopt = \langle\rangle\\
    \vt = \tty\\
    \newenv = \declarelocal{\tenv, \vx, \vt, \ldk}
  }
  {
...
\begin{mathpar}
  ...
  \inferrule{
    ...
    \tenv' = \declarelocal{\tenv, \name, \tty, \texttt{LDK\_Let}}\\
    \texttt{new\_stmt'} = \annotateblock{\tenv', \texttt{stmt}}
  }
  {
    \annotatecatcher{\tenv, \texttt{name\_opt}, \tty, \texttt{stmt}} = (\texttt{name\_opt}, \tty, \texttt{new\_stmt'})
