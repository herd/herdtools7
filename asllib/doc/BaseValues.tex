...
\begin{itemize}
    \item \parameterizedintegertypesterm{} ---
          ...
          invocation, there is no need to define a \basevalueterm{} for them; \identi{WVQZ}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item checking that $\length$ is greater than or equal to $0$ yields $\True$\ProseTerminateAs{\NoBaseValue};
    ...
    \begin{itemize}
        ...
        \ProseNote{See \RequirementRef{NoBaseValue}}
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item checking whether $\vzminlist$ is empty yields $\True$ or a \typingerrorterm{} with \errorcodeterm{} \NoBaseValue;
    ...
    \begin{itemize}
        \item $\vt$ is the enumerated array type over the enumeration $\venum$ and labels $\vlabels$ and element type $\tty$
              ($\TArray(\ArrayLengthEnum(\venum, \vlabels), \tty)$);
...
\inferrule[t\_bits\_static]{
    ...
    \vzopt \neq \None\hva\\\\
    \vzopt \reverseeqdef \some{\length}\\
...
\begin{mathpar}
\inferrule[t\_named]{
    \makeanonymous(\tenv, \TNamed(\id)) \typearrow \vtp \OrTypeError\hva\\\\
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item the single element list for $\vvone$, if $\vvone$ is non-negative and $\vvtwo$ is greater than or equal to $\vvone$.
        \end{itemize}
    \end{itemize}
\end{itemize}
...
\inferrule[exact]{
    \reducetozopt(\tenv, \ve) \typearrow \vzopt\\
    \techeck(\vzopt \neq \None, \NoBaseValue) \typearrow \True \OrTypeError\hva\\\\
    \vzopt \reverseeqdef \some{\vz}
}{
...
\inferrule[range]{
    ...
    \techeck(\vzoptone \neq \None, \NoBaseValue) \typearrow \True \OrTypeError\hva\\\\
    \vzoptone \reverseeqdef \some{\vvone}\\
    \reducetozopt(\tenv, \vetwo) \typearrow \vzopttwo\hva\\
    \techeck(\vzopttwo \neq \None, \NoBaseValue) \typearrow \True \OrTypeError\\\\
    \vzopttwo \reverseeqdef \some{\vvtwo}\\
    {
    \vzs \eqdef
        \begin{cases}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item define $\vz$ based on $\vzone$ and $\vztwo$ according to the following cases:
        \begin{itemize}
...
\inferrule[more\_than\_one]{
    ...
    \listminabs(\vltwo) = \vztwo\\
    {
        \vz \eqdef \begin{cases}
            \vzone & \absvalueop{\vzone} < \absvalueop{\vztwo}\\
            \vztwo & \absvalueop{\vzone} > \absvalueop{\vztwo}\\
            \vzone & \vzone = \vztwo\\
            \absvalueop{\vzone} & \absvalueop{\vzone} = \absvalueop{\vztwo} \land \vzone \neq \vztwo
        \end{cases}
    }
}{
