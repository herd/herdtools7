...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \secref{AssignableExpressions} defines the AST for assignable expressions;
  ...
  \item \secref{AbstractSyntaxAbbreviations} defines AST abbreviations used by inference rules throughout this reference.
\end{itemize}
...
\subsection{Assignable Expressions \label{sec:AssignableExpressions}}
...
\begin{itemize}
  ...
  \item $\EEnumArray\{\EArrayEnum: \name, \EArrayLabels: \vl_{1..k}, \EArrayValue: \vetwo\}$ is used
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
This section describes how to transform a parse tree into the corresponding AST
via recursively traversing the parse tree and applying a \emph{builder} function
or a builder relation for each non-terminal node.
...
We refer to builder functions and builder relations collectively as \emph{builders}.

\hypertarget{def-tbuilderror}{}
\hypertarget{def-builderrorconfig}{}
For each non-terminal $N \derives R_1 \;|\; \ldots \;|\; R_k$, we define a builder
$\textsf{build\_}N $ which takes a parse tree for $\parsenode{N}$ and returns the corresponding
AST or a \emph{build error configuration} $\BuildErrorConfig \in \TBuildError$.
%
Build errors and build error codes are defined next:
\RenderTypes{build_error_and_codes}

The builder is defined in terms of one inference rule per alternative $R_i$.
...
\begin{itemize}
  \item \ASTRuleRef{ClistoneToList}
  \item \ASTRuleRef{ClistzeroToList}
  \item \ASTRuleRef{ClisttwoToList}
  \item \ASTRuleRef{TClistoneToList}
  \item \ASTRuleRef{TClistzeroToList}
  \item \ASTRuleRef{PlistzeroToList}
  \item \ASTRuleRef{PlisttwoToList}
  \item \ASTRuleRef{poptionToOption}
  \item \ASTRuleRef{Identifier}
\end{itemize}

\ASTRuleDef{ClistoneToList}
The function $\clistonetolist{\overname{\vx}{\ClistOne{T}}} \aslto \KleenePlus{T}$ takes a parse node for a non-empty comma-separated list and returns a list of the elements.

\begin{mathpar}
\inferrule[one\_element]{}{
  \clistonetolist{(\vx)} \astarrow [\vx]
}
\and
\inferrule[multiple\_elements]{
  \clistonetolist{\vxs} \astarrow \astversion{\vxs}
}{
  \clistonetolist{(\vx, \Tcomma, \vxs)} \astarrow [\vx] \concat \astversion{\vxs}
}
\end{mathpar}

\ASTRuleDef{ClistzeroToList}
The function $\clistzerotolist{\overname{\vx}{\ClistZero{T}}} \aslto \KleeneStar{T}$ takes a parse node for a possibly-empty comma-separated list and returns a list of the elements.

\begin{mathpar}
\inferrule[empty]{}{
  \clistzerotolist{\emptysentence} \astarrow [\ ]
}
\and
\inferrule[non\_empty]{
  ...
  \clistzerotolist{\vxs} \astarrow \astversion{\vxs}
}
\end{mathpar}

\ASTRuleDef{ClisttwoToList}
The function $\clisttwotolist{\overname{\vx}{\Clisttwo{T}}} \aslto \KleenePlus{T}$ takes a parse node for a comma-separated list with at least two elements and returns a list of the elements.

\begin{mathpar}
\inferrule{
  \clistonetolist{\vxs} \astarrow \astversion{\vxs}
}{
  \clisttwotolist{(\vx, \Tcomma, \vxs)} \astarrow [\vx] \concat \astversion{\vxs}
}
\end{mathpar}

\ASTRuleDef{TClistoneToList}
The function $\tclistonetolist{\overname{\vx}{\TClistOne{T}}} \aslto \KleenePlus{T}$ takes a parse node for a non-empty comma-separated trailing list and returns a list of the elements.

\begin{mathpar}
\inferrule[one\_element\_no\_comma]{}{
  \tclistonetolist{(\vx, \emptysentence)} \astarrow [\vx]
}
\and
\inferrule[one\_element\_comma]{}{
  \tclistonetolist{(\vx, \Tcomma)} \astarrow [\vx]
}
\and
\inferrule[multiple\_elements]{
  \tclistonetolist{\vxs} \astarrow \astversion{\vxs}
...
\ASTRuleDef{TClistzeroToList}
The function $\tclistzerotolist{\overname{\vx}{\TClistZero{T}}} \aslto \KleeneStar{T}$ takes a parse node for a possibly-empty comma-separated trailing list and returns a list of the elements.
...
\ASTRuleDef{PlistzeroToList}
The function $\plistzerotolist{\overname{\vx}{\PlistZero{T}}} \aslto \KleeneStar{T}$ takes a parse node for a possibly-empty parenthesized comma-separated list and returns a list of the elements.

\begin{mathpar}
\inferrule{
  \clistzerotolist{\vxs} \astarrow \astversion{\vxs}
}{
  \plistzerotolist{(\Tlpar, \vxs, \Trpar)} \astarrow \astversion{\vxs}
}
\end{mathpar}

\ASTRuleDef{PlisttwoToList}
The function $\plisttwotolist{\overname{\vx}{\Plisttwo{T}}} \aslto \KleenePlus{T}$ takes a parse node for a parenthesized comma-separated list with at least two elements and returns a list of the elements.

\begin{mathpar}
\inferrule{
  \clisttwotolist{\vxs} \astarrow \astversion{\vxs}
}{
  \plisttwotolist{(\Tlpar, \vxs, \Trpar)} \astarrow \astversion{\vxs}
}
\end{mathpar}

\ASTRuleDef{poptionToOption}
The function $\poptiontoOption{\overname{\vx}{\option{T}}} \aslto \Option{T}$ takes a parse node for an optional element and returns the corresponding optional value.

\begin{mathpar}
\inferrule[none]{}{
  \poptiontoOption{\emptysentence} \astarrow \none
}
\and
\inferrule[some]{}{
  \poptiontoOption{(\vx)} \astarrow \some{\vx}
}
\end{mathpar}


\ASTRuleDef{Identifier}
\RenderRelation{build_identifier}
\RenderProseAndFormally{build_identifier}
...
\begin{array}{lcl}
  ...
  \torexpr(\LESetFields(\vle, [\vx_{1..k}])) &=& \EGetFields(\torexpr(\vle), [\vx_{1..k}])\\
  \torexpr(\LEDiscard) &=& \EVar(\discardvar)\\
  \torexpr(\LEDestructuring([\vle_{1..k}])) &=& \ETuple([i=1..k: \torexpr(\vle_i)])\\
\end{array}
\]

\section{Abstract Syntax Abbreviations\label{sec:AbstractSyntaxAbbreviations}}
