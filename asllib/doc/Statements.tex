...
\begin{mathpar}
\inferrule[int]{
  \checknoprecisionloss{\rhs} \checktransarrow \True \OrTypeError\\\\
...
\]
checks whether a type has not suffered any precision loss in its constraints (see for example \TypingRuleRef{ApplyBinopTypes}).

\ProseParagraph
\OneApplies
\begin{itemize}
  \item $\tt$ is a \wellconstrainedintegertype{} with a precision $\PrecisionFull$ and no \typingerrorterm{} is raised;
  \item $\tt$ is a  \wellconstrainedintegertype{} with a precision $\PrecisionLost$ and a \typingerrorterm{} is raised;
  \item $\tt$ is not a \wellconstrainedintegertype{} and no \typingerrorterm{} is raised.
\end{itemize}
...
\begin{mathpar}
  \inferrule{%
    ...
    \checknoprecisionloss{\TInt(\wellconstrained(_, p))} \typearrow \True
  }
\end{mathpar}
...
\begin{itemize}
  ...
  \item $\newtenv$ is $\tenv$ (notice that this means $\vindexname$ is only declared for annotating $\vbodyp$ but then goes
        out of scope);
  \item \Prosenonconflictingunion{$\vsesblock$ and $\vsescond$}{$\vses$}\ProseOrError.
\end{itemize}
\FormallyParagraph
\begin{mathpar}
\inferrule{
...
\begin{itemize}
  ...
  \item define $\vsess$ as the list of $\vxs_i$, for every \Proselistrange{$i$}{$\vargs$};
