...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item annotating the local declaration item $\ldi$ with type $\vte$, local declaration keyword $\ldk$, initializing expression
          $\vep$ in $\tenv$
          yields $(\tenvone, \ldione)$;
    \item define $\ldityped$ as $\ldione$ if $\ldione$ is one of $\LDIDiscard$ or $\LDITyped$ and $\LDITyped(\ldione, \vte)$, otherwise.
          This is made to ensure that the typed local declaration is always annotated by a type;
    \item One of the following applies:
    \begin{itemize}
      ...
      \begin{itemize}
        ...
        \item declaring a local constant of type $\vte$, literal $\vv$ and local declaration item $\ldityped$ in $\tenvone$ yields $\newtenv$;
        \item $\news$ is a declaration with $\ldk$, $\ldityped$ and an expression $\vep$.
      \end{itemize}
...
\inferrule[constant]{
  ...
  \ldityped \eqdef \choice{\ldione\in\{\LDIDiscard, \LDITyped\}}{\ldione}{\LDITyped(\ldione, \vte)}\\
  \commonprefixline\\\\
  \ldk = \LDKConstant\\
  \reduceconstants(\tenvone, \ve) \typearrow \vv \OrTypeError\\\\
  \declarelocalconstant(\tenvone, \vv, \ldityped) \typearrow \newtenv\\
  \news \eqdef \SDecl(\LDKConstant, \ldityped, \langle\vep\rangle)
}{
...
\inferrule[non\_constant]{
  ...
  \commonprefixline\\\\
  \ldk \neq \LDKConstant\\
  \news \eqdef \SDecl(\ldk, \ldityped, \langle\vep\rangle)
}{
  \annotatestmt(\tenv, \overname{\SDecl(\ldk, \ldi, \langle\ve\rangle)}{\vs}) \typearrow (\news, \overname{\tenvone}{\newtenv})
}
\end{mathpar}
...
\subsection{Semantics}
\subsubsection{SemanticsRule.SCase \label{sec:SemanticsRule.SCase}}
\subsubsection{Example}
The specification:
