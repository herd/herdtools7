...
\subsection{Abstract Syntax}
\begin{flalign*}
  \stmt \derives\ & \SPrint(\overtext{\expr^*}{args}, \overtext{\Bool}{newline}, \overtext{\Bool}{debug}) &
\end{flalign*}
...
\inferrule{
  ...
  \astarrow
  \overname{\SPrint(\astversion{\vargs}, \vnewline, \vdebug)}{\vastnode}
}
\\
\inferrule{%
  \buildplist[\Nexpr](\vargs) \astarrow \astversion{\vargs} \\
...
All of the following apply:
\begin{itemize}
  \item $\vs$ denotes the print statement with arguments $\vargs$;
  ...
  \item $\news$ denotes the print statement with arguments $\vargsp$;
  \item $\newtenv$ is $\tenv$.
\end{itemize}
...
\begin{mathpar}
  \inferrule{%
    k \eqdef |\vargs| \\
    i=1..k: \annotateexpr{\vargs_i, \tenv} \typearrow (\vt_i, \vargsp_i)\OrTypeError \\
    i=1..k: \checktrans{\issingular(\vt_i)}{\BadPrintType} \typearrow \True \OrTypeError \\
  }{%
    \annotatestmt (\tenv, \SPrint(\vargs, \vnewline, \vb)) \typearrow (\SPrint(\vargsp, \vnewline, \vb), \tenv)
  }
\end{mathpar}
...
\begin{itemize}
  ...
  \item \hyperlink{print-literals}{prints} all the elements in $\elist$, without separator;
