...
\section{Declaration statements with an elided parameter \label{DeclarationStatementsElidedParameter}}
...
\item all of the following apply (\textsc{continue}):
  \begin{itemize}
    ...
    \item applying $\ticklooplimit$ to $\vlimitopt$ yields $\vlimitoptp$\ProseOrError;
...
\begin{itemize}
  ...
  \item evaluating the optional limit expression $\velimitopt$ via $\evallimit$ in $\env$
        yields $(\vlimitopt, \vgone)$\ProseOrError;
  \item applying $\ticklooplimit$ to $\vlimitoptone$ yields $\vlimitopttwo$\ProseOrError;
