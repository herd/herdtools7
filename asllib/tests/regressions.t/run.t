...
Parameterized integers:
  ...
  File same-precedence.asl, line 6, characters 10 to 15:
    let x = a + b - c;
            ^^^^^
  ASL Grammar error: Cannot parse. Binary operators of equal precedence.
  [1]
  ...
  File binop-non-assoc.asl, line 3, characters 6 to 11:
    - = 3 - 2 - 1;
        ^^^^^
  ASL Grammar error: Cannot parse. Non-associative binary operators.
  [1]
...
Bounds checks
  ...
  File elided-parameter-non-bits.asl, line 3, characters 20 to 27:
    let x : integer = Zeros{};
                      ^^^^^^^
  ASL Grammar error: Cannot parse.
    Cannot desugar elided parameter: left-hand side must have a `bits(...)` type annotation.
  [1]
