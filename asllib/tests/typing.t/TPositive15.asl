...
begin
    ...
    let testD : bits(8) = 0xA55A1234[x * 8 +: 8];
    let testE : bits(n) = 0xA55A1234[x * n +: n]; // RHS is type bits(n), which is a constrained integer

    var testF           = Zeros{32};
    testF[x * n +: n]       = Zeros{n};           // Same rules apply to bit slices on LHS
