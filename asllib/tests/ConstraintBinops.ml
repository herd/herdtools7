open Asllib
open AST
open StaticOperations

let eval_binop x (op : int3_binop) y =
  let pos = ASTUtils.dummy_annotated and t = Error.Static in
  match Operations.binop_values pos t (op :> binop) (L_Int x) (L_Int y) with
  | L_Int z -> z
  | _ -> assert false
...
let z_in_constraint z c =
  match filter_reduce_constraint_div c |> Option.value ~default:c with
...
let property (op : int3_binop) (x, y, cs1, cs2) =
...
let print_test (op : int3_binop) (x, y, cs1, cs2) =
  try
    let res = eval_binop x op y in
    let cs = constraint_binop op cs1 cs2 in
    let reduced_cs =
