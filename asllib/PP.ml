...
let pp_local_decl_item f =
  let pp_ldi_tuple_item f = function
    | Some x -> pp_print_string f x
    | None -> pp_print_string f "-"
  in
  function
  | LDI_Discard -> pp_print_string f "-"
  | LDI_Var x -> pp_print_string f x
  | LDI_Tuple ldis ->
      fprintf f "@[(%a)@]" (pp_comma_list pp_ldi_tuple_item) ldis
