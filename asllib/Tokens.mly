...
%token CATCH COLON COLON_COLON COMMA CONFIG CONSTANT DIV DIVRM DO DOT
%token DOWNTO ELSE ELSIF END ENUMERATION EOF EOR EQ EQ_OP EXCEPTION FOR FUNC
%token GEQ GETTER GT IF IMPL IN INTEGER LBRACE LBRACKET LEQ LET LOOPLIMIT LPAR
%token LT MINUS MOD MUL NEQ NOT OF OR OTHERWISE PASS PLUS PLUS_COLON POW PRAGMA
%token PRINTLN PRINT RBRACE RBRACKET RDIV REAL RECORD RECURSELIMIT REPEAT
%token RETURN RPAR STAR_COLON SEMI_COLON SETTER SHL SHR SLICING STRING SUBTYPES
%token THEN THROW TO TRY TYPE ARBITRARY UNREACHABLE UNTIL VAR WHEN WHERE WHILE
%token WITH LLBRACKET RRBRACKET
