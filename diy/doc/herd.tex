...
identifiers~\synt{\NT{id}} and tags~\synt{\NT{tag}}.
...
Tuples include a constant, the empty tuple \synth{\T{(}\T{)}},
...
Notice that postfix operators operate on relations only, exect the complement
operator~\T{\textilde} that also operates on sets.
...
and postfix operators bind tighter then infix operators.
