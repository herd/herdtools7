...
--- See Sec.~\ref{sec:with}.
Equivalently, we may assume that \verb+co+ and~\verb+fr+ are pre-defined.
...
program order~\textrel{po} and of communication relations
and final writes to memory (last write to a given location).
\footnote{Alternatively, we may adopt the simpler view that
...
Things are a bit more complex in~\herd{}, due to the presence of initial writes.
Those are write events that initialise the memory locations.
...
Up to now we have ignored initial writes, we are now going to
integrate them explicitly.
From the model perspective the initial write~$w_i$ of location~$x$
should precede any other write~$w$ to~$x$.
Such a constraint can be expressed in \cat{} as a relation:
\begin{verbatim}
let preS = loc & (IW * (W \ IW))
\end{verbatim}
Where \verb+loc+ is a predefined relation that relates all events
that access the same location.


In the SC case,  the constraint on initial writes means that $w_i$ should
precede~$w$ in the scheduling order~$S$.
Hence, the order~$S$ is not just any order on memory events (predefined
set~\verb+M+, which includes initial writes),
it is a topological order that extends the pre-order~\verb+preSC+.
We may generate all such topological orders with \cat{} primitive
\verb+linearisations+:
\begin{verbatim}
let allS = linearisations(M,preS)
\end{verbatim}
that con \verb+rf+ is defined (thanks to initial writes) without ambiguity.
Hence we have the following implementation \afile{lamport.cat}:
