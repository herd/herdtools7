...
type 'op binop =
  ...
  (* Test if two variables may be equal modulo the equality of two PAC fields *)
  | CollisionPossible
...
module
  ...
  = struct
    ...
    let addErrorCode key =
      ...
      | Symbolic (Virtual a) ->
          let name = extract_name a.name in
    ...
    (* Return whether a hash collision between two PAC fields is possible. *)
