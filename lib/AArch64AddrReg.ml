...
(*
  ...
  TODO:
  pp_norm is needed for registers with more than just one field.
  Awaiting decision on the generalization of AddrReg. For the
  purposes of PAR_El1, this can be removed.

  Has effects on toolsConstant.ml, symbConstant.ml.

  Would be interesting to know whether the change applies to *Constant.ml
  and whether there is a rule in the naming of these files.
