...
(* generic *)
  ...
  (* An error generated by `op1` and `op` to inform the caller that the answer
   * can be different depending of the presence of a hash collision, the error
 ...
 *)
  ...
  | Val s1, Val s2 when Option.is_some (collision s1 s2) ->
      let p1,p2 = Option.get (collision s1 s2) in
      raise (CollisionPAC (p1,p2,one,zero))
...
(* Assume concrete and others always to differ *)
  ...
  | Val s1, Val s2 when Option.is_some (collision s1 s2) ->
      let p1,p2 = Option.get (collision s1 s2) in
      raise (CollisionPAC (p1,p2,zero,one))
  | _, _ ->
