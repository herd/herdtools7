...
let pp_lower_key k = String.lowercase_ascii (pp_upper_key k)
...
let compare_signature_err p1 p2 =
  ...
  | 0 -> Bool.compare p1.is_key_a p2.is_key_a
  | r -> r
...
let get_name p = Option.map signature_name (PacSet.choose_opt p)
...
let pp_litmus pac s =
  let do_rec = function
    ...
    | Ok ok :: [] ->
        if ok.offset <> 0 then
          Warn.user_error "pp_litmus: PAC offset must be 0, got %d" ok.offset;
...
let compare_solver_state s1 s2 =
  ...
  | 0 ->
      let ineqs1 = List.sort PacSet.compare s1.inequalities
      and ineqs2 = List.sort PacSet.compare s2.inequalities in
      Misc.list_compare PacSet.compare ineqs1 ineqs2
  | r -> r
...
let normalize (x: t) {equalities; _} = simplify x equalities
...
let add_equality (x: t) (y: t) (state: solver_state) : solver_state option =
  ...
  | Some pivot ->
    ...
    (* simplify all the current inequalities with the new one *)
