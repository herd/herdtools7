...
let get_raised_flags s =
  ...
  List.fold_left (fun acc line ->
    ...
    if String.length line >= 6 && string_starts_with ~prefix:"Flag " line then
...
let get_each_litmus_in_dir dir =
  ...
  let rec loop acc =
    ...
    | None -> Unix.closedir handle; acc
  in
  loop []

let run flags =
  let cat_flags = get_cat_flags flags.axioms_file flags.libdir in
  let dirs_and_confs = get_dirs_and_confs flags.dirs_and_confs_file in
  let debug = flags.debug in
  let remaining_flags = List.fold_left (fun remaining_flags (dir, conf) ->
    Printf.printf "Checking axioms against %s ...\n%!" dir;
    let litmuses = get_each_litmus_in_dir dir in
    let remaining_flags = List.fold_left (fun remaining_flags litmus ->
      let _, stdout, stderr = TestHerd.run_herd ~bell:None ~cat:(Some flags.axioms_file)
        ~conf:conf
        ~variants:[]
        ~libdir:flags.libdir
        flags.herd [litmus] in
      let stdout = String.concat "\n" stdout in
      let stderr = String.concat "\n" stderr in
      if debug && not (stderr = "") then
        Printf.printf "Stderr for test %s is %s\n\n" litmus stderr;
      let assert_regex = Str.regexp "[^ \t\n\r]+[ \t\n\r]+assertion failed" in
      try
...
let () =
  ...
  let options = [
    ...
    Args.is_file  ("-dirs-and-confs-path", Arg.Set_string dirs_and_confs_file, "path to \
      the file specifying the directory of litmus tests and configurations to use") ;
    Args.is_file ("-axioms-path", Arg.Set_string axioms_file, "path to the axioms cat file") ;
    ArgUtils.parse_bool "-debug" debug "show debug output for this test runner" ;
  ] in
  ...
  if !axioms_file = "" then
    exit_with_error "Must set -axioms-path" ;
