        ...
        let remaining_flags =
          List.fold_left
            (fun remaining_flags litmus ->
              let _, stdout, stderr =
                TestHerd.run_herd ~bell:None ~cat:(Some flags.axioms_file) ~conf
                  ~variants:[] ~libdir:flags.libdir flags.herd [ litmus ]
              in
...
let () =
  ...
  let options =
    [
      ...
      Args.is_file
        ( "-axioms-path",
          Arg.Set_string axioms_file,
          "path to the axioms cat file" );
  ...
  if !axioms_file = "" then exit_with_error "Must set -axioms-path";
