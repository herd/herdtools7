...
previous notes are focussed arout the internal structure of the solver and the
memory model but this not is focussed on the user interface, and how to use it
to simplify understant the bahaviour of the hash collisions in a concurrent
program.
...
introduce a new set of instructions to
- Add a cryptographic hash in a pointer
- Remove a cryptographic hash from a pointer
- Authenticate a pointer and remove it's hash only in case of success

As example this assembly program add a hash in the pointer `x0` before doing a
successfull authentication and loading a value from the pointer:
...
And this program do the same thing without the authentication:
...
and depending of the enabled features on the processor, a failing authentication
...
Here `pacda(pacdb(...),...)` represent the combination of the two hashs using an
exclusive `OR`, if the hashs are not equals then their `EOR` must be different to
...
assembly parallel programs like this one:
...
`eor x4,x2,x2; add x0,x0,x4` sequence because this add an address dependency
...
`pacda x0,x0; xpacd x0` in this sequence because it generate a
`basic-dependency` between it's register-read/write events:
...
`autda Xd,Xn` only create a `pick-basic-dependency` between it's read event in
...
`autda Xd,Xn` that fail, we also expect to see a `basic-dependency` between the
...
PAC add cryptographic hashs in the most significant bits of the virtual
...
enumerate all the possible collisions in a concurrent program. And we can use
...
dependencies are strong enough the ordering of the two loads, because if the
anthentication pass, then `P2` can't raise a fault.
